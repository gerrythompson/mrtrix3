...
Window::Window(bool is_response_coefs)
    : lighting_dialog(nullptr), current(0), is_response(is_response_coefs) {
  GUI::Dialog::init();
  ...
  connect(hide_negative_lobes_action,
          SIGNAL(triggered(bool)),
          this,
          SLOT(hide_negative_lobes_slot(bool)));
  ...
  connect(colour_by_direction_action,
          SIGNAL(triggered(bool)),
          this,
          SLOT(colour_by_direction_slot(bool)));
  ...
  response_action->setStatusTip(tr("Assume each row of values consists only of\nthe m=0 (axially "
                                   "symmetric) even SH coefficients"));
