...
#define MODE_OPTION(classname, specifier, name, description) MODE(classname, specifier, name,
description)
...
Window::GLArea::GLArea(Window &parent) : GL::Area(&parent) {
  ...
  // grabGesture (Qt::PanGesture); // deactivated to prevent sticky pan:
  // https://github.com/MRtrix3/mrtrix3/issues/761
  QFont font_ = font();
...
void Window::GLArea::dropEvent(QDropEvent *event) {
  ...
  if (mimeData->hasUrls()) {
    ...
    for (int i = 0; i < urlList.size() && i < 32; ++i) {
      try {
        list.push_back(
            make_unique<MR::Header>(MR::Header::open(urlList.at(i).path().toUtf8().constData())));
      } catch (Exception &e) {
        e.display();
      }
    }
...
Window::Window()
  ...
  properties_action =
      image_menu->addAction(tr("Properties..."), this, SLOT(image_properties_slot()));
  properties_action->setToolTip(
  ...
  prev_slice_action =
      image_menu->addAction(tr("Previous slice"), this, SLOT(slice_previous_slot()));
  ...
  next_image_volume_action =
      image_menu->addAction(tr("Next volume"), this, SLOT(image_next_volume_slot()));
  next_image_volume_action->setShortcut(tr("Right"));
  addAction(next_image_volume_action);

  prev_image_volume_action =
      image_menu->addAction(tr("Previous volume"), this, SLOT(image_previous_volume_slot()));
  prev_image_volume_action->setShortcut(tr("Left"));
  addAction(prev_image_volume_action);

  goto_image_volume_action =
      image_menu->addAction(tr("Go to volume..."), this, SLOT(image_goto_volume_slot()));
  ...
  prev_image_volume_group_action = image_menu->addAction(
  ...
  goto_image_volume_group_action = image_menu->addAction(
      tr("Go to volume group..."), this, SLOT(image_goto_volume_group_slot()));
  ...
  prev_image_action =
      image_menu->addAction(tr("Previous image"), this, SLOT(image_previous_slot()));
  ...
  reset_windowing_action =
      colourmap_menu->addAction(tr("Reset brightness/contrast"), this, SLOT(image_reset_slot()));
  reset_windowing_action->setShortcut(tr("Esc"));
  addAction(reset_windowing_action);

  image_interpolate_action =
      colourmap_menu->addAction(tr("Interpolate"), this, SLOT(image_interpolate_slot()));
...
#define MODE_OPTION(classname, specifier, name, description)                                       \
  MODE(classname, specifier, name, description)
  {
    size_t n = 1;
#include "gui/mrview/mode/list.h"
  }
...
#undef MODE_OPTION
  ...
  show_orientation_labels_action =
      menu->addAction(tr("Show orientation labels"), glarea, SLOT(update()));
  show_orientation_labels_action->setShortcut(tr("O"));
  show_orientation_labels_action->setCheckable(true);
  show_orientation_labels_action->setChecked(
      File::Config::get_bool("MRViewShowOrientationLabel", true));
  ...
  connect(mode_action_group,
          SIGNAL(triggered(QAction *)),
          this,
          SLOT(select_mouse_mode_slot(QAction *)));
  ...
  snap_to_image_action = toolbar->addAction(
      QIcon(":/lock.svg"), tr("Snap to image"), this, SLOT(snap_to_image_slot()));
...
#include "gui/mrview/tool/list.h"
  ...
  if (!colourbar_position)
    WARN("invalid specifier \"" + cbar_pos +
         "\" for config file entry \"MRViewColourBarPosition\"");
  ...
  if (!tools_colourbar_position)
    WARN("invalid specifier \"" + cbar_pos +
         "\" for config file entry \"MRViewToolsColourBarPosition\"");
}
    ...
    if (MR::App::option.size()) {
      ...
      if (MR::App::option.size() > first_option) {
        ...
        if (first_option < last_arg_pos)
          throw Exception(
              "options must appear after the last argument - see help page for details");
      }
    }
...
void Window::image_open_slot() {
  vector<std::string> image_list =
      Dialog::File::get_images(this, "Select images to open", &current_folder);
  if (image_list.empty())
    return;
...
void Window::image_import_DICOM_slot() {
  std::string folder =
      Dialog::File::get_folder(this, "Select DICOM folder to import", &current_folder);
  if (folder.empty())
    return;
...
void Window::selected_custom_colour(const QColor &colour, const ColourMapButton &) {
  Image *imagep = image();
  if (imagep) {
    std::array<GLubyte, 3> c_colour{
        {GLubyte(colour.red()), GLubyte(colour.green()), GLubyte(colour.blue())}};
...
void Window::background_colour_slot() {
  QColor colour = QColorDialog::getColor(
      Qt::black, this, "Select background colour", QColorDialog::DontUseNativeDialog);
...
void Window::set_image_volume(size_t axis, ssize_t index) {
  ...
  if (index < 0)
    ...
    index = wrap_volumes_action->isChecked() ? index - image()->image.size(axis)
                                             : image()->image.size(axis) - 1;
...
void Window::image_goto_volume_slot() {
  ...
  bool ok;
  ssize_t vol = QInputDialog::getInt(
      this, tr("Go to..."), qstr(label), image()->image.index(3), 0, maxvol, 1, &ok);
  if (ok)
    set_image_volume(3, vol);
}
...
void Window::image_goto_volume_group_slot() {
  ...
  bool ok;
  ssize_t grp = QInputDialog::getInt(
      this, tr("Go to..."), qstr(label), image()->image.index(4), 0, maxvolgroup, 1, &ok);
  if (ok)
...
void Window::about_slot() {
  std::string message = std::string("<h1>MRView</h1>The MRtrix viewer, version ") +
                        MR::App::mrtrix_version +
                        "<br>"
                        "<em>" +
                        str(8 * sizeof(size_t)) +
                        " bit "
...
#define TOOL(classname, name, description)                                                         \
  ...
  if (stub.compare(0, stub.size(), std::string(opt.opt->id), 0, stub.size()) == 0) {               \
    ...
    if (dynamic_cast<Tool::__Action__ *>(tool_group->actions()[tool_id])                           \
...
#include "gui/mrview/tool/list.h"
    ...
    if (opt.opt->is("size")) {
      ...
      if (glsize.size() != 2)
        throw Exception("invalid argument \"" + std::string(opt.args[0]) +
                        "\" to -size batch command");
      if (glsize[0] < 1 || glsize[1] < 1)
        throw Exception("values provided to -size option must be positive");
      QSize oldsize = glarea->size();
      QSize winsize = size();
      resize(winsize.width() - oldsize.width() + glsize[0],
             winsize.height() - oldsize.height() + glsize[1]);
      return;
    }
    ...
    if (opt.opt->is("focus")) {
      try {
        ...
        if (pos.size() != 3)
          throw Exception(
              "-focus option expects a comma-separated list of 3 floating-point values");
        set_focus(Eigen::Vector3f{float(pos[0]), float(pos[1]), float(pos[2])});
      } catch (Exception &E) {
        try {
          show_crosshairs_action->setChecked(to<bool>(opt[0]));
        } catch (Exception &E2) {
          throw Exception("-focus option expects a boolean or a comma-separated list of 3 "
    ...
    if (opt.opt->is("target")) {
      if (image()) {
        ...
        if (pos.size() != 3)
          throw Exception(
              "-target option expects a comma-separated list of 3 floating-point values");
    ...
    if (opt.opt->is("orientation")) {
      if (image()) {
        ...
        if (pos.size() != 4)
          throw Exception(
              "-orientation option expects a comma-separated list of 4 floating-point values");
    ...
    if (opt.opt->is("voxel")) {
      if (image()) {
        ...
        if (pos.size() != 3)
          throw Exception(
              "-voxel option expects a comma-separated list of 3 floating-point values");
        set_focus(image()->voxel2scanner() *
                  Eigen::Vector3f{float(pos[0]), float(pos[1]), float(pos[2])});
        glarea->update();
      }
      return;
    }
    ...
    if (opt.opt->is("colourmap")) {
      ...
      if (n < 0 || n >= static_cast<int>(colourmap_button->colourmap_actions.size()))
        throw Exception("invalid image colourmap index \"" + str(n + 1) +
                        "\" requested in batch command");
    ...
    if (opt.opt->is("intensity_range")) {
      if (image()) {
        ...
        if (param.size() != 2)
          throw Exception(
              "-intensity_range options expects comma-separated list of two floating-point values");
...
void Window::add_commandline_options(MR::App::OptionList &options) {
  options + OptionGroup("View options")
      ...
      + Option(
            "orientation",
            "Set the orientation of the camera for the viewing window, in the form of a quaternion "
            "representing the rotation away from the z-axis. This should be provided as a list of "
            "4 comma-separated floating point values (this will be automatically normalised).") +
      ...
      + Option("select_image",
               "Switch to image number specified, with reference to the list of currently loaded "
               "images.")
            .allow_multiple() +
