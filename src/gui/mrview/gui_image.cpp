...
void ImageBase::render3D(Displayable::Shader &shader_program,
                         const Projection &projection,
                         const float depth) {
  update_texture3D();
...
template <typename ValueType> inline void Image::copy_texture_3D() {
  ...
  for (V.index(2) = 0; V.index(2) < V.size(2); ++V.index(2)) {
    ...
    upload_data(
        {{0, 0, V.index(2)}}, {{V.size(0), V.size(1), 1}}, reinterpret_cast<void *>(&data[0]));
    ++progress;
  }
}
