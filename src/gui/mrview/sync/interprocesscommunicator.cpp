...
InterprocessCommunicator::InterprocessCommunicator() : QObject(0) {
  ...
  for (int i_attempt = 0; i_attempt < 100; i_attempt++) {
    ...
    for (int i = 0; i < MAX_NO_ALLOWED; i++) {
      ...
      if (state != QLocalSocket::ConnectedState) {
        ...
        QLocalServer::removeServer(
            serverName); // this can be needed when previous processes used this but were forcibly
                         // killed, leaving a temporary file. If that file exists, then we will get
                         // an exception ("address in use") when trying to use this Id
        break;
      }
    }
    if (freeEntry == -1) {
    ...
    // else we didn't connect OK. This can occur when two processes start simultaneously and both
    // try for the same number. Loop back and re-try
  }
  ...
  if (!listenerSetUp) {
    ...
    throw std::runtime_error("Could not find any free id to use for listening. Have you reached "
                             "the maximum number of open windows allowed?");
  }
...
/**
 * Fires when another process tries to set up a connection from them --> us. I.e. when another
 * MRView calls TryConnectTo with our id as the argument
 */
...
/**
 * Tries to set an outgoing connection to the specified ID (another process), and tells that process
 * what our id is, so it can connect back to us
 */
...
void InterprocessCommunicator::OnDataReceived(vector<std::shared_ptr<QByteArray>> allMessages) {
  ...
  // It is up to those listeners to check their validity and let us know if they have changed their
  // value
  if (toSync.size()) {
