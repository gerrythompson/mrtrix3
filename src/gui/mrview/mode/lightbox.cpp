...
void LightBox::draw_plane_primitive(int axis,
...
void LightBox::paint(Projection &) {
  ...
  if (render_volumes()) {
    ...
    if (original_slice_index + volume_increment * (n_rows * n_cols - 1 - current_slice_index) >=
        image()->image.size(3))
      current_slice_index = n_rows * n_cols - 1 -
                            (image()->image.size(3) - 1 - original_slice_index) / volume_increment;
...
void LightBox::set_focus_event() {
  ...
  if (render_volumes()) {
    ssize_t vol =
        image()->image.index(3) + volume_increment * (new_slice_index - current_slice_index);
...
void LightBox::image_changed_event() {
  ...
  if (image()) {
    ...
    if (prev_image_name.empty()) {
      float slice_inc =
          std::pow(header.spacing(0) * header.spacing(1) * header.spacing(2), 1.f / 3.f);
