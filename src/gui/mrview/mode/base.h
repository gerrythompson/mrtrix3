...
public:
  ...
  Eigen::Vector3f get_through_plane_translation(float distance,
  ...
  Eigen::Vector3f get_through_plane_translation_FOV(int increment,
                                                    const ModelViewProjection &projection) {
    return get_through_plane_translation(MOVE_IN_OUT_FOV_MULTIPLIER * increment * FOV(),
                                         projection);
  }
