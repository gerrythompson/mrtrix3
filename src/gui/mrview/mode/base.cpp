...
void Base::paintGL() {
...

  {
    ...
    if (window().show_voxel_info()) {
      ...
      ssize_t vox[] = {ssize_t(std::round(voxel[0])),
                       ssize_t(std::round(voxel[1])),
                       ssize_t(std::round(voxel[2]))};
      ...
      projection.render_text(
          printf("position: [ %.4g %.4g %.4g ] mm", focus()[0], focus()[1], focus()[2]),
          LeftEdge | BottomEdge);
...
void Base::slice_move_event(const ModelViewProjection &proj, float x) {
  if (window().active_camera_interactor() &&
...
void Base::panthrough_event(const ModelViewProjection &proj) {
  if (window().active_camera_interactor() &&
      window().active_camera_interactor()->panthrough_event(proj))
    return;
...
void Base::setup_projection(const int axis, ModelViewProjection &with_projection) const {
  const GL::mat4 M =
      snap_to_image() ? GL::mat4(image()->image2scanner().matrix()) : GL::mat4(orientation());
  setup_projection(adjust_projection_matrix(GL::transpose(M), axis), with_projection);
}

void Base::setup_projection(const Eigen::Quaternionf &V,
                            ModelViewProjection &with_projection) const {
...
void Base::setup_projection(const GL::mat4 &M, ModelViewProjection &with_projection) const {
  ...
  const float depth =
      std::sqrt(Math::pow2(image()->header().spacing(0) * image()->header().size(0)) +
...
void Base::rotate_event(const ModelViewProjection &proj) {
  if (window().active_camera_interactor() &&
      window().active_camera_interactor()->rotate_event(proj))
    return;
