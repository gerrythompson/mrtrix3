...
bool Transform::panthrough_event(const ModelViewProjection &projection) {
  auto move = window().get_current_mode()->get_through_plane_translation_FOV(
      window().mouse_displacement().y(), projection);
...
bool Transform::tilt_event(const ModelViewProjection &projection) {
  ...
  const auto rot =
      window().get_current_mode()->get_tilt_rotation(projection).cast<double>().inverse();
  if (!rot.coeffs().allFinite())
    return true;

  const Eigen::Vector3d origin = window().focus().cast<double>();
  transform_type M = transform_type(rot).pretranslate(origin).translate(-origin) *
                     window().image()->header().transform();
...
bool Transform::rotate_event(const ModelViewProjection &projection) {
  ...
  transform_type M = transform_type(rot).inverse().pretranslate(origin).translate(-origin) *
                     window().image()->header().transform();
