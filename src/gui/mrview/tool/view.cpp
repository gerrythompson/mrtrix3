...
public:
  ...
  void reset(ClipPlane &p, const Image &image, int proj) {
    ...
    const Eigen::Vector3f centre =
        image.voxel2scanner() * Eigen::Vector3f{image.header().size(0) / 2.0f,
                                                image.header().size(1) / 2.0f,
                                                image.header().size(2) / 2.0f};
...
View::View(Dock *parent) : Base(parent) {
  ...
  clip_highlight_check_box->setToolTip(
      "Helps to identify selected clip planes that can be interacted with.");
  clip_highlight_check_box->setChecked(true);
  connect(clip_highlight_check_box,
          SIGNAL(toggled(bool)),
          this,
          SLOT(clip_planes_toggle_highlight_slot()));
  ...
  connect(clip_intersectionmode_check_box,
          SIGNAL(toggled(bool)),
          this,
          SLOT(clip_planes_toggle_intersectionmode_slot()));
  ...
  connect(
      clip_planes_new_axial_action, SIGNAL(triggered()), this, SLOT(clip_planes_add_axial_slot()));
  submenu->addAction(clip_planes_new_axial_action);

  clip_planes_new_sagittal_action = new QAction("&sagittal", this);
  connect(clip_planes_new_sagittal_action,
          SIGNAL(triggered()),
          this,
          SLOT(clip_planes_add_sagittal_slot()));
  submenu->addAction(clip_planes_new_sagittal_action);

  clip_planes_new_coronal_action = new QAction("&coronal", this);
  connect(clip_planes_new_coronal_action,
          SIGNAL(triggered()),
          this,
          SLOT(clip_planes_add_coronal_slot()));
  ...
  connect(clip_planes_reset_axial_action,
          SIGNAL(triggered()),
          this,
          SLOT(clip_planes_reset_axial_slot()));
  submenu->addAction(clip_planes_reset_axial_action);

  clip_planes_reset_sagittal_action = new QAction("&sagittal", this);
  connect(clip_planes_reset_sagittal_action,
          SIGNAL(triggered()),
          this,
          SLOT(clip_planes_reset_sagittal_slot()));
  submenu->addAction(clip_planes_reset_sagittal_action);

  clip_planes_reset_coronal_action = new QAction("&coronal", this);
  connect(clip_planes_reset_coronal_action,
          SIGNAL(triggered()),
          this,
          SLOT(clip_planes_reset_coronal_slot()));
...
void View::showEvent(QShowEvent *) {
  ...
  connect(
      &window(), SIGNAL(imageVisibilityChanged(bool)), this, SLOT(onImageVisibilityChanged(bool)));
...
void View::update_ortho_mode_gui(const Mode::Ortho &mode) {
  ...
  connect(
      ortho_view_in_row_check_box, SIGNAL(toggled(bool)), &mode, SLOT(set_show_as_row_slot(bool)));
}
...
void View::update_lightbox_mode_gui(const Mode::LightBox &mode) {
  ...
  connect(
      light_box_show_4d, SIGNAL(toggled(bool)), this, SLOT(light_box_toggle_volumes_slot(bool)));
...
bool View::pan_event(const ModelViewProjection &projection) {
  ...
  if (!clip.size())
    return false;
  Eigen::Vector3f move =
      projection.screen_to_model_direction(window().mouse_displacement(), window().target());
  for (size_t n = 0; n < clip.size(); ++n) {
    GL::vec4 &p(*clip[n]);
...
bool View::panthrough_event(const ModelViewProjection &projection) {
  ...
  if (!clip.size())
    return false;
  move_clip_planes_in_out(projection,
                          clip,
                          MOVE_IN_OUT_FOV_MULTIPLIER * window().mouse_displacement().y() *
                              window().FOV());
  return true;
}
