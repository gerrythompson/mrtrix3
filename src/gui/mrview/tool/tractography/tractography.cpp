...
Tractography::Tractography(Dock *parent)
  ...
  const std::string default_geom_type =
      File::Config::get("MRViewDefaultTractGeomType", tractogram_geometry_types[0]);
  try {
...
void Tractography::tractogram_open_slot() {
  ...
  vector<std::string> list = Dialog::File::get_files(
      this, "Select tractograms to open", "Tractograms (*.tck)", &current_folder);
  add_tractogram(list);
}
...
void Tractography::add_commandline_options(MR::App::OptionList &options) {
  ...
  options + OptionGroup("Tractography tool options")
      ...
      + Option("tractography.lighting", "Toggle the use of lighting of tractogram geometry")
            .allow_multiple() +
      ...
      + Option("tractography.tsf_load", "Load the specified tractography scalar file.")
            .allow_multiple() +
      ...
      + Option("tractography.tsf_thresh",
               "Set thresholds for the tractography scalar file. Requires -tractography.tsf_load "
               "already provided.")
            .allow_multiple() +
      ...
      + Option("tractography.tsf_colourmap",
               ...
               "Requires -tractography.tsf_load already.")
            .allow_multiple() +
...
/*
  ...
  are in the list view, no action is taken.
*/
...
bool Tractography::process_commandline_option(const MR::App::ParsedOption &opt) {
  ...
  if (opt.opt->is("tractography.tsf_colourmap")) {
    try {
      ...
      if (process_commandline_option_tsf_check_tracto_loaded()) {
        ...
        if (indices.size() != 1)
          throw Exception(
              "-tractography.tsf_colourmap option requires one tractogram to be selected");
  ...
  if (opt.opt->is("tractography.colour")) {
    try {
      ...
      if (values.size() != 3)
        throw Exception(
            "must provide exactly three comma-separated values to the -tractography.colour option");
      const float max_value = std::max({values[0], values[1], values[2]});
      if (std::min({values[0], values[1], values[2]}) < 0.0 || max_value > 255)
        throw Exception("values provided to -tractogram.colour must be either between 0.0 and 1.0, "
                        "or between 0 and 255");
      const float multiplier = max_value <= 1.0 ? 1.0 : 1.0 / 255.0;

      // input need to be a float *
      float colour_input[3] = {multiplier * float(values[0]),
...
bool Tractography::process_commandline_option_tsf_check_tracto_loaded() {
  ...
  if (count == 0) {
    ...
    std::cerr << "TSF argument specified but no tractography loaded. Ensure TSF arguments follow "
                 "the tractography.load argument.\n";
  }
  return count != 0;
}

/*Checks whether legal to apply tsf options and prepares the scalar_file_options to do so. Returns
 * the vector of floats parsed from the options, or null on fail*/
