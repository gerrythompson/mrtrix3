...
void Tractogram::render(const Projection &transform) {
  ...
  if (tractography_tool.do_crop_to_slab) {
    gl::Uniform3fv(
        gl::GetUniformLocation(track_shader, "screen_normal"), 1, transform.screen_normal().data());
    gl::Uniform1f(gl::GetUniformLocation(track_shader, "crop_var"),
                  window().focus().dot(transform.screen_normal()) -
                      tractography_tool.slab_thickness / 2);
  ...
  if (color_type == TrackColourType::ScalarFile) {
    gl::Uniform1f(gl::GetUniformLocation(track_shader, "offset"),
                  display_midpoint - 0.5f * display_range);
    gl::Uniform1f(gl::GetUniformLocation(track_shader, "scale"), 1.0 / display_range);
  }

  if (tractography_tool.use_lighting) {
    gl::UniformMatrix4fv(
        gl::GetUniformLocation(track_shader, "MV"), 1, gl::FALSE_, transform.modelview());
    gl::Uniform3fv(
        gl::GetUniformLocation(track_shader, "light_pos"), 1, tractography_tool.lighting->lightpos);
    gl::Uniform1f(gl::GetUniformLocation(track_shader, "ambient"),
                  tractography_tool.lighting->ambient);
    gl::Uniform1f(gl::GetUniformLocation(track_shader, "diffuse"),
                  tractography_tool.lighting->diffuse);
    gl::Uniform1f(gl::GetUniformLocation(track_shader, "specular"),
                  tractography_tool.lighting->specular);
  ...
  float point_size_screenspace = Tractogram::default_point_size *
                                 std::exp(2.0e-3f * line_thickness) * original_fov *
...
inline void Tractogram::render_streamlines() {
  ...
  for (size_t buf = 0, N = vertex_buffers.size(); buf < N; ++buf) {
    ...
    if (vao_dirty) {
      ...
      case TrackColourType::ScalarFile:
        ...
        gl::VertexAttribPointer(3,
                                ...
                                (void *)(sample_stride * sizeof(float)));
        break;
      default:
        break;
      }
      ...
      gl::VertexAttribPointer(
          1, 3, gl::FLOAT, gl::FALSE_, 3 * sample_stride * sizeof(float), (void *)0);
      ...
      gl::VertexAttribPointer(2,
                              ...
                              gl::FALSE_,
                              3 * sample_stride * sizeof(float),
                              (void *)(6 * sample_stride * sizeof(float)));
      ...
      for (size_t j = 0, M = track_sizes[buf].size(); j < M; ++j) {
        ...
        track_starts[buf][j] =
            (GLint)std::floor(original_track_starts[buf][j] / (float)sample_stride);

        // Vertex attributes are packed prev, curr, next
        // So ensure first curr does indeed correspond to track start
        if (original_track_starts[buf][j] - sample_stride * track_starts[buf][j] <
            sample_stride - 1)
          --track_starts[buf][j];
    ...
    gl::MultiDrawArrays(
        mode, &track_starts[buf][0], &track_sizes[buf][0], num_tracks_per_buffer[buf]);
  }
...
inline void Tractogram::update_stride() {
  ...
  if (geometry_type == TrackGeometryType::Pseudotubes && std::isfinite(step_size)) {
    const auto geom_size = geometry_type == TrackGeometryType::Pseudotubes
                               ? Tractogram::default_line_thickness
...
void Tractogram::load_intensity_track_scalars(const std::string &filename) {
  ...
  if (Path::has_suffix(filename, ".tsf")) {
    ...
    for (vector<size_t>::const_iterator i = num_tracks_per_buffer.begin();
         i != num_tracks_per_buffer.end();
         ++i)
      total_num_tracks += *i;
    if (size_t(scalars.size()) != total_num_tracks)
      throw Exception("The scalar text file does not contain the same number of elements as the "
                      "selected tractogram");
...
void Tractogram::load_tracks_onto_GPU(vector<Eigen::Vector3f> &buffer,
  ...
  gl::BufferData(
      gl::ARRAY_BUFFER, buffer.size() * sizeof(Eigen::Vector3f), &buffer[0][0], gl::STATIC_DRAW);
