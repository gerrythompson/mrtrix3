...
TrackScalarFileOptions::TrackScalarFileOptions(Tractography *parent)
  ...
  connect(intensity_file_button,
          SIGNAL(clicked()),
          this,
          SLOT(open_intensity_track_scalar_file_slot()));
  hlayout->addWidget(intensity_file_button);

  // Colourmap menu:
  colourmap_menu = new QMenu(tr("Colourmap menu"), this);

  MRView::create_colourmap_menu(
      this, colourmap_group, colourmap_menu, colourmap_actions, false, false);
  ...
  show_colour_bar =
      colourmap_menu->addAction(tr("Show colour bar"), this, SLOT(show_colour_bar_slot()));
  ...
  QAction *reset_intensity =
      colourmap_menu->addAction(tr("Reset intensity"), this, SLOT(reset_intensity_slot()));
  ...
  connect(
      threshold_file_combobox, SIGNAL(activated(int)), this, SLOT(threshold_scalar_file_slot(int)));
...
void TrackScalarFileOptions::render_tractogram_colourbar(const Tractogram &tractogram) {
  ...
  window().colourbar_renderer.render(tractogram.colourmap,
...
void TrackScalarFileOptions::update_UI() {
  ...
  if (tractogram->get_color_type() == TrackColourType::ScalarFile) {
    ...
    intensity_file_button->setText(
        qstr(shorten(Path::basename(tractogram->intensity_scalar_filename), 35, 0)));
  ...
  case TrackThresholdType::SeparateFile:
    ...
    threshold_file_combobox->addItem(
        qstr(shorten(Path::basename(tractogram->threshold_scalar_filename), 35, 0)));
  ...
  const bool show_threshold_controls =
      (tractogram->get_threshold_type() != TrackThresholdType::None);
...
bool TrackScalarFileOptions::open_intensity_track_scalar_file_slot() {
  std::string scalar_file =
      Dialog::File::get_file(this,
                             "Select scalar text file or Track Scalar file (.tsf) to open",
                             "",
                             &tool->current_folder);
  ...
  case 1:
    if (tractogram->get_color_type() == TrackColourType::ScalarFile) {
      ...
      QMessageBox::warning(QApplication::activeWindow(),
                           ...
                           tr("Can only threshold based on scalar file used for streamline "
                              "colouring if that colour mode is active"),
                           QMessageBox::Ok,
                           QMessageBox::Ok);
      threshold_file_combobox->blockSignals(true);
      switch (tractogram->get_threshold_type()) {
