...
void Capture::on_translation_type(int index) {
  translation_type =
      static_cast<TranslationType>(translation_type_combobox->itemData(index).toInt());
}
...
void Capture::run(bool with_capture) {
  ...
  for (size_t i = first_index; i < first_index + frames_value; ++i) {
    ...
    Eigen::Vector3f axis{
        rotation_axis_x->value(), rotation_axis_y->value(), rotation_axis_z->value()};
    axis.normalize();
