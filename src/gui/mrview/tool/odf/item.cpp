...
ODF_Item::ODF_Item(MR::Header &&H,
  ...
  try {
    ...
    try {
      dixel->set_header();
      DEBUG("Image " + image.header().name() +
            " initialised as dixel ODF using header directions field");
    } catch (...) {
      try {
        dixel->set_internal(image.header().size(3));
        DEBUG("Image " + image.header().name() +
              " initialised as dixel ODF using internal direction set");
      } catch (...) {
  ...
  if (entry != H.keyval().end()) {
    try {
      ...
      if (lines.size() != size_t(H.size(3)))
        throw Exception("malformed directions field in image \"" + H.name() +
