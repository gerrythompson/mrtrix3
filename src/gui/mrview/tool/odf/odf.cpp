...
ODF::ODF(Dock *parent)
    : Base(parent), preview(nullptr), renderer(nullptr), lighting_dock(nullptr), lmax(0) {
  ...
  connect(hide_negative_values_box,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(hide_negative_values_slot(int)));
  ...
  connect(colour_by_direction_box,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(colour_by_direction_slot(int)));
  ...
  main_grid_box->setToolTip(tr("Show individual ODFs at the spatial resolution of the main image "
                               "instead of the ODF image's own spatial resolution"));
  ...
  colour_relative_to_projection_box->setToolTip(
      tr("Colour ODFs according their direction relative to the camera,\nrather than relative to "
  ...
  connect(
      lighting_settings_button, SIGNAL(clicked(bool)), this, SLOT(lighting_settings_slot(bool)));
...
void ODF::draw(const Projection &projection, bool is_3D, int, int) {
  ...
  if (settings->odf_type == odf_type_t::DIXEL &&
      settings->dixel->dir_type == ODF_Item::DixelPlugin::dir_t::NONE)
    return;
  ...
  if (!hide_all_button->isChecked()) {
    ...
    if (colour_relative_to_projection_box->isChecked()) {
      rot = GL::inv(window().snap_to_image() ? GL::mat4(image.transform().matrix())
                                             : GL::mat4(window().orientation()));
      rotation = &rot;
    }
    ...
    pos += projection.screen_normal() *
           (projection.screen_normal().dot(window().focus() - window().target()));
    ...
    Eigen::Vector3f x_dir =
        projection.screen_to_model_direction(1.0, 0.0, projection.depth_of(pos));
    x_dir.normalize();
    ...
    Eigen::Vector3f y_dir =
        projection.screen_to_model_direction(0.0, 1.0, projection.depth_of(pos));
    y_dir.normalize();
    ...
    const Eigen::Vector3f x_width = projection.screen_to_model_direction(
        projection.width() / 2.0, 0.0, projection.depth_of(pos));
    const int nx = std::ceil(x_width.norm() / x_dir.norm());
    const Eigen::Vector3f y_width = projection.screen_to_model_direction(
        0.0, projection.height() / 2.0, projection.depth_of(pos));
...
void ODF::get_values(Eigen::VectorXf &values,
                     ODF_Item &item,
  ...
  if (interp) {
    ...
    if (linear_interp.scanner(pos)) {
      for (linear_interp.index(3) = 0;
           linear_interp.index(3) < std::min(ssize_t(values.size()), linear_interp.size(3));
  ...
  if (item.odf_type == odf_type_t::DIXEL &&
      item.dixel->dir_type == ODF_Item::DixelPlugin::dir_t::DW_SCHEME) {
...
void ODF::setup_ODFtype_UI(const ODF_Item *image) {
  ...
  level_of_detail_label->setVisible(image->odf_type == odf_type_t::SH ||
                                    image->odf_type == odf_type_t::TENSOR);
  level_of_detail_selector->setVisible(image->odf_type == odf_type_t::SH ||
                                       image->odf_type == odf_type_t::TENSOR);
    ...
    for (size_t i = 0; i != image->dixel->shells->count(); ++i) {
      if (!(*image->dixel->shells)[i].is_bzero())
        shell_selector->addItem(
            QString::fromStdString(str(int(std::round((*image->dixel->shells)[i].get_mean())))));
    }
    if (shell_selector->count() &&
...
void ODF::tensor_open_slot() {
  vector<std::string> list =
      Dialog::File::get_images(&window(), "Select tensor images to open", &current_folder);
  if (list.empty())
    return;
  ...
  try {
    ...
    case 0: // DW scheme
      if (!settings->dixel->num_DW_shells())
        throw Exception(
            "Cannot draw orientation information from DW scheme: no such scheme stored in header");
    ...
    case 4: // From file
      const std::string path = Dialog::File::get_file(
          this, "Select directions file", "Text files (*.txt)", &current_folder);
      if (!path.size()) {
    ...
    shell_selector->setEnabled(dir_type == 0 && settings->dixel->shells &&
                               settings->dixel->shells->count() > 1);
    if (dir_type == 3) {
      if (preview)
        preview->render_frame->clear_dixels();
    } else {
