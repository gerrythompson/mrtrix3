...
Overlay::Overlay(Dock *parent) : Base(parent) {
  ...
  connect(lower_threshold_check_box,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(lower_threshold_changed(int)));
  ...
  connect(upper_threshold_check_box,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(upper_threshold_changed(int)));
...
void Overlay::image_open_slot() {
  vector<std::string> overlay_names =
...
void Overlay::add_images(vector<std::unique_ptr<MR::Header>> &list) {
  ...
  image_list_view->selectionModel()->select(QItemSelection(first, last),
...
void Overlay::render_image_colourbar(const Image &image) {
  float min_value =
      image.use_discard_lower() ? image.scaling_min_thresholded() : image.scaling_min();

  float max_value =
      image.use_discard_upper() ? image.scaling_max_thresholded() : image.scaling_max();

  window().colourbar_renderer.render(image.colourmap,
                                     image.scale_inverted(),
                                     min_value,
                                     max_value,
                                     image.scaling_min(),
                                     image.display_range,
                                     Eigen::Vector3f{image.colour[0] / 255.0f,
...
void Overlay::update_selection() {
  ...
  if (indices.size() == 1) {
    ...
    // volume_box->setVisible(overlay->header().ndim() > 3); // causes shift in FOV due to resizing
    // of tool pane
    for (size_t d = 3; d < overlay->image.ndim(); ++d) {
      ...
      volume_index_layout->addWidget(
          vol_index, volume_index_layout->count() / 3, volume_index_layout->count() % 3);
  ...
  lower_threshold_check_box->setCheckState(
      num_lower_threshold
          ? (num_lower_threshold == indices.size() ? Qt::Checked : Qt::PartiallyChecked)
          : Qt::Unchecked);
  ...
  upper_threshold_check_box->setCheckState(
      num_upper_threshold
          ? (num_upper_threshold == indices.size() ? Qt::Checked : Qt::PartiallyChecked)
          : Qt::Unchecked);
...
void Overlay::add_commandline_options(MR::App::OptionList &options) {
  ...
  options + OptionGroup("Overlay tool options")
      ...
      + Option("overlay.threshold_min", "Set the lower threshold value of the overlay")
            .allow_multiple() +
      Argument("value").type_float()

      + Option("overlay.threshold_max", "Set the upper threshold value of the overlay")
            .allow_multiple() +
      Argument("value").type_float()

      + Option("overlay.no_threshold_min", "Disable the lower threshold for the overlay")
            .allow_multiple() +
      Option("overlay.no_threshold_max", "Disable the upper threshold for the overlay")
          .allow_multiple()

      + Option("overlay.interpolation", "Enable or disable overlay image interpolation.")
            .allow_multiple() +
...
bool Overlay::process_commandline_option(const MR::App::ParsedOption &opt) {
  ...
  if (opt.opt->is("overlay.colour")) {
    try {
      ...
      if (values.size() != 3)
        throw Exception(
            "must provide exactly three comma-separated values to the -overlay.colour option");
      const float max_value = std::max({values[0], values[1], values[2]});
      if (std::min({values[0], values[1], values[2]}) < 0.0 || max_value > 255)
        throw Exception("values provided to -overlay.colour must be either between 0.0 and 1.0, or "
                        "between 0 and 255");
      const float multiplier = max_value <= 1.0 ? 255.0 : 1.0;
      QColor colour(
          int(values[0] * multiplier), int(values[1] * multiplier), int(values[2] * multiplier));
  ...
  if (opt.opt->is("overlay.intensity")) {
    try {
      ...
      if (values.size() != 2)
        throw Exception(
            "must provide exactly two comma-separated values to the -overlay.intensity option");
