...
Connectome::Connectome(Dock *parent)
  ...
  connect(node_visibility_combobox,
          SIGNAL(activated(int)),
          this,
          SLOT(node_visibility_selection_slot(int)));
  ...
  node_visibility_warning_icon->setPixmap(
      warning_icon.pixmap(node_visibility_combobox->height(), Qt::KeepAspectRatio));
  node_visibility_warning_icon->setScaledContents(true);
  node_visibility_warning_icon->setToolTip("Changes to node visualisation will have no apparent "
                                           "effect if node visibility is set to \'none\'");
  ...
  connect(node_visibility_threshold_button,
          SIGNAL(valueChanged()),
          this,
  ...
  connect(node_geometry_combobox,
          SIGNAL(activated(int)),
          this,
          SLOT(node_geometry_selection_slot(int)));
  ...
  connect(node_geometry_sphere_lod_spinbox,
          SIGNAL(valueChanged(int)),
          this,
          SLOT(sphere_lod_slot(int)));
  ...
  connect(node_geometry_overlay_interp_checkbox,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(overlay_interp_slot(int)));
  ...
  connect(
      node_colour_combobox, SIGNAL(activated(int)), this, SLOT(node_colour_selection_slot(int)));
  ...
  connect(node_colour_matrix_operator_combobox,
          SIGNAL(activated(int)),
          this,
          SLOT(node_colour_matrix_operator_slot(int)));
  ...
  connect(node_colour_fixedcolour_button,
          SIGNAL(clicked()),
          this,
          SLOT(node_fixed_colour_change_slot()));
  hlayout->addWidget(node_colour_fixedcolour_button, 1);
  node_colour_colourmap_button =
      new ColourMapButton(this, node_colourmap_observer, false, false, true);
  ...
  connect(
      node_colour_lower_button, SIGNAL(valueChanged()), this, SLOT(node_colour_parameter_slot()));
  ...
  connect(
      node_colour_upper_button, SIGNAL(valueChanged()), this, SLOT(node_colour_parameter_slot()));
  ...
  node_size_matrix_operator_combobox->setToolTip(
      tr("If node sizes are determined from a matrix file, and multiple\n"
         "nodes are selected, this operator defines how the entries from\n"
         "the corresponding rows of the matrix are combined to produce a\n"
  ...
  connect(node_size_matrix_operator_combobox,
          SIGNAL(activated(int)),
          this,
          SLOT(node_size_matrix_operator_slot(int)));
  ...
  connect(
      node_size_invert_checkbox, SIGNAL(stateChanged(int)), this, SLOT(node_size_parameter_slot()));
  ...
  node_alpha_matrix_operator_combobox->setToolTip(
      tr("If node opacity is determined from a matrix file, and multiple\n"
         "nodes are selected, this operator defines how the entries from\n"
  ...
  connect(node_alpha_matrix_operator_combobox,
          SIGNAL(activated(int)),
          this,
          SLOT(node_alpha_matrix_operator_slot(int)));
  ...
  connect(node_alpha_invert_checkbox,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(node_alpha_parameter_slot()));
  ...
  connect(edge_visibility_combobox,
          SIGNAL(activated(int)),
          this,
          SLOT(edge_visibility_selection_slot(int)));
  gridlayout->addWidget(edge_visibility_combobox, 0, 2);
  edge_visibility_warning_icon = new QLabel();
  edge_visibility_warning_icon->setPixmap(
      warning_icon.pixmap(edge_visibility_combobox->height(), Qt::KeepAspectRatio));
  edge_visibility_warning_icon->setScaledContents(true);
  edge_visibility_warning_icon->setToolTip("Changes to edge visualisation will have no apparent "
                                           "effect if edge visibility is set to \'none\'");
  ...
  connect(edge_visibility_threshold_button,
          SIGNAL(valueChanged()),
          this,
  ...
  edge_visibility_by_nodes_checkbox->setToolTip(
      tr("Toggle whether or not an edge that connects to an invisible node (whether hidden through "
         "\"visibility\" or \"opacity\" features) should also be made invisible"));
  edge_visibility_by_nodes_checkbox->setTristate(false);
  edge_visibility_by_nodes_checkbox->setChecked(true);
  connect(edge_visibility_by_nodes_checkbox,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(edge_visibility_parameter_slot()));
  ...
  connect(edge_geometry_combobox,
          SIGNAL(activated(int)),
          this,
          SLOT(edge_geometry_selection_slot(int)));
  ...
  edge_geometry_cylinder_lod_spinbox->setToolTip(
      tr("Level of Detail for drawing cylinders / streamtubes"));
  ...
  connect(edge_geometry_cylinder_lod_spinbox,
          SIGNAL(valueChanged(int)),
          this,
          SLOT(cylinder_lod_slot(int)));
  ...
  connect(edge_geometry_line_smooth_checkbox,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(edge_size_value_slot()));
  ...
  connect(
      edge_colour_combobox, SIGNAL(activated(int)), this, SLOT(edge_colour_selection_slot(int)));
  ...
  edge_colour_colourmap_button =
      new ColourMapButton(this, edge_colourmap_observer, false, false, true);
  ...
  connect(
      edge_colour_lower_button, SIGNAL(valueChanged()), this, SLOT(edge_colour_parameter_slot()));
  ...
  connect(
      edge_colour_upper_button, SIGNAL(valueChanged()), this, SLOT(edge_colour_parameter_slot()));
  ...
  connect(
      edge_size_invert_checkbox, SIGNAL(stateChanged(int)), this, SLOT(edge_size_parameter_slot()));
  ...
  connect(edge_alpha_invert_checkbox,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(edge_alpha_parameter_slot()));
  ...
  lighting_checkbox->setToolTip(
      tr("Toggle whether lighting should be applied to compatible elements"));
  ...
  connect(
      crop_to_slab_checkbox, SIGNAL(stateChanged(int)), this, SLOT(crop_to_slab_toggle_slot(int)));
  ...
  show_node_list_button->setToolTip(
      tr("Open window that displays list of nodes and enables their selection"));
  ...
  connect(&node_selection_settings,
          SIGNAL(dataChanged()),
          this,
          SLOT(node_selection_settings_changed_slot()));
...
size_t Connectome::visible_number_colourbars() {
  if (!buffer)
    return 0;
  return (
      ((((node_colour == node_colour_t::CONNECTOME && matrix_list_model->rowCount()) ||
         node_colour == node_colour_t::VECTOR_FILE || node_colour == node_colour_t::MATRIX_FILE) &&
        show_node_colour_bar)
           ? 1
           : 0) +
...
void Connectome::image_open_slot() {
  ...
  try {
    ...
    e.display();
    // If importing a new image has failed, but another image was loaded previously, keep existing
    // data
  }
  window().updateGL();
}
...
void Connectome::crop_to_slab_toggle_slot(int /*value*/) {
  ...
  node_geometry_overlay_3D_warning_icon->setVisible(node_geometry == node_geometry_t::OVERLAY &&
                                                    is_3D);
  window().updateGL();
}
...
void Connectome::initialise(const std::string &path) {
  ...
  if (!H.datatype().is_integer())
    throw Exception("Input parcellation image must have an integer datatype; try running mrconvert "
                    "-datatype uint32");
  if (H.ndim() != 3)
    ...
    for (auto loop = Loop(*buffer)(*buffer); loop; ++loop) {
      ...
      if (node_index) {
        ...
        if (node_index >= max_index) {
          ...
          node_lower_corners.resize(node_index + 1,
                                    Eigen::Array3i{int(H.size(0)), int(H.size(1)), int(H.size(2))});
          node_upper_corners.resize(node_index + 1, Eigen::Array3i{-1, -1, -1});
          max_index = node_index;
        }

        const Eigen::Vector3f voxel{
            float(buffer->index(0)), float(buffer->index(1)), float(buffer->index(2))};
        ...
        for (size_t axis = 0; axis != 3; ++axis) {
          node_lower_corners[node_index][axis] =
              std::min(node_lower_corners[node_index][axis], int(voxel[axis]));
          node_upper_corners[node_index][axis] =
              std::max(node_upper_corners[node_index][axis], int(voxel[axis]));
        }
      }
    }
  }
    ...
    for (size_t node_index = 1; node_index <= max_index; ++node_index) {
      if (node_volumes[node_index]) {
        ...
        for (size_t axis = 0; axis != 3; ++axis) {
          ...
          dim[axis] =
              node_upper_corners[node_index][axis] - node_lower_corners[node_index][axis] + 1;
        }
        MR::Adapter::Subset<MR::Image<node_t>> subset(*buffer, from, dim);
        MR::Image<bool> node_mask(
            MR::Image<bool>::scratch(subset, "Node " + str(node_index) + " mask"));
        ...
        nodes.push_back(
            Node(node_coms[node_index], node_volumes[node_index], pixheight, node_mask));
  ...
  if (node_visibility != node_visibility_t::NONE) {
    ...
    if (node_geometry == node_geometry_t::OVERLAY) {
      ...
      if (is_3D) {
        ...
        node_overlay->render3D(
            node_overlay->slice_shader, projection, projection.depth_of(window().focus()));
      ...
      if (node_colour == node_colour_t::VECTOR_FILE &&
          ColourMap::maps[node_colourmap_index].is_colour)
        gl::Uniform3fv(
            gl::GetUniformLocation(node_shader, "colourmap_colour"), 1, node_fixed_colour.data());
      ...
      if (use_lighting()) {
        gl::UniformMatrix4fv(
            gl::GetUniformLocation(node_shader, "MV"), 1, gl::FALSE_, projection.modelview());
      ...
      if (crop_to_slab) {
        gl::Uniform3fv(gl::GetUniformLocation(node_shader, "screen_normal"),
                       1,
                       projection.screen_normal().data());
        if (is_3D) {
      ...
      for (auto it = node_ordering.rbegin(); it != node_ordering.rend(); ++it) {
        ...
        if (node_visibility_given_selection(it->second)) {
          ...
          gl::Uniform1f(node_size_ID,
                        node_size_given_selection(it->second) * node_size_scale_factor);
          switch (node_geometry) {
          case node_geometry_t::SPHERE:
            if (alpha) {
              gl::CullFace(gl::FRONT);
              gl::Uniform1f(specular_ID,
                            (1.0 - node_alpha_given_selection(it->second) * node_fixed_alpha) *
                                lighting.specular);
...
void Connectome::draw_edges(const Projection &projection) {
  ...
  if (edge_visibility != edge_visibility_t::NONE) {
    ...
    if (use_lighting()) {
      gl::UniformMatrix4fv(
          gl::GetUniformLocation(edge_shader, "MV"), 1, gl::FALSE_, projection.modelview());
    ...
    if (crop_to_slab) {
      gl::Uniform3fv(gl::GetUniformLocation(edge_shader, "screen_normal"),
                     1,
                     projection.screen_normal().data());
      if (is_3D) {
    ...
    if ((edge_colour == edge_colour_t::CONNECTOME || edge_colour == edge_colour_t::MATRIX_FILE) &&
      ...
      gl::Uniform3fv(
          gl::GetUniformLocation(edge_shader, "colourmap_colour"), 1, edge_fixed_colour.data());
    ...
    for (auto it = edge_ordering.rbegin(); it != edge_ordering.rend(); ++it) {
      ...
      if (edge_visibility_given_selection(edge)) {
        ...
        case edge_geometry_t::CYLINDER:
          ...
          gl::Uniform1f(
              radius_ID,
              std::sqrt(edge_size_given_selection(edge) * edge_size_scale_factor / Math::pi));
          if (alpha) {
            gl::CullFace(gl::FRONT);
            gl::Uniform1f(specular_ID,
                          (1.0 - edge_alpha_given_selection(edge) * edge_fixed_alpha) *
                              lighting.specular);
...
bool Connectome::import_vector_file(FileDataVector &data, const std::string &attribute) {
  ...
  try {
    ...
    if (data.size() != num_nodes()) {
      ...
      throw Exception("File " + Path::basename(path) + " contains " + str(numel) +
...
void Connectome::load_properties() {
  if (lut.empty()) {
    ...
    if (absent_entry_count) {
      WARN(str(absent_entry_count) +
           " indices present in parcellation image with no entry in lookup table; "
...
void Connectome::calculate_node_colours() {
  if (node_colour == node_colour_t::FIXED) {
    ...
    if (lut.size()) {
      for (node_t node_index = 1; node_index <= num_nodes(); ++node_index) {
        ...
        if (i == lut.end())
          ...
          nodes[node_index].set_colour(Eigen::Array3f(i->second.get_colour().cast<float>()) /
                                       255.0f);
      }
    } else {
      for (auto i = nodes.begin(); i != nodes.end(); ++i)
    ...
    if (list.size() && selected_node_count) {
      ...
      const float lower = node_colour_lower_button->value(),
                  upper = node_colour_upper_button->value();
      for (node_t i = 1; i <= num_nodes(); ++i) {
        if (selected_nodes[i]) {
          nodes[i].set_colour(node_selection_settings.get_node_selected_colour());
        } else {
          float min = std::numeric_limits<float>::infinity(), sum = 0.0f,
  ...
  update_node_overlay();
  // Need to indicate to the node list view that data have changed (specifically the node colour
  // pixmaps)
  ...
  if (node_alpha == node_alpha_t::FIXED) {
    ...
    if (list.size() && selected_node_count) {
      ...
      const float lower = node_alpha_lower_button->value(),
                  upper = node_alpha_upper_button->value();
...
void Connectome::calculate_edge_colours() {
  if (edge_colour == edge_colour_t::FIXED) {
    ...
    for (auto i = edges.begin(); i != edges.end(); ++i)
      i->set_colour(
          Eigen::Array3f{abs(i->get_dir()[0]), abs(i->get_dir()[1]), abs(i->get_dir()[2])});
    ...
    if (list.size()) {
      ...
      const float lower = edge_colour_lower_button->value(),
                  upper = edge_colour_upper_button->value();
      for (size_t i = 0; i != num_edges(); ++i) {
  ...
  if (edge_alpha == edge_alpha_t::FIXED) {
    ...
    if (list.size()) {
      ...
      const float lower = edge_alpha_lower_button->value(),
                  upper = edge_alpha_upper_button->value();
...
Eigen::Array3f Connectome::node_colour_given_selection(const node_t index) {
  if (selected_nodes[index]) {
    ...
    return ((fade * node_selection_settings.get_node_selected_colour()) +
    ...
    return ((fade * node_selection_settings.get_node_other_colour()) +
            ((1.0f - fade) * nodes[index].get_colour()));
  } else {
...
float Connectome::node_size_given_selection(const node_t index) {
  if (selected_nodes[index]) {
    ...
    for (auto e = edges.begin(); e != edges.end(); ++e) {
      if (e->is_visible() && (e->get_node_index(0) == index || e->get_node_index(1) == index) &&
          (selected_nodes[e->get_node_index(0)] || selected_nodes[e->get_node_index(1)])) {
        return (node_selection_settings.get_node_associated_size_multiplier() *
                nodes[index].get_size());
      }
    }
...
float Connectome::node_alpha_given_selection(const node_t index) {
  if (selected_nodes[index]) {
    return (node_selection_settings.get_node_selected_alpha_multiplier() *
            nodes[index].get_alpha());
  } else if (selected_node_count) {
    for (auto e = edges.begin(); e != edges.end(); ++e) {
      if (e->is_visible() && (e->get_node_index(0) == index || e->get_node_index(1) == index) &&
          (selected_nodes[e->get_node_index(0)] || selected_nodes[e->get_node_index(1)])) {
        return (node_selection_settings.get_node_associated_alpha_multiplier() *
                nodes[index].get_alpha());
      }
    }
...
void Connectome::update_controls_node_visibility(const float min,
                                                 const float mean,
                                                 const float max) {
...
void Connectome::update_controls_edge_visibility(const float min,
                                                 const float mean,
                                                 const float max) {
...
void Connectome::limit_min_max_controls(AdjustButton *const lower_button,
                                        AdjustButton *const upper_button) const {
...
void Connectome::update_control(AdjustButton *const button,
                                const float min,
                                const float mean,
                                const float max) {
...
void Connectome::get_meshes() {
  ...
  if (meshes.size() != nodes.size())
    throw Exception("Mesh file contains " + str(meshes.size()) + " objects; expected " +
                    str(nodes.size()));
  have_meshes = false;
  GL::Context::Grab context;
  for (node_t i = 1; i <= num_nodes(); ++i)
...
bool Connectome::use_alpha_nodes() const {
  ...
  if (selected_node_count &&
      (node_selection_settings.get_node_selected_alpha_multiplier() < 1.0f ||
...
bool Connectome::use_alpha_edges() const {
  ...
  if (selected_node_count &&
      (node_selection_settings.get_edge_selected_alpha_multiplier() < 1.0f ||
...
float Connectome::calc_line_width(const float desired_width, const bool is_smooth) const {
  if (is_smooth) {
    if (line_thickness_range_smooth[0] &&
        std::round(desired_width) < line_thickness_range_smooth[0])
      return line_thickness_range_smooth[0];
    if (line_thickness_range_smooth[1] &&
        std::round(desired_width) > line_thickness_range_smooth[1])
      return line_thickness_range_smooth[1];
    return desired_width;
  } else {
    if (line_thickness_range_aliased[0] &&
        std::round(desired_width) < line_thickness_range_aliased[0])
      return line_thickness_range_aliased[0];
    if (line_thickness_range_aliased[1] &&
        std::round(desired_width) > line_thickness_range_aliased[1])
