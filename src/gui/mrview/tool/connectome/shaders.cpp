...
void NodeShader::update(const Connectome &parent) {
  ...
  if (geometry == node_geometry_t::SPHERE || geometry == node_geometry_t::CUBE ||
      geometry == node_geometry_t::MESH) {
  ...
  if (!is_3D) {
    ...
    geometry_shader_source += "void main() {\n"
                              ...
                              "    if (mu >= 0.0 && mu <= 1.0) {\n"
                              "      gl_Position = gl_in[v1].gl_Position + (mu * "
                              "(gl_in[v2].gl_Position - gl_in[v1].gl_Position));\n"
                              "      normal" +
                              GS_out + " = normalize(((1.0 - mu) * normal" + GS_in +
                              "[v1]) + (mu * normal" + GS_in +
                              "[v2]));\n"
                              "      EmitVertex();\n"
                              "    }\n"
...
void EdgeShader::update(const Connectome &parent) {
  ...
  case edge_geometry_t::CYLINDER:
    ...
    if (use_lighting) {
      vertex_shader_source += "  normal" + GS_in +
                              " = normalize (mat3(MV) * (vertexNormal_modelspace * rot_matrix));\n";
    }
    break;
  ...
  if (!is_3D) {
    ...
    case edge_geometry_t::LINE:
    case edge_geometry_t::STREAMLINE:
      geometry_shader_source += "  float mu = depth[0] / (depth[0] - depth[1]);\n"
                                "  if (mu >= 0.0 && mu <= 1.0) {\n"
                                "    gl_Position = gl_in[0].gl_Position + (mu * "
                                "(gl_in[1].gl_Position - gl_in[0].gl_Position));\n";
      if (use_lighting) {
        geometry_shader_source += "    tangent" + GS_out + " = normalize(((1.0 - mu) * tangent" +
                                  GS_in + "[1]) + (mu * tangent" + GS_in + "[0]));\n";
      }
      if (colour == edge_colour_t::DIRECTION && geometry == edge_geometry_t::STREAMLINE) {
        geometry_shader_source += "      dir" + GS_out + " = normalize(((1.0 - mu) * dir" + GS_in +
                                  "[v1]) + (mu * dir" + GS_in + "[v2]));\n";
      }
    ...
    case edge_geometry_t::CYLINDER:
    case edge_geometry_t::STREAMTUBE:
      geometry_shader_source += "  for (int v1 = 0; v1 != 3; ++v1) {\n"
                                "    int v2 = (v1 == 2) ? 0 : v1+1;\n"
                                "    float mu = depth[v1] / (depth[v1] - depth[v2]);\n"
                                "    if (mu >= 0.0 && mu <= 1.0) {\n"
                                "      gl_Position = gl_in[v1].gl_Position + (mu * "
                                "(gl_in[v2].gl_Position - gl_in[v1].gl_Position));\n";
      if (use_lighting) {
        geometry_shader_source += "      normal" + GS_out + " = normalize(((1.0 - mu) * normal" +
                                  GS_in + "[v1]) + (mu * normal" + GS_in + "[v2]));\n";
      }
  ...
  if (colour == edge_colour_t::DIRECTION &&
    ...
    if (use_alpha) {
      fragment_shader_source += "  color.rgb = vec3 (abs(dir" + GS_out + "[0]), abs(dir" + GS_out +
                                "[1]), abs(dir" + GS_out + "[2]));\n";
    } else {
      fragment_shader_source += "  color = vec3 (abs(dir" + GS_out + "[0]), abs(dir" + GS_out +
                                "[1]), abs(dir" + GS_out + "[2]));\n";
    }
  ...
  if (use_lighting) {
    if (geometry == edge_geometry_t::CYLINDER || geometry == edge_geometry_t::STREAMTUBE) {
      fragment_shader_source +=
          "  color *= ambient + diffuse * clamp (dot (normal" + GS_out +
          ", light_pos), 0, 1);\n"
          "  color += specular * pow (clamp (dot (reflect (-light_pos, normal" +
          GS_out + "), vec3(0.0,0.0,1.0)), 0, 1), shine);\n";
    } else { // Line and streamline
