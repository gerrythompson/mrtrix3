...
Edge::Edge(const node_t one,
           const node_t two,
           const Eigen::Vector3f &c_one,
           const Eigen::Vector3f &c_two)
  ...
  if (is_diagonal()) {
    ...
    const Eigen::Matrix<float, 3, 3> matrix(
        Eigen::Quaternionf(Eigen::AngleAxisf(angle, v)).matrix());
...
Edge::Streamline::Streamline(const Exemplar &data) {
  ...
  if (data.vertices.size())
    gl::BufferData(gl::ARRAY_BUFFER,
                   data.vertices.size() * sizeof(Eigen::Vector3f),
                   &data.vertices[0][0],
                   gl::STATIC_DRAW);
  ...
  if (data.tangents.size())
    gl::BufferData(gl::ARRAY_BUFFER,
                   data.tangents.size() * sizeof(Eigen::Vector3f),
                   &data.tangents[0][0],
                   gl::STATIC_DRAW);
...
Edge::Streamtube::Streamtube(const Exemplar &data) : count(data.vertices.size()) {
  ...
  for (vector<Eigen::Vector3f>::const_iterator i = data.vertices.begin(); i != data.vertices.end();
       ++i) {
  ...
  for (vector<Eigen::Vector3f>::const_iterator i = data.tangents.begin(); i != data.tangents.end();
       ++i) {
    for (size_t j = 0; j != N; ++j)
  ...
  if (tangents.size())
    gl::BufferData(gl::ARRAY_BUFFER,
                   tangents.size() * sizeof(Eigen::Vector3f),
                   &tangents[0][0],
                   gl::STATIC_DRAW);
  ...
  for (size_t i = 0; i != shared.points_per_vertex(); ++i)
    normal_multipliers.push_back(
        std::make_pair(std::cos(i * angle_multiplier), std::sin(i * angle_multiplier)));
  vector<Eigen::Vector3f> normals;
  normals.reserve(vertices.size());
  for (size_t i = 0; i != data.vertices.size(); ++i) {
    for (vector<std::pair<float, float>>::const_iterator j = normal_multipliers.begin();
  ...
  if (normals.size())
    gl::BufferData(gl::ARRAY_BUFFER,
                   normals.size() * sizeof(Eigen::Vector3f),
                   &normals[0][0],
                   gl::STATIC_DRAW);
