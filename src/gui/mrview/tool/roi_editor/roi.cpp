...
void ROI::new_slot() {
  ...
  list_view->selectionModel()->select(
      list_model->index(list_model->rowCount() - 1, 0, QModelIndex()), QItemSelectionModel::Select);
  updateGL();
...
void ROI::open_slot() {
  vector<std::string> names =
      Dialog::File::get_images(this, "Select ROI images to open", &current_folder);
  if (names.empty())
    return;
...
int ROI::normal2axis(const Eigen::Vector3f &normal, const ROI_Item &roi) const {
  float x_dot_n =
      abs((roi.image2scanner().rotation().cast<float>() * Eigen::Vector3f{1.0f, 0.0f, 0.0f})
              .dot(normal));
  float y_dot_n =
      abs((roi.image2scanner().rotation().cast<float>() * Eigen::Vector3f{0.0f, 1.0f, 0.0f})
              .dot(normal));
  float z_dot_n =
      abs((roi.image2scanner().rotation().cast<float>() * Eigen::Vector3f{0.0f, 0.0f, 1.0f})
              .dot(normal));
  if (x_dot_n > y_dot_n)
...
void ROI::slice_copy_slot(QAction *action) {
  ...
  if (!proj)
    return;
  const Eigen::Vector3f current_origin =
  ...
  const int source_slice =
      current_slice + ((action == copy_from_above_button->defaultAction()) ? 1 : -1);
...
bool ROI::mouse_press_event() {
  ...
  if (brush_button->isChecked()) {
    if (brush_size_button->isMin())
      ...
      roi->current().draw_circle(
          *roi, current_origin, insert_mode_value, brush_size_button->value());
...
bool ROI::process_commandline_option(const MR::App::ParsedOption &opt) {
  ...
  if (opt.opt->is("roi.colour")) {
    try {
      ...
      if (values.size() != 3)
        throw Exception(
            "must provide exactly three comma-separated values to the -roi.colour option");
      const float max_value = std::max({values[0], values[1], values[2]});
      if (std::min({values[0], values[1], values[2]}) < 0.0 || max_value > 255)
        throw Exception("values provided to -roi.colour must be either between 0.0 and 1.0, or "
                        "between 0 and 255");
