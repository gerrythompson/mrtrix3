...
ROI_UndoEntry::ROI_UndoEntry(ROI_Item &roi, int current_axis, int current_slice) {
  ...
  gl::TexImage2D(
      gl::TEXTURE_2D, 0, gl::R8, tex_size[0], tex_size[1], 0, gl::RED, gl::UNSIGNED_BYTE, nullptr);
  GL_CHECK_ERROR;
...
void ROI_UndoEntry::draw_line(ROI_Item &roi,
  ...
  do {
    if (v[0] >= 0 && v[0] < roi.header().size(0) && v[1] >= 0 && v[1] < roi.header().size(1) &&
        v[2] >= 0 && v[2] < roi.header().size(2))
...
void ROI_UndoEntry::draw_fill(ROI_Item &roi,
                              const Eigen::Vector3f &pos,
                              const bool insert_mode_value) {
  ...
  const size_t seed_index =
      seed_voxel[0] - from[0] +
      size[0] * (seed_voxel[1] - from[1] + size[1] * (seed_voxel[2] - from[2]));
  ...
  while (buffer.size()) {
    ...
    for (size_t i = 0; i != 4; ++i) {
      ...
      if (adj[0] >= 0 && adj[0] < int(roi.header().size(0)) && adj[1] >= 0 &&
          adj[1] < int(roi.header().size(1)) && adj[2] >= 0 && adj[2] < int(roi.header().size(2))) {
        const size_t adj_index =
            adj[0] - from[0] + size[0] * (adj[1] - from[1] + size[1] * (adj[2] - from[2]));
