...
void Fixel::render_fixel_colourbar(const Tool::BaseFixel &fixel) {
  GL::assert_context_is_current();
  float min_value =
      fixel.use_discard_lower() ? fixel.scaling_min_thresholded() : fixel.scaling_min();

  float max_value =
      fixel.use_discard_upper() ? fixel.scaling_max_thresholded() : fixel.scaling_max();
...
void Fixel::add_images(vector<std::string> &list) {
  ...
  if (previous_size < new_size) {
    ...
    fixel_list_view->selectionModel()->select(QItemSelection(first, last),
...
void Fixel::update_gui_tracking_controls() {
  ...
  track_main_volume->setCheckState(
      num_checked ? (num_checked == n_images ? Qt::Checked : Qt::PartiallyChecked) : Qt::Unchecked);
}
...
void Fixel::line_thickness_slot(int thickness) {
  ...
  for (int i = 0; i < indices.size(); ++i)
    fixel_list_model->get_fixel_image(indices[i])
        ->set_line_thickness(static_cast<float>(thickness) / 1.0e5f);
  window().updateGL();
}
...
void Fixel::length_multiplier_slot() {
  ...
  for (int i = 0; i < indices.size(); ++i)
    fixel_list_model->get_fixel_image(indices[i])
        ->set_line_length_multiplier(length_multiplier->value());
  window().updateGL();
}
...
void Fixel::on_set_scaling_slot() {
  ...
  for (int i = 0; i < indices.size(); ++i)
    fixel_list_model->get_fixel_image(indices[i])
        ->set_windowing(min_value->value(), max_value->value());
  window().updateGL();
}
...
void Fixel::add_commandline_options(MR::App::OptionList &options) {
  ...
  options + OptionGroup("Fixel plot tool options")
      ...
      + Option("fixel.load",
               "Load a fixel file (any file inside a fixel directory, or an old .msf / .msh legacy "
               "format file) into the fixel tool.")
            .allow_multiple() +
