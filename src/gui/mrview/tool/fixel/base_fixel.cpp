...
BaseFixel::BaseFixel(const std::string &filename, Fixel &fixel_tool)
  ...
  voxel_size_length_multipler =
      0.45 * (header.spacing(0) + header.spacing(1) + header.spacing(2)) / 3;
}
  ...
  if (!fixel_tool.is_cropped_to_slab()) {
    ...
    for (size_t x = 0, N = slice_fixel_indices[0].size(); x < N; ++x) {
      if (slice_fixel_counts[0][x])
        gl::MultiDrawArrays(gl::POINTS,
                            &slice_fixel_indices[0][x][0],
                            &slice_fixel_sizes[0][x][0],
                            slice_fixel_counts[0][x]);
    }
  } else {
...
void BaseFixel::update_interp_image_buffer(const Projection &projection,
  ...
  Eigen::Vector3f y_width = projection.screen_to_model_direction(
      0.0f, projection.height() / 2.0f, projection.depth_of(p));
  ...
  for (int y = -ny; y <= ny; ++y) {
    for (int x = -nx; x <= nx; ++x) {
      ...
      std::array<int, 3> voxel{{(int)std::round(voxel_pos[0]),
                                (int)std::round(voxel_pos[1]),
                                (int)std::round(voxel_pos[2])}};
...
void BaseFixel::load_image(const std::string &filename) {
  ...
  const auto initial_col_key =
      std::find(colour_types.begin(), colour_types.end(), Path::basename(filename));
  ...
  gl::BufferData(gl::ARRAY_BUFFER,
                 pos_buffer_store.size() * sizeof(Eigen::Vector3f),
                 &(pos_buffer_store[0][0]),
                 gl::STATIC_DRAW);
  gl::EnableVertexAttribArray(0);
  gl::VertexAttribPointer(0, 3, gl::FLOAT, gl::FALSE_, 0, (void *)0);
...
void BaseFixel::reload_directions_buffer() {
  ...
  gl::BufferData(gl::ARRAY_BUFFER,
                 dir_buffer_store.size() * sizeof(Eigen::Vector3f),
                 &(dir_buffer_store)[0][0],
                 gl::STATIC_DRAW);
  gl::EnableVertexAttribArray(1);
  gl::VertexAttribPointer(1, 3, gl::FLOAT, gl::FALSE_, 0, (void *)0);
...
void BaseFixel::reload_values_buffer() {
  ...
  gl::BufferData(
      gl::ARRAY_BUFFER, val_buffer.size() * sizeof(float), &(val_buffer)[0], gl::STATIC_DRAW);
  gl::EnableVertexAttribArray(2);
  gl::VertexAttribPointer(2, 1, gl::FLOAT, gl::FALSE_, 0, (void *)0);
