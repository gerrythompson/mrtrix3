...
void Image4D::reload_image_buffer() {
  ...
  for (auto l = Loop(*fixel_data, 0, 3)(*fixel_data); l; ++l) {
    ...
    const std::array<int, 3> voxel{
        {int(fixel_data->index(0)), int(fixel_data->index(1)), int(fixel_data->index(2))}};
