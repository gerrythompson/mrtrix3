...
void ColourMapButton::init_core_menu_items(bool create_shortcuts) {
  ...
  connect(core_colourmaps_actions,
          SIGNAL(triggered(QAction *)),
          this,
          SLOT(select_colourmap_slot(QAction *)));
...
void ColourMapButton::init_customise_state_menu_items() {
  auto show_colour_bar =
      colourmap_menu->addAction(tr("Show colour bar"), this, SLOT(show_colour_bar_slot(bool)));
  show_colour_bar->setCheckable(true);
  show_colour_bar->setChecked(true);
  addAction(show_colour_bar);

  invert_scale_action =
      colourmap_menu->addAction(tr("Invert"), this, SLOT(invert_colourmap_slot(bool)));
...
void ColourMapButton::select_colour_slot() {
  QColor colour =
      QColorDialog::getColor(Qt::red, this, "Select Color", QColorDialog::DontUseNativeDialog);
