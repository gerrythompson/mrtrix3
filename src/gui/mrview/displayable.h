...
public:
  ...
  void set_colour(std::array<GLubyte, 3> &c) { colour = c; }
  ...
  void set_shader_variables(Shader &shader_program,
                            float scaling = 1.0,
    ...
    gl::Uniform1f(gl::GetUniformLocation(shader_program, (with_prefix + "scale").c_str()),
                  scaling / display_range);
    if (use_discard_lower())
      gl::Uniform1f(gl::GetUniformLocation(shader_program, (with_prefix + "lower").c_str()),
                    lessthan / scaling);
    if (use_discard_upper())
      gl::Uniform1f(gl::GetUniformLocation(shader_program, (with_prefix + "upper").c_str()),
                    greaterthan / scaling);
    if (use_transparency()) {
    ...
    if (ColourMap::maps[colourmap].is_colour)
      gl::Uniform3f(
          gl::GetUniformLocation(shader_program, (with_prefix + "colourmap_colour").c_str()),
          colour[0] / 255.0,
          colour[1] / 255.0,
          colour[2] / 255.0);
  }
