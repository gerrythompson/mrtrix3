...
void ColourBars::render(size_t colourmap,
  ...
  if (!program || !frame_program || colourmap != current_colourmap_index ||
      current_colourmap_inverted != inverted)
  ...
  float max_frac =
      std::min(std::max(0.0f, (local_max_value - global_min_value) / global_range), 1.0f);
  float min_frac =
      std::min(std::max(0.0f, (local_min_value - global_min_value) / global_range), max_frac);
  ...
  if (current_position & Position::Right) {
    x_offset = current_projection->width() -
               (max_bars_per_row - column_index) * (scaled_width + inset + colourbar_padding) +
               colourbar_padding;
    halign = 1;
  ...
  frame_program.start();
  gl::Uniform1f(gl::GetUniformLocation(frame_program, "scale_x"),
                2.0f / current_projection->width());
  gl::Uniform1f(gl::GetUniformLocation(frame_program, "scale_y"),
                2.0f / current_projection->height());
