...
public:
  Volume(MR::Header &&header)
      : Displayable(header.name()),
        ...
        interpolation(File::Config::get_bool("ImageInterpolation", true) ? gl::LINEAR
                                                                         : gl::NEAREST),
  ...
  Eigen::Transform<float, 3, Eigen::AffineCompact> voxel2scanner() const {
    ...
    return T.scale(Eigen::Vector3d(_header.spacing(0), _header.spacing(1), _header.spacing(2)))
        .cast<float>();
  }
  ...
  Eigen::Transform<float, 3, Eigen::AffineCompact> scanner2voxel() const {
    ...
    return T
        .prescale(Eigen::Vector3d(
            1.0 / _header.spacing(0), 1.0 / _header.spacing(1), 1.0 / _header.spacing(2)))
        .cast<float>();
  }
  ...
  float focus_rate() const {
    return 1.0e-3 * (std::pow(_header.size(0) * _header.spacing(0) * _header.size(1) *
                                  _header.spacing(1) * _header.size(2) * _header.spacing(2),
                              float(1.0 / 3.0)));
  }
  ...
  inline void upload_data(const std::array<ssize_t, 3> &x,
                          const std::array<ssize_t, 3> &size,
                          const void *data) {
    gl::TexSubImage3D(
        gl::TEXTURE_3D, 0, x[0], x[1], x[2], size[0], size[1], size[2], format, type, data);
  }
...
protected:
  ...
  void set_vertices_for_slice_render(const Projection &projection, float depth) {
    vertices[0] = projection.screen_to_model(
        projection.x_position(), projection.y_position() + projection.height(), depth);
    vertices[2] =
        projection.screen_to_model(projection.x_position(), projection.y_position(), depth);
    vertices[4] = projection.screen_to_model(
        projection.x_position() + projection.width(), projection.y_position(), depth);
    vertices[6] = projection.screen_to_model(projection.x_position() + projection.width(),
                                             projection.y_position() + projection.height(),
                                             depth);
