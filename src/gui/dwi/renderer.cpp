...
void Renderer::start(const Projection &projection,
  ...
  shader.start(mode,
               ...
               hide_neg_values,
               orthographic,
               colour_relative_to_projection);

  gl::UniformMatrix4fv(gl::GetUniformLocation(shader, "MV"), 1, gl::FALSE_, projection.modelview());
  gl::UniformMatrix4fv(
      gl::GetUniformLocation(shader, "MVP"), 1, gl::FALSE_, projection.modelview_projection());
  if (colour_relative_to_projection)
    gl::UniformMatrix4fv(
        gl::GetUniformLocation(shader, "rotation"), 1, gl::FALSE_, *colour_relative_to_projection);
...
void Renderer::Shader::start(mode_t mode,
  ...
  GL_CHECK_ERROR;
  if (!(*this) || mode != mode_ || use_lighting != use_lighting_ ||
...
std::string Renderer::Shader::vertex_shader_source() const {
  ...
  if (colour_by_direction_) {
    ...
    source += "  vertex_" + std::string(mode_ == mode_t::DIXEL ? "orig_" : "") +
              "color = constant_color;\n";
  }

  if (mode_ == mode_t::SH || mode_ == mode_t::TENSOR) {
    source += "  vec3 pos = " +
              std::string(mode_ == mode_t::TENSOR ? "new_vertex" : "vertex * amplitude") +
              " * scale;\n"
              "  if (reverse != 0)\n"
              "    pos = -pos;\n";
    if (orthographic_) {
...
std::string Renderer::Shader::fragment_shader_source() const {
  ...
  if (use_lighting_) {
    ...
    source += "  if (amplitude < 0.0)\n"
              ...
              "  final_color += specular * pow (clamp (dot (reflect (-light_pos, norm), "
              "normalize(vertex_position)), 0, 1), shine);\n";
  }
...
void Renderer::SH::initGL() {
  ...
  gl::VertexAttribPointer(
      0, 3, gl::FLOAT, gl::FALSE_, sizeof(Shapes::HalfSphere::Vertex), (void *)0);
...
void Renderer::SH::set_data(const vector_t &r_del_daz, int buffer_ID) const {
  ...
  surface_buffer.bind(gl::ARRAY_BUFFER);
  gl::BufferData(
      gl::ARRAY_BUFFER, r_del_daz.size() * sizeof(float), &r_del_daz[0], gl::STREAM_DRAW);
  gl::VertexAttribPointer(1, 3, gl::FLOAT, gl::FALSE_, 3 * sizeof(GLfloat), (void *)0);
}
  ...
  for (size_t n = 0; n < vertices.size(); ++n) {
    ...
    for (int m = 1; m <= lmax; m++) {
      ...
      for (int l = 2 * ((m + 1) / 2); l <= lmax; l += 2) {
        ...
        if (l > m)
          transform(3 * n + 1, idx) +=
              transform(3 * n, idx + 1) * sqrt(float((l - m) * (l + m + 1)));
...
void Renderer::Tensor::set_data(const vector_t &data, int buffer_ID) const {
  (void)buffer_ID; // to silence unused-parameter warnings
  // For tensor overlay, send the (inverse) tensor coefficients and colour directly to the shader as
  // a uniform
  assert(data.size() == 6);
  tensor_t D;
  D(0, 0) = data[0];
  ...
  if (data[0] <= 0.0f || data[1] <= 0.0f || data[2] <= 0.0f || Dinv.diagonal().minCoeff() < 0.0f) {
    ...
    gl::UniformMatrix3fv(
        gl::GetUniformLocation(parent.shader, "inv_tensor"), 1, gl::FALSE_, Dinv.data());
...
void Renderer::Dixel::update_dixels(const MR::DWI::Directions::Set &dirs) {
  ...
  gl::BufferData(gl::ARRAY_BUFFER,
                 dirs.size() * sizeof(Eigen::Vector3f),
                 &directions_data[0],
                 gl::STATIC_DRAW);
  gl::VertexAttribPointer(0, 3, gl::FLOAT, gl::FALSE_, 3 * sizeof(GLfloat), (void *)0);
  index_buffer.bind();
  gl::BufferData(gl::ELEMENT_ARRAY_BUFFER,
                 indices_data.size() * sizeof(std::array<GLint, 3>),
                 &indices_data[0],
                 gl::STATIC_DRAW);
  GL_CHECK_ERROR;
