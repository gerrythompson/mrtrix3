...
void init() {
  ...
  if (gl_version == 0) {
    WARN("unable to determine OpenGL version - operation may be unstable if actual version is less "
         "than 3.3");
  ...
  /*
          ...
          switch (status) {
            ...
            case gl::FRAMEBUFFER_INCOMPLETE_ATTACHMENT: throw Exception ("default framebuffer is
     incomplete!"); case gl::FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT: throw Exception ("default
     framebuffer has no image attached!"); case gl::FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER: throw
     Exception ("default framebuffer has incomplete draw buffer!"); case
     gl::FRAMEBUFFER_UNSUPPORTED: throw Exception ("default framebuffer does not support requested
     format!"); case gl::FRAMEBUFFER_COMPLETE: break; default: throw Exception ("default framebuffer
     cannot be used for unknown reason!");
          }
          */
}
