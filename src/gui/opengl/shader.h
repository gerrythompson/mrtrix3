...
public:
  ...
  static const std::string type() {
    return TYPE == gl::VERTEX_SHADER ? "vertex"
                                     : (TYPE == gl::FRAGMENT_SHADER ? "fragment" : "geometry");
  }
...
public:
  ...
  template <GLint TYPE> void attach(const Object<TYPE> &object) {
    ...
    GL_DEBUG("attached OpenGL " + Object<TYPE>::type() + " shader ID " + str(object.index_) +
             " to program ID " + str(index_));
  }
  template <GLint TYPE> void detach(const Object<TYPE> &object) {
    assert(index_);
    assert(object.index_);
    gl::DetachShader(index_, object.index_);
    GL_DEBUG("detached OpenGL " + Object<TYPE>::type() + " shader ID " + str(object.index_) +
             " from program ID " + str(index_));
  }
  void link() {
