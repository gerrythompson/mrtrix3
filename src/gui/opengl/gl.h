...
public:
  ...
  void attach_color(Texture &tex, size_t attachment) const {
    assert(tex);
    bind();
    GL_DEBUG("texture ID " + str(tex) + " attached to framebuffer ID " + str(id) +
             " at color attachement " + str(attachment));
    gl::FramebufferTexture(
        gl::FRAMEBUFFER, GLenum(size_t(gl::COLOR_ATTACHMENT0) + attachment), tex, 0);
  }
  ...
  void draw_buffers(size_t first, size_t second) const {
    check_context();
    GLenum list[2] = {GLenum(size_t(gl::COLOR_ATTACHMENT0) + first),
                      GLenum(size_t(gl::COLOR_ATTACHMENT0) + second)};
