...
public:
  ...
  void
  render_text_align(int x, int y, const std::string &text, int halign = 0, int valign = 0) const {
    QString s(qstr(text));
  ...
  void set(GL::Shader::Program &shader_program) const {
    ...
    gl::UniformMatrix4fv(
        gl::GetUniformLocation(shader_program, "MVP"), 1, gl::FALSE_, modelview_projection());
  }
