...
void Cylinder::LOD(const size_t level_of_detail) {
  ...
  for (int i = 0; i != N; ++i) {
    vertices.push_back(
        Eigen::Vector3f{std::cos(i * angle_multiplier), std::sin(i * angle_multiplier), 0.0f});
    normals.push_back(Eigen::Vector3f{0.0f, 0.0f, -1.0f});
    if (i == N - 1)
  ...
  for (int i = 0; i != N; ++i) {
    vertices.push_back(
        Eigen::Vector3f{std::cos(i * angle_multiplier), std::sin(i * angle_multiplier), 1.0f});
    normals.push_back(Eigen::Vector3f{0.0f, 0.0f, 1.0f});
    if (i == N - 1)
      indices.push_back(Eigen::Array3i{
          far_face_centre_index, far_face_centre_index + 1, far_face_centre_index + i});
    else if (i)
      indices.push_back(Eigen::Array3i{
          far_face_centre_index, far_face_centre_index + i + 1, far_face_centre_index + i});
  }
  ...
  gl::BufferData(
      gl::ARRAY_BUFFER, normals.size() * sizeof(Eigen::Vector3f), &normals[0][0], gl::STATIC_DRAW);

  num_indices = 3 * indices.size();
  index_buffer.gen();
  index_buffer.bind();
  gl::BufferData(gl::ELEMENT_ARRAY_BUFFER,
                 indices.size() * sizeof(Eigen::Array3i),
                 &indices[0],
                 gl::STATIC_DRAW);
}
