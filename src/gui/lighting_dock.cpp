...
LightingSettings::LightingSettings(QWidget *parent, GL::Lighting &lighting)
  ...
  elevation_slider->setSliderPosition(
      int((1000.0 / Math::pi) *
          acos(-info.lightpos[1] / Eigen::Map<Eigen::Matrix<float, 3, 1>>(info.lightpos).norm())));
  ...
  azimuth_slider->setSliderPosition(
      int((1000.0 / Math::pi) * atan2(info.lightpos[0], info.lightpos[2])));
