...
public:
  Fixel_map(const Header &H)
      : _header(H), _accessor(Image<MapVoxel *>::scratch(_header, "fixel map voxels")) {
    ...
    // This allows index 0 to be used as an error code, simplifying the implementation of MapVoxel
    // and Iterator
...
public:
  ...
  MapVoxel(const size_t first, const size_t size)
      : first_fixel_index(first), count(size), lookup_table(nullptr) {}
