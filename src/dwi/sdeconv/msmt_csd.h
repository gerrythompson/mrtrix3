    ...
    void init() {
      ...
      for (size_t t = 0; t != num_tissues(); ++t) {
        ...
        responses[t].conservativeResizeLike(
            Eigen::MatrixXd::Zero(num_shells(), Math::ZSH::NforL(lmax[t])));
      }
      ...
      INFO("initialising multi-tissue CSD for " + str(num_tissues()) + " tissue types, with " +
           str(nparams) + " parameters");
      ...
      problem = Math::ICLS::Problem<double>(C,
                                            A,
                                            Eigen::VectorXd(),
                                            0,
