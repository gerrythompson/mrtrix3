...
const OptionGroup ResampleOption =
    ...
    Argument("num").type_integer(2) + Argument("start").type_sequence_float() +
    ...
    Argument("num").type_integer(2) + Argument("start").type_sequence_float() +
    Argument("mid").type_sequence_float() + Argument("end").type_sequence_float();
...
Base *get_resampler() {
  const size_t count =
      (get_options("upsample").size() ? 1 : 0) + (get_options("downsample").size() ? 1 : 0) +
