...
const OptionGroup AssignmentOptions =
    ...
    + Option("assignment_radial_search",
             ...
             "Argument is the maximum radius in mm; if no node is found within this radius, the "
    ...
    + Option("assignment_reverse_search",
             "traverse from each streamline endpoint inwards along the streamline, in search of "
             "the last node traversed by the streamline. "
             "Argument is the maximum traversal length in mm (set to 0 to allow search to continue "
             "to the streamline midpoint).") +
    ...
    + Option("assignment_all_voxels",
             ...
             "(note that this means a streamline may be assigned to more than two nodes, or indeed "
             "none at all)");
...
void setup_metric(Metric &metric, Image<node_t> &nodes_data) {
  ...
  if (opt.size()) {
    try {
      ...
      throw Exception(e,
                      "-scale_file option expects a file containing a list of numbers (one for "
                      "each streamline); "
                      "file \"" +
