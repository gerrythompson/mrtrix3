...
bool Selector::operator()(const NodePair &nodes) const {
  ...
  if (exact_match && list.size() == 2)
    return ((nodes.first == list[0] && nodes.second == list[1]) ||
            (nodes.first == list[1] && nodes.second == list[0]));
    ...
    for (size_t i = 0; i != nodes.size(); ++i) {
      ...
      for (size_t j = i; j != nodes.size(); ++j) {
        ...
        exemplars.push_back(Exemplar(
            index++, length, std::make_pair(one, two), std::make_pair(COMs[one], COMs[two])));
      }
    }
  } else {
...
void WriterExtraction::add(const vector<node_t> &list,
                           const std::string &path,
                           const std::string weights_path = "") {
