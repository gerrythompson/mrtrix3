...
node_t Tck2nodes_end_voxels::select_node(const Tractography::Streamline<> &tck,
                                         Image<node_t> &v,
...
void Tck2nodes_radial::initialise_search() {
  const int max_axis_offset(
      std::ceil((max_dist + max_add_dist) /
                std::min({nodes.spacing(0), nodes.spacing(1), nodes.spacing(2)})));
  ...
  for (offset[2] = -max_axis_offset; offset[2] <= +max_axis_offset; ++offset[2]) {
    for (offset[1] = -max_axis_offset; offset[1] <= +max_axis_offset; ++offset[1]) {
      for (offset[0] = -max_axis_offset; offset[0] <= +max_axis_offset; ++offset[0]) {
        const default_type dist = std::sqrt(Math::pow2(offset[2] * nodes.spacing(2)) +
                                            Math::pow2(offset[1] * nodes.spacing(1)) +
...
node_t Tck2nodes_radial::select_node(const Tractography::Streamline<> &tck,
                                     Image<node_t> &v,
                                     const bool end) const {
  ...
  const Eigen::Vector3d p = (end ? tck.back() : tck.front()).cast<default_type>();
  const Eigen::Vector3d v_float = transform->scanner2voxel * p;
  const voxel_type centre{
      int(std::round(v_float[0])), int(std::round(v_float[1])), int(std::round(v_float[2]))};

  for (vector<voxel_type>::const_iterator offset = radial_search.begin();
       offset != radial_search.end();
       ++offset) {

    const voxel_type this_voxel(centre + *offset);
    const Eigen::Vector3d p_voxel(transform->voxel2scanner *
                                  this_voxel.matrix().cast<default_type>());
...
node_t Tck2nodes_revsearch::select_node(const Tractography::Streamline<> &tck,
                                        Image<node_t> &v,
                                        const bool end) const {
...
node_t Tck2nodes_forwardsearch::select_node(const Tractography::Streamline<> &tck,
                                            Image<node_t> &v,
                                            const bool end) const {
  ...
  if (tck.size() > 2) {
    const Eigen::Vector3d second_last_step =
        (tck[index + step] - tck[index + 2 * step]).cast<default_type>();
...
default_type Tck2nodes_forwardsearch::get_cf(const Eigen::Vector3d &p,
...
void Tck2nodes_all_voxels::select_nodes(const Streamline<> &tck,
                                        Image<node_t> &v,
                                        vector<node_t> &out) const {
