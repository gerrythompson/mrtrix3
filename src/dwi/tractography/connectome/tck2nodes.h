...
protected:
  ...
  virtual node_t
  select_node(const Tractography::Streamline<> &tck, Image<node_t> &v, const bool end) const {
  ...
  public:
    ...
    bool operator<(const Eigen::Array<int, 3, 1> &that) const {
      return ((*this)[2] == that[2])
                 ? (((*this)[1] == that[1]) ? ((*this)[0] < that[0]) : ((*this)[1] < that[1]))
                 : ((*this)[2] < that[2]);
    }
  };
};
...
private:
  node_t
  select_node(const Tractography::Streamline<float> &, Image<node_t> &, const bool) const override;
};
...
public:
  Tck2nodes_radial(const Image<node_t> &nodes_data, const default_type radius)
      : Tck2nodes_base(nodes_data, true),
        max_dist(radius),
        max_add_dist(std::sqrt(Math::pow2(0.5 * nodes.spacing(2)) +
                               Math::pow2(0.5 * nodes.spacing(1)) +
...
private:
  node_t
  select_node(const Tractography::Streamline<> &, Image<node_t> &, const bool) const override;

  void initialise_search();
  vector<voxel_type> radial_search;
  const default_type max_dist;
  // Distances are sub-voxel from the precise streamline termination point, so the search order is
  // imperfect.
  //   This parameter controls when to stop the radial search because no voxel within the search
  //   space can be closer than the closest voxel with non-zero node index processed thus far.
...
public:
  ...
  Tck2nodes_revsearch(const Tck2nodes_revsearch &that)
      : Tck2nodes_base(that), max_dist(that.max_dist) {}
...
// Forward search - form a diamond-like shape emanating from the streamline endpoint in the
