...
template <typename T> void Matrix<T>::write_assignments(const std::string &path) const {
  if (!track_assignments)
    throw Exception(
        "Cannot write streamline assignments to file as they were not stored during processing");
...
void Matrix<T>::save(const std::string &path,
  ...
  Eigen::IOFormat fmt(Eigen::FullPrecision,
                      Eigen::DontAlignCols,
                      std::string(1, Path::delimiter(path)),
                      "\n",
                      "",
                      "",
                      "",
...
template <typename T>
void Matrix<T>::apply_data(const size_t node_one,
                           const size_t node_two,
                           const T value,
                           const T weight) {
  assert(mat2vec);
