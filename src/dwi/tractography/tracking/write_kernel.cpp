...
bool WriteKernel::operator()(const GeneratedTrack &tck) {
  ...
  if (tck.size() && output_seeds) {
    ...
    (*output_seeds) << str(writer.count) << "," << str(tck.get_seed_index()) << "," << str(p[0])
                    << "," << str(p[1]) << "," << str(p[2]) << ",\n";
  }
  ...
  progress.update(
      [&]() {
        return printf("%8" PRIu64 " seeds, %8" PRIu64 " streamlines, %8" PRIu64 " selected",
                      seeds,
                      streamlines,
                      selected);
      },
  ...
  if (early_exit(seeds, selected)) {
    WARN("Track generation terminating prematurely: Highly unlikely to reach target number of "
         "streamlines (p<" +
