...
const OptionGroup TrackOption =
    ...
    + Option("step",
             ...
             "for first-order algorithms, " +
                 str(Defaults::stepsize_voxels_firstorder, 2) + " x voxelsize; " +
                 "if using RK4, " + str(Defaults::stepsize_voxels_rk4, 2) +
                 " x voxelsize; "
                 "for iFOD2: " +
    ...
    + Option("cutoff",
             ...
             "(defaults: " +
                 ...
                 "threshold multiplied by " + str(Defaults::cutoff_act_multiplier) +
                 " when using ACT).") +
    Argument("value").type_float(0.0)
...
void load_streamline_properties_and_rois(Properties &properties) {
  ...
  if (get_options("include_ordered").size() && !get_options("seed_unidirectional").size())
    throw Exception(
        "-include_ordered requires that -seed_unidirectional is set, but this is not so");
