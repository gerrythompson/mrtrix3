...
SharedBase::SharedBase(const std::string &diff_path, Properties &property_set)
    : source(Image<float>::open(diff_path).with_direct_io(3)),
      ...
      ,
      debug_header(
          Header::open(properties.find("act") == properties.end() ? diff_path : properties["act"])),
      transform(debug_header)
#endif
{
  if (properties.find("max_num_tracks") == properties.end())
    max_num_tracks =
        (properties.find("max_num_seeds") == properties.end()) ? Defaults::num_selected_tracks : 0;
  ...
  for (size_t i = 0; i != TERMINATION_REASON_COUNT; ++i) {
    ...
    debug_images[i] =
        new Image<uint32_t>(Image<uint32_t>::create("terms_" + name + ".mif", debug_header));
  }
#endif
}
...
void SharedBase::set_step_and_angle(const float voxel_frac,
  ...
  min_dist = is_act() ? (Defaults::minlength_voxels_withact * vox())
...
void SharedBase::set_num_points(const float angle_minradius_preds, const float max_step_postds) {
  ...
  DEBUG(
      "For tracking step size " + str(step_size) + "mm, " +
      (std::isfinite(max_angle_ho)
           ? ("max change in fibre orientation angle per step " +
              str(max_angle_ho * 180.0 / Math::pi, 6) + " deg (using RK4)")
      ...
      str(downsampler.get_ratio()) + ": " + "minimum length of " + str(min_dist) +
      "mm requires at least " + str(min_num_points_preds) +
      " vertices pre-DS, is tested explicitly for " + str(min_num_points_postds) +
      " vertices or less post-DS; " + "maximum length of " + str(max_dist) +
      "mm will stop tracking after " + str(max_num_points_preds) +
      " vertices pre-DS, is tested explicitly for " + str(max_num_points_postds) +
