...
// If this is enabled, images will be output in the current directory showing the density of
// streamline terminations due to different termination mechanisms throughout the brain
...
public:
  ...
  // This gets overloaded for iFOD2, as each sample is output rather than just each step, and there
  // are
  ...
  void add_rejection(const reject_t i) const {
    rejections[i].fetch_add(1, std::memory_order_relaxed);
  }
