...
public:
  static void run(const std::string &diff_path,
    ...
    if (properties.find("seed_dynamic") == properties.end()) {
      ...
      Thread::run_queue(
          Thread::multi(tracker), Thread::batch(GeneratedTrack(), TRACKING_BATCH_SIZE), writer);
      ...
      if (max_num_tracks.empty())
        throw Exception("Dynamic seeding requires setting the desired number of tracks using the "
                        "-select option");
...
private:
  ...
  bool seed_track(GeneratedTrack &tck) {
    ...
    if (S.properties.seeds.is_finite()) {
      ...
      FAIL("Failed to find suitable seed point after " + str(MAX_NUM_SEED_ATTEMPTS) +
           " attempts - aborting");
      return false;
    }
  }
  ...
  void apply_priors(term_t &termination) {
    ...
    if (S.is_act()) {
      ...
      case EXIT_SGM:
      case TERM_IN_SGM:
        throw Exception("\nFIXME: Have received ACT-based termination for non-ACT tracking in "
                        "apply_priors()\n");
  ...
  bool track_rejected(const GeneratedTrack &tck) {
    ...
    // seedtest algorithm uses min_num_points_preds = 1; should be 2 or more for all other
    // algorithms
  ...
  bool satisfy_wm_requirement(const vector<Eigen::Vector3f> &tck) {
    // If using the Seed_test algorithm (indicated by max_num_points == 2), don't want to execute
    // this check
    ...
    if (method.act().seed_in_sgm && !method.act().sgm_seed_to_wm)
      return false;
    // Used these in the ACT paper, but wasn't entirely happy with the method; can change these
    // #defines to re-enable ACT instead now defaults to a 2-voxel minimum length
    ...
    for (const auto &i : tck) {
      if (method.act().fetch_tissue_data(i)) {
        ...
        if (((integral += (Math::pow2(wm) * S.internal_step_size())) > ACT_WM_INT_REQ) &&
            (max_value > ACT_WM_ABS_REQ))
          return true;
      }
    }
    return false;
  ...
  void truncate_maxlength(GeneratedTrack &tck) {
    ...
    //   - Re-run this function recursively in order to truncate from the opposite end of the
    //   streamline
    ...
    tck.resize(index + 1);
    tck[index] =
        tck[index - 1] + ((tck[index] - tck[index - 1]).normalized() * (S.max_dist - length_sum));
  }
