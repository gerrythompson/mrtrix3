...
bool EarlyExit::operator()(const size_t num_seeds, const size_t num_tracks) {
  ...
  const Eigen::Array<default_type, 2, 1> b(
      {default_type(num_tracks + 1), default_type(num_tracks + 1)});
  const Eigen::Array<default_type, 2, 1> x(
      {1.0 - (max_num_tracks / default_type(max_num_seeds)), 1.0});
  ...
  DEBUG("tckgen early exit: Target " + str(max_num_tracks) + "/" + str(max_num_seeds) + " (" +
        str(max_num_tracks / default_type(max_num_seeds), 3) + "), " + "current " +
        str(num_tracks) + "/" + str(num_seeds) + " (" +
        str(num_tracks / default_type(num_seeds), 3) + "), " + "conditional probability " +
        str(conditional, 3) + ", hypothesis prior probability " + str(prob_hypothesis_prior, 3) +
        ", " + "observation probability " + str(prob_observation, 3) + ", posterior " +
        str(posterior, 3));
  ...
  DEBUG("tckgen early exit: Target " + str(max_num_tracks) + "/" + str(max_num_seeds) + " (" +
        str(target_ratio, 3) + "), " + "current " + str(num_tracks) + "/" + str(num_seeds) + " (" +
        str(current_ratio, 3) + "), " + "variance " + str(variance, 3) + ", threshold " +
        str(threshold, 3));
