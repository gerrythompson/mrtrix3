...
void MethodBase::truncate_track(GeneratedTrack &tck,
...
bool MethodBase::check_seed() {
  ...
  if ((S.properties.mask.size() && !S.properties.mask.contains(pos)) ||
      (S.properties.exclude.contains(pos)) || (S.is_act() && !act().check_seed(pos))) {
    pos = {NaN, NaN, NaN};
    return false;
  }
...
Eigen::Vector3f MethodBase::rotate_direction(const Eigen::Vector3f &reference,
