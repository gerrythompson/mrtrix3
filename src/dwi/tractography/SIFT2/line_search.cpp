...
LineSearchFunctor::LineSearchFunctor(const SIFT::track_t index, const TckFactor &tckfactor)
    : track_index(index),
      ...
      // Pre-scale reg_tv by total streamline contribution; each fixel then contributes (PM *
      // length),
      //   and the whole thing is appropriately normalised
      reg_tv(tckfactor.reg_multiplier_tv /
             tckfactor.contributions[track_index]->get_total_contribution()) {
...
LineSearchFunctor::Result LineSearchFunctor::get(const double dFs) const {
  ...
  for (vector<Fixel>::const_iterator i = fixels.begin(); i != fixels.end(); ++i) {
    ...
    data_result.third_deriv +=
        2.0 * i->PM * i->cost_frac * scaled_contribution * ((3.0 * roc_contribution) + diff);
