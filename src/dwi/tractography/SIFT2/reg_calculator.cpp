...
RegularisationCalculator::RegularisationCalculator(TckFactor &tckfactor,
                                                   double &cf_reg_tik,
                                                   double &cf_reg_tv)
    : master(tckfactor),
...
bool RegularisationCalculator::operator()(const SIFT::TrackIndexRange &range) {
  for (SIFT::track_t track_index = range.first; track_index != range.second; ++track_index) {
    ...
    for (size_t j = 0; j != this_contribution.dim(); ++j) {
      ...
      this_tv_sum += fixel.get_weight() * this_contribution[j].get_length() *
                     contribution_multiplier * fixel_coeff_cost;
    }
    tv_sum += this_tv_sum;
  }
  return true;
