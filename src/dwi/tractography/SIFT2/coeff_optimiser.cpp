...
#ifdef SIFT2_COEFF_OPTIMISER_DEBUG
  fprintf(stderr,
          "%ld of %ld initial searches failed, %ld in wrong direction, %ld steps truncated, %ld "
...
CoefficientOptimiserGSS::CoefficientOptimiserGSS(TckFactor &tckfactor,
                                                 ...
                                                 BitSet &fixels_to_exclude,
                                                 double &sum_costs)
    : CoefficientOptimiserBase(tckfactor,
                               step_stats,
                               coefficient_stats,
                               nonzero_streamlines,
                               fixels_to_exclude,
...
double CoefficientOptimiserQLS::get_coeff_change(const SIFT::track_t track_index) const {
  ...
  if (!std::isfinite(dFs)) {
    ...
    if (dFs > 0.99 * master.max_coeff_step && line_search_functor(master.max_coeff_step) < cost)
      dFs = master.max_coeff_step;
    else if (dFs < -0.99 * master.max_coeff_step &&
             line_search_functor(-master.max_coeff_step) < cost)
      dFs = -master.max_coeff_step;
#ifdef SIFT2_COEFF_OPTIMISER_DEBUG
    ++failed;
#endif
...
CoefficientOptimiserIterative::CoefficientOptimiserIterative(
    const CoefficientOptimiserIterative &that)
    : CoefficientOptimiserBase(that)
#ifdef SIFT2_COEFF_OPTIMISER_DEBUG
      ,
      iter_count(0)
...
double CoefficientOptimiserIterative::get_coeff_change(const SIFT::track_t track_index) const {
  ...
  do {
    ...
    // const double denominator = (2.0 * Math::pow2 (result.second_deriv)) - (result.first_deriv *
    // result.third_deriv); change = denominator ? (-numerator / denominator) : 0.0;
