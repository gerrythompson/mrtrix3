...
void check_counts(const Properties &a,
                  const Properties &b,
  ...
  auto f = [](const ROISetBase &rois, const std::string &type, const float threshold) {
    for (size_t i = 0; i != rois.size(); ++i) {
      if (rois[i].min_featurelength() < threshold) {
        WARN("Streamline step size is large compared to " + type + " ROI \"" +
             rois[i].parameters() + "; " +
...
std::ostream &operator<<(std::ostream &stream, const Properties &P) {
  ...
  stream << "include: " << P.include << ", ordered_include: " << P.ordered_include
         << ", exclude: " << P.exclude << ", mask: " << P.mask << ", dict: ";
