...
public:
  ...
  public:
    Shared(const std::string &diff_path, DWI::Tractography::Properties &property_set)
      ...
      // max_angle_1o needs to be set because it influences the cone in which FOD amplitudes are
      // sampled
      if (rk4) {
...
protected:
  ...
  public:
    ...
    float operator()(float el) {
      return std::pow(
          Math::SH::value(P.values, Eigen::Vector3f(std::sin(el), 0.0, std::cos(el)), P.S.lmax),
          P.S.fod_power);
    }
