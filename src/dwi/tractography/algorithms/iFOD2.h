...
public:
  ...
  public:
    Shared(const std::string &diff_path, DWI::Tractography::Properties &property_set)
      ...
      const float angle_minradius_preds =
          2.0 * std::asin(step_size / (2.0 * min_radius)) / float(num_samples);
    ...
    ~Shared() {
      ...
      if (mean_truncations) {
        INFO("mean number of steps between rejection sampling truncations = " +
    ...
    void update_stats(double mean_samples_per_run,
                      double mean_truncations_per_run,
                      double max_truncation) const {
  ...
  void truncate_track(GeneratedTrack &tck,
                      const size_t length_to_revert_from,
                      const size_t revert_step) override {
    // OK, if we know length_to_revert_from, we can reconstruct what sample_idx was at that point
    size_t sample_idx_at_full_length =
        (length_to_revert_from - tck.get_seed_index()) % S.num_samples;
    ...
    if (S.is_act())
      act().sgm_depth =
          (act().sgm_depth > points_to_remove) ? act().sgm_depth - points_to_remove : 0;
  }
...
private:
  ...
  // Generate an arc only when required, and on the majority of next() calls, simply return the next
  // point
  //   in the arc - more dense structural image sampling
  size_t sample_idx;

  FORCE_INLINE float FOD(const Eigen::Vector3f &direction) const {
    return (S.precomputer ? S.precomputer.value(values, direction)
...
protected:
  void get_path(vector<Eigen::Vector3f> &positions,
    ...
    if (theta) {
      ...
      positions[S.num_samples - 1] =
          pos + R * (std::sin(theta) * dir + (float(1.0) - cos_theta) * curv);
...
private:
  ...
  public:
    Calibrate(iFOD2 &method)
      ...
      init_log_prob =
          0.5 * std::log(Math::SH::value(P.values, Eigen::Vector3f(0.0, 0.0, 1.0), P.S.lmax));
    }
