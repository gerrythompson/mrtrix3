...
public:
  ...
  double stageShift(const Particle *par, const Point_t &pos, const Point_t &dir) {
    ...
    if (par->hasPredecessor()) {
      ...
      ep -= Particle::L * dir;
      dEint += calcEnergy(
          pos, ep, par->getPredecessor()->getPosition(), par->getPredecessor()->getEndPoint(a));
    }
    if (par->hasSuccessor()) {
      ...
      ep += Particle::L * dir;
      dEint += calcEnergy(
          pos, ep, par->getSuccessor()->getPosition(), par->getSuccessor()->getEndPoint(a));
    }
...
protected:
  ...
  double calcEnergy(const Particle *P1, const int ep1, const Particle *P2, const int ep2) {
    return calcEnergy(
        P1->getPosition(), P1->getEndPoint(ep1), P2->getPosition(), P2->getEndPoint(ep2));
  }

  double
  calcEnergy(const Point_t &pos1, const Point_t &ep1, const Point_t &pos2, const Point_t &ep2) {
    Point_t Xm = (pos1 + pos2) * 0.5; // midpoint between both segments
    double Ucon =
        ((ep1 - Xm).squaredNorm() + (ep2 - Xm).squaredNorm()) / (Particle::L * Particle::L);
    return Ucon - cpot;
  }
