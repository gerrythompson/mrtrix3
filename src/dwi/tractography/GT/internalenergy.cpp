...
void InternalEnergyComputer::scanNeighbourhood(const Particle *p,
                                               const int alpha0,
  ...
  for (int i = -1; i <= 1; i++) {
    for (int j = -1; j <= 1; j++) {
      for (int k = -1; k <= 1; k++) {
        ...
        for (ParticleGrid::ParticleVectorType::const_iterator it = pvec->begin(); it != pvec->end();
             ++it) {
          pe.par = *it;
          if (pe.par == p)
            continue;
          ...
          if ((pe.alpha == -1)
                  ...
                  : (pe.par->hasSuccessor() &&
                     pe.par->getSuccessor() != p)) // Exclude connected endpoints, unless they are
                                                   // connected to the current particle.
            continue;
...
ParticleEnd InternalEnergyComputer::pickNeighbour() {
  ...
  auto neighbour =
      std::find_if(neighbourhood.begin(), neighbourhood.end(), [&](const ParticleEnd &particle) {
        sum += particle.p_suc;
        return sum >= t;
      });
