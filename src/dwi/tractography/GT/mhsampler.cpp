...
void MHSampler::birth() {
  ...
  double R =
      std::exp(-dE) * props.density / (pGrid.getTotalCount() + 1) * props.p_death / props.p_birth;
  if (R > rng_uniform()) {
    E->acceptChanges();
    pGrid.add(pos, dir);
    stats.incNa('b');
...
bool MHSampler::inMask(const Point_t p) {
  if ((p[0] <= -0.5) || (p[0] >= dims[0] - 0.5) || (p[1] <= -0.5) || (p[1] >= dims[1] - 0.5) ||
...
void MHSampler::moveRandom(const Particle *par, Point_t &pos, Point_t &dir) {
  pos = par->getPosition() +
        Point_t(rng_normal() * sigpos, rng_normal() * sigpos, rng_normal() * sigpos);
  dir = par->getDirection() +
        Point_t(rng_normal() * sigdir, rng_normal() * sigdir, rng_normal() * sigdir);
  dir.normalize();
}
