...
public:
  ...
  double get_diff(const double mu) const {
    return ((TD.load(std::memory_order_relaxed) * mu) - FOD);
  }
  ...
  float get_ratio(const double mu) const {
    return ((mu * TD.load(std::memory_order_relaxed)) / FOD);
  }
  ...
  float get_cumulative_prob(const uint64_t track_count) {
    ...
    if (track_count > track_count_at_last_update) {
      cumulative_prob = ((track_count_at_last_update * old_prob) +
                         ((track_count - track_count_at_last_update) * applied_prob)) /
                        float(track_count);
...
private:
  ...
  std::atomic<double> TD; // Protect against concurrent reads & writes, though perfect thread
                          // concurrency is not necessary
  bool update; // For small / noisy fixels, exclude the seeding probability from being updated

  // Multiple values to update - use an atomic boolean in a similar manner to a mutex, but with less
  // overhead
