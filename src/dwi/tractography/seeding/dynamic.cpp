...
Dynamic::~Dynamic() {
  ...
  for (size_t fixel_index = 1; fixel_index != fixels.size(); ++fixel_index) {
    ...
    if (!force_seed) {
      ...
      seed_prob =
          cumulative_prob * (current_trackcount / float(target_trackcount - current_trackcount)) *
  ...
  auto out_probs = buffer_probs.voxel(), out_logprobs = buffer_logprobs.voxel(),
       out_ratios = buffer_ratios.voxel();
  ...
  while (1) {
    ...
    if (fixel.can_update()) {
      ...
      if (!force_seed) {
        ...
        // Target track count is double the current track count, until this exceeds the actual
        // target number
        ...
        seed_prob = (ratio < 1.0) ? (cumulative_prob * (Szero - (current_trackcount * ratio)) /
                                     (ratio * (Szero - current_trackcount)))
                                  : 0.0;

#ifdef DYNAMIC_SEED_DEBUGGING
        // if (fixel_index == test_fixel)
        //   std::cerr << "Ratio " << ratio << ", tracks " << current_trackcount << "/" <<
        //   target_trackcount << ", seeds " << fixel_seeds << "/" << total_seeds << ", prob " <<
        //   cumulative_prob << " -> " << seed_prob << "\n";
#endif
    ...
    if (seed_prob > uniform_float(rng)) {
      ...
      const Eigen::Vector3f vp(v[0] + uniform_float(rng) - 0.5,
                               v[1] + uniform_float(rng) - 0.5,
                               v[2] + uniform_float(rng) - 0.5);
...
void Dynamic::output_fixel_images() {
  ...
  auto out_probs = buffer_probs.voxel(), out_logprobs = buffer_logprobs.voxel(),
       out_ratios = buffer_ratios.voxel(), out_FDs = buffer_FDs.voxel(),
       out_TDs = buffer_TDs.voxel();
  VoxelAccessor v(accessor);
  Image::Loop loop;
  for (auto loopiter = loop(v, out_probs, out_logprobs, out_ratios, out_FDs, out_TDs); loopiter;
       ++loopiter) {
    if (v.value()) {
      out_probs.value().set_size((*v.value()).num_fixels());
      out_logprobs.value().set_size((*v.value()).num_fixels());
  ...
  for (loop.start(v, out); loop.ok(); loop.next(v, out)) {
    if (v.value()) {
      ...
      for (Fixel_map<Fixel_TD_seed>::ConstIterator f = begin(v); f; ++f, ++index) {
        Image::Fixel::Legacy::FixelMetric fixel(
            f().get_dir(), f().get_FOD(), (f().can_update() ? 1.0f : 0.0f));
...
bool WriteKernelDynamic::operator()(const Tracking::GeneratedTrack &in,
                                    Tractography::Streamline<> &out) {
  ...
  if (!WriteKernel::operator()(in)) {
    ...
    out.weight = 0.0f;
    // Actually need to pass this down the queue so that the seeder thread receives it and knows to
    // terminate
    return true;
  }
  out = in;
