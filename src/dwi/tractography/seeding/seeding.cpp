...
const OptionGroup SeedMechanismOption =
    ...
    +
    Option("seed_sphere", "spherical seed as four comma-separated values (XYZ position and radius)")
        .allow_multiple() +
    Argument("spec").type_sequence_float()

    + Option("seed_random_per_voxel",
             "seed a fixed number of streamlines per voxel in a mask image; random placement of "
             "seeds in each voxel")
          .allow_multiple() +
    Argument("image").type_image_in() + Argument("num_per_voxel").type_integer(1)

    + Option("seed_grid_per_voxel",
             "seed a fixed number of streamlines per voxel in a mask image; place seeds on a 3D "
             "mesh grid "
    ...
    + Option(
          ...
          "Input image should be a 3D seeding volume; seeds drawn within this image will be "
          "optimised to the "
    ...
    Option(
        "seed_dynamic",
        "determine seed points dynamically using the SIFT model (must not provide any other "
        "seeding mechanism). "
        "Note that while this seeding mechanism improves the distribution of reconstructed "
        "streamlines density, "
...
const OptionGroup SeedParameterOption =
    ...
    + Option("max_attempts_per_seed",
             ...
             "to track from, as many attempts will still be made in vain for such seeds. (default: "
             "1000)") +
...
void load_seed_mechanisms(Properties &properties) {
  ...
  // Can't instantiate the dynamic seeder here; internal FMLS segmenter has to use the same
  // Directions::Set as TrackMapperDixel
  opt = get_options("seed_dynamic");
  if (opt.size()) {
    if (list.num_seeds())
      throw Exception(
          "If performing dynamic streamline seeding, cannot specify any other type of seed!");
