...
public:
  Receiver(const std::string &path, const Properties &properties, const size_t n, const size_t s)
      : writer(path, properties),
        ...
        segments(0),
        progress(std::string("       0 read,        0 written") +
                 (crop ? ",        0 segments" : "")) {}
  ...
  ~Receiver() {
    ...
    // of scope
    progress.set_text(
        std::string(printf("%8" PRIu64 " read, %8" PRIu64 " written", total_count, count)) +
        (crop ? printf(", %8" PRIu64 " segments", segments) : ""));
    if (number && (count != number))
      WARN("User requested " + str(number) + " streamlines, but only " + str(count) +
