...
void TrackMapperTWI::set_factor(const Streamline<> &tck, SetVoxelExtras &out) const {
  ...
  case CURVATURE:
    ...
    case GAUSSIAN:
      throw Exception("Gaussian track-wise statistic should not be used in TrackMapperTWI class; "
                      "use Mapping::Gaussian::TrackMapper instead");
    ...
    default:
      throw Exception(
          "FIXME: Undefined / unsupported track statistic in TrackMapperTWI::get_factor()");
    }
    break;
  ...
  default:
    throw Exception(
        "FIXME: Undefined / unsupported contrast mechanism in TrackMapperTWI::get_factor()");
  }
...
void TrackMapperTWI::add_twdfc_static_image(Image<float> &image) {
  ...
  if (track_statistic != ENDS_CORR)
    throw Exception("For fMRI correlation mapping, only the endpoint correlation track-wise "
                    "statistic is valid");
...
void TrackMapperTWI::add_twdfc_dynamic_image(Image<float> &image,
  ...
  if (contrast != SCALAR_MAP)
    throw Exception(
        "For sliding time-window fMRI mapping, mapper must be set to SCALAR_MAP contrast");
  if (track_statistic != ENDS_CORR)
    throw Exception("For sliding time-window fMRI mapping, only the endpoint correlation "
                    "track-wise statistic is valid");
...
void TrackMapperTWI::load_factors(const Streamline<> &tck) const {
  ...
  // However don't want to just take a pair of distant points and get the tangent that way; would
  // rather
  //   find a way to 'smooth' the curvature in a non-scalar fashion i.e. inverted curvature cancels
  // Ideally would like to get a curvature measurement & azimuth at each point; these can be
  // averaged
  ...
  // Remember: tangent vectors are unit length, but for principal normal vectors length must be
  // preserved!

  vector<Eigen::Vector3d> smoothed_tangents;
  smoothed_tangents.reserve(tangents.size());

  static const default_type gaussian_theta =
      CURVATURE_TRACK_SMOOTHING_FWHM / (2.0 * sqrt(2.0 * log(2.0)));
