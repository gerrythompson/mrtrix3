    ...
    if (statistic == ENDS_CORR) {
      ...
      for (; index >= 0 && index < ssize_t(tck.size()); index += step) {
        ...
        const Eigen::Array3i v(
            {int(std::round(p[0])), int(std::round(p[1])), int(std::round(p[2]))});
...
const Streamline<>::point_type TWIImagePluginBase::get_end_point(const Streamline<> &tck,
...
void TWIScalarImagePlugin::load_factors(const Streamline<> &tck,
                                        vector<default_type> &factors) const {
  if (statistic == ENDS_MIN || statistic == ENDS_MEAN || statistic == ENDS_MAX ||
      statistic == ENDS_PROD) {
...
void TWIFODImagePlugin::load_factors(const Streamline<> &tck, vector<default_type> &factors) const {
  ...
  if (statistic == ENDS_MAX || statistic == ENDS_MEAN || statistic == ENDS_MIN ||
      statistic == ENDS_PROD) {
      ...
      if (index > 0) {
        if (interp.scanner(tck[index])) {
          ...
          const Eigen::Vector3d dir =
              (tck[(index == ssize_t(tck.size() - 1)) ? index : (index + 1)] -
               tck[index ? (index - 1) : 0])
                  .cast<default_type>()
                  .normalized();
    ...
    for (size_t i = 0; i != tck.size(); ++i) {
      if (interp.scanner(tck[i])) {
        ...
        const Eigen::Vector3d dir =
            (tck[(i == tck.size() - 1) ? i : (i + 1)] - tck[i ? (i - 1) : 0])
                .cast<default_type>()
                .normalized();
        factors.push_back(precomputer->value(sh_coeffs, dir));
      } else {
...
void TWDFCStaticImagePlugin::load_factors(const Streamline<> &tck,
                                          vector<default_type> &factors) const {
  ...
  const default_type means[2] = {sums[0] / default_type(interp.size(3)),
                                 sums[1] / default_type(interp.size(3))};
...
void TWDFCDynamicImagePlugin::load_factors(const Streamline<> &tck,
                                           vector<default_type> &factors) const {
  assert(statistic == ENDS_CORR);
  factors.assign(1, NaN);
