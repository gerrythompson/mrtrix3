...
// Unlike standard TWI, with Gaussian smoothing the TWI factor is stored per point rather than per
// streamline However, it's handy from a code perspective to still use the same base class
/*
...
public:
  ...
  inline void insert(const Eigen::Vector3i &v,
                     const Eigen::Vector3d &d,
                     const default_type l,
                     const default_type f) {
