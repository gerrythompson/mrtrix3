...
public:
  ...
  // Also means that the call to run_queue() must cast to Gaussian::TrackMapper rather than using
  // base class
...
protected:
  ...
  //   factor value (TrackMapperTWI member field factors[] only contains one entry per pre-upsampled
  //   point)
...
template <class Cont> void TrackMapper::voxelise_precise(const Streamline<> &tck, Cont &out) const {
  ...
  do {
    ...
    Eigen::Vector3d traversal_vector =
        (p_voxel_exit - p_voxel_entry).cast<default_type>().normalized();
  ...
  for (size_t end = 0; end != 2; ++end) {
    ...
    if (check(vox, info)) {
      const Eigen::Vector3d dir =
          (end ? (tck[tck.size() - 1] - tck[tck.size() - 2]) : (tck[0] - tck[1]))
              .cast<default_type>()
              .normalized();
