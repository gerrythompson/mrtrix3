...
size_t determine_upsample_ratio(const Header &header, const float step_size, const float ratio) {
  ...
  if (step_size && std::isfinite(step_size))
    upsample_ratio = std::ceil(
        step_size /
        (std::min(header.spacing(0), std::min(header.spacing(1), header.spacing(2))) * ratio));
  return upsample_ratio;
}

size_t
determine_upsample_ratio(const Header &header, const std::string &tck_path, const float ratio) {
  Properties properties;
  Reader<> reader(tck_path, properties);
  return determine_upsample_ratio(header, properties, ratio);
}

size_t determine_upsample_ratio(const Header &header,
...
void generate_header(Header &header,
                     const std::string &tck_file_path,
                     const vector<default_type> &voxel_size) {
