...
template <class HeaderType> inline bool check(const Eigen::Vector3i &V, const HeaderType &H) {
  return (V[0] >= 0 && V[0] < H.size(0) && V[1] >= 0 && V[1] < H.size(1) && V[2] >= 0 &&
          V[2] < H.size(2));
}
...
public:
  ...
  bool operator<(const Voxel &V) const {
    return (((*this)[2] == V[2])
                ? (((*this)[1] == V[1]) ? ((*this)[0] < V[0]) : ((*this)[1] < V[1]))
                : ((*this)[2] < V[2]));
  }
...
// TOD class: tore the SH coefficients in the voxel class so that aPSF generation can be
// multi-threaded Provide a normalize() function to remove any length dependence, and have unary
// contribution per streamline
...
public:
  default_type
      factor;   // For TWI, when contribution to the map is uniform along the length of the track
  size_t index; // Index of the track
  default_type weight; // Cross-sectional multiplier for the track
};

// Set classes that give sensible behaviour to the insert() function depending on the base voxel
// class
