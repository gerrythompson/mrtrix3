  ...
  MapWriter(const Header &header,
    ...
    if ((type != DEC && voxel_statistic == V_MEAN) ||
      ...
      if (type == DEC || type == TOD)
        H_counts.ndim() = 3;
      counts.reset(
          new Image<float>(Image<float>::scratch(H_counts, "TWI streamline count buffer")));
    }
  }
  ...
  void finalise() override {
    ...
    case V_MEAN:
      if (type == GREYSCALE) {
        ...
        assert(counts);
        // TODO For dixels, should this be a voxel mean i.e. normalise each non-zero voxel to unit
        // density,
...
private:
  ...
  default_type get_factor(const VoxelDEC &element, const SetVoxelDEC &set) const {
    return set.factor;
  }
  default_type get_factor(const Dixel &element, const SetDixel &set) const { return set.factor; }
  default_type get_factor(const VoxelTOD &element, const SetVoxelTOD &set) const {
    return set.factor;
  }
  default_type get_factor(const Gaussian::Voxel &element, const Gaussian::SetVoxel &set) const {
    return element.get_factor();
  }
  default_type get_factor(const Gaussian::VoxelDEC &element,
                          const Gaussian::SetVoxelDEC &set) const {
    return element.get_factor();
  }
  default_type get_factor(const Gaussian::Dixel &element, const Gaussian::SetDixel &set) const {
    return element.get_factor();
  }
  default_type get_factor(const Gaussian::VoxelTOD &element,
                          const Gaussian::SetVoxelTOD &set) const {
    return element.get_factor();
  }
