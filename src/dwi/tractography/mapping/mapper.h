...
// Didn't bother making this a command-line option, since curvature contrast results were very poor
// regardless of smoothing
...
protected:
  ...
  inline void add_to_set(SetVoxelDEC &,
                         ...
                         const Eigen::Vector3d &,
                         const default_type) const;
  inline void add_to_set(SetVoxelDir &,
                         const Eigen::Vector3i &,
                         const Eigen::Vector3d &,
                         const default_type) const;
...
void TrackMapperBase::voxelise_precise(const Streamline<> &tck, Cont &out) const {
  ...
  do {
    ...
    const Eigen::Vector3d traversal_vector =
...
void TrackMapperBase::voxelise_ends(const Streamline<> &tck, Cont &out) const {
  ...
  for (size_t end = 0; end != 2; ++end) {
    ...
    if (check(vox, info)) {
      ...
      if (tck.size() > 1)
        dir = (end ? (tck[tck.size() - 1] - tck[tck.size() - 2]) : (tck[0] - tck[1]))
                  .cast<default_type>()
                  .normalized();
