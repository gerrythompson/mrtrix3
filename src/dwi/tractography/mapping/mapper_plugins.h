...
public:
  ...
  DWI::Directions::index_type operator()(const Eigen::Vector3d &d) const {
    return dirs.select_direction(d);
  }
...
public:
  ...
  void operator()(VectorType &sh, const UnitVectorType &d) const {
    (*generator)(sh, d);
  }
...
protected:
  ...
  // Helper functions; find the last point on the streamline from which valid image information can
  // be read
...
public:
  TWDFCDynamicImagePlugin(Image<float> &input_image,
