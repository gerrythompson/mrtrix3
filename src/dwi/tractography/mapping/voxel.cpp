...
std::ostream &operator<<(std::ostream &stream, const VoxelDEC &v) {
  stream << "[" << Eigen::Vector3i(v).transpose() << "]: [" << v.get_colour().transpose() << "] "
         << v.get_length();
  return stream;
}

std::ostream &operator<<(std::ostream &stream, const VoxelDir &v) {
  stream << "[" << Eigen::Vector3i(v).transpose() << "]: [" << v.get_dir().transpose() << "] "
         << v.get_length();
  return stream;
}
...
std::ostream &operator<<(std::ostream &stream, const VoxelTOD &v) {
  stream << "[" << Eigen::Vector3i(v).transpose() << "]: " << v.get_tod().transpose() << " "
         << v.get_length();
  return stream;
}
