...
public:
  ...
  TrackScalar(TrackScalar &&that)
      : vector<value_type>(std::move(that)), DataIndex(std::move(that)) {}
...
public:
  ...
  Streamline(size_t size, const point_type &fill)
      : vector<point_type>(size, fill), weight(value_type(1.0)) {}

  Streamline(const Streamline &) = default;
  Streamline &operator=(const Streamline &that) = default;

  Streamline(Streamline &&that)
      : vector<point_type>(std::move(that)), DataIndex(std::move(that)), weight(that.weight) {
    that.weight = 1.0f;
  }
