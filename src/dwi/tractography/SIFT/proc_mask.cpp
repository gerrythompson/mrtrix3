...
const App::OptionGroup SIFTModelProcMaskOption =
    ...
    + App::Option("proc_mask",
                  "provide an image containing the processing mask weights for the model; image "
                  "spatial dimensions must match the fixel image") +
...
void initialise_processing_mask(Image<float> &in_dwi,
  ...
  if (opt.size()) {
    ...
    if (!dimensions_match(out_mask, image, 0, 3))
      throw Exception("Dimensions of processing mask image provided using -proc_mask option must "
                      "match relevant fixel image");
    ...
    if (opt.size()) {
      ...
      if (dimensions_match(out_5tt, in_5tt, 0, 3)) {
        ...
        auto threaded_loop =
            ThreadedLoop("resampling ACT 5TT image to fixel image space", in_dwi, 0, 3);
      ...
      for (auto l = Loop(out_5tt, 0, 3)(out_5tt, out_mask); l; ++l) {
        const float value = Math::pow2<float>(
            out_5tt.value()); // Processing mask value is the square of the WM fraction
