...
template <class Fixel> void ModelBase<Fixel>::output_target_sh(const std::string &path) const {
  ...
  for (auto l = Loop(0, 3)(out, v); l; ++l) {
    if (v.value()) {
      Eigen::Matrix<default_type, Eigen::Dynamic, 1> sum =
          Eigen::Matrix<default_type, Eigen::Dynamic, 1>::Zero(N);
      for (typename Fixel_map<Fixel>::ConstIterator i = begin(v); i; ++i) {
        if (i().get_FOD()) {
...
void ModelBase<Fixel>::output_errors_voxel(const std::string &dirpath,
  ...
  const default_type current_mu = mu();
  auto out_max_abs_diff =
      Image<float>::create(Path::join(dirpath, max_abs_diff_path), Fixel_map<Fixel>::header());
