...
public:
  Track_fixel_contribution(const uint32_t fixel_index, const float length) {
    const uint32_t length_as_int =
        std::min(uint32_t(255), uint32_t(std::round(scale_to_storage * length)));
  ...
  bool add(const float length) {
    // Allow summing of multiple contributions to a fixel, UNLESS it would cause truncation, in
    // which
  ...
  static void set_scaling(const Header &H) {
    const float max_length =
        std::sqrt(Math::pow2(H.spacing(0)) + Math::pow2(H.spacing(1)) + Math::pow2(H.spacing(2)));
...
/*
// This is a 'safe' version of Track_fixel_contribution that does not use byte-sharing, but requires
double the RAM
...
public:
  ...
  TrackContribution()
      : Min_mem_array<Track_fixel_contribution>(), total_contribution(0.0), total_length(0.0) {}
