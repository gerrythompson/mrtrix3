...
public:
  ...
  bool operator<(const Cost_fn_gradient_sort &that) const {
...
// Sorting of the gradient vector in SIFT is done in a multi-threaded fashion, in a number of
// stages:
...
//     The entry in the set corresponding to the gradient vector is removed, but the iterator WITHIN
//     THE BLOCK is incremented and re-written to the set; this allows multiple streamlines from a
//     single block to be filtered in a single iteration, provided the gradient is less than that of
//     the candidate streamline from all other blocks
...
private:
  ...
  public:
    BlockSender(const track_t count, const track_t size)
        : num_tracks(count), block_size(size), counter(0) {}
