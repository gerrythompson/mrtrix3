...
MT_gradient_vector_sorter::MT_gradient_vector_sorter(MT_gradient_vector_sorter::VecType &in,
                                                     const track_t block_size)
    : end(in.end()) {
...
bool MT_gradient_vector_sorter::Sorter::operator()(const TrackIndexRange &in,
                                                   VecItType &out) const {
  ...
  for (; from_start < from_end; ++from_start) {
    if (from_start->get_gradient_per_unit_length() >= 0.0) {
      for (--from_end; from_end > from_start && from_end->get_gradient_per_unit_length() >= 0.0;
           --from_end)
        ;
