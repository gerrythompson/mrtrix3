...
void SIFTer::perform_filtering() {
  ...
  // For streamlines that do not contribute to the map, remove an equivalent proportion of length to
  ...
  try {
    gradient_vector.assign(num_tracks(),
                           Cost_fn_gradient_sort(num_tracks(),
  ...
  if (tracks_remaining < term_number)
    throw Exception("Filtering failed; desired number of filtered streamlines is greater than or "
                    "equal to the size of the input dataset");
  ...
  if (!csv_path.empty()) {
    ...
    csv_out << "0,0,0," << str(tracks_remaining) << "," << str(init_cf) << "," << str(TD_sum) << ","
            << str(mu()) << ",Start,\n";
  }
  ...
  do {
    ...
    // * Simulate a gradient vector using a Gaussian distribution of gradients (ignore length
    // dependence)
    // * Simulate sorting this gradient vector; need to simulate both the sort, and some number of
    // get() calls
    // * Perform a golden section search to find the optimal block size
    // This wasn't implemented as the optimal block size seems pretty stable regardless of gradient
    // vector size

    // Ideally the sorting block size should change dynamically as streamlines are filtered
    // This is to reduce the load on the single-threaded section as the multi-threaded sorting
    // complexity declines
    //   (as more streamlines are no longer present, hence have a null gradient and are excluded
    //   from the full sort)
    // Tried for an algebraic solution but the numbers didn't line up with my experiments
    // Trying a heuristic for now; go for a sort size of 1000 following initial sort, assuming half
    // of all
    //   remaining streamlines have a negative gradient

    const track_t sort_size =
        std::min(std::ceil(num_tracks() / double(Thread::number_of_threads())),
                 std::round(2000.0 * double(num_tracks()) / double(tracks_remaining)));
    MT_gradient_vector_sorter sorter(gradient_vector, sort_size);

    // Remove candidate streamlines one at a time, and correspondingly modify the fixels to which
    ...
    do {
      ...
      if (!output_at_counts.empty() && (tracks_remaining == output_at_counts.back())) {
        ...
        INFO("\nProportionality coefficient at " + str(tracks_remaining) + " streamlines is " +
             str(mu()));
      ...
      // Determine whether or not it is appropriate to remove a non-contributing streamline at this
      // point
      if (sum_noncontributing_length &&
        ...
        const double streamline_density_ratio =
            candidate->get_cost_gradient() /
            (sum_contributing_length - contributing_length_removed);
        ...
        for (size_t f = 0; f != candidate_contribution.dim(); ++f) {
          ...
          const double change_remove_tck =
              this_fixel.get_cost_wo_track(new_mu, length) - this_fixel.get_cost(old_mu);
          this_actual_cf_change = this_actual_cf_change - undo_change_mu_only + change_remove_tck;
        }

        const double required_cf_change_quantisation =
            enforce_quantisation ? (-0.5 * quantisation) : 0.0;
        const double this_nonlinearity = (candidate->get_cost_gradient() - this_actual_cf_change);

        if (this_actual_cf_change <
            std::min(
                {required_cf_change_ratio, required_cf_change_quantisation, this_nonlinearity})) {
          ...
          if (!removed_this_iteration) {
            // If filtering has been completed to convergence, but the user does not want to filter
            // to convergence
            //   (i.e. they have defined a desired termination criterion but it has not yet been
            //   met), disable the quantisation check to give the algorithm a chance to meet the
            //   user's termination request
            if (enforce_quantisation && (term_number || term_ratio || term_mu)) {
              if (App::log_level)
                fprintf(stderr, "\n");
              CONSOLE("filtering has reached quantisation error, but user's desired termination "
                      "criterion has not yet been met;");
              CONSOLE("  disabling cost function quantisation check and continuing filtering to "
                      "try to satisfy user's request");
  ...
  end_iteration:
    ...
    if (!csv_path.empty()) {
      File::OFStream csv_out(csv_path,
                             std::ios_base::out | std::ios_base::app | std::ios_base::ate);
      csv_out << str(iteration) << "," << str(removed_this_iteration) << ","
              << str(num_tracks() - tracks_remaining) << "," << str(tracks_remaining) << ","
              << str(cf_end_iteration) << "," << str(TD_sum) << "," << str(mu()) << ",";
      switch (recalculate) {
      case UNDEFINED:
...
void SIFTer::output_filtered_tracks(const std::string &input_path,
  ...
  for (vector<size_t>::const_iterator i = block_sizes.begin(); i != block_sizes.end(); ++i) {
    ...
    try {
      ...
      for (size_t candidate_count = 0; candidate_count < num_tracks / 1000; ++candidate_count)
        sorter.get();
      std::cerr << "Time required for sorting " << num_tracks << " tracks, block size "
                << block_size << " = " << timer.elapsed() * 1000.0 << "ms\n";
    } catch (...) {
...
double SIFTer::calc_gradient(const track_t index,
                             const double current_mu,
                             const double current_roc_cost) const {
  ...
  for (size_t f = 0; f != tck_cont.dim(); ++f) {
    ...
    const double gradient_remove_tck =
        fixel.get_cost_wo_track(mu_if_removed, tck_cont[f].get_length()) -
        fixel.get_cost(current_mu);
...
bool SIFTer::TrackGradientCalculator::operator()(const TrackIndexRange &in) const {
  for (track_t track_index = in.first; track_index != in.second; ++track_index) {
    if (master.contributions[track_index]) {
      ...
      const double grad_per_unit_length =
          master.contributions[track_index]->get_total_contribution()
