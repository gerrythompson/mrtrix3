...
public:
  ...
  FixelBase(const default_type amp, const Eigen::Vector3d &d)
      : FOD(amp), TD(0.0), weight(1.0), dir(d) {}

  FixelBase(const FMLS::FOD_lobe &lobe)
      : FOD(lobe.get_integral()), TD(0.0), weight(1.0), dir(lobe.get_mean_dir()) {}
...
// This class stores the necessary fixel information (including streamline densities), but does not
// retain the
//   list of fixels traversed by each streamline. If this information is necessary, use the Model
//   class (model.h)
...
protected:
  ...
  void output_errors_voxel(const std::string &,
                           const std::string &,
                           const std::string &,
...
template <class Fixel> void ModelBase<Fixel>::perform_FOD_segmentation(Image<float> &data) {
  ...
  Thread::run_queue(writer,
                    Thread::batch(FMLS::SH_coefs()),
                    Thread::multi(fmls),
                    Thread::batch(FMLS::FOD_lobes()),
                    *this);
...
template <class Fixel> void ModelBase<Fixel>::scale_FDs_by_GM() {
  ...
  for (auto l = Loop(v)(v, act_5tt); l; ++l) {
    ...
    const default_type multiplier =
        1.0 - tissues.get_cgm() - (0.5 * tissues.get_sgm()); // Heuristic
...
template <class Fixel> void ModelBase<Fixel>::map_streamlines(const std::string &path) {
  ...
  const track_t count =
      (properties.find("count") == properties.end()) ? 0 : to<track_t>(properties["count"]);
  if (!count)
  ...
  mapper.set_upsample_ratio(
      Mapping::determine_upsample_ratio(Fixel_map<Fixel>::header(), properties, 0.1));
...
template <class Fixel> void ModelBase<Fixel>::output_proc_mask(const std::string &path) {
  save(proc_mask, path);
}
...
void ModelBase<Fixel>::initialise_debug_image_output(const std::string &dirpath) const {
  ...
  Image<float> directions_image =
      Image<float>::create(Path::join(dirpath, "directions.mif"), H_directions);
  VoxelAccessor v(accessor());
...
template <class Fixel>
void ModelBase<Fixel>::output_all_debug_images(const std::string &dirpath,
  ...
  output_errors_voxel(dirpath,
                      prefix + "_maxabsdiff_voxel.mif",
                      prefix + "_diff_voxel.mif",
                      prefix + "_cost_voxel.mif");
#ifdef SIFT_MODEL_OUTPUT_FIXEL_IMAGES
