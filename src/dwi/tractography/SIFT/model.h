...
public:
  template <class Set>
  Model(Set &dwi, const DWI::Directions::FastLookupSet &dirs) : ModelBase<Fixel>(dwi, dirs) {
  ...
  // For debugging purposes - make sure the sum of TD in the fixels is equal to the sum of TD in the
  // streamlines
  void check_TD();
...
template <class Fixel> Model<Fixel>::~Model() {
  for (vector<TrackContribution *>::iterator i = contributions.begin(); i != contributions.end();
       ++i) {
    if (*i) {
      delete *i;
      *i = nullptr;
...
template <class Fixel> void Model<Fixel>::map_streamlines(const std::string &path) {
...

  {
    ...
    TrackMappingWorker worker(
        *this, Mapping::determine_upsample_ratio(Fixel_map<Fixel>::header(), properties, 0.1));
  ...
  if (!contributions.back()) {
    ...
    for (track_t i = 0; i != contributions.size(); ++i) {
      ...
      WARN("Only " + str(num_tracks) + " tracks read from input track file; expected " +
...
template <class Fixel> void Model<Fixel>::remove_excluded_fixels() {
  ...
  TrackIndexRangeWriter writer(
      SIFT_TRACK_INDEX_BUFFER_SIZE, num_tracks(), "Removing excluded fixels");
...
template <class Fixel> void Model<Fixel>::check_TD() {
  ...
  for (vector<TrackContribution *>::const_iterator i = contributions.begin();
       i != contributions.end();
       ++i) {
    if (*i)
