...
const App::OptionGroup FMLSSegmentOption =
    ...
    + App::Option("fmls_peak_value",
                  ...
                  "Any lobe for which the maximal peak amplitude is smaller than this threshold "
                  "will be discarded. "
                  "Default: " +
    ...
    + App::Option(
          ...
          "Specify the ratio between a given FOD amplitude sample between two lobes, and the "
          "smallest peak amplitude of the adjacent lobes, above which those lobes will be merged. "
          "This is the amplitude of the FOD at the 'bridge' point between the two lobes, divided "
          "by the peak amplitude of the smaller of the two adjoining lobes. "
          "A value of 1.0 will never merge two lobes into one; a value of 0.0 will always merge "
          "lobes unless they are bisected by a zero-valued crossing. "
          "Default: " +
...
IntegrationWeights::IntegrationWeights(const DWI::Directions::Set &dirs) : data(dirs.size()) {
  ...
  // Integrating an FOD with constant amplitude 1 (l=0 term = sqrt(4pi) should produce a value of
  // 4pi Every other integral should produce zero
  ...
  for (const auto &i : data_in_order) {
    ...
    if (adj_lobes.empty()) {
      ...
      if (abs(i.first) / out[adj_lobes.back()].get_max_peak_value() > lobe_merge_ratio) {
        ...
        for (auto j = retrospective_assignments.begin(); j != retrospective_assignments.end();
             ++j) {
          bool modified = false;
  ...
  for (auto i = out.begin(); i != out.end();) { // Empty increment
    ...
    if (i->is_negative() || i->get_integral() < integral_threshold) {
      ...
      for (size_t peak_index = 0; peak_index != i->num_peaks(); ++peak_index) {
        ...
        const default_type newton_peak_value =
            Math::SH::get_peak(in, lmax, newton_peak_dir, &(*precomputer));
  ...
  do {
    ...
    for (dir_t d = 0; d != dirs.size(); ++d) {
      if (lobe.get_values()[d]) {
        ...
        // Transform unit direction onto tangent plane defined by the current mean direction
        // estimate
