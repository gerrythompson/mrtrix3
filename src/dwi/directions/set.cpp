...
void Set::initialise_adjacency() {
  ...
  class Plane {
  public:
    Plane(const vector<Vertex> &vertices,
          const index_type one,
          const index_type two,
          const index_type three)
        : indices{{one, two, three}},
          normal(((vertices[two].dir - vertices[one].dir)
                      .cross(vertices[three].dir - vertices[two].dir))
                     .normalized()),
          dist(std::max({vertices[one].dir.dot(normal),
                         vertices[two].dir.dot(normal),
                         vertices[three].dir.dot(normal)})) {}
  ...
  for (index_type i = 0; i != 6; ++i) {
    for (index_type j = i + 1; j != 6; ++j) {
      const default_type dist_sq =
          (vertices[all_extrema[j]].dir - vertices[all_extrema[i]].dir).squaredNorm();
    ...
    if (i != distant_pair.first && i != distant_pair.second) {
      const default_type dist =
          (vertices[all_extrema[i]].dir - (vertices[all_extrema[distant_pair.first]].dir))
              ...
              .norm() /
          (vertices[all_extrema[distant_pair.second]].dir -
           vertices[all_extrema[distant_pair.first]].dir)
              .norm();
      if (dist > max_dist) {
        max_dist = dist;
        third_point = i;
  ...
  planes.push_back(Plane(vertices,
                         all_extrema[distant_pair.first],
                         all_extrema[distant_pair.second],
                         all_extrema[third_point]));
  ...
  while (planes.size()) {
    ...
    if (max_index == vertices.size()) {
      ...
      // Find the matching edges from multiple faces, and construct new triangles going up to the
      // new point Remove any shared edges; non-shared edges are the projection horizon
...
void FastLookupSet::initialise() {
  ...
  for (size_t i = 0; i != total_num_angle_grids; ++i) {
    ...
    for (size_t point_index = 0; point_index != 4; ++point_index) {
      ...
      for (vector<index_type>::const_iterator d = grid_lookup[i].begin();
  ...
  for (size_t grid_index = 0; grid_index != total_num_angle_grids; ++grid_index) {
    ...
    for (size_t index_to_expand = 0; index_to_expand != num_to_expand; ++index_to_expand) {
      ...
      for (vector<index_type>::const_iterator adj = get_adj_dirs(dir_to_expand).begin();
        ...
        // Size of lookup tables could potentially be reduced by being more prohibitive of adjacent
        // direction inclusion in the lookup table for this grid

        bool is_present = false;
        for (vector<index_type>::const_iterator i = this_grid.begin();
             !is_present && i != this_grid.end();
             ++i)
          is_present = (*i == *adj);
        if (!is_present)
