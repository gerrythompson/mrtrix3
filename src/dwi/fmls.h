...
#define FMLS_INTEGRAL_THRESHOLD_DEFAULT                                                            \
  0.0 // By default, don't threshold by integral (tough to get a good number)
#define FMLS_PEAK_VALUE_THRESHOLD_DEFAULT 0.1
#define FMLS_MERGE_RATIO_BRIDGE_TO_PEAK_DEFAULT                                                    \
  1.0 // By default, never perform merging of lobes generated from discrete peaks such that a single
      // lobe contains multiple peaks

// By default, the mean direction of each FOD lobe is calculated by taking a weighted average of the
//   Euclidean unit vectors (weights are FOD amplitudes). This is not strictly mathematically
//   correct, and a method is provided for optimising the mean direction estimate based on
//   minimising the weighted sum of squared geodesic distances on the sphere. However the coarse
//   estimate is in fact accurate enough for our applications. Uncomment this line to activate the
//   optimal mean direction calculation (about a 20% performance penalty).
...
// These are for configuring the FMLS segmenter at the command line, particularly for fod_metric
// command
...
public:
  FOD_lobe(const DWI::Directions::Set &dirs,
           const index_type seed,
           const default_type value,
           const default_type weight)
      : mask(dirs),
  ...
  // This is used for creating a `null lobe' i.e. an FOD lobe with zero size, containing all
  // directions not
  ...
  void revise_peak(const size_t index,
...
// Store a vector of weights to be applied when computing integrals, to account for non-uniformities
// in direction distribution These weights are applied to the amplitude along each direction as the
// integral for each lobe is summed,
...
private:
  ...
  default_type
      lobe_merge_ratio;  // Determines whether two lobes get agglomerated into one, depending on the
                         // FOD amplitude at the current point and how it compares to the maximal
                         // amplitudes of the lobes to which it could be assigned
  bool create_null_lobe; // If this is set, an additional lobe will be created after segmentation
                         // with zero size, containing all directions not assigned to any other lobe
  bool create_lookup_table; // If this is set, an additional lobe will be created after segmentation
                            // with zero size, containing all directions not assigned to any other
                            // lobe
  bool dilate_lookup_table; // If this is set, the lookup table created for each voxel will be
                            // dilated so that all directions correspond to the nearest positive
                            // non-zero FOD lobe

  void verify_settings() const {
    if (create_null_lobe && dilate_lookup_table)
      throw Exception("For FOD segmentation, options 'create_null_lobe' and 'dilate_lookup_table' "
                      "are mutually exclusive");
    if (!create_lookup_table && dilate_lookup_table)
      throw Exception("For FOD segmentation, 'create_lookup_table' must be set in order for lookup "
                      "tables to be dilated ('dilate_lookup_table')");
  }
