...
public:
  ...
  Unring2D(const Unring2D &other)
      : row_fft(other.row_fft.size(), FFTW_FORWARD),
        ...
        unring1d_row(
            row_ifft, other.unring1d_row.nsh, other.unring1d_row.minW, other.unring1d_row.maxW),
        unring1d_col(
            col_ifft, other.unring1d_col.nsh, other.unring1d_col.minW, other.unring1d_col.maxW),
...
private:
  ...
  template <typename fft_obj, typename Derived>
  FORCE_INLINE void FFT(fft_obj &fft, Eigen::MatrixBase<Derived> &M) {
  ...
  FORCE_INLINE void FFT(Math::FFT1D &fft, Eigen::MatrixBase<Derived> &&M) {
    FFT(fft, M);
  }

  template <typename Derived> FORCE_INLINE void row_FFT(Eigen::MatrixBase<Derived> &mat) {
    FFT(row_fft, mat);
  }
  template <typename Derived> FORCE_INLINE void row_iFFT(Eigen::MatrixBase<Derived> &mat) {
    FFT(row_ifft, mat);
  }
