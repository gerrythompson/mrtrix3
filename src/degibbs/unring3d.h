  ...
  void operator()(const Iterator &pos) {
    ...
    for (int f = 0; f < 2 * num_shifts + 1; f++) {
      for (int n = 0; n < lsize; ++n)
        ifft[f][n] =
            fft[n] * exp(j * 2.0 * indexshift(n, lsize) * Math::pi * shift_ind[f] / double(lsize));
      if (!(lsize & 1))
        ifft[f][lsize / 2] = 0.0;
      ifft[f].run();
    }
  ...
  int optimumshift(int n, int lsize) {
    ...
    for (int f = 0; f < 2 * num_shifts + 1; f++) {
      ...
      for (int k = minW; k <= maxW; ++k) {
        sum_left += abs(ifft[f][wraparound(n - k, lsize)].real() -
                        ifft[f][wraparound(n - k - 1, lsize)].real());
        sum_left += abs(ifft[f][wraparound(n - k, lsize)].imag() -
                        ifft[f][wraparound(n - k - 1, lsize)].imag());

        sum_right += abs(ifft[f][wraparound(n + k, lsize)].real() -
                         ifft[f][wraparound(n + k + 1, lsize)].real());
        sum_right += abs(ifft[f][wraparound(n + k, lsize)].imag() -
                         ifft[f][wraparound(n + k + 1, lsize)].imag());
      }
