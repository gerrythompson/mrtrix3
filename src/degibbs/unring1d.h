  ...
  template <typename Derived> FORCE_INLINE void operator()(Eigen::MatrixBase<Derived> &data) {
    ...
    for (int l = 0; l < n; ++l) {
      ...
      for (int j = 0; j < 2 * nsh + 1; ++j) {
        ...
        TV1arr[j] += abs(shifted((l - minW + 1 + n) % n, j).real() -
                         shifted((l - (minW) + n) % n, j).real());
        TV1arr[j] -= abs(shifted((l - maxW + n) % n, j).real() -
                         shifted((l - (maxW + 1) + n) % n, j).real());
        TV2arr[j] += abs(shifted((l + maxW + 1 + n) % n, j).real() -
                         shifted((l + (maxW + 2) + n) % n, j).real());
        TV2arr[j] -= abs(shifted((l + minW + n) % n, j).real() -
                         shifted((l + (minW + 1) + n) % n, j).real());

        TV1arr[j] += abs(shifted((l - minW + 1 + n) % n, j).imag() -
                         shifted((l - (minW) + n) % n, j).imag());
        TV1arr[j] -= abs(shifted((l - maxW + n) % n, j).imag() -
                         shifted((l - (maxW + 1) + n) % n, j).imag());
        TV2arr[j] += abs(shifted((l + maxW + 1 + n) % n, j).imag() -
                         shifted((l + (maxW + 2) + n) % n, j).imag());
        TV2arr[j] -= abs(shifted((l + minW + n) % n, j).imag() -
                         shifted((l + (minW + 1) + n) % n, j).imag());
      }
