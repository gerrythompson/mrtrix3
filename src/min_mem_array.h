...
public:
  ...
  Min_mem_array(const Min_mem_array &that) : n(that.n), d(new T[n]) {
  ...
  bool operator<(const Min_mem_array<T> &that) const {
    // If one is empty and the other is not, one is 'less than' the other; but if both are empty,
    ...
    // If code reaches this point, then all of the elements up to the minimum of the two lengths are
    // identical Therefore, only return true for '<' if the length of that is longer than this (if
    // they are equivalent in length, '<' does not hold so return false)
    return (n < that.n);
  }
