...
public:
  ...
  void set_scale_factor(const vector<default_type> &scalefactor) {
    ...
    for (size_t level = 0; level < scalefactor.size(); ++level) {
      if (scalefactor[level] <= 0 || scalefactor[level] > 1.0)
        throw Exception("the linear registration scale factor for each multi-resolution level must "
                        "be between 0 and 1");
  ...
  void set_stage_iterations(const vector<uint32_t> &it) {
    ...
    if (it.size() == stages.size()) {
      ...
      throw Exception("the number of stage iterations must be defined for all stages (1 or " +
                      str(stages.size()) + ")");
    for (auto &stage : stages) {
  ...
  void run_masked(MetricType &metric,
    ...
    if (init_translation_type == Transform::Init::mass)
      ...
      Transform::Init::initialise_using_image_centres(
          im1_image, im2_image, im1_mask, im2_mask, transform, init);
    else if (init_translation_type ==
             Transform::Init::set_centre_mass) // doesn't change translation or linear matrix
      Transform::Init::set_centre_via_mass(
          im1_image, im2_image, im1_mask, im2_mask, transform, init, contrasts);
    else if (init_translation_type ==
             Transform::Init::set_centre_geometric) // doesn't change translation or linear matrix
      Transform::Init::set_centre_via_image_centres(
          im1_image, im2_image, im1_mask, im2_mask, transform, init);
    ...
    //   transformation_search.run_masked (metric, transform, im1_image, im2_image, im1_mask,
    //   im2_mask, init);
    //   // transform.debug();
    // }
    ...
    // using Im1ImageInterpolatorType = Interp::SplineInterp<Im1ImageType,
    // Math::UniformBSpline<typename Im1ImageType::value_type>,
    // Math::SplineProcessingType::ValueAndDerivative>; using Im2ImageInterpolatorType =
    ...
    // Interp::SplineInterp<ProcessedImageType, Math::UniformBSpline<typename
    // ProcessedImageType::value_type>, Math::SplineProcessingType::ValueAndDerivative>;
#else
    ...
    using ProcessedImageInterpolatorType =
        Interp::LinearInterp<ProcessedImageType,
                             Interp::LinearInterpProcessingType::ValueAndDerivative>;
#endif
    ...
    midway_image_header = compute_minimum_average_header(im1_image,
                                                         im2_image,
                                                         transform.get_transform_half_inverse(),
                                                         transform.get_transform_half());

    for (size_t istage = 0; istage < stages.size(); istage++) {
      auto &stage = stages[istage];

      CONSOLE("linear stage " + str(istage + 1) + "/" + str(stages.size()) + ", " +
      ...
      if (stage_contrasts.size()) {
        ...
        MultiContrastSetting mc(
            im1_image.ndim() < 4 ? 1 : im1_image.size(3), do_reorientation, stage.fod_lmax);
      ...
      auto im1_smoothed = Registration::multi_resolution_lmax(
          im1_image, stage.scale_factor, do_reorientation, stage_contrasts);
      ...
      ParamType parameters(
          transform, im1_smoothed, im2_smoothed, midway_resized, im1_mask, im2_mask);
      ...
      Eigen::Vector3d spacing(midway_image_header.spacing(0),
      ...
      default_type reg_coherence_len =
          File::Config::get_float("RegCoherenceLen", 3.0); // = 3 stdev blur
      ...
      for (auto stage_iter = 1U; stage_iter <= stage.stage_iterations; ++stage_iter) {
        if (stage.gd_max_iter > 0 and stage.optimisers[stage_iter - 1] == OptimiserAlgoType::bbgd) {
          Math::GradientDescentBB<Metric::Evaluate<MetricType, ParamType>,
                                  typename TransformType::UpdateType>
          ...
          optim.run(
              stage.gd_max_iter, grad_tolerance, analyse_descent ? std::cout.rdbuf() : log_stream);
          parameters.optimiser_update(optim, evaluate.overlap());
          INFO("    iteration: " + str(stage_iter) + "/" + str(stage.stage_iterations) +
               " GD iterations: " + str(optim.function_evaluations()) +
               " cost: " + str(optim.value()) + " overlap: " + str(evaluate.overlap()));
        ...
        if (stage.diagnostics_images.size()) {
          ...
          parameters.make_diagnostics_image(
              stage.diagnostics_images[stage_iter - 1],
      ...
      midway_image_header =
          compute_minimum_average_header(im1_image,
                                         im2_image,
  ...
  //   const bool do_reorientation, const bool input_is_one, const std::string& out_path, const
  //   Header& h_midway) { if (do_reorientation and aPSF_directions.size() == 0)
  ...
  //   }
  //   image_midway = Image<typename ImageType::value_type>::create (out_path,
  //   midway_header).with_direct_io(); if (input_is_one) {
  ...
  //       transformation.get_transform_half(), aPSF_directions);
  //   } else {
  //     Filter::reslice<Interp::Cubic> (input, image_midway,
  //     transformation.get_transform_half_inverse(), Adapter::AutoOverSample, 0.0); if
  //     (do_reorientation)
  //  //     60 directions by default!
  //       Transform::reorient ("reorienting...", image_midway, image_midway,
