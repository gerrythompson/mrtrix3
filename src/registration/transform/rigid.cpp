...
void project_linear2rotation(const Eigen::Matrix<default_type, 3, 3> &linear,
  ...
  assert(
      (rotation * rotation.transpose().eval())
          .isApprox(Eigen::Matrix<default_type, Eigen::Dynamic, Eigen::Dynamic>::Identity(3, 3)) &&
      "project_linear2rotation orthonormal?");
}

bool RigidLinearNonSymmetricUpdate::operator()(
    Eigen::Matrix<default_type, Eigen::Dynamic, 1> &newx,
  ...
  if (control_points.size()) {
    ...
    if (cnt > 0)
      INFO("rigid: gradient descent step size was too large. Multiplied by factor " +
...
Rigid::get_jacobian_vector_wrt_params(const Eigen::Vector3d &p) const {
...
void Rigid::set_parameter_vector(
    const Eigen::Matrix<Rigid::ParameterType, Eigen::Dynamic, 1> &param_vector) {
  this->trafo.matrix() =
      Eigen::Map<const Eigen::Matrix<ParameterType, 3, 4, Eigen::RowMajor>>(&param_vector(0));
  this->compute_halfspace_transformations();
}

void Rigid::get_parameter_vector(
    Eigen::Matrix<Rigid::ParameterType, Eigen::Dynamic, 1> &param_vector) const {
