...
public:
  ...
  using ParamType = Metric::Params<
      ...
      Image<default_type>,
      Image<default_type>,
      // use Interp::LinearInterpProcessingType::ValueAndDerivative for metric that calculates
      // gradients
  ...
  void write_images(const std::string &im1_path, const std::string &im2_path) {
    ...
    Filter::reslice<Interp::Cubic>(
        im1, image1_midway, local_trafo.get_transform_half(), Adapter::AutoOverSample, 0.0);
    Filter::reslice<Interp::Cubic>(
  ...
  void run(bool debug = false) {
    ...
    size_t iterations =
        global_search ? global_search_iterations : (rot_angles.size() * local_search_directions);
    ...
    if (translation_extent != 0) {
      ...
      extent << midway_image_header.spacing(0) * translation_extent *
                    (midway_image_header.size(0) - 0.5),
    ...
    while (++iteration < iterations) {
      ...
      DEBUG("rotation search: iteration " + str(iteration) + " cost: " + str(cost) +
            " cnt: " + str(cnt));
      if (debug)
    ...
    {
      auto max_ =
          Eigen::MatrixXd::Constant(cost_it.rows(), 1, std::numeric_limits<default_type>::max());
      // default_type max_overlap = overlap_it.maxCoeff();
      default_type mean_overlap =
...
private:
  FORCE_INLINE ParamType get_parameters() {
    ...
    //   Eigen::Transform<default_type, 3, Eigen::Projective> init_trafo_2 =
    //   local_trafo.get_transform_half(); init_transforms.push_back (init_trafo_1);
  ...
  FORCE_INLINE void gen_random_quaternion() {
    ...
    quat = Eigen::Quaternion<default_type>(
  ...
  // ENH: less brute-force approach (for instance: fixed set of electrostatic repulsion directions,
  // rotate all to gap centre)
  FORCE_INLINE void gen_uniform_rotation_axes(const size_t &n_dir,
                                              const default_type &max_cone_angle_deg) {
    assert(n_dir > 1);
    ...
    default_type a = (1.0 - std::cos(Math::pi / 180.0 * default_type(max_cone_angle_deg))) /
                     (default_type(n_dir - 1));
  ...
  FORCE_INLINE void gen_local_quaternion() {
    ...
    quat = Eigen::Quaternion<default_type>(
        Eigen::AngleAxis<default_type>(rot_angles[idx_angle], xyz.row(idx_dir)));
    ++idx_dir;
  }
