  ...
  if (control_points.size()) {
    ...
    while (true) {
      ...
      if (projection_type == TransformProjectionType::rigid_nonsym) {
        ...
        Eigen::JacobiSVD<Eigen::Matrix<default_type, 3, 3>> svd(
            L, Eigen::ComputeFullU | Eigen::ComputeFullV);
        default_type x =
        ...
        assert((R * R.transpose().eval())
                   .isApprox(Eigen::Matrix<default_type, Eigen::Dynamic, Eigen::Dynamic>::Identity(
                       3, 3)) &&
    ...
    throw Exception(
        "no control points defined. deactivated as we cannot regularise the update properly!");
    // reduce step size if determinant of matrix becomes negative (happens rarely at first few
    // iterations) size_t cnt = 0; const default_type factor(0.9); while (true) {
    //   delta = g * step_size;
    //   Registration::Transform::param_vec2mat(delta, Delta);
...
Eigen::Matrix<default_type, 4, 1>
Affine::get_jacobian_vector_wrt_params(const Eigen::Vector3d &p) const {
...
void Affine::set_parameter_vector(
    const Eigen::Matrix<Affine::ParameterType, Eigen::Dynamic, 1> &param_vector) {
  this->trafo.matrix() =
      Eigen::Map<const Eigen::Matrix<ParameterType, 3, 4, Eigen::RowMajor>>(&param_vector(0));
  this->compute_halfspace_transformations();
}

void Affine::get_parameter_vector(
    Eigen::Matrix<Affine::ParameterType, Eigen::Dynamic, 1> &param_vector) const {
