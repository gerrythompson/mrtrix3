...
void calc_median_trafo(const MR::vector<transform_type> &candid_trafo,
  ...
  for (size_t i = 0; i < n_vertices; ++i) {
    ...
    Math::median_weiszfeld(
        candid_vertices[i], median_vertex, weiszfeld_iterations, weiszfeld_precision);
    median_vertices_4.col(i) << median_vertex, 1.0;
  }

  Eigen::ColPivHouseholderQR<Eigen::Matrix<default_type, 4, n_vertices>> dec(
      vertices_4.transpose());
