...
namespace Transform {
namespace Init {
bool get_sorted_eigen_vecs_vals(
    const Eigen::Matrix<default_type, 3, 3> &mat,
  ...
  if (es.info() != Eigen::Success)
    return false;
  const Eigen::Matrix<default_type, Eigen::Dynamic, Eigen::Dynamic> &evec =
      es.eigenvectors().real();
  const Eigen::Matrix<default_type, Eigen::Dynamic, 1> eval(
      es.eigenvalues().real()); // copy on purpose
  ...
  std::sort(std::begin(eval_idx_vec),
            std::end(eval_idx_vec),
            std::greater<std::pair<default_type, ssize_t>>());
...
public:
  ...
  template <class ImType> void operator()(ImType &image) {
    ...
    voxel_pos << (default_type)image.index(0), (default_type)image.index(1),
        (default_type)image.index(2);
...
void initialise_using_rotation_search(
  ...
  if (contrast_settings.size() > 1)
    WARN("rotation search does not support multiple contrasts. using only first volume of first "
         "contrasts");
...
void initialise_using_image_mass(Image<default_type> &im1,
  ...
  get_centre_of_mass(im1,
                     init.init_translation.unmasked1 ? bogus_mask : mask1,
                     im1_centre_of_mass,
                     contrast_settings);
  get_centre_of_mass(im2,
                     init.init_translation.unmasked2 ? bogus_mask : mask2,
                     im2_centre_of_mass,
                     contrast_settings);
...
void MomentsInitialiser::run() {
  ...
  // Maaref, A., & Aïssa, S. (2007). Eigenvalue distributions of wishart-type random matrices with
  // application to the performance analysis of MIMO MRC systems. IEEE Transactions on Wireless
  // Communications, 6(7), 2678–2689. doi:10.1109/TWC.2007.05990 Edelman, A. (1991). The
  // distribution and moments of the smallest eigenvalue of a random matrix of wishart type. Linear
  // Algebra and Its Applications, 159, 55–80. doi:10.1016/0024-3795(91)90076-9 CONSOLE("before
  // permutation"); MAT(im1_evec); VEC(im1_eval); MAT(im2_evec); VEC(im2_eval);
  ...
  //   default_type dotprod = (im2_evec_perm.array() *
  //   im1_evec.array()).colwise().sum().abs().sum(); if (dotprod > bestdotprod) bestperm = perm;
  // }
  // im2_evec = (im2_evec * bestperm).eval();
  // im2_eval = (im2_eval * bestperm).eval();

  // flip eigenvectors to same handedness
  im2_evec.array().rowwise() *=
      (im2_evec.array() * im1_evec.array())
          .colwise()
          .sum()
...
void FODInitialiser::init(Image<default_type> &im,
  ...
  if (mask.valid()) {
    for (auto i = Loop(0, 3)(im, mask); i; ++i) {
      if (mask.value()) {
        im_mass += im.value();
        voxel_pos << (default_type)im.index(0), (default_type)im.index(1),
            (default_type)im.index(2);
