...
inline void
param_mat2vec(const Eigen::Matrix<ValueType, 3, 4, Eigen::RowMajor> &transformation_matrix,
...
template <class ValueType>
inline void
param_mat2vec(const Eigen::Matrix<ValueType, 4, 4, Eigen::RowMajor> &transformation_matrix,
              Eigen::Matrix<ValueType, Eigen::Dynamic, 1> &param_vector) {
  assert(param_vector.size() == 12);
  param_vector =
      Eigen::Map<Eigen::MatrixXd>((transformation_matrix.template block<3, 4>(0, 0)).data(), 12, 1);
}
template <class ValueType>
inline void
param_mat2vec(const Eigen::Matrix<ValueType, 3, 4, Eigen::ColMajor> &transformation_matrix,
              Eigen::Matrix<ValueType, Eigen::Dynamic, 1> &param_vector) {
  assert(param_vector.size() == 12);
...
template <class ValueType>
inline void
param_mat2vec(const Eigen::Matrix<ValueType, 4, 4, Eigen::ColMajor> &transformation_matrix,
              Eigen::Matrix<ValueType, Eigen::Dynamic, 1> &param_vector) {
  assert(param_vector.size() == 12);
  // create temporary copy of the matrix in row major order and map it's memory as a vector
  param_vector = Eigen::Map<Eigen::Matrix<ValueType, 12, 1>>(
      (Eigen::Matrix<default_type, 3, 4, Eigen::RowMajor>(
