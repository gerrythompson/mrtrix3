...
FORCE_INLINE Eigen::MatrixXd aPSF_weights_to_FOD_transform(const int num_SH,
...
FORCE_INLINE vector<vector<ssize_t>>
multiContrastSetting2start_nvols(const vector<MultiContrastSetting> &mcsettings, size_t &max_n_SH) {
  max_n_SH = 0;
    ...
    for (const auto &mc : mcsettings) {
      if (mc.do_reorientation && mc.lmax > 0) {
        start_nvols.emplace_back(
            std::initializer_list<ssize_t>{(ssize_t)mc.start, (ssize_t)mc.nvols});
...
public:
  LinearKernelMultiContrast(ssize_t n_vol,
    ...
    if (modulate) {
      Eigen::VectorXd modulation_factors = transformed_directions.colwise().norm() /
                                           linear_transform.linear().inverse().determinant();
...
public:
  NonLinearKernelMultiContrast(ssize_t n_vol,
      ...
      : max_n_SH(max_n_SH),
        ...
        fod(n_vol) {
    for (auto const &sn : start_nvols)
      map_FOD_to_aPSF_transform[sn[1]] =
          Math::pinv(aPSF_weights_to_FOD_transform(sn[1], directions));
    assert(n_vol > 0);
    assert(start_nvols.size());
  }
  ...
  void operator()(FODImageType &image) {
    ...
    if (modulate) {
      ...
      // TODO: make this work?
      // transform.noalias() = (aPSF_weights_to_FOD_transform (max_n_SHvox,
      // transformed_directions).colwise() * modulation_factors.transpose()) *
      // map_FOD_to_aPSF_transform[max_n_SHvox];
    } else {
      transform.noalias() = aPSF_weights_to_FOD_transform(max_n_SHvox, transformed_directions) *
                            map_FOD_to_aPSF_transform[max_n_SHvox];
    }
...
public:
  NonLinearKernel(const ssize_t n_SH,
                  Image<default_type> &warp,
                  const Eigen::MatrixXd &directions,
                  const bool modulate)
      : n_SH(n_SH),
  ...
  void operator()(FODImageType &image) {
    ...
    if (image.value() > 0) { // only reorient voxels that contain a FOD
      ...
      if (modulate) {
        Eigen::MatrixXd modulation_factors =
            transformed_directions.colwise().norm() / jacobian.determinant();
        transformed_directions.colwise().normalize();
        ...
        transformed_directions.colwise().normalize();
        transform.noalias() =
            aPSF_weights_to_FOD_transform(n_SH, transformed_directions) * FOD_to_aPSF_transform;
      }
      fod = image.row(3);
      fod = transform * fod;
      image.row(3) = fod;
...
void reorient_warp(
  ...
  if (start_nvols.size()) {
    ...
    ThreadedLoop(progress_message, fod_image, 0, 3)
        .run(NonLinearKernel<FODImageType>(fod_image.size(3), warp, directions, modulate),
             fod_image);
  }
}
