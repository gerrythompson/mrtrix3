...
public:
  /** requires_precompute:
  type_trait to distinguish metric types that require a call to precompute before the operator() is
  called */
  ...
  template <class ParamType> default_type precompute(ParamType &parameters) {
    ...
    // using Im1MaskInterpolatorType = Interp::LinearInterp<Im1MaskType,
    // Interp::LinearInterpProcessingType::Value>; using Im2MaskInterpolatorType =
    // Interp::LinearInterp<Im2MaskType, Interp::LinearInterpProcessingType::Value>;
    ...
    // processed_image: 2 volumes: interpolated image1 value, interpolated image2 value if both
    // masks' values are >= 0.5
    ...
    auto loop =
        ThreadedLoop("precomputing cross correlation data...", parameters.processed_image, 0, 3);
...
private:
  ...
  struct CCNoGradientPrecomputeFunctor {
    ...
    void operator()(ProcessedImageType &pimage, MaskImageType &mask) {
      ...
      vox = Eigen::Vector3d(default_type(pimage.index(0)),
                            default_type(pimage.index(1)),
                            default_type(pimage.index(2)));
      pos = v2s * vox;
