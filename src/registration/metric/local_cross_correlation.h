...
template <typename ImageType1, typename ImageType2> struct LCCPrecomputeFunctorMasked_Naive {
  ...
  LCCPrecomputeFunctorMasked_Naive(const vector<size_t> &ext,
                                   ImageType1 &adapter1,
                                   ImageType2 &adapter2)
...
public:
  /** typedef int is_neighbourhood: type_trait to distinguish voxel-wise and neighbourhood based
   * metric types */
  using is_neighbourhood = int;
  /** requires_precompute int is_neighbourhood: type_trait to distinguish metric types that require
   * a call to precompute before the operator() is called */
  ...
  template <class ParamType> default_type precompute(ParamType &parameters) {
    ...
    auto cc_image = cc_image_header.template get_image<ProcessedImageValueType>().with_direct_io(
        Stride::contiguous_along_axis(3));
    vector<uint32_t> NoOversample;
    {
      ...
      if (parameters.im1_mask.valid() and !parameters.im2_mask.valid())
        Filter::reslice<Interp::Nearest>(
            parameters.im1_mask, cc_mask, parameters.transformation.get_transform_half());
        ...
        auto both = [](decltype(cc_mask) &cc_mask,
    ...
    Adapter::Reslice<Interp::Linear, Im1Type> interp1(
    ...
    Adapter::Reslice<Interp::Linear, Im2Type> interp2(
    ...
    parameters.processed_mask_interp.reset(
    ...
    loop.run(LCCPrecomputeFunctorMasked_Naive<decltype(interp1), decltype(interp2)>(
                 extent, interp1, interp2),
  ...
  default_type operator()(Params &params,
    ...
    const Eigen::Vector3d pos = Eigen::Vector3d(
        default_type(iter.index(0)), default_type(iter.index(0)), default_type(iter.index(0)));
    ...
    // 2.0 * sfm / (sff * smm) * ((i2 - sfm / smm * i1 ) * im1_gradient.value() - (i1 - sfm / sff *
    // i2 ) * im2_gradient.value());

    // ITK:
    // derivWRTImage[dim] = 2.0 * sFixedMoving / (sFixedFixed_sMovingMoving) * (fixedI -
    // sFixedMoving / sMovingMoving * movingI) * movingImageGradient[dim];
    Eigen::Vector3d derivWRTImage =
        -A_BC * ((val2 - A / B * val1) * grad1 - 0.0 * (val1 - A / C * val2) * grad2);
