...
public:
  ...
  void operator()(const Im1ImageType &im1_meansubtracted,
    ...
    if (im1_meansubtracted.index(0) == 0 ||
        im1_meansubtracted.index(0) == im1_meansubtracted.size(0) - 1 ||
        im1_meansubtracted.index(1) == 0 ||
        im1_meansubtracted.index(1) == im1_meansubtracted.size(1) - 1 ||
        im1_meansubtracted.index(2) == 0 ||
        im1_meansubtracted.index(2) == im1_meansubtracted.size(2) - 1) {
    ...
    Eigen::Matrix<typename Im1ImageType::value_type, 3, 1> grad =
        2.0 * sfm / (sff * smm) *
        ((i2 - sfm / smm * i1) * im1_gradient.value() -
         (i1 - sfm / sff * i2) * im2_gradient.value());
