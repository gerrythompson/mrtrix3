...
struct metric_requires_precompute<MetricType,
                                  typename Void2<typename MetricType::requires_precompute>::type> {
  using yes = int;
};
...
struct metric_requires_initialisation<
    MetricType,
    typename Void2<typename MetricType::requires_initialisation>::type> {
  using yes = int;
};
} // namespace
//! \endcond
...
public:
  ...
  Evaluate(const MetricType &metric_,
  ...
  //  metric_requires_precompute<U>::yes: operator() loops over processed_image instead of
  //  midway_image
  template <class U = MetricType>
  default_type operator()(const Eigen::Matrix<default_type, Eigen::Dynamic, 1> &x,
    ...
    if (directions.cols()) {
      ...
      im1_image_reoriented =
          make_shared<Image<default_type>>(Image<default_type>::scratch(params.im1_image));
      im2_image_reoriented =
          make_shared<Image<default_type>>(Image<default_type>::scratch(params.im2_image));
    ...
    metric.precompute(params);
    {
      ...
      ThreadKernel<MetricType, ParamType> kernel(
          metric, params, overall_cost_function, gradient, &overlap_count);
      {
        LogLevelLatch log_level(0);
        ThreadedLoop(params.processed_image, 0, 3).run(kernel);
      }
    }
    DEBUG("Metric evaluate iteration: " + str(iteration++) +
          ", cost: " + str(overall_cost_function.transpose()));
  ...
  //       if (overlap_count)
  //         *overlap_count = 0;
  //       StochasticThreadKernel <MetricType, ParamType> functor (loop.inner_axes,
  //       params.loop_density, metric, params, cost, gradient, rng, overlap_count); loop.run_outer
  //       (functor);
  //     }
  //     else {
  //       if (overlap_count)
  ...
  //         params.robust_estimate_subset_from, params.robust_estimate_subset_size); ThreadedLoop
  //         (midway_subset, 0, 3).run (kernel);
  //       } else {
  ...
  default_type operator()(const Eigen::Matrix<default_type, Eigen::Dynamic, 1> &x,
    ...
    if (params.loop_density < 1.0) {
      ...
      if (params.robust_estimate_subset) {
        ...
        Adapter::Subset<decltype(params.processed_mask)> subset(params.processed_mask,
                                                                params.robust_estimate_subset_from,
                                                                params.robust_estimate_subset_size);
        LogLevelLatch log_level(0);
        // single threaded as we loop over small VOIs. multi-threading of small VOIs is VERY slow
