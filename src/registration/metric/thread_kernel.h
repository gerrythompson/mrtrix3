...
struct is_neighbourhood_metric<MetricType,
                               typename Void<typename MetricType::is_neighbourhood>::type> {
  using yes = int;
};
...
template <class MetricType>
struct use_processed_image<MetricType,
                           typename Void<typename MetricType::requires_precompute>::type> {
  using yes = int;
};
...
public:
  ThreadKernel(const MetricType &metric,
    ...
    if (params.robust_estimate_subset) {
      ...
      auto spacing = Eigen::DiagonalMatrix<default_type, 3>(params.midway_image.spacing(0),
                                                            params.midway_image.spacing(1),
                                                            params.midway_image.spacing(2));
      for (size_t j = 0; j < 3; ++j)
        for (size_t i = 0; i < 3; ++i)
          i2s(i, 3) +=
              params.robust_estimate_subset_from[j] * params.midway_image.spacing(j) * i2s(i, j);
  ...
  void operator()(const Iterator &iter,
    ...
    Eigen::Vector3d voxel_pos(
        (default_type)iter.index(0), (default_type)iter.index(1), (default_type)iter.index(2));
  ...
  void operator()(const Iterator &iter,
    ...
    params.transformation.transform_half(
        im1_point,
        im2_point); // transform_half is full transformation, transform_half_inverse is identity
    if (params.im1_mask_interp) {
      params.im1_mask_interp->scanner(im1_point);
      if (params.im1_mask_interp->value() < 0.5)
        return;
...
public:
  ...
  void operator()(const Iterator &iter) {
    auto engine = std::default_random_engine{
        static_cast<std::default_random_engine::result_type>(rng.get_seed())};
    auto kern =
        ThreadKernel<MetricType, ParamType>(metric, params, cost_function, gradient, overlap_count);
