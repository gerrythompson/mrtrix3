...
FORCE_INLINE void check_image_output(const std::string &image_name, const Header &reference) {
  ...
  if (Path::exists(image_name) && !App::overwrite_files)
    throw Exception("output image \"" + image_name +
...
struct MultiContrastSetting {
  ...
  ssize_t image_lmax; // lmax available in image (preloaded, might differ from original image), 0 if
                      // not an FOD image
  default_type weight;
...
inline std::ostream &operator<<(std::ostream &o, const MultiContrastSetting &a) {
  o << "MultiContrast: [start:" << a.start << ", nvols:" << a.nvols << ", lmax:" << a.lmax
    << ", image_lmax:" << a.image_lmax << ", reorient:" << a.do_reorientation
    << ", weight:" << a.weight << "]";
  return o;
}
