...
public:
  ...
  void operator()(InputDeformationFieldType &deform_input,
                  OutputDeformationFieldType &deform_output) {
...
public:
  ...
  ComposeLinearDispKernel(const transform_type &linear_transform,
                          const DisplacementFieldType &disp_in)
  ...
  void operator()(DisplacementFieldType &disp_input, DeformationFieldType &deform_output) {
    ...
    deform_output.row(3) = linear_transform * (image_transform.voxel2scanner * voxel +
                                               Eigen::Vector3d(disp_input.row(3)));
  }
...
public:
  ComposeDispKernel(Image<default_type> &disp_input1,
                    Image<default_type> &disp_input2,
                    default_type step)
  ...
  void operator()(Image<default_type> &disp_input1, Image<default_type> &disp_output) {
    Eigen::Vector3d voxel((default_type)disp_input1.index(0),
                          (default_type)disp_input1.index(1),
                          (default_type)disp_input1.index(2));
...
public:
  ...
  void operator()(Image<default_type> &deform) {
    Eigen::Vector3d voxel((default_type)deform.index(0),
                          (default_type)deform.index(1),
                          (default_type)deform.index(2));
...
// Compose a linear transform and a displacement field. The output field is a deformation field. The
...
// Compose a linear transform and a deformation field. The output field is a deformation field. The
// input and output can be the same image.
template <class InputDeformationFieldType, class OutputDeformationFieldType>
...
// Compose two displacement fields and output a displacement field. The input and output can be the
// same image.
...
// Compose two displacement fields and output a displacement field using scaling and squaring.  The
// input and output can be the same image.
FORCE_INLINE void update_displacement_scaling_and_squaring(Image<default_type> &input,
  ...
  default_type min_vox_size = static_cast<default_type>(
      std::min(input.spacing(0), std::min(input.spacing(1), input.spacing(2))));

  // if the maximum update is larger than half a voxel, perform scaling and squaring to ensure the
  // displacement field remains diffeomorphic
  ...
  if (max_norm * step < min_vox_size / 2.0) {
    ...
    scale_factor =
        std::pow(2, std::ceil(std::log((max_norm * step) / (min_vox_size / 2.0)) / std::log(2.0)));
...
template <class DeformationField1Type,
...
FORCE_INLINE WarpType compute_full_deformation(WarpType &warp,
  ...
  if (from == 1) {
    ...
    Registration::Warp::compute_full_deformation(
        linear2.inverse(), mid_to_im2, im1_to_mid, linear1, deform);
  } else {
    index[0] = 1;
    ...
    Registration::Warp::compute_full_deformation(
        linear1.inverse(), mid_to_im1, im2_to_mid, linear2, deform);
  }
  return deform;
}
