...
public:
  ...
  void operator()(Image<default_type> &inv_deform) {
    Eigen::Vector3d voxel((default_type)inv_deform.index(0),
                          (default_type)inv_deform.index(1),
                          (default_type)inv_deform.index(2));
    Eigen::Vector3d truth = transform.voxel2scanner * voxel;
...
FORCE_INLINE void invert_deformation(Image<default_type> &deform_field,
  ...
  error_tolerance *=
      (deform_field.spacing(0) + deform_field.spacing(1) + deform_field.spacing(2)) / 3;

  if (!is_initialised)
    displacement2deformation(inv_deform_field, inv_deform_field);

  ThreadedLoop("inverting warp field...", inv_deform_field, 0, 3)
      .run(DeformationThreadKernel(deform_field, inv_deform_field, max_iter, error_tolerance),
           inv_deform_field);
}

/*! Estimate the inverse of a displacement field, output the inverse as a deformation field
 * Note that the output inv_warp can be passed as either a zero field or an initial estimate (as a
 * deformation field)
 */
...
FORCE_INLINE void invert_displacement(Image<default_type> &disp_field,
  ...
  ThreadedLoop("inverting displacement field...", inv_disp_field, 0, 3)
      .run(DisplacementThreadKernel(disp_field, inv_disp_field, max_iter, error_tolerance),
           inv_disp_field);
}
