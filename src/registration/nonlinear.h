...
public:
  ...
  void run(TransformType linear_transform,
    ...
    if (!is_initialised) {
      ...
      // define transfomations that will be applied to the image header when the common space is
      // calculated
      midway_image_header =
          compute_minimum_average_header(im1_image,
                                         im2_image,
                                         linear_transform.get_transform_half_inverse(),
                                         linear_transform.get_transform_half());
    } else {
    ...
    if (max_iter.size() == 1)
      ...
      throw Exception("the max number of non-linear iterations needs to be defined for each "
                      "multi-resolution level (scale_factor)");
    ...
    for (size_t level = 0; level < scale_factor.size(); level++) {
      if (is_initialised) {
        ...
        if (do_reorientation) {
          CONSOLE("nonlinear stage " + str(level + 1) + ", scale factor " +
                  str(scale_factor[level]) + ", lmax " + str(fod_lmax[level]));
        } else {
          CONSOLE("nonlinear stage " + str(level + 1) + ", scale factor " +
                  str(scale_factor[level]));
        }
      }
      ...
      default_type update_smoothing_mm =
          update_smoothing *
          ((midway_image_header_resized.spacing(0) + midway_image_header_resized.spacing(1) +
      ...
      if (stage_contrasts.size()) {
        ...
        MultiContrastSetting mc(
            im1_image.ndim() < 4 ? 1 : im1_image.size(3), do_reorientation, fod_lmax[level]);
      ...
      auto im1_smoothed = Registration::multi_resolution_lmax(
          im1_image, scale_factor[level], do_reorientation, stage_contrasts);
      ...
      default_type grad_step_altered =
          gradient_step *
          (field_header.spacing(0) + field_header.spacing(1) + field_header.spacing(2)) / 3.0;
      ...
      while (!converged) {
        ...
        if (iteration > 1) {
          ...
          Warp::update_displacement_scaling_and_squaring(
              *im1_to_mid, *im1_update, *im1_to_mid_new, grad_step_altered);
          Warp::update_displacement_scaling_and_squaring(
              *im2_to_mid, *im2_update, *im2_to_mid_new, grad_step_altered);
          ...
          Registration::Warp::compose_linear_displacement(
              im1_to_mid_linear, *im1_to_mid_new, im1_deform_field);
          Registration::Warp::compose_linear_displacement(
              im2_to_mid_linear, *im2_to_mid_new, im2_deform_field);
        } else {
          Registration::Warp::compose_linear_displacement(
              im1_to_mid_linear, *im1_to_mid, im1_deform_field);
          Registration::Warp::compose_linear_displacement(
              im2_to_mid_linear, *im2_to_mid, im2_deform_field);
        }
        ...
        if (do_reorientation && fod_lmax[level]) {
          ...
          Registration::Transform::reorient_warp(
              im1_warped, im1_deform_field, aPSF_directions, false, stage_contrasts);
          Registration::Transform::reorient_warp(
              im2_warped, im2_deform_field, aPSF_directions, false, stage_contrasts);
        }
        ...
        if (im1_image.ndim() == 4) {
          ...
          Metric::Demons4D<Im1ImageType, Im2ImageType, Im1MaskType, Im2MaskType> metric(
              ...
              im2_mask_warped,
              &stage_contrasts);
          ThreadedLoop(im1_warped, 0, 3)
              .run(metric, im1_warped, im2_warped, *im1_update_new, *im2_update_new);
        } else {
          if (use_cc) {
        ...
        if (converged && diagnostics_image_prefix.size()) {
          ...
          //   throw Exception ("diagnostics image file \"" + oss.str() + "\" already exists (use
          //   -force option to force overwrite)");
    ...
    if (has_negative_jacobians(*im1_to_mid) || has_negative_jacobians(*im2_to_mid) ||
      ...
      WARN("final warp computed is not diffeomorphic (negative jacobian determinants detected). "
           "Try increasing -nl_disp_smooth or -nl_update_smooth regularisation.");
  }
...
protected:
  std::shared_ptr<Image<default_type>> reslice(Image<default_type> &image, Header &header) {
    std::shared_ptr<Image<default_type>> temp =
        make_shared<Image<default_type>>(Image<default_type>::scratch(header));
  ...
  // Internally the warp is stored as a displacement field to enable easy smoothing near the
  // boundaries
