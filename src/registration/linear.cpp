...
const OptionGroup adv_init_options =
    ...
    Option("init_translation.unmasked2",
           "disregard mask2 for the translation initialisation (affects 'mass')")
    ...
    Option("init_rotation.search.scale",
           "relative size of the images used for the rotation search. (Default: 0.15)") +
    Argument("scale").type_float(0.0001, 1.0) +
    Option("init_rotation.search.directions",
           "number of rotation axis for local search. (Default: 250)") +
...
const OptionGroup lin_stage_options =
    ...
    Option("linstage.iterations",
           "number of iterations for each registration stage, not to be confused with -rigid_niter "
           "or -affine_niter. "
           "This can be used to generate intermediate diagnostics images "
           "(-linstage.diagnostics.prefix) "
           "or to change the cost function optimiser during registration (without the need to "
           "repeatedly resize the images). (Default: 1 == no repetition)") +
    Argument("num or comma separated list").type_sequence_int()

    // TODO linstage.loop_density

    // TODO linstage.robust: Start each stage repetition with the estimated parameters from the
    // previous stage. choose parameter consensus criterion: maximum overlap, min cost

    + Option("linstage.optimiser.first",
             "Cost function optimisation algorithm to use at first iteration of all stages. "
             "Valid choices: bbgd (Barzilai-Borwein gradient descent) or gd (simple gradient "
             "descent). (Default: bbgd)") +
    Argument("algorithm").type_choice(linear_optimisation_algo_choices) +
    Option("linstage.optimiser.last",
           "Cost function optimisation algorithm to use at last iteration of all stages (if there "
           "are more than one). "
           "Valid choices: bbgd (Barzilai-Borwein gradient descent) or gd (simple gradient "
           "descent). (Default: bbgd)") +
    Argument("algorithm").type_choice(linear_optimisation_algo_choices) +
    Option("linstage.optimiser.default",
           "Cost function optimisation algorithm to use at any stage iteration other than first or "
           "last iteration. "
           "Valid choices: bbgd (Barzilai-Borwein gradient descent) or gd (simple gradient "
           "descent). (Default: bbgd)") +
    Argument("algorithm").type_choice(linear_optimisation_algo_choices)
...
const OptionGroup rigid_options =
    ...
    + Option("rigid_init_rotation",
             ...
             "moments (rotation based on directions of intensity variance with respect to centre "
             "of mass), \n"
    ...
    + Option("rigid_init_matrix",
             ...
             "Note that this overrides rigid_init_translation and rigid_init_rotation "
             "initialisation ") // TODO definition of centre
    ...
    + Option("rigid_niter",
             "the maximum number of gradient descent iterations per stage. This can be specified "
             "either as a single number "
    ...
    // + Option ("rigid_loop_density", "density of gradient descent 1 (batch) to 0.0 (max
    // stochastic) (Default: 1.0)")
    ...
    + Option("rigid_lmax",
             "explicitly set the lmax to be used per scale factor in rigid FOD registration. By "
             "default FOD registration will "
             "use lmax 0,2,4 with default scale factors 0.25,0.5,1.0 respectively. Note that no "
...
const OptionGroup affine_options =
    ...
    + Option("affine_init_matrix",
             ...
             "Note that this overrides affine_init_translation and affine_init_rotation "
             "initialisation ") // TODO definition of centre
    + Argument("file").type_file_in()
    ...
    // + Option ("affine_loop_density", "density of gradient descent 1 (batch) to 0.0 (max
    // stochastic) (Default: 1.0)")
    //   + Argument ("num").type_sequence_float () // TODO
    ...
    + Option("affine_lmax",
             "explicitly set the lmax to be used per scale factor in affine FOD registration. By "
             "default FOD registration will "
             "use lmax 0,2,4 with default scale factors 0.25,0.5,1.0 respectively. Note that no "
             "reorientation will be performed with lmax = 0.") +
    Argument("num").type_sequence_int()
...
const OptionGroup fod_options =
    ...
    Argument("file", "a list of directions [az el] generated using the gendir command.")
        .type_file_in()
