...
void read_annot(const std::string &path, label_vector_type &labels, Connectome::LUT &lut) {
  ...
  if (!in.good()) {
    WARN("FreeSurfer annotation file \"" + Path::basename(path) +
         "\" does not contain colortable information");
  ...
  if (num_entries > 0) {
    ...
    for (int32_t i = 0; i != num_entries_to_read; ++i) {
      ...
      const int32_t id = r + (g << 8) + (b << 16) + (flag << 24);
      lut.insert(
          std::make_pair(structure, Connectome::LUT_node(std::string(struct_name.get()), r, g, b)));
