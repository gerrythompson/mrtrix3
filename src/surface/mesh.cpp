      ...
      if (line.substr(0, 6) == "POINTS") {
        ...
        while (polygon_count < num_polygons && element_count < num_elements) {
          ...
          if (vertex_count != 3 && vertex_count != 4)
            throw Exception("Could not parse file \"" + path +
                            "\";  only suppport 3- and 4-vertex polygons");
...
void Mesh::load_stl(const std::string &path) {
  ...
  if (strncmp(init, "solid", 5)) {
    ...
    while (in.read(reinterpret_cast<char *>(normal.data()), 3 * sizeof(float))) {
      ...
      triangles.push_back(vector<uint32_t>{uint32_t(vertices.size() - 3),
    ...
    while (std::getline(in, line)) {
      ...
      if (line.substr(0, 12) == "facet normal") {
        if (!inside_solid)
          throw Exception("Error parsing STL file " + Path::basename(path) +
                          ": facet outside solid");
        if (inside_facet)
        ...
        if (!inside_loop)
          throw Exception("Error parsing STL file " + Path::basename(path) +
                          ": loop ending without start");
        if (!inside_facet)
          throw Exception("Error parsing STL file " + Path::basename(path) +
                          ": loop ending outside facet");
        inside_loop = false;
      } else if (line.substr(0, 8) == "endfacet") {
        if (inside_loop)
          throw Exception("Error parsing STL file " + Path::basename(path) +
                          ": facet ending inside loop");
        if (!inside_facet)
          throw Exception("Error parsing STL file " + Path::basename(path) +
                          ": facet ending without start");
        inside_facet = false;
        if (vertex_index != 3)
          throw Exception("Error parsing STL file " + Path::basename(path) + ": facet ended with " +
        ...
        if (inside_facet)
          throw Exception("Error parsing STL file " + Path::basename(path) +
                          ": solid ending inside facet");
        inside_solid = false;
      } else if (line.substr(0, 5) == "solid") {
        throw Exception("Error parsing STL file " + Path::basename(path) +
                        ": multiple solids in file");
      } else {
        throw Exception("Error parsing STL file " + Path::basename(path) + ": unknown key (" +
                        line + ")");
      }
    }
    if (inside_solid)
  ...
  while (std::getline(in, line)) {
    ...
    if (prefix == "v") {
      ...
      if (elements.size() != 3 && elements.size() != 4)
        throw Exception("Malformed face information in input OBJ file (face with neither 3 nor 4 "
                        "vertices; line " +
                        str(counter) + ")");
      ...
      for (vector<std::string>::iterator i = elements.begin(); i != elements.end(); ++i) {
        ...
        if (!values_per_element)
          ...
          throw Exception("Malformed face information in input OBJ file (inconsistent vertex / "
                          "texture / normal detail); line " +
                          str(counter));
      ...
      // The OBJ format allows defining different vertex-based normals for different faces that
      // reference the same vertex This isn't consistent with the internal storage mechanism used in
      // the Mesh class, and isn't really a feature
  ...
  if (magic_number == FreeSurfer::triangle_file_magic_number) {
    ...
    auto load_triangles = [&]() {
      ...
      try {
        ...
        throw Exception("Error reading FreeSurfer file: Memory allocation (" + str(num_vertices) +
                        " vertices, " + str(num_polygons) + " polygons = erroneous?)");
      }
      for (int32_t i = 0; i != num_vertices; ++i) {
        ...
        if (!in.good())
          throw Exception("Error reading FreeSurfer file: EOF reached after " +
                          str(vertices.size()) + " of " + str(num_vertices) + " vertices");
      ...
      for (int32_t i = 0; i != num_polygons; ++i) {
        ...
        if (!in.good())
          throw Exception("Error reading FreeSurfer file: EOF reached after " +
                          str(triangles.size()) + " of " + str(num_polygons) + " triangles");
        triangles.push_back(Triangle(temp));
      }
    };
...
void Mesh::save_vtk(const std::string &path, const bool binary) const {
  ...
  if (binary) {
    ...
    out << "POLYGONS " + str(triangles.size() + quads.size()) + " " +
               str(4 * triangles.size() + 5 * quads.size()) + "\n";
    for (TriangleList::const_iterator i = triangles.begin(); i != triangles.end(); ++i) {
...
void Mesh::save_obj(const std::string &path) const {
  ...
  for (QuadList::const_iterator q = quads.begin(); q != quads.end(); ++q)
    out << "f " << str((*q)[0] + 1) << " " << str((*q)[1] + 1) << " " << str((*q)[2] + 1) << " "
        << str((*q)[3] + 1) << "\n";
}
