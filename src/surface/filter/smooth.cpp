...
void Smooth::operator()(const Mesh &in, Mesh &out) const {
  ...
  for (TriangleList::const_iterator p = in.triangles.begin(); p != in.triangles.end(); ++p) {
    centroids.push_back((in.vertices[(*p)[0]] + in.vertices[(*p)[1]] + in.vertices[(*p)[2]]) *
                        (1.0 / 3.0));
  ...
  for (size_t iter = 0; iter != 8; ++iter) {
    for (uint32_t v = 0; v != V; ++v) {
      ...
      // Find polygons at the outer edge of this expanding front, and add them to the neighbourhood
      // for this vertex
  ...
  for (uint32_t v = 0; v != V; ++v) {
    ...
    for (std::set<uint32_t>::const_iterator it = vert_polys[v].begin(); it != vert_polys[v].end();
         ++it) {
      const uint32_t i = *it;
  ...
  VertexList tangents;
  for (TriangleList::const_iterator p = mollified_mesh.triangles.begin();
       p != mollified_mesh.triangles.end();
       ++p)
