...
Scalar::Scalar(const std::string &path, const Mesh &mesh) {
  ...
  if (size_t(size()) != mesh.num_vertices())
    throw Exception("Input surface scalar file \"" + path +
                    "\" has incorrect number of vertices (" + str(size()) + ", mesh has " +
                    str(mesh.num_vertices()) + ")");
  name = Path::basename(path);
}

void Scalar::save(const std::string &path) const { save_vector(*this, path); }
  ...
  for (int32_t i = 0; i != num_entries; ++i) {
    ...
    if (size_t(index) >= mesh.num_vertices())
      throw Exception("Error opening file \"" + path +
                      "\" as FreeSurfer w-file: invalid vertex index (" + str(index) +
...
void Scalar::load_fs_curv(const std::string &path, const Mesh &mesh) {
  ...
  if (magic_number == FreeSurfer::new_curv_file_magic_number) {
    ...
    if (size_t(num_vertices) != mesh.num_vertices())
      throw Exception("Error opening file \"" + path +
                      "\" as Freesurfer curv file: Incorrect number of vertices (" +
    ...
    if (size_t(num_faces) != mesh.num_polygons())
      throw Exception("Error opening file \"" + path +
                      "\" as Freesurfer curv file: Incorrect number of polygons (" +
