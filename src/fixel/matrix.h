...
public:
  ...
  InitElement() : fixel_index(std::numeric_limits<fixel_index_type>::max()), track_count(0) {}
  InitElement(const fixel_index_type fixel_index) : fixel_index(fixel_index), track_count(1) {}
  InitElement(const fixel_index_type fixel_index, const ValueType track_count)
      : fixel_index(fixel_index), track_count(track_count) {}
  ...
  FORCE_INLINE bool operator<(const InitElement &that) const {
    return fixel_index < that.fixel_index;
  }
...
public:
  ...
  FORCE_INLINE void exponentiate(const ValueType C) {
    connectivity_value = std::pow(connectivity_value, C);
  }
...
public:
  ...
  NormFixel() : norm_multiplier(connectivity_value_type(1)) {}
  NormFixel(const BaseType &i) : BaseType(i), norm_multiplier(connectivity_value_type(1)) {}
  NormFixel(BaseType &&i) : BaseType(std::move(i)), norm_multiplier(connectivity_value_type(1)) {}
  void normalise() {
    norm_multiplier = connectivity_value_type(0);
    for (const auto &c : *this)
      norm_multiplier += c.value();
    norm_multiplier = norm_multiplier ? (connectivity_value_type(1) / norm_multiplier)
                                      : connectivity_value_type(0);
  }
