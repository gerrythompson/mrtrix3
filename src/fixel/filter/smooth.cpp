...
// TODO Provide an alternative functor for when the input & output images contain more than one
// column
//   (more efficient to construct the smoothing kernel once per fixel, then loop over image
//   channels)
void Smooth::operator()(Image<float> &input, Image<float> &output) const {
        ...
        for (const auto &c : connectivity) {
          ...
          if (mask.value() && std::isfinite(static_cast<float>(input.value()))) {
            const Matrix::connectivity_value_type weight =
                ...
                std::exp(master.gaussian_const2 *
                         (master.fixel_positions[c.index()] - pos).squaredNorm());
  ...
  Thread::run_queue(
      Source(input.size(0)), Thread::batch(size_t()), Thread::multi(Worker(*this, input, output)));
}
