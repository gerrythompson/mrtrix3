...
void Connect::operator()(Image<float> &input, Image<float> &output) const {
  ...
  if (input.ndim() > 1 && input.size(1) != 1)
    throw Exception("Fixel connected component filter cannot be applied to fixel data files with "
      ...
      if (input.value() >= value_threshold) {
        ...
        while (to_expand.size()) {
          ...
          for (const auto &c : connections) {
            ...
            if (!processed[c.index()] && c.value() >= connectivity_threshold &&
                input.value() >= value_threshold) {
              ++cluster_size;
              processed[c.index()] = true;
              to_expand.push(c.index());
            } // Finish adding the other fixel in this connection to the list of fixels to expand
              // the cluster
  ...
  std::sort(
      cluster_sizes.begin(),
      cluster_sizes.end(),
      [](const IndexAndSize &a, const IndexAndSize &b) -> bool { return (a.second > b.second); });
