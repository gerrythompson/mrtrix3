...
void InitFixel::add(const vector<index_type> &indices) {
  ...
  // For each output vector location, need to determine whether it should come from copying an
  // existing entry,
...
init_matrix_type generate(const std::string &track_filename,
  ...
  public:
    ...
    bool operator()(const DWI::Tractography::Streamline<> &tck, vector<index_type> &out) const {
  ...
  DWI::Tractography::Mapping::TrackLoader loader(
      track_file, num_tracks, "computing fixel-fixel connectivity matrix");
  DWI::Tractography::Mapping::TrackMapperBase mapper(index_image);
  mapper.set_upsample_ratio(
      DWI::Tractography::Mapping::determine_upsample_ratio(index_image, properties, 0.333f));
  mapper.set_use_precise_mapping(true);
  TrackProcessor track_processor(
      mapper, index_image, directions_image, fixel_mask, angular_threshold);
...
void normalise_and_write(init_matrix_type &matrix,
  ...
  index_type num_connections = 0;
  {
    ...
    for (size_t fixel_index = 0; fixel_index != matrix.size(); ++fixel_index) {
      const connectivity_value_type normalisation_factor =
          connectivity_value_type(1) / connectivity_value_type(matrix[fixel_index].count());
  ...
  try {
    ...
    value_image =
        Image<connectivity_value_type>::create(Path::join(path, "values.mif"), value_header);
  } catch (Exception &e) {
    throw Exception(
        e, "Unable to allocate space on filesystem for fixel-fixel connectivity matrix data");
  }

  ProgressBar progress("Normalising and writing fixel-fixel connectivity matrix to directory \"" +
                           path + "\"",
                       matrix.size());
  for (size_t fixel_index = 0; fixel_index != matrix.size(); ++fixel_index) {
...
Reader::Reader(const std::string &path, const Image<bool> &mask)
    : directory(path), mask_image(mask) {
  try {
    ...
    if (value_image.size(0) != fixel_image.size(0))
      throw Exception("Number of fixels in value image (" + str(value_image.size(0)) +
                      ") does not match number of fixels in fixel image (" +
                      str(fixel_image.size(0)) + ")");
    if (mask_image.valid() && size_t(mask_image.size(0)) != size())
      throw Exception("Fixel image \"" + mask_image.name() + "\" has different number of fixels (" +
                      str(mask_image.size(0)) + ") to fixel-fixel connectivity matrix (" +
                      str(size()) + ")");
  } catch (Exception &e) {
    throw Exception(e,
                    "Unable to load path \"" + directory + "\" as fixel-fixel connectivity data");
  }
}
