...
const Entry maps[] = {
    Entry("Gray",
          ...
          [](float amplitude) {
            return Eigen::Array3f(clamp(amplitude), clamp(amplitude), clamp(amplitude));
          }),

    Entry("Hot",
          "color.rgb = vec3 (2.7213 * amplitude, 2.7213 * amplitude - 1.0, 3.7727 * amplitude - "
          "2.7727);\n",
          [](float amplitude) {
            return Eigen::Array3f(clamp(2.7213f * amplitude),
                                  clamp(2.7213f * amplitude - 1.0f),
                                  clamp(3.7727f * amplitude - 2.7727f));
          }),

    Entry("Cool",
          "color.rgb = 1.0 - (vec3 (2.7213 * (1.0 - amplitude), 2.7213 * (1.0 - amplitude) - 1.0, "
          "3.7727 * (1.0 - amplitude) - 2.7727));\n",
          [](float amplitude) {
    ...
    // The Inferno and Viridis colour maps are implemented using a 6th order polynomial
    // approximation of the originals, ported from https://www.shadertoy.com/view/WlfXRN (public
    // domain; CC0 license).
    ...
    Entry(
        ...
        "color.rgb = clamp( c0 + amplitude*(c1 + amplitude*(c2 + amplitude*(c3 + amplitude*(c4 + "
        "amplitude*(c5 + amplitude*c6))))), 0.0, 1.0);\n",
        [](float amplitude) {
          ...
          Eigen::Array3f rgb =
              ...
              amplitude *
                  (c1 + amplitude *
                            (c2 + amplitude *
                                      (c3 + amplitude * (c4 + amplitude * (c5 + amplitude * c6)))));
    ...
    Entry("PET",
          ...
          "color.g = clamp (2.0 * (0.25 - abs (amplitude - 0.25)), 0.0, 1.0) + clamp "
          "(2.0*amplitude - 1.0, 0.0, 1.0);\n"
          "color.b = 1.0 - (clamp (1.0 - 2.0 * amplitude, 0.0, 1.0) + clamp (1.0 - 4.0 * abs "
          "(amplitude - 0.75), 0.0, 1.0));\n",
          [](float amplitude) {
