...
LUT::file_format LUT::guess_file_format(const std::string &path) {
    ...
    if (line.size() > 1 && line[0] != '#') {
      ...
      if (!(split_by_quotes.size() % 2))
        throw Exception("Line " + str(line_counter) + " of LUT file \"" + Path::basename(path) +
                        "\" contains an odd number of quotation marks, and hence cannot be "
                        "properly split up according to quotation marks");
      ...
      if (entries.size()) {
        if (columns.size() && entries.size() != columns.size()) {
          Exception E("Inconsistent number of columns in LUT file \"" + Path::basename(path) +
                      "\"");
          E.push_back("Initial file contents contain " + str(columns.size()) +
                      " columns, but line " + str(line_counter) + " contains " +
                      str(entries.size()) + " entries:");
          E.push_back("\"" + line + "\"");
          throw E;
        }
        if (columns.empty())
  ...
  if (columns.size() == 2 && columns[0].is_integer() && !columns[1].is_numeric()) {
    DEBUG("LUT file \"" + Path::basename(path) +
          "\" contains 1 integer, 1 string per line: Basic format");
    return LUT_BASIC;
  }
  if (columns.size() == 6 && columns[0].is_integer() && !columns[1].is_numeric() &&
      columns[2].is_8bit() && columns[3].is_8bit() && columns[4].is_8bit() &&
      columns[5].is_8bit()) {
    DEBUG("LUT file \"" + Path::basename(path) +
  ...
  if (columns.size() == 8 && columns[0].is_integer() && columns[1].is_8bit() &&
      columns[2].is_8bit() && columns[3].is_8bit() && columns[4].is_unary_range_float() &&
      columns[5].is_integer() && columns[6].is_integer() && !columns[7].is_numeric()) {
    DEBUG("LUT file \"" + Path::basename(path) +
          "\" contains an integer, 3 8-bit integers, a float, two integers, and a string per line: "
          "ITKSNAP format");
    return LUT_ITKSNAP;
  }
  if (columns.size() == 7 && columns[0].is_integer() && !columns[1].is_numeric() &&
      !columns[2].is_numeric() && columns[1].mean_length() < columns[2].mean_length() &&
...
void LUT::parse_line_itksnap(const std::string &line) {
  ...
  char name[80];
  sscanf(
      line.c_str(), "%u %u %u %u %f %u %u %s", &index, &r, &g, &b, &a, &label_vis, &mesh_vis, name);
  if (index != std::numeric_limits<node_t>::max()) {
