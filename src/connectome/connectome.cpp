...
void check(const Header &H) {
  if (!(H.ndim() == 3 || (H.ndim() == 4 && H.size(3) == 1)))
    throw Exception("Image \"" + H.name() +
                    "\" is not 3D, and hence is not a volume of node parcel indices");
  if (H.datatype().is_floating_point()) {
    ...
    // WARN ("Image \"" + H.name() + "\" stored as floating-point; it is preferable to store label
    // images using an unsigned integer type");
  } else if (H.datatype().is_signed()) {
    CONSOLE("Image \"" + H.name() +
            "\" stored with signed integer type; need to check for negative values");
    ...
    // WARN ("Image \"" + H.name() + "\" stored as signed integer; it is preferable to store label
    // images using an unsigned integer type");
  }
}
