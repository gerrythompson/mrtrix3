...
PreProcessor::PreProcessor(const std::shared_ptr<Math::Stats::GLM::TestBase> stats_calculator,
    ...
    : stats_calculator(stats_calculator),
      ...
      enhanced_sum(
          matrix_type::Zero(stats_calculator->num_elements(), stats_calculator->num_hypotheses())),
      enhanced_count(count_matrix_type::Zero(stats_calculator->num_elements(),
                                             stats_calculator->num_hypotheses())),
...
Processor::Processor(const std::shared_ptr<Math::Stats::GLM::TestBase> stats_calculator,
    ...
    : stats_calculator(stats_calculator),
      ...
      null_dist_contribution_counter(count_matrix_type::Zero(stats_calculator->num_elements(),
                                                             stats_calculator->num_hypotheses())),
      global_uncorrected_pvalue_counter(global_uncorrected_pvalue_counter),
      uncorrected_pvalue_counter(count_matrix_type::Zero(stats_calculator->num_elements(),
                                                         stats_calculator->num_hypotheses())),
      mutex(new std::mutex()) {
  assert(stats_calculator);
}
...
void precompute_empirical_stat(const std::shared_ptr<Math::Stats::GLM::TestBase> stats_calculator,
  ...
  empirical_statistic =
      matrix_type::Zero(stats_calculator->num_elements(), stats_calculator->num_hypotheses());
  count_matrix_type global_enhanced_count(count_matrix_type::Zero(
      stats_calculator->num_elements(), stats_calculator->num_hypotheses()));
  {
    Math::Stats::Shuffler shuffler(
        stats_calculator->num_inputs(),
        true,
        "Pre-computing empirical statistic for non-stationarity correction");
    PreProcessor preprocessor(
        stats_calculator, enhancer, skew, empirical_statistic, global_enhanced_count);
    ...
    for (size_t ie = 0; ie != stats_calculator->num_elements(); ++ie) {
      if (global_enhanced_count(ie, contrast) > 0)
        empirical_statistic(ie, contrast) =
            std::pow(empirical_statistic(ie, contrast) /
                         static_cast<default_type>(global_enhanced_count(ie, contrast)),
                     1.0 / skew);
      else
...
void precompute_default_permutation(
  ...
  assert(stats_calculator);
  ProgressBar progress(std::string("Running GLM ") +
                           (enhancer ? "and enhancement algorithm " : "") +
                           "for default permutation",
                       4);
...
void run_permutations(const std::shared_ptr<Math::Stats::GLM::TestBase> stats_calculator,
  ...
  count_matrix_type global_uncorrected_pvalue_count(count_matrix_type::Zero(
      stats_calculator->num_elements(), stats_calculator->num_hypotheses()));
  {
  ...
  uncorrected_pvalues =
      global_uncorrected_pvalue_count.cast<default_type>() / default_type(shuffler.size());
}
