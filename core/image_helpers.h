...
FORCE_INLINE auto __ndim(const AxesType &axes) -> decltype(axes.size(), size_t()) {
  return axes.size();
}

template <class AxesType>
FORCE_INLINE auto __ndim(const AxesType &axes) -> decltype(axes.ndim(), size_t()) {
  return axes.ndim();
}

template <class AxesType>
FORCE_INLINE auto __get_index(const AxesType &axes, size_t axis)
...
template <class AxesType>
FORCE_INLINE auto __set_index(AxesType &axes, size_t axis, ssize_t index)
...
template <class ImageType, typename ReturnType> struct enable_if_image_type {
  typedef decltype((void)(std::declval<ImageType>().ndim() + std::declval<ImageType>().size(0) +
                          ...
                          std::declval<ImageType>().value() + std::declval<ImageType>().index(0)),
                   std::declval<ReturnType>()) type;
};
...
template <class ImageType> struct is_pure_image {
  static bool const value =
      std::is_same<ImageType, ::MR::Image<typename ImageType::value_type>>::value;
};
...
//! error if the image does not represent spatial data: need 3 spatial axes all with size greater
//! than 1 requirement for anything that performs 3D interpolation, or erosion (& maybe others not
//! thought of yet)
...
//! error if the image has dimensionality of at least \a N, allowing for higher singleton
//! dimensions. For example, [ x y z ], [ x y z 1 1 ] can both be considered 3D, but [ x y z 1 n ]
//! will throw an exception.
template <class HeaderType>
FORCE_INLINE void check_effective_dimensionality(const HeaderType &in, size_t N) {
  if (in.ndim() < N)
    throw Exception("Image \"" + in.name() + "\" does not represent " + str(N) +
                    "D data (too few dimensions)");
  for (size_t i = N; i < in.ndim(); ++i)
    if (in.size(i) != 1)
      throw Exception("Image \"" + in.name() + "\" does not represent " + str(N) + "D data (axis " +
                      str(i) + " has size " + str(in.size(i)) + ")");
}
...
template <typename ValueType> inline int64_t footprint(int64_t count) {
...
footprint(const HeaderType &in,
          size_t from_dim = 0,
          size_t up_to_dim = std::numeric_limits<size_t>::max()) {
...
template <class HeaderType>
inline typename std::enable_if<std::is_class<HeaderType>::value, int64_t>::type
footprint(const HeaderType &in, const char *specifier) {
...
inline bool spacings_match(const HeaderType1 &in1,
  ...
  for (size_t n = 0; n < axes.size(); ++n) {
    if (in1.ndim() <= axes[n] || in2.ndim() <= axes[n])
      return false;
    if (abs(in1.spacing(axes[n]) - in2.spacing(axes[n])) >
...
template <class HeaderType1, class HeaderType2>
inline bool
dimensions_match(const HeaderType1 &in1, const HeaderType2 &in2, size_t from_axis, size_t to_axis) {
  assert(from_axis < to_axis);
  if (to_axis > in1.ndim() || to_axis > in2.ndim())
    return false;
  for (size_t n = from_axis; n < to_axis; ++n)
...
template <class HeaderType1, class HeaderType2>
inline bool
dimensions_match(const HeaderType1 &in1, const HeaderType2 &in2, const vector<size_t> &axes) {
  for (size_t n = 0; n < axes.size(); ++n) {
    if (in1.ndim() <= axes[n] || in2.ndim() <= axes[n])
      return false;
...
template <class HeaderType1, class HeaderType2>
inline void
check_dimensions(const HeaderType1 &in1, const HeaderType2 &in2, size_t from_axis, size_t to_axis) {
  if (!dimensions_match(in1, in2, from_axis, to_axis))
    throw Exception("dimension mismatch between \"" + in1.name() + "\" and \"" + in2.name() +
                    "\" between axes " + str(from_axis) + " and " + str(to_axis - 1) + " (" +
                    dim2str(in1) + " vs. " + dim2str(in2) + ")");
}

template <class HeaderType1, class HeaderType2>
inline void
check_dimensions(const HeaderType1 &in1, const HeaderType2 &in2, const vector<size_t> &axes) {
  if (!dimensions_match(in1, in2, axes))
    throw Exception("dimension mismatch between \"" + in1.name() + "\" and \"" + in2.name() +
                    "\" for axes [" + join(axes, ",") + "]" + " (" + dim2str(in1) + " vs. " +
                    dim2str(in2) + ")");
}

template <class HeaderType1, class HeaderType2>
inline void check_voxel_grids_match_in_scanner_space(const HeaderType1 &in1,
                                                     const HeaderType2 &in2,
...
//! returns true if the image to scanner transformation and voxel sizes of in1 and in2 are within
//! tolerance tol: tolerance of FOV corner displacement in voxel units
template <class HeaderType1, class HeaderType2>
inline bool voxel_grids_match_in_scanner_space(const HeaderType1 in1,
                                               const HeaderType2 in2,
                                               const double tol = 1.0e-3) {
  ...
  double diff_in_scannercoord =
      std::sqrt((vs1.asDiagonal() * in1.transform().matrix() * voxel_coord -
...
public:
  FORCE_INLINE Index(ImageType &image, size_t axis) : image(image), axis(axis) {
...
public:
  ConstRow(ImageType &image, size_t axis) : axis(axis), image(image) {
    assert(axis < image.ndim());
  }
...
public:
  ...
  FORCE_INLINE Helper::Index<Derived> index(size_t axis) {
    return {static_cast<Derived &>(*this), axis};
  }
  FORCE_INLINE ssize_t index(size_t axis) const {
   ...
   * \code
   * */
  FORCE_INLINE Helper::ConstRow<Derived> row(size_t axis) const {
    return {static_cast<Derived &>(*this), axis};
  }
  FORCE_INLINE Helper::Row<Derived> row(size_t axis) {
    return {static_cast<Derived &>(*this), axis};
  }
};
