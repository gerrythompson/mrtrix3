...
public:
  ...
  friend std::ostream &operator<<(std::ostream &stream, const Image &V) {
    stream << "\"" << V.name() << "\", datatype " << DataType::from<Image::value_type>().specifier()
           << ", index [ ";
   ...
   * auto image = header.get_image<float>().with_direct_io (Stride::contiguous_along_axis (3,
   * header)); \endcode and \code auto image = Image<float>::open (filename).with_direct_io (-1);
   * // or;
   ...
   * \code
   * auto header = Header::open (filename);
   * auto image = header.get_image<float>().with_direct_io (Stride::contiguous_along_spatial_axes
   * (header)); \endcode
   */
  ...
  static Image create(const std::string &image_name,
                      const Header &template_header,
                      bool add_to_command_history = true) {
    return Header::create(image_name, template_header, add_to_command_history)
        .get_image<ValueType>();
  }
...
public:
  ...
  FORCE_INLINE ValueType get_value(size_t offset) const {
    ...
    return fetch_func(io->segment(nseg),
                      offset - nseg * io->segment_size(),
                      intensity_offset(),
                      intensity_scale());
  }
...
Image<ValueType>::Buffer::Buffer(Header &H, bool read_write_if_existing) : Header(H) {
...
template <typename ValueType> void *Image<ValueType>::Buffer::get_data_pointer() {
  ...
  if (io->nsegments() == 1 && datatype() == DataType::from<ValueType>() &&
...
template <typename ValueType>
FORCE_INLINE Image<ValueType>::Image() : data_pointer(nullptr), data_offset(0) {}

template <typename ValueType>
Image<ValueType>::Image(const std::shared_ptr<Image<ValueType>::Buffer> &buffer_p,
                        const Stride::List &desired_strides)
    : buffer(buffer_p),
  ...
  DEBUG("image \"" + name() + "\" initialised with strides = " + str(strides) +
        ", start = " + str(data_offset) + ", using " + (is_direct_io() ? "" : "in") + "direct IO");
}
    ...
    if (buffer->get_io()) {
      if (buffer->get_io()->is_image_readwrite() && buffer->data_buffer) {
        ...
        threaded_copy_with_progress_message(
            "writing back direct IO buffer for \"" + name() + "\"", src, dest);
      }
    }
  }
}
...
Image<ValueType> Image<ValueType>::with_direct_io(Stride::List with_strides) {
  ...
  bool preload =
      (buffer->datatype() != DataType::from<ValueType>()) || (buffer->get_io()->files.size() > 1);
...
std::string Image<ValueType>::dump_to_mrtrix_file(std::string filename, bool) const {
  ...
  if (!out.good())
    throw Exception("error writing back contents of file \"" + data_filename +
                    "\": " + strerror(errno));
  out.close();

  // If data_size exceeds some threshold, ostream artificially increases the file size beyond that
  // required at close()
