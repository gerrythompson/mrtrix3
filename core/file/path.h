...
inline std::string dirname(const std::string &name) {
  ...
  return (i == std::string::npos ? std::string("")
                                 : (i ? name.substr(0, i) : std::string(1, PATH_SEPARATORS[0])));
}
...
inline bool has_suffix(const std::string &name,
                       const std::initializer_list<const std::string> &suffix_list) {
  return std::any_of(suffix_list.begin(), suffix_list.end(), [&](const std::string &suffix) {
