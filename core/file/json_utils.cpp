...
void read(const nlohmann::json &json, Header &header, const bool realign) {
  ...
  if (pe_scheme.rows()) {
    if (do_realign) {
      ...
      INFO("Phase encoding information read from JSON file modified to conform to prior MRtrix3 "
           "internal transform realignment of image \"" +
           header.name() + "\"");
    } else {
  ...
  if (slice_encoding_it != header.keyval().end()) {
    if (do_realign) {
      ...
      INFO("Slice encoding direction read from JSON file modified to conform to prior MRtrix3 "
           "internal transform realignment of input image");
    } else {
...
void write(const Header &header, nlohmann::json &json, const std::string &image_path) {
  ...
  if (pe_scheme.rows()) {
    ...
    INFO("Phase encoding information written to JSON file modified according to expected output "
         "NIfTI header transform realignment");
  }
  ...
  if (slice_encoding_it != H_adj.keyval().end()) {
    ...
    slice_encoding_it->second = Axes::dir2id(new_dir);
    INFO("Slice encoding direction written to JSON file modified according to expected output "
         "NIfTI header transform realignment");
  }
