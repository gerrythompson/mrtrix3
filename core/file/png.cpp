...
Reader::Reader(const std::string &filename)
  ...
  DEBUG("PNG image \"" + filename + "\":  " + str(width) + "x" + str(height) + "; " +
...
Writer::Writer(const Header &H, const std::string &filename)
  ...
  if (Path::exists(filename) && !App::overwrite_files)
    throw Exception("output file \"" + filename +
  ...
  case 4:
    ...
    default:
      png_destroy_write_struct(&png_ptr, &info_ptr);
      throw Exception("Unsupported number of volumes (" + str(H.size(3)) + ") in image \"" +
                      filename + "\" for PNG writer");
    }
    break;
  default:
    png_destroy_write_struct(&png_ptr, &info_ptr);
    throw Exception("Unsupported image dimensionality (" + str(H.ndim()) + ") in image \"" +
                    filename + "\" for PNG writer");
  }
  ...
  if (data_type.is_floating_point()) {
    INFO("Data to be converted to PNG is floating-point; "
         "image will be scaled to integer representation assuming input data is in the range 0.0 - "
         "1.0");
  }
  ...
  png_time modtime;
  png_convert_from_time_t(&modtime,
