...
MMap::MMap(const Entry &entry, bool readwrite, bool preload, int64_t mapped_size)
  ...
  if (readwrite) {
    ...
    if (GetVolumePathName(Entry::name.c_str(), root_path, length)) { // Returns non-zero on success
      ...
      case 0: // DRIVE_UNKNOWN
        DEBUG("cannot get filesystem information on file \"" + Entry::name +
              "\": " + strerror(errno));
      ...
      case 3: // DRIVE_FIXED
        ...
        break;
      case 4: // DRIVE_REMOTE
        DEBUG("Drive for file \"" + Entry::name +
              "\" detected as network - using delayed write-back");
        delayed_writeback = true;
        break;
      case 5: // DRIVE_CDROM
        DEBUG("Drive for file \"" + Entry::name +
              "\" detected as CD-ROM - using delayed write-back");
        delayed_writeback = true;
        break;
      ...
      DEBUG("unable to query root drive path for file \"" + Entry::name +
            "\"; using delayed write-back");
      delayed_writeback = true;
    }
#else
    struct statfs fsbuf;
    ...
    if (fsbuf.f_type == 0xff534d42 /* CIFS */ || fsbuf.f_type == 0x6969 /* NFS */ ||
      ...
      DEBUG("\"" + Entry::name +
            "\" appears to reside on a networked filesystem - using delayed write-back");
      delayed_writeback = true;
    }
#endif
    ...
    if (delayed_writeback) {
      ...
      if (preload) {
        ...
        if (!in.good())
          throw Exception("error preloading contents of file \"" + Entry::name +
                          "\": " + strerror(errno));
      } else
        memset(first, 0, msize);
      DEBUG("file \"" + Entry::name + "\" held in RAM at " + str((void *)first) + ", size " +
            str(msize));
  ...
  try {
    ...
    addr = static_cast<uint8_t *>(MapViewOfFile(
        handle, (readwrite ? FILE_MAP_ALL_ACCESS : FILE_MAP_READ), 0, 0, start + msize));
    if (!addr)
      throw 0;
    CloseHandle(handle);
#else
    addr = static_cast<uint8_t *>(mmap((char *)0,
                                       start + msize,
                                       (readwrite ? PROT_WRITE | PROT_READ : PROT_READ),
                                       MAP_SHARED,
                                       fd,
                                       0));
    if (addr == MAP_FAILED)
