...
inline bool check(Header &H, size_t num_axes) {
  ...
  case DataType::Float64:
    ...
    break;
  default:
    throw Exception("Unsupported data type for MGH format (" +
...
template <class Input> void read_other(Header &H, Input &in) {
  ...
  auto read_mri_frame = [&](Input &in, const int64_t len) {
    ...
    for (size_t frame_index = 0; frame_index != nframes; ++frame_index) {
      ...
      // FreeSurfer loads TM with a float32 here, even though TM is in the diffusion section and is
      // a float64...
      fetch<float32>(in);
      frame.TM = 0.0;
      ...
      std::string line =
          str(frame.type) + "," + str(frame.TE) + "," + str(frame.TR) + "," + str(frame.flip) +
      ...
      if (frame.type == MGH_FRAME_TYPE_DIFFUSION_AUGMENTED) {
        ...
        line += "," + str(frame.DX) + "," + str(frame.DY) + "," + str(frame.DZ) + "," +
                ...
                str(frame.bvalue) + "," + str(frame.TM) + "," + str(frame.D1_ramp) + "," +
                str(frame.D1_flat) + "," + str(frame.D1_amp) + "," + str(frame.D2_ramp) + "," +
                str(frame.D2_flat) + "," + str(frame.D2_amp) + "," + str(frame.D3_ramp) + "," +
                str(frame.D3_flat) + "," + str(frame.D3_amp) + "," + str(frame.D4_ramp) + "," +
                str(frame.D4_flat) + "," + str(frame.D4_amp);
      }
  ...
  try {
    ...
    H.keyval()["MGH_flip"] =
        str(fetch<float32>(in) * 180.0 / Math::pi, 6); // Radians in MGHO -> degrees in header
    ...
    do {
      ...
      else if (id == MGH_TAG_OLD_SURF_GEOM || id == MGH_TAG_OLD_USEREALRAS ||
...
template <class Output> void write_other(const Header &H, Output &out) {
  ...
  auto write_matrix = [](const Eigen::Matrix<default_type, 4, 4> &M, Output &out) {
    ...
    snprintf(buffer,
             ...
             "AutoAlign %10lf %10lf %10lf %10lf %10lf %10lf %10lf %10lf %10lf %10lf %10lf %10lf "
             "%10lf %10lf %10lf %10lf",
             M(0, 0),
             M(0, 1),
             M(0, 2),
  ...
  auto write_mri_frames = [&](const std::string &table, Output &out) {
    ...
    if (lines.size() != nframes) {
      WARN("Error writing MRI frame data to output image (image has " + str(nframes) +
           " volumes, frame data tables has " + str(lines.size()) +
           " rows); omitting information from output image");
      return;
    }
    ...
    for (size_t frame_index = 0; frame_index != nframes; ++frame_index) {
      ...
      if (entries.size() != 24 && entries.size() != 45) {
        WARN("Error writing MRI frame data to output image (frame data table has line with " +
             str(entries.size()) +
      ...
      // When stored in an MRtrix3 header key:value entry, TM is NOT included in the list at this
      // point
      ...
      frame.m_ras2vox =
          new Eigen::Matrix<default_type, 4, 4>(Eigen::Matrix<default_type, 4, 4>::Zero());
      const auto M = split(entries[21], " ");
      if (M.size() != 16) {
        WARN("Error writing MRI frame data to output image (expected RAS2vox matrix with 16 "
             "entries, read " +
      ...
      if (frame.type == MGH_FRAME_TYPE_DIFFUSION_AUGMENTED) {
        if (entries.size() != 45) {
          WARN("Error writing MRI frame data to output image (frame indicated as "
               "diffusion-augmented, but does not have sufficient data); omitting information from "
               "output image");
          return;
        }
  ...
  auto write_colourtable_V1 = [](const std::string &table, Output &out) {
    ...
    for (const auto &line : lines) {
      ...
      if (entries.size() != 5)
        throw Exception("Error writing colour table to file: Line has " + str(entries.size()) +
                        " fields, expected 5");
  ...
  for (auto entry : H.keyval()) {
    ...
    if (entry.first == "MGH_TR") {
      ...
      if (id == MGH_TAG_MRI_FRAME) {
        ...
        auto_align_matrix.reset(
            new Eigen::Matrix<default_type, 4, 4>(Eigen::Matrix<default_type, 4, 4>::Zero()));
        const auto lines = split_lines(entry.second);
        if (lines.size() != 4)
          throw Exception(
              "Error parsing auto align header entry for MGH format: Invalid number of lines (" +
              str(lines.size()) + "; should be 4)");
        for (size_t row = 0; row != 4; ++row) {
          const auto entries = split(strip(lines[row]), ", ", true);
          if (entries.size() != 4)
            throw Exception("Error parsing auto align header entry for MGH format: Invalid number "
                            "of entries on line " +
  ...
  // Although we could theoretically avoid writing any metadata here at all if there were no
  // interesting
  //   data to write, the fact that "command_history" will always have at least one entry
  //   (corresponding to the currently-executing command) means that MGH_TAG_CMDLINE will always
  ...
  if (colour_table.size()) {
    ...
    case 6:
      ...
      break;
    default:
      WARN("Malformed colour table in header (incorrect number of columns); not written to output "
           "image");
      break;
    }
  }
