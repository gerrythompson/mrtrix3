...
bool Config::get_bool(const std::string &key, bool default_value) {
  ...
  try {
    ...
    WARN("malformed boolean entry \"" + value + "\" for key \"" + key +
...
int Config::get_int(const std::string &key, int default_value) {
  ...
  try {
    ...
    WARN("malformed integer entry \"" + value + "\" for key \"" + key +
         "\" in configuration file - ignored");
    return default_value;
  }
}
...
float Config::get_float(const std::string &key, float default_value) {
  ...
  try {
    ...
    WARN("malformed floating-point entry \"" + value + "\" for key \"" + key +
         "\" in configuration file - ignored");
    return default_value;
  }
}
...
void Config::get_RGB(
    const std::string &key, float *ret, float default_R, float default_G, float default_B) {
  std::string value = get(key);
  if (value.size()) {
    try {
