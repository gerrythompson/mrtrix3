...
std::unique_ptr<MR::ImageIO::Base> dicom_to_mapper(MR::Header &H,
  ...
  for (const auto &series_it : series) {
    try {
      ...
      E.display();
      throw Exception("error reading series " + str(series_it->number) + " of DICOM image \"" +
                      H.name() + "\"");
    }
    ...
    for (auto image_it : *series_it) {
      if (!image_it->transfer_syntax_supported) {
        ...
        E.push_back("consider using third-party tools to convert your data to standard "
  ...
  if (nchannels == 1 && !image.frames.size()) {
    ...
    if (nchannels > 1)
      INFO("data segment is larger than expected from image dimensions - interpreting as "
           "multi-channel data");
  }
  ...
  for (size_t n = 1; n < frames.size(); ++n) { // check consistency of data scaling:
    if (frames[n]->scale_intercept != frames[n - 1]->scale_intercept ||
      ...
      if (image.images_in_mosaic)
        throw Exception(
            "unable to load series due to inconsistent data scaling between DICOM mosaic frames");
      inconsistent_scaling = true;
      INFO("DICOM images contain inconsistency scaling - data will be rescaled and stored in "
  ...
  if (image.images_in_mosaic) {
    ...
    for (size_t n = 0; n != dim[1]; ++n)
      min_acquisition_time =
          std::min(min_acquisition_time, default_type(frames[n]->acquisition_time));
    for (size_t n = 0; n != dim[1]; ++n)
      slices_timing_float.push_back(default_type(frames[n]->acquisition_time) -
                                    min_acquisition_time);
  }
  if (slices_timing_float.size()) {
    const size_t slices_acquired_at_zero =
        std::count(slices_timing_float.begin(), slices_timing_float.end(), 0.0f);
  ...
  if (image.images_in_mosaic) {
    ...
    if (frame.acq_dim[0] > size_t(H.size(0)) || frame.acq_dim[1] > size_t(H.size(1))) {
      WARN("acquisition matrix [ " + str(frame.acq_dim[0]) + " " + str(frame.acq_dim[1]) +
           " ] is smaller than expected [ " + str(H.size(0)) + " " + str(H.size(1)) +
           " ] in DICOM mosaic");
    ...
    if (H.size(0) * mosaic_size != frame.dim[0] || H.size(1) * mosaic_size != frame.dim[1]) {
      WARN("dimensions of DICOM mosaic [ " + str(frame.dim[0]) + " " + str(frame.dim[1]) +
           " ] do not match expected size [ " + str(H.size(0) * mosaic_size) + " " +
           str(H.size(0) * mosaic_size) + " ]");
      WARN("  assuming data are stored as " + str(mosaic_size) + "x" + str(mosaic_size) +
           " mosaic of " + str(H.size(0)) + "x" + str(H.size(1)) + " slices.");
      WARN("  image may be incorrectly reformatted");
    }
    ...
    io_handler.reset(
        new MR::ImageIO::Mosaic(H, frame.dim[0], frame.dim[1], H.size(0), H.size(1), H.size(2)));
