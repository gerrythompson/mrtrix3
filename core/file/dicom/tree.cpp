...
std::shared_ptr<Patient> Tree::find(const std::string &patient_name,
...
void Tree::read_file(const std::string &filename) {
  ...
  std::shared_ptr<Study> study = patient->find(
      reader.study, reader.study_ID, reader.study_UID, reader.study_date, reader.study_time);
