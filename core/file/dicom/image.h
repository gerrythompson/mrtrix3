...
public:
  Frame() {
    acq_dim[0] = acq_dim[1] = dim[0] = dim[1] = instance = series_num = acq = sequence =
        echo_index = grad_number = UINT_MAX;
    samples_per_pixel = 1;
    ...
    pixel_bandwidth = bandwidth_per_pixel_phase_encode = echo_time = inversion_time =
        repetition_time = flip_angle = partial_fourier = time_after_start = NaN;
    echo_train_length = 0;
  ...
  default_type distance, pixel_size[2], slice_thickness, slice_spacing, scale_slope,
      scale_intercept, bvalue;
  ...
  default_type pixel_bandwidth, bandwidth_per_pixel_phase_encode, echo_time, inversion_time,
      repetition_time, flip_angle, partial_fourier, time_after_start;
  ...
  static std::string get_DW_scheme(const vector<Frame *> &frames,
                                   const size_t nslices,
                                   const transform_type &image_transform);
...
public:
  Image(Series *parent = nullptr)
      : series(parent), images_in_mosaic(0), is_BE(false), in_frames(false) {}
