...
bool QuickScan::read(const std::string &file_name,
          ...
          if (item.element == 0x1010U || item.element == 0x1020U || item.element == 0x1110U ||
            ...
            while (entry.parse()) {
              ...
              if ((print_Phoenix && is_phoenix) || (print_CSA_fields && !is_phoenix)) {
                ...
                print(std::string("[CSA] ") + entry.key() + " (" + str(entry.num_items()) +
                      " items): <");
                const auto data = entry.get_string();
                size_t line_count = 0;
                for (const auto &entry : data) {
...
std::ostream &operator<<(std::ostream &stream, const QuickScan &file) {
  stream << "file: \"" << file.filename << "\" [" << file.modality
         << "]:\n    patient: " << file.patient << " " << format_ID(file.patient_ID) << " - "
         << format_date(file.patient_DOB)
         << "\n    study: " << (file.study.size() ? file.study : "[unspecified]") << " "
         << format_ID(file.study_ID) << " - " << format_date(file.study_date) << " "
         << format_time(file.study_time) << "\n    series: [" << file.series_number << "] "
