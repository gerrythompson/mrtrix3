...
public:
  ...
  template <class SingleFunctor> static SingleFunctor &get(X & /*f*/, SingleFunctor &functor) {
    return functor;
  }
};
...
public:
  ...
  Queue(const std::string &description = "unnamed",
        size_t buffer_size = MRTRIX_QUEUE_DEFAULT_CAPACITY)
  ...
  void status() {
    ...
    std::cerr << "Thread::Queue \"" + name + "\": " << writer_count << " writer"
              << (writer_count > 1 ? "s" : "") << ", " << reader_count << " reader"
              << (reader_count > 1 ? "s" : "") << ", items waiting: " << size() << "\n";
  }
