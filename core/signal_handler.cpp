...
void handler(int i) noexcept {
  ...
  if (!flag.test_and_set()) {
    ...
    // Try to do a tempfile cleanup before printing the error, since the latter's not guaranteed to
    // work... Don't use File::remove: may throw an exception
...
#undef __SIGNAL
    ...
    // Don't use std::cerr << here: Use basic C string-handling functions and a write() call to
    // STDERR_FILENO Don't attempt to use any terminal colouring
    char str[256];
    str[255] = '\0';
