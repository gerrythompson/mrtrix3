...
//
// When the "-shell" option is present, it is first applied, and next the constraints (if any are
// requested) are checked. If any constraint is violated, an error (exception) occurs. The logic is
// that the user's
// "-shell" selection is a conscious choice that should always be honoured, and no unexpected
// modifications (e.g. including the bzeros) are made to it.
//
// When the "-shell" option is not present, by default all shells are included. If constraints are
// requested, they are made true for the convenience of the user, if possible (e.g. just selecting
// the highest b-value shell if "force_singleshell" is requested, excluding the bzeros if
// "force_without_bzero" is present, etc.). However, if they simply can't be made true (e.g.
// "force_with_bzero" when there are no bzeros in the dataset), an error (exception) will still
// occur.
//
// So long story short: multi-shell (as in "all shells") is default, but constraints can be
// specified if strictly required for certain commands or algorithms.
Shells &Shells::select_shells(const bool force_singleshell,
  ...
  if (force_without_bzero && force_with_bzero)
    throw Exception(
        "Incompatible constraints: command tries to enforce proceeding both with and without b=0");
  ...
  if (opt.size()) {
    ...
    for (vector<default_type>::const_iterator b = desired_bvalues.begin();
         b != desired_bvalues.end();
         ++b) {
      ...
      if (*b <= bzero_threshold()) {
        ...
        if (has_bzero()) {
          if (!bzero_selected) {
            to_retain[0] = true;
            bzero_selected = true;
            DEBUG("User requested b-value " + str(*b) +
                  "; got b=0 shell : " + str(smallest().get_mean()) + " +- " +
        ...
        // * Compute number of standard deviations away from each shell mean, see if there's a clear
        // winner
        ...
        for (size_t s = 0; s != count(); ++s) {
          if ((*b >= shells[s].get_min()) && (*b <= shells[s].get_max())) {
            if (!to_retain[s]) {
              ...
              shell_selected = true;
              DEBUG("User requested b-value " + str(*b) + "; got shell " + str(s) + ": " +
          ...
          if (shell_selected && !ambiguous) {
            if (!to_retain[best_shell]) {
              ...
              DEBUG("User requested b-value " + str(*b) + "; got shell " + str(best_shell) + ": " +
                    ...
                    str(shells[best_shell].get_stdev()) + " with " +
                    str(shells[best_shell].count()) + " volumes");
            } else {
              throw Exception(
                  "User selected a shell more than once: " + str(shells[best_shell].get_mean()) +
                  " +- " + str(shells[best_shell].get_stdev()) + " with " +
                  str(shells[best_shell].count()) + " volumes");
            }
          } else {

            // First, check to see if all non-zero shells have (effectively) non-zero standard
            // deviation (If one non-zero shell has negligible standard deviation, assume a Poisson
        ...
        } // End checking if the shell can be selected because of lying within the numerical range
          // of a shell
    ...
    if (force_with_bzero && !bzero_selected)
      throw Exception(
          "User did not select b=0 shell, but the command requires the presence of b=0 data");
    ...
    if (force_singleshell && !is_single_shell()) {
      ...
      WARN(
          "Multiple non-zero b-value shells detected, automatically selecting largest b-value: b=" +
...
Shells::Shells(const Eigen::MatrixXd &grad) {
  ...
  if ((num_shells < 1) || (num_shells > std::sqrt(default_type(grad.rows()))))
    throw Exception("DWI volumes could not be classified into b-value shells; gradient encoding "
                    "may not represent a HARDI sequence");
  ...
  if (smallest().is_bzero()) {
    INFO("Diffusion gradient encoding data clustered into " + str(num_shells - 1) +
         " non-zero shells and " + str(smallest().count()) + " b=0 volumes");
  } else {
    INFO("Diffusion gradient encoding data clustered into " + str(num_shells) +
         " shells (no b=0 volumes)");
  }
  DEBUG("Shells: b = { " + str([&] {
          std::string m;
          for (auto &s : shells)
