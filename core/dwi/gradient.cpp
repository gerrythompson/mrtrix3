...
Eigen::MatrixXd load_bvecs_bvals(const Header &header,
  ...
  try {
    ...
    throw Exception(e,
                    "Unable to import files \"" + bvecs_path + "\" and \"" + bvals_path +
  ...
  if (bvals.rows() != 1) {
    if (bvals.cols() == 1)
      ...
      throw Exception("bvals file must contain 1 row or column only (file \"" + bvals_path +
                      "\" has " + str(bvals.rows()) + ")");
  }
  if (bvecs.rows() != 3) {
    if (bvecs.cols() == 3)
      ...
      throw Exception("bvecs file must contain exactly 3 rows or columns (file \"" + bvecs_path +
                      "\" has " + str(bvecs.rows()) + ")");
  }

  if (bvals.cols() != bvecs.cols())
    throw Exception("bvecs and bvals files must have same number of diffusion directions (file \"" +
                    bvecs_path + "\" has " + str(bvecs.cols()) + ", file \"" + bvals_path +
                    "\" has " + str(bvals.cols()) + ")");

  const size_t num_volumes = header.ndim() < 4 ? 1 : header.size(3);
  if (size_t(bvals.cols()) != num_volumes)
    throw Exception("bvecs and bvals files must have same number of diffusion directions as "
                    "DW-image (gradients: " +
...
Eigen::MatrixXd get_raw_DW_scheme(const Header &header) {
  ...
  if (opt_fsl.size()) {
    if (opt_mrtrix.size())
      throw Exception("Diffusion gradient table can be provided using either -grad or -fslgrad "
                      "option, but NOT both");
...
Eigen::MatrixXd get_DW_scheme(const Header &header, BValueScalingBehaviour bvalue_scaling) {
  try {
    ...
    Eigen::Array<default_type, Eigen::Dynamic, 1> squared_norms =
        grad.leftCols(3).rowwise().squaredNorm();
    ...
    if ((requires_bvalue_scaling && bvalue_scaling == BValueScalingBehaviour::Auto) ||
      ...
      if (requires_bvalue_scaling) {
        ...
        WARN("use of -bvalue_scaling option had no effect: gradient vector norms are all within "
             "tolerance "
             "(maximum scaling factor = " +
             str(max_scaling_factor) + ")");
      }
    ...
    // - vector normalisation effect is large, regardless of whether or not b-value scaling was
    // applied
    ...
    throw Exception(e,
                    "error importing diffusion gradient table for image \"" + header.name() + "\"");
  }
}
