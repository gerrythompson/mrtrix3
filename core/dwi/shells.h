...
// Don't expect these values to change depending on the particular command that is initialising the
// Shells class;
...
// Minimum number of volumes within DWI_SHELL_EPSILON necessary to continue expansion of the cluster
// selection
...
FORCE_INLINE default_type bzero_threshold() {
  static const default_type value =
      File::Config::get_float("BZeroThreshold", DWI_SHELLS_BZERO_THREHSOLD);
  return value;
}
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &stream, const Shell &S) {
    stream << "Shell: " << S.volumes.size() << " volumes, b-value " << S.mean << " +- " << S.stdev
           << " (range [" << S.min << " - " << S.max << "])";
    return stream;
  }
...
public:
  ...
  Shells &select_shells(const bool force_singleshell,
