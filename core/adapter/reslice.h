...
typename std::enable_if<!std::is_same<value_type, bool>::value &&
                        ...
                        value_type>::type inline normalise(const default_type sum,
                                                           const default_type norm) {
...
public:
  ...
  Reslice(const ImageType &original,
          ...
          const value_type value_when_out_of_bounds =
              Interp::Base<ImageType>::default_out_of_bounds_value())
      : interp(original, value_when_out_of_bounds),
        ...
        direct_transform(Transform(original).scanner2voxel * transform *
                         Transform(reference).voxel2scanner) {
    using namespace Eigen;
    assert(ndim() >= 3);

    const bool is_nearest =
        std::is_same<typename Interp::Nearest<ImageType>, decltype(interp)>::value;
