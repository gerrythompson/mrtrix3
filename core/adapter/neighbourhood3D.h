...
class NeighbourhoodCoord : public Base<NeighbourhoodCoord<ImageType>, ImageType> {
public:
  ...
  NeighbourhoodCoord(const ImageType &original, const VectorType &extent, const Iterator &iter)
    ...
    for (size_t i = 0; i < ndim(); ++i) {
      ...
      size_[i] =
          (from_[i] + extent[i] >= original.size(i)) ? original.size(i) - from_[i] - 1 : extent[i];
      assert(from_[n] + size_[n] < original.size(n));
    }

    // for (size_t n = 0; n < ndim(); ++n)
    //   if (from_[n] + size_[n] > original.size(n))
    //     throw Exception ("FIXME: dimensions requested for NeighbourhoodCoord adapter are out of
    //     bounds!");
