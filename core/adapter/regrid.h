...
public:
  ...
  Regrid(const ImageType &original,
      ...
      : base_type(original),
        ...
        index_invalid_lower_upper([&] {
          ...
          for (size_t d = 0; d < from_.size(); ++d) {
            v.push_back(vector<ssize_t>{from_[d] < 0 ? -(ssize_t)from_[d] - 1 : -1,
                                        original.size(d) - from_[d]});
          }
          return v;
        }()),
  ...
  value_type value() {
    for (size_t axis = 0; axis < index_.size(); ++axis)
      if (index_requires_bound_check[axis] && (index_[axis] >= index_invalid_lower_upper[axis][1] ||
                                               index_[axis] <= index_invalid_lower_upper[axis][0]))
        return fill_;
    return parent().value();
  }
