    ...
    do {
      ...
      if (last_char == ':') {
        ...
        if (i) {
          if (i != 2)
            throw Exception(
                "For floating-point ranges, must specify three numbers (start:step:end)");
          default_type first = range_spec[0], inc = range_spec[1], last = range_spec[2];
          if (!inc || (inc * (last - first) < 0.0) || !std::isfinite(first) ||
              !std::isfinite(inc) || !std::isfinite(last))
            throw Exception("Floating-point range does not form a finite set");
          default_type value = first;
          for (size_t mult = 0;
               (inc > 0.0f ? value < last - 0.5f * inc : value > last + 0.5f * inc);
               ++mult)
