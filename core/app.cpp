...
#define MRTRIX_CORE_REFERENCE                                                                      \
  "Tournier, J.-D.; Smith, R. E.; Raffelt, D.; Tabbara, R.; Dhollander, T.; Pietsch, M.; "         \
  "Christiaens, D.; Jeurissen, B.; Yeh, C.-H. & Connelly, A. "                                     \
...
OptionGroup __standard_options =
    ...
    Option("nthreads",
           "use this number of threads in multi-threaded applications (set to 0 to disable "
           "multi-threading).") +
...
inline void
get_matches(vector<const Option *> &candidates, const OptionGroup &group, const std::string &stub) {
...
std::string
paragraph(const std::string &header, const std::string &text, int header_indent, int indent) {
...
std::string help_head(int format) {
  ...
  std::string version_string = project_version ? std::string("Version ") + project_version
...
std::string Example::syntax(int format) const {
  std::string s =
      paragraph("", format ? underline(title + ":") + "\n" : title + ": ", HELP_PURPOSE_INDENT);
...
std::string Argument::usage() const {
  ...
  stream << "ARGUMENT " << id << " " << (flags & Optional ? '1' : '0') << " "
...
std::string Option::usage() const {
  std::ostringstream stream;
  stream << "OPTION " << id << " " << (flags & Optional ? '1' : '0') << " "
         << (flags & AllowMultiple ? '1' : '0') << "\n";
...
std::string get_help_string(int format) {
  return help_head(format) + help_synopsis(format) + usage_syntax(format) +
         ARGUMENTS.syntax(format) + DESCRIPTION.syntax(format) + EXAMPLES.syntax(format) +
         OPTIONS.syntax(format) + __standard_options.header(format) +
         __standard_options.contents(format) + __standard_options.footer(format) +
         help_tail(format);
}
...
void print_help() {
  ...
  if (help_display_command.size()) {
    ...
    if (!file) {
      ...
      INFO("error sending help page to display command \"" + help_display_command +
           "\": " + strerror(errno));
    }
...
const Option *match_option(const char *arg) {
  if (consume_dash(arg) && *arg && !isdigit(*arg) && *arg != '.') {
    ...
    if (std::all_of(++candidates.begin(), candidates.end(), [&cid](const Option *cand) {
          return cand->id == cid;
        }))
      return candidates[0];
...
void parse() {
  ...
  for (size_t i = 0; i < ARGUMENTS.size(); ++i) {
    ...
    if (ARGUMENTS[i].flags) {
      if (flags && flags != ARGUMENTS[i].flags)
        throw Exception("FIXME: all arguments declared optional() or allow_multiple() should have "
                        "matching flags in command-line syntax");
  ...
  size_t num_arg_per_multi =
      num_optional_arguments ? num_extra_arguments / num_optional_arguments : 0;
  ...
  for (size_t i = 0; i < OPTIONS.size(); ++i) {
    for (size_t j = 0; j < OPTIONS[i].size(); ++j) {
      ...
      if (count < 1 && !(OPTIONS[i][j].flags & Optional))
        throw Exception(std::string("mandatory option \"-") + OPTIONS[i][j].id +
                        "\" must be specified");

      if (count > 1 && !(OPTIONS[i][j].flags & AllowMultiple))
        throw Exception(std::string("multiple instances of option \"-") + OPTIONS[i][j].id +
                        "\" are not allowed");
    }
  }
  ...
  // check for the existence of all specified input files (including optional ones that have been
  // provided) if necessary, also check for pre-existence of any output files with known paths
  ...
  for (const auto &i : option) {
    for (size_t j = 0; j != i.opt->size(); ++j) {
      ...
      if (arg.type == ArgDirectoryIn) {
        if (!Path::exists(text))
          throw Exception("input directory \"" + text + "\" for option \"-" +
                          std::string(i.opt->id) + "\" not found");
        if (!Path::is_dir(text))
          throw Exception("input \"" + text + "\" for option \"-" + std::string(i.opt->id) +
                          "\" is not a directory");
      }
      ...
      if (arg.type == TracksOut && !Path::has_suffix(text, ".tck"))
        throw Exception("output track file \"" + text + "\" for option \"-" +
                        std::string(i.opt->id) + "\" must use the .tck suffix");
    }
  }
...
int64_t App::ParsedArgument::as_int() const {
  if (arg->type == Integer) {
    ...
    if (alpha_count) {
      if (alpha_is_last) {
        ...
        case 'T':
          ...
          break;
        default:
          throw Exception("error converting string " + str(p) +
                          " to integer: unexpected postfix \'" + postfix + "\'");
        }
    ...
    if (retval < min || retval > max) {
      ...
      msg += "\" is out of bounds (valid range: " + str(min) + " to " + str(max) +
  ...
  if (arg->type == Choice) {
    ...
    if (opt)
      ...
      msg += std::string("argument \"") + arg->id;
    msg += std::string("\" (received \"" + std::string(p) + "\"; valid choices are: ") +
           join(choices, ", ") + ")";
    throw Exception(msg);
  }
  assert(0);
