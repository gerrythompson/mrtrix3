...
/** \note upon completion, the (l,m) value will be stored in \c array[l]. Entries in \a array for
 * l<m will be left undefined. */
template <typename VectorType>
inline void
Plm_sph(VectorType &array, const int lmax, const int m, const typename VectorType::Scalar x) {
...
/** \note this function expects the previously computed array of associated Legendre functions to be
 * stored in \a array, (as computed by Plm_sph (VectorType& array, const int lmax, const int m,
 * const ValueType x)) and will overwrite the values in \a array with the derivatives */
template <typename VectorType>
inline void
Plm_sph_deriv(VectorType &array, const int lmax, const int m, const typename VectorType::Scalar x) {
  using value_type = typename VectorType::Scalar;
  value_type x2 = pow2(x);
  if (x2 >= 1.0) {
    for (int n = m; n <= lmax; n++)
  ...
  for (int n = lmax; n > m; n--)
    array[n] = x2 * (n * x * array[n] -
                     (n + m) * std::sqrt((2.0 * n + 1.0) * (n - m) / ((2.0 * n - 1.0) * (n + m))) *
                         array[n - 1]);
  array[m] *= x2 * m * x;
}
