...
init_transform(const MatrixType &dirs, const int lmax) {
...
template <class MatrixType>
Eigen::Matrix<typename MatrixType::Scalar, Eigen::Dynamic, Eigen::Dynamic>
init_transform_cart(const MatrixType &dirs, const int lmax) {
  using namespace Eigen;
  using value_type = typename MatrixType::Scalar;
  if (dirs.cols() != 3)
...
inline Eigen::Matrix<typename VectorType::Scalar, Eigen::Dynamic, 1>
invert(const VectorType &coefs) {
...
value(const VectorType1 &coefs, const VectorType2 &unit_dir, int lmax) {
...
get_peak(const VectorType &sh,
  ...
  for (int i = 0; i < 50; i++) {
    ...
    value_type d2SH_dt2 =
        daz * daz * d2SH_daz2 + 2.0 * daz * del * d2SH_deldaz + del * del * d2SH_del2;
...
inline void derivatives(const VectorType &sh,
  ...
  for (int m = 1; m <= lmax; m++) {
    ...
    for (int l = ((m & 1) ? m + 1 : m); l <= lmax; l += 2) {
      ...
      if (m == 1)
        tmp2 -= sqrt(value_type((l + m) * (l - m + 1) * (l + m - 1) * (l - m + 2))) *
                AL[index_mpos(l, 1)];
      else
        tmp2 += sqrt(value_type((l + m) * (l - m + 1) * (l + m - 1) * (l - m + 2))) *
                AL[index_mpos(l, m - 2)];
