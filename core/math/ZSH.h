...
Eigen::Matrix<value_type, Eigen::Dynamic, Eigen::Dynamic> init_amp_transform(const VectorType &els,
                                                                             const size_t lmax) {
...
init_deriv_transform(const VectorType &els, const size_t lmax) {
...
public:
  ...
  Transform(const MatrixType &dirs, const size_t lmax)
      : ZSHT(init_amp_transform(
            dirs.col(1), lmax)), // Elevation angles are second column of aximuth/elevation matrix
        iZSHT(pinv(ZSHT)) {}
...
template <class VectorType>
inline typename VectorType::Scalar derivative(const VectorType &coefs,
...
inline VectorType &FA2ZSH(VectorType &zsh,
  ...
  for (size_t i = 0; i < precision; i++) {
    ...
    sigs[i] =
        exp(-bvalue * (ev1 * std::cos(el) * std::cos(el) + ev2 * std::sin(el) * std::sin(el)));
