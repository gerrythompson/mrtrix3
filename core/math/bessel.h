...
//* Compute the scaled regular modified cylindrical Bessel function of zeroth order exp(-|x|)
// I_0(x). */
...
template <typename T> inline T I0_scaled(const T x) {
  ...
  if (x <= 8.0)
    return ((0.375 + Chebyshev::eval(
                         coef_bI0, (sizeof(T) > 4 ? 20 : 13), -1.0, 1.0, (48.0 / x - 11.0) / 5.0)) /
            sqrt(x));
  return (
      (0.375 + Chebyshev::eval(coef_cI0, (sizeof(T) > 4 ? 21 : 11), -1.0, 1.0, 16.0 / x - 1.0)) /
      sqrt(x));
}
...
template <typename T> inline T I1_scaled(const T x) {
  ...
  if (x <= 8.0) {
    T b = (0.375 + Chebyshev::eval(
                       coef_bI1, (sizeof(T) > 4 ? 20 : 11), -1.0, 1.0, (48.0 / x - 11.0) / 5.0)) /
          sqrt(x);
    return (x > 0.0 ? b : -b);
  }
  T b = (0.375 + Chebyshev::eval(coef_cI1, (sizeof(T) > 4 ? 21 : 9), -1.0, 1.0, 16.0 / x - 1.0)) /
        sqrt(x);
  return (x > 0.0 ? b : -b);
}
