...
namespace {
inline std::string direction_str(int direction) {
  return (direction == FFTW_FORWARD ? "forward" : "backward");
}
...
inline void
FFT(ImageTypeIn &in, ImageTypeOut &out, size_t axis, int direction, bool centre_FFT = false) {
  class FFTFunctor {
  public:
    FFTFunctor(
        const ImageTypeIn &in, const ImageTypeOut &out, size_t axis, int direction, bool centre_FFT)
        : in(in),
          out(out),
    ...
    void operator()(const Iterator &pos) {
      ...
      for (auto l = Loop(axis, axis + 1)(out); l; ++l)
        out.value() =
            fft[shift(out.index(axis), out.size(axis), centre_FFT, direction == FFTW_BACKWARD)];
    }
  ...
  ThreadedLoop(
      "performing " + direction_str(direction) + " FFT along axis " + str(axis), in, outer_axes, 1)
