...
App::OptionGroup glm_options(const std::string &element_name) {
  ...
  OptionGroup result =
      ...
      + Option("variance",
               ...
               "measurements for which the expected variance is equivalent should contain the same "
               "index") +
      ...
      + Option("fonly",
               "only assess F-tests; do not perform statistical inference on entries in the "
               "contrast matrix")
      ...
      Option(
          ...
          "add a column to the design matrix corresponding to subject " + element_name +
              "-wise values "
              "(note that the contrast matrix must include an additional column for each use of "
              "this option); "
...
void check_design(const matrix_type &design, const bool extra_factors) {
  ...
  if (decomp.rank() < design.cols()) {
    if (extra_factors) {
      ...
      WARN("Design matrix is rank-deficient; processing may proceed, but manually checking your "
           "matrix is advised");
    }
  } else {
    ...
    if (cond > 100.0) {
      ...
      CONSOLE(std::string("Design matrix condition number") +
              (extra_factors ? " (without element-wise columns)" : "") + ": " + str(cond, 6));
    }
  }
}
  ...
  try {
    ...
    if (size_t(data.size()) != num_inputs)
      throw Exception("Number of entries in variance group file \"" + std::string(opt[0][0]) +
                      "\" (" + str(data.size()) + ") does not match number of inputs (" +
                      str(num_inputs) + ")");
  ...
  if (opt.size()) {
    ...
    for (ssize_t ftest_index = 0; ftest_index != ftest_matrix.rows(); ++ftest_index) {
      if (!ftest_matrix.row(ftest_index).count())
        throw Exception("Row " + str(ftest_index + 1) +
                        " of F-test matrix does not contain any ones");
    ...
    throw Exception("Cannot perform F-tests exclusively (-fonly option): No F-test matrix was "
                    "provided (-ftests option)");
  }
  return hypotheses;
}
...
vector_type abs_effect_size(const matrix_type &measurements,
...
matrix_type abs_effect_size(const matrix_type &measurements,
                            const matrix_type &design,
...
matrix_type stdev(const matrix_type &measurements,
                  const matrix_type &design,
  ...
  const matrix_type R(matrix_type::Identity(design.rows(), design.rows()) -
                      (design * Math::pinv(design)));
  // Residuals
...
vector_type std_effect_size(const matrix_type &measurements,
                            const matrix_type &design,
                            const Hypothesis &hypothesis) {
  if (hypothesis.is_F())
    return vector_type::Constant(measurements.cols(), NaN);
  return abs_effect_size(measurements, design, hypothesis).array() /
         stdev(measurements, design).array().col(0);
}

matrix_type std_effect_size(const matrix_type &measurements,
                            const matrix_type &design,
                            const vector<Hypothesis> &hypotheses) {
...
void all_stats(const matrix_type &measurements,
  ...
  std::cerr << "Betas: " << betas.rows() << " x " << betas.cols() << ", max "
...
#ifdef GLM_ALL_STATS_DEBUG
  std::cerr << "abs_effect_size: " << abs_effect_size.rows() << " x " << abs_effect_size.cols()
            << ", max " << abs_effect_size.array().maxCoeff() << "\n";
#else
  if (progress)
    ++*progress;
#endif
...
#ifdef GLM_ALL_STATS_DEBUG
  std::cerr << "std_effect_size: " << std_effect_size.rows() << " x " << std_effect_size.cols()
            << ", max " << std_effect_size.array().maxCoeff() << "\n";
#endif
}
...
void all_stats(const matrix_type &measurements,
  ...
  public:
    Source(const size_t num_elements)
        : num_elements(num_elements),
          counter(0),
          progress(new ProgressBar("Calculating basic properties of default permutation",
                                   num_elements)) {}
  ...
  public:
    ...
    bool operator()(const size_t &element_index) {
      ...
      //   present in either the input data or imported from the element-wise design matrix column
      //   data
...
matrix_type Hypothesis::check_rank(const matrix_type &in, const size_t index) const {
  ...
  if (decomp.rank() == in.rows())
    return in;
  WARN("F-test " + str(index + 1) +
       " is rank-deficient; row-space matrix decomposition will instead be used");
...
TestFixedHomoscedastic::TestFixedHomoscedastic(const matrix_type &measurements,
  ...
  // When the design matrix is fixed, we can pre-calculate the model partitioning for each
  // hypothesis
  for (const auto &h : hypotheses) {
    partitions.emplace_back(h.partition(design));
    XtX.emplace_back(partitions.back().X.transpose() * partitions.back().X);
    one_over_dof.push_back(1.0 /
                           (num_inputs() - partitions.back().rank_x - partitions.back().rank_z));
  }
}
  ...
  for (size_t ih = 0; ih != c.size(); ++ih) {
    ...
    for (size_t ie = 0; ie != num_elements(); ++ie) {
      ...
      const default_type F =
          ((beta.transpose() * XtX[ih] * beta)(0, 0) / c[ih].rank()) / (one_over_dof * sse[ie]);
      if (!std::isfinite(F)) {
...
void TestFixedHeteroscedastic::operator()(const matrix_type &shuffling_matrix,
  ...
  for (size_t ih = 0; ih != c.size(); ++ih) {
    ...
    VAR(sse.cols());
#endif
    // These terms are what appears in the weighting matrix based on the VG to which each input
    // belongs;
    ...
    for (size_t ie = 0; ie != num_elements(); ++ie) {
      ...
      const default_type numerator =
          ...
          (c[ih].matrix() * (M.transpose() * W.asDiagonal() * M).inverse() *
      ...
      for (size_t vg_index = 0; vg_index != num_vgs; ++vg_index)
        // Since Wnn is the same for every n in the variance group, can compute that summation as
        // the product of:
        //   - the value inserted in W for that particular VG
        //   - the number of inputs that are a part of that VG
        gamma += inv_Rnn_sums[vg_index] *
                 Math::pow2(1.0 - ((Wterms(vg_index, ie) * inputs_per_vg[vg_index]) / W_trace));
      ...
      if (!std::isfinite(G)) {
        ...
        if (c[ih].is_F() && c[ih].rank() > 1) {
          ...
          const default_type dof =
              Math::welch_satterthwaite(Wterms.col(ie).inverse(), inputs_per_vg);
#ifdef GLM_TEST_DEBUG
          VAR(dof);
#endif
  ...
  for (ssize_t ie = 0; ie != y.cols(); ++ie) {
    ...
    if (finite_count < std::min(num_inputs(), 2 * num_factors())) {
      ...
      if (!std::isfinite(condition_number) || condition_number > 1e5) {
        ...
        Rm.noalias() =
            matrix_type::Identity(finite_count, finite_count) - (Mfull_masked * pinvMfull_masked);
        ...
        for (size_t ih = 0; ih != c.size(); ++ih) {
          ...
          if (dof(ie, ih) < 1) {
            ...
            const default_type F =
                ((beta.transpose() * XtX * beta)(0, 0) / c[ih].rank()) / (sse / dof(ie, ih));
...
void TestVariableHomoscedastic::get_mask(const size_t ie,
                                         BitSet &mask,
    ...
    for (size_t in_index = 0; in_index != num_inputs(); ++in_index) {
      if (mask[in_index]) {
        ...
        Mfull_masked.block(out_index, M.cols(), 1, extra_column_data.cols()) =
            extra_column_data.row(in_index);
...
TestVariableHeteroscedastic::TestVariableHeteroscedastic(const vector<CohortDataImport> &importers,
                                                         ...
                                                         const bool nans_in_data,
                                                         const bool nans_in_columns)
    : TestVariableHomoscedastic(
          importers, measurements, design, hypotheses, nans_in_data, nans_in_columns),
      VG(variance_groups),
      num_vgs(VG.maxCoeff() + 1),
      gamma_weights(vector_type::Zero(num_hypotheses())) {
  for (size_t ih = 0; ih != c.size(); ++ih) {
        ...
        if (VG_counts.minCoeff() <= 1) {
          ...
          for (size_t ih = 0; ih != c.size(); ++ih) {
            ...
            const default_type numerator =
                ...
                (c[ih].matrix() *
                 (Mfull_masked.transpose() * W.asDiagonal() * Mfull_masked).inverse() *
                 c[ih].matrix().transpose())
                    .inverse() *
            ...
            default_type gamma(0.0);
            for (size_t vg_index = 0; vg_index != num_vgs; ++vg_index)
              gamma += Math::pow2(1.0 - ((Wterms[vg_index] * VG_counts[vg_index]) / W_trace)) /
                       Rnn_sums[vg_index];
            gamma = 1.0 + (gamma_weights[ih] * gamma);
            ...
            if (!std::isfinite(G)) {
              ...
              stats(ie, ih) =
                  c[ih].is_F() ? G
                               : std::sqrt(G) *
                                     ((c[ih].matrix() * lambda.matrix()).sum() > 0.0 ? 1.0 : -1.0);
              if (c[ih].is_F() && c[ih].rank() > 1) {
                const default_type dof =
                    2.0 * default_type(c[ih].rank() - 1) / (3.0 * (gamma - 1.0));
                zstats(ie, ih) = stat2z->F2z(G, c[ih].rank(), dof);
              } else {
