...
public:
  ...
  Hypothesis(matrix_type::ConstRowXpr &in, const size_t index)
      : c(in), r(Math::rank(c)), F(false), i(index) {
    check_nonzero();
  }
 ...
 * @return the matrix containing the output absolute effect sizes (one column of element effect
 * sizes per contrast)
 */
 ...
 * @param design the design matrix
 * @param hypothesis a Hypothesis class instance defining the effect of interest
 * @return the matrix containing the output standardised effect sizes (one column of element effect
 * sizes per contrast)
 */
...
class TestBase {
public:
  TestBase(const matrix_type &measurements,
           const matrix_type &design,
           const vector<Hypothesis> &hypotheses)
   ...
   */
  virtual void
  operator()(const matrix_type &shuffling_matrix, matrix_type &stat, matrix_type &zstat) const = 0;
...
public:
   ...
   * @param zstats the vector containing the Z-transformed output statistics (one column per
   * hypothesis)
   */
  void operator()(const matrix_type &shuffling_matrix,
                  matrix_type &stats,
                  matrix_type &zstats) const override;
...
public:
   ...
   * @param design the design matrix
   * @param hypotheses a vector of Hypothesis instances
   * @param variance_groups a vector of integers corresponding to variance group assignments (should
   * be indexed from zero)
   */
...
public:
   ...
   * @param zstat the vector containing the Z-transformed output statistics (one column per
   * hypothesis)
   *
  ...
  void operator()(const matrix_type &shuffling_matrix,
                  matrix_type &stat,
                  matrix_type &zstat) const override;
...
protected:
  ...
  void
  apply_mask_VG(const BitSet &mask, index_array_type &VG_masked, index_array_type &VG_counts) const;
};
