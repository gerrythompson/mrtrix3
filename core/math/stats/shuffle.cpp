...
App::OptionGroup shuffle_options(const bool include_nonstationarity,
                                 const default_type default_skew) {
  using namespace App;
  ...
  OptionGroup result =
      ...
      + Option("notest",
               "don't perform statistical inference; only output population statistics (effect "
               "size, stdev etc)")

      + Option("errors",
               "specify nature of errors for shuffling; options are: " + join(error_types, ",") +
                   " (default: ee)") +
      Argument("spec").type_choice(error_types)

      + Option("exchange_within",
               "specify blocks of observations within each of which data may undergo restricted "
               "exchange") +
      ...
      + Option("nshuffles",
               "the number of shuffles (default: " + str(DEFAULT_NUMBER_SHUFFLES) + ")") +
      Argument("number").type_integer(1)

      + Option("permutations",
               "manually define the permutations (relabelling). The input should be a text file "
               "defining a m x n matrix, "
               "where each relabelling is defined as a column vector of size m, and the number of "
               "columns, n, defines "
               "the number of permutations. Can be generated with the palm_quickperms function in "
  ...
  if (include_nonstationarity) {
    ...
    result + Option("nonstationarity", "perform non-stationarity correction")
        ...
        + Option("skew_nonstationarity",
                 "specify the skew parameter for empirical statistic calculation (default for this "
                 "command is " +
                     str(default_skew) + ")") +
        ...
        + Option("nshuffles_nonstationarity",
                 ...
                 "for non-stationarity correction (default: " +
                     str(DEFAULT_NUMBER_SHUFFLES_NONSTATIONARITY) + ")") +
        Argument("number").type_integer(1)

        + Option("permutations_nonstationarity",
                 "manually define the permutations (relabelling) for computing the emprical "
                 "statistics for non-stationarity correction. "
                 "The input should be a text file defining a m x n matrix, where each relabelling "
                 "is defined as a column vector of size m, "
                 "and the number of columns, n, defines the number of permutations. Can be "
...
Shuffler::Shuffler(const size_t num_rows, const bool is_nonstationarity, const std::string msg)
    : rows(num_rows),
      nshuffles(is_nonstationarity ? DEFAULT_NUMBER_SHUFFLES_NONSTATIONARITY
  ...
  if (opt.size()) {
    try {
      ...
      throw Exception(e,
                      "Unable to read file \"" + std::string(opt[0][0]) +
                          "\" as within-block exchangeability");
    }
  }
  ...
  if (opt.size()) {
    ...
    try {
      ...
      throw Exception(e,
                      "Unable to read file \"" + std::string(opt[0][0]) +
                          "\" as whole-block exchangeability");
    }
  }
...
void Shuffler::initialise(const error_t error_types,
  ...
  auto safe2pow = [](const size_t i) {
    return (i >= 8 * sizeof(size_t)) ? (std::numeric_limits<size_t>::max()) : ((size_t(1) << i));
  };
  const size_t max_num_signflips =
      eb_whole.size() ? safe2pow(eb_whole.maxCoeff() + 1) : safe2pow(rows);
  ...
  if (max_shuffles < nshuffles) {
    if (nshuffles_explicit) {
      ...
      WARN("Only " + str(max_shuffles) +
           " unique shuffles can be generated, which is less than the default number of " +
           str(nshuffles) + " for " +
           (is_nonstationarity ? "non-stationarity correction" : "null distribution generation"));
    }
  ...
  //   - Interleave one of the two of them, so that every combination of paired permutation-signflip
  //   is unique
  // - If using a fixed number of shuffles:
  //   - If fixed number is less than the maximum number of permutations, generate that number
  //   randomly
  //   - If fixed number is equal to the maximum number of permutations, generate the full set
  //   - If fixed number is greater than the maximum number of permutations, generate that number
  //   randomly,
  ...
  if (ee && !permutations.size()) {
    if (ise) {
      if (nshuffles == max_shuffles) {
        ...
        // - Permit duplicates (specifically of permutations only) if an adequate number cannot be
        // generated
        generate_random_permutations(nshuffles,
                                     rows,
                                     eb_within,
                                     eb_whole,
                                     !is_nonstationarity,
                                     nshuffles > max_num_permutations);
      }
    } else if (nshuffles < max_shuffles) {
      generate_random_permutations(
          nshuffles, rows, eb_within, eb_whole, !is_nonstationarity, false);
    } else {
      generate_all_permutations(rows, eb_within, eb_whole);
      assert(permutations.size() == max_shuffles);
    }
    ...
    if (ee) {
      if (nshuffles == max_shuffles) {
        ...
        for (size_t i = 0; i != max_num_permutations; ++i)
          duplicated_signflips.insert(
              duplicated_signflips.end(), signflips.begin(), signflips.end());
        ...
        generate_random_signflips(
            nshuffles, rows, eb_whole, !is_nonstationarity, nshuffles > max_num_signflips);
      }
    } else if (nshuffles < max_shuffles) {
...
index_array_type Shuffler::load_blocks(const std::string &filename, const bool equal_sizes) {
  ...
  for (size_t i = 0; i <= max_coeff; ++i) {
    if (counts[i] < 2)
      throw Exception("Sequential indices in file \"" + filename +
                      "\" must contain at least two entries each");
  }
  if (equal_sizes) {
    for (size_t i = 1; i <= max_coeff; ++i) {
      if (counts[i] != counts[0])
        throw Exception("Indices in file \"" + filename +
