...
template <class SubjectDataImport>
void CohortDataImport::initialise(const std::string &listpath,
  ...
  ProgressBar progress("Configuring data import from files listed in \"" +
                       Path::basename(listpath) + "\" as found relative to directory \"" +
                       load_from_dir + "\"");

  for (const auto &line : lines) {
    try {
      std::shared_ptr<SubjectDataImport> subject(
          new SubjectDataImport(Path::join(load_from_dir, line)));
