...
namespace MR {
namespace Math {
double matrix_average(vector<Eigen::MatrixXd> const &mat_in,
...
Header compute_minimum_average_header(
    ...
    Eigen::Matrix<default_type, 4, 1> padding =
        Eigen::Matrix<default_type, 4, 1>(1.0, 1.0, 1.0, 1.0));

template <class ImageType1, class ImageType2>
Header
compute_minimum_average_header(const ImageType1 &im1,
                               ...
                               Eigen::Matrix<default_type, 4, 1> padding =
                                   Eigen::Matrix<default_type, 4, 1>(1.0, 1.0, 1.0, 1.0),
                               int voxel_subsampling = 1) {
  vector<Eigen::Transform<default_type, 3, Eigen::Projective>> init_transforms{transform_1,
                                                                               transform_2};
