...
default_type Zstatistic::LookupBase::interp(const default_type stat,
  ...
  if (index_float >= 1.0 && index_float < data.size() - 2) {
    ...
    return hermite.value(
        data[index_int - 1], data[index_int], data[index_int + 1], data[index_int + 2]);
  }
  return func(stat);
}
...
Zstatistic::Lookup_t2z::Lookup_t2z(const size_t dof)
    : dof(dof), offset(-t2z_max - t2z_step), scale(1.0 / t2z_step) {
...
Zstatistic::Lookup_F2z::Lookup_F2z(const size_t rank, const size_t dof)
  ...
  data_upper =
      Math::sqrt2 *
      (2.0 * Math::betaincreg(0.5 * d_two, 0.5 * d_one, x_upper)).unaryExpr(&Math::erfcinv);
#else
  data_upper.resize(x_upper.size());
  for (ssize_t i = 0; i != x_upper.size(); ++i)
    data_upper[i] =
        Math::sqrt2 * Math::erfcinv(2.0 * Math::betaincreg(0.5 * dof, 0.5 * rank, x_upper[i]));
#endif
  ...
  data_lower =
      Math::sqrt2 *
      (2.0 * Math::betaincreg(0.5 * d_one, 0.5 * d_two, x_lower) - 1.0).unaryExpr(&Math::erfinv);
#else
  data_lower.resize(x_lower.size());
  for (ssize_t i = 0; i != x_lower.size(); ++i)
    data_lower[i] =
        Math::sqrt2 * Math::erfinv(2.0 * Math::betaincreg(0.5 * rank, 0.5 * dof, x_lower[i]) - 1.0);
#endif
}
