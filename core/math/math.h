...
void save_matrix(const MatrixType &M,
  ...
  Eigen::IOFormat fmt(Eigen::FullPrecision,
                      Eigen::DontAlignCols,
                      std::string(1, Path::delimiter(filename)),
                      "\n",
                      "",
                      "",
                      "",
...
vector<vector<ValueType>> load_matrix_2D_vector(const std::string &filename,
  ...
  if (!stream)
    throw Exception("Unable to open numerical data text file \"" + filename +
                    "\": " + strerror(errno));
...
inline transform_type load_transform(const std::string &filename, VectorType &centre) {
  ...
  if (V.size() != 3 && V.size() != 4)
    throw Exception("transform in file " + filename +
...
inline void save_transform(const transform_type &M,
  ...
  Eigen::IOFormat fmt(
      Eigen::FullPrecision, Eigen::DontAlignCols, std::string(1, d), "\n", "", "", "", "");
