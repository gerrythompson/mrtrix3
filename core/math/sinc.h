...
public:
  ...
  void set(const ImageType &image, const size_t axis, const value_type position) {
    ...
    for (size_t i = 0; i != window_size; ++i) {
      ...
      // const value_type hann_cos_term = Math::pi * offset /
      // (value_type(max_offset_from_kernel_centre) + 0.5); const value_type hann_factor   = (abs
      // (hann_cos_term) < Math::pi) ? 0.5 * (1.0 + std::cos (hann_cos_term)) : 0.0; const
      // value_type this_weight   = hann_factor * sinc;

      const value_type lanczos_sinc_term =
          abs(Math::pi * offset / (double(max_offset_from_kernel_centre) + 0.5));
