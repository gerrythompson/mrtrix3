...
template <typename T>
inline T lnP(const T measured, T actual, const T one_over_noise_squared, T &dP_dactual) {
  assert(measured >= 0.0);
...
lnP(const T measured, const T actual, const T one_over_noise_squared, T &dP_dactual, T &dP_dN) {
  ...
  dP_dN = 0.5 * pow2(m_a) - 1.0 / one_over_noise_squared +
          (actual_is_positive ? -actual_pos : actual_pos);
  return 0.5 * nm_a * m_a - log(nm * F0);
}
