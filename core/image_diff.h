    ...
    if (std::isfinite(in1.spacing(i)))
      if (abs((in1.spacing(i) - in2.spacing(i)) / (in1.spacing(i) + in2.spacing(i))) > 1e-4)
        throw Exception("images \"" + in1.name() + "\" and \"" + in2.name() +
                        "\" do not have matching voxel spacings on axis " + str(i) + " (" +
...
inline void check_images_frac(ImageType1 &in1, ImageType2 &in2, const double tol = 0.0) {
  ...
  ThreadedLoop(in1).run(
      [&tol](const ImageType1 &a, const ImageType2 &b) {
        if (abs((cdouble(a.value()) - cdouble(b.value())) /
                (0.5 * (cdouble(a.value()) + cdouble(b.value())))) > tol)
          throw Exception("images \"" + a.name() + "\" and \"" + b.name() +
...
//! check images are the same within a fractional tolerance relative to the maximum value in the
//! voxel
template <class ImageType1, class ImageType2>
  ...
  auto func = [&tol](decltype(in1) &a, decltype(in2) &b) {
    ...
    for (auto l = Loop(3)(a, b); l; ++l) {
      if (abs(cdouble(a.value()) - cdouble(b.value())) > threshold)
        throw Exception("images \"" + a.name() + "\" and \"" + b.name() +
                        "\" do not match within " + str(tol) + " of maximal voxel value" + " (" +
                        str(cdouble(a.value())) + " vs " + str(cdouble(b.value())) + ")");
    }
  };
...
inline void check_keyvals(const HeaderType1 &in1, const HeaderType2 &in2) {
  const static std::set<std::string> reserved{
      "command_history", "mrtrix_version", "project_version"};
...
inline bool headers_match(HeaderType1 &in1, HeaderType2 &in2) {
  if (!dimensions_match(in1, in2))
    return false;
  if (!spacings_match(in1, in2, 1e-6)) // implicitly checked in voxel_grids_match_in_scanner_space
                                       // but with different tolerance
    return false;
