...
public:
  ...
  void set_size(const uint32_t n) {
    // Handler allocates new memory if necessary, and sets the relevant number of elements flag in
    // the sparse image data It returns the file offset necessary to access the relevant memory, so
    // update the raw image value accordingly
...
public:
  Image(const std::string &image_name)
      : ::MR::Image<uint64_t>(::MR::Image<uint64_t>::open(image_name)), io(nullptr) {
    check();
  }

  Image(Header &header) : ::MR::Image<uint64_t>(header.get_image<uint64_t>()), io(nullptr) {
    check();
  }
  ...
  Image(const std::string &image_name, const Header &template_header)
      : ::MR::Image<uint64_t>(::MR::Image<uint64_t>::create(image_name, template_header)),
        io(nullptr) {
    check();
  }
...
protected:
  ...
  void check() {
    ...
    if (name_it == keyval().end())
      throw Exception("cannot create sparse image without knowledge of underlying class type in "
                      "the image header");
    // TODO temporarily disabled this to allow updated_syntax tests to pass with files generated
    // with master branch.
    //          const std::string& class_name = name_it->second;
    //          if (str(typeid(DataType).name()) != class_name)
    //            throw Exception ("class type of sparse image buffer (" +
    //            str(typeid(DataType).name()) + ") does not match that in image header (" +
    //            class_name + ")");
    KeyValues::const_iterator size_it = keyval().find(Fixel::Legacy::size_key);
    if (size_it == keyval().end())
      throw Exception("cannot create sparse image without knowledge of underlying class size in "
                      "the image header");
    ...
    DEBUG("Sparse image verified for accessing " + name() + " using type " +
          str(typeid(DataType).name()));
  }
};
