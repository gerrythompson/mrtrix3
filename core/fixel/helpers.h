...
FORCE_INLINE void check(const Header &in) {
  ...
  try {
    ...
    throw Exception(e,
                    "Image \"" + in.name() + "\" is not a valid peaks image: Expect 4 dimensions");
  }
  if (in.size(3) % 3)
    throw Exception("Image \"" + in.name() +
...
FORCE_INLINE bool fixels_match(const IndexHeaderType &index_header,
  ...
  if (is_index_image(index_header)) {
    if (index_header.keyval().count(n_fixels_key)) {
      fixels_match =
          std::stoul(index_header.keyval().at(n_fixels_key)) == (index_type)data_header.size(0);
    } else {
      auto index_image = Image<index_type>::open(index_header.name());
      index_image.index(3) = 1;
      index_type num_fixels = 0;
...
FORCE_INLINE void check_fixel_size(const Header &index_h, const Header &data_h) {
  ...
  if (!fixels_match(index_h, data_h))
    throw InvalidImageException("Fixel number mismatch between index image " + index_h.name() +
                                " and data image " + data_h.name());
}

FORCE_INLINE void check_fixel_directory(const std::string &path,
  ...
  if (check_if_empty && Path::Dir(path_temp).read_name().size() != 0)
    throw Exception("Output fixel directory \"" + path_temp + "\" is not empty" +
                    (App::overwrite_files ? " (-force option cannot safely be applied on "
                                            "directories; please erase manually instead)"
                                          : ""));
}
...
FORCE_INLINE Header find_index_header(const std::string &fixel_directory_path) {
  ...
  if (!header.valid())
    throw InvalidFixelDirectoryException("Could not find index image in directory " +
                                         fixel_directory_path);
...
//! Generate a header for a fixel directions data file (Nx3x1) based on knowledge of the number of
//! fixels
...
FORCE_INLINE void copy_fixel_file(const std::string &input_file_path,
...
FORCE_INLINE void copy_index_file(const std::string &input_directory,
                                  const std::string &output_directory) {
  ...
  if (Path::exists(output_path)) {
    ...
    if (!images_match_abs(input_image, output_image))
      throw Exception("output fixel directory \"" + output_directory +
    ...
    auto output_image = Image<index_type>::create(
        Path::join(output_directory, Path::basename(input_header.name())), input_header);
    auto input_image = input_header.get_image<index_type>();
    threaded_copy(input_image, output_image);
  }
}
...
FORCE_INLINE void copy_directions_file(const std::string &input_directory,
                                       const std::string &output_directory) {
  ...
  if (Path::exists(output_path)) {
    ...
    auto output_image = Image<float>::create(
        Path::join(output_directory, Path::basename(input_header.name())), input_header);
    auto input_image = input_header.get_image<float>();
    threaded_copy(input_image, output_image);
  }
...
//! Copy all data files in a fixel directory into another directory. Data files do not include the
//! index or directions file.
FORCE_INLINE void copy_all_data_files(const std::string &input_directory,
                                      const std::string &output_directory) {
  for (auto &input_header :
       Fixel::find_data_headers(input_directory, Fixel::find_index_header(input_directory)))
