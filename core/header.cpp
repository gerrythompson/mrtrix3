...
std::string short_description(const Header &H) {
  ...
  return " with dimensions " + join(dims, "x") + ", voxel spacing " + join(vox, "x") +
  ...
  try {
    ...
    if (num.size()) {
      ...
      std::function<void(Header &, vector<Header> &, const size_t)> import =
        ...
        // The nested concatenation may still include the very first header that has already been
        // read;
...
Header Header::create(const std::string &image_name,
  ...
  try {
    ...
    if (!check_strides_match(strides, strides_aftercheck)) {
      INFO("output strides for image " + image_name + " modified to " + str(strides_aftercheck) +
           " - requested strides " + str(strides) + " are not supported in " +
           (*format_handler)->description + " format");
    }
...
std::string Header::description(bool print_all) const {
  ...
  if (io) {
    ...
    desc += std::string("  Data type:         ") +
            (datatype().description() ? datatype().description() : "invalid") + "\n";
    desc += "  Intensity scaling: offset = " + str(intensity_offset()) +
            ", multiplier = " + str(intensity_scale()) + "\n";
  }
...
void Header::realign_transform() {
  ...
  INFO("Axes and transform of image \"" + name() +
       "\" altered to approximate RAS coordinate system");
  ...
  if (pe_scheme.rows()) {
    ...
    INFO("Phase encoding scheme modified to conform to MRtrix3 internal header transform "
         "realignment");
  }
  ...
  if (slice_encoding_it != keyval().end()) {
    ...
    for (size_t axis = 0; axis != 3; ++axis)
      new_dir[axis] =
          orig_dir[realign_perm_[axis]] * (realign_flip_[realign_perm_[axis]] ? -1.0 : 1.0);
    slice_encoding_it->second = Axes::dir2id(new_dir);
    INFO("Slice encoding direction has been modified to conform to MRtrix3 internal header "
         "transform realignment");
  }
}

Header concatenate(const vector<Header> &headers,
                   const size_t axis_to_concat,
                   const bool permit_datatype_mismatch) {
  Exception e("Unable to concatenate " + str(headers.size()) + " images along axis " +
              str(axis_to_concat) + ": ");
  ...
  for (const auto &H : headers) {
    ...
    for (this_max_nonunity_dim = H.ndim() - 1;
         this_max_nonunity_dim >= 0 && H.size(this_max_nonunity_dim) <= 1;
         --this_max_nonunity_dim)
      ;
    global_max_nonunity_dim =
        std::max(global_max_nonunity_dim, size_t(std::max(ssize_t(0), this_max_nonunity_dim)));
  }
  ...
  for (size_t i = 1; i != headers.size(); ++i) {
    ...
    for (size_t axis = 0; axis <= global_max_nonunity_dim; ++axis) {
      if (axis != axis_to_concat && axis < H.ndim() && H.size(axis) != result.size(axis)) {
        e.push_back("Images \"" + result.name() + "\" and \"" + H.name() +
                    "\" have inequal sizes along axis " + str(axis_to_concat) + " (" +
                    str(result.size(axis)) + " vs " + str(H.size(axis)) + ")");
        throw e;
      }
    }
    ...
    if (datatype_test(result.datatype().is_integer() && !result.datatype().is_signed() &&
    ...
    if (datatype_test(result.datatype().bytes() < H.datatype().bytes()))
      result.datatype() =
          (result.datatype()() & DataType::Attributes) + (H.datatype()() & DataType::Type);
  }
