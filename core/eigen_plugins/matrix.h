...
template <class ImageType> Matrix(const MR::Helper::ConstRow<ImageType> &row) : Base() {
  operator=(row);
}
template <class ImageType> Matrix(const MR::Helper::Row<ImageType> &row) : Base() {
  operator=(row);
}

#define MRTRIX_OP(ARG)                                                                             \
  template <class ImageType>                                                                       \
  inline Matrix &operator ARG(const MR::Helper::ConstRow<ImageType> &row) {                        \
