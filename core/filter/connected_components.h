...
public:
  ...
  void run(vector<Cluster> &, vector<uint32_t> &, const VectorType &, const float) const;
...
private:
  ...
  void depth_first_search(
      const uint32_t, Cluster &, vector<uint32_t> &, const VectorType &, const float) const;
};
  ...
  void set_axes(const vector<bool> &i) {
    if (i.size() != ndim())
      throw Exception("Length of axis selection flag vector (" + str(i.size()) +
                      ") does not match dimensionality of connected-component filter (" +
                      str(ndim()) + "D)");
    enabled_axes = i;
  }
