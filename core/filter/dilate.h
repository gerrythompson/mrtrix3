...
public:
  template <class HeaderType> Dilate(const HeaderType &in) : Base(in), npass(1) {
    datatype_ = DataType::Bit;
  }

  template <class HeaderType>
  Dilate(const HeaderType &in, const std::string &message) : Base(in, message), npass(1) {
    datatype_ = DataType::Bit;
  }
  ...
  void operator()(InputImageType &input, OutputImageType &output) {
    ...
    std::shared_ptr<ProgressBar> progress(message.size() ? new ProgressBar(message, npass + 1)
                                                         : nullptr);
