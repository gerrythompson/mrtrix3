...
public:
  ...
  void operator()(InputImageType &input, MaskType &spatial_prior, OutputImageType &output) {
    ...
    ssize_t cnt_lower = std::max<size_t>(
        10000, std::floor(fov_min * input.size(0) * input.size(1) * input.size(2)));
    ...
    while (cnt >= cnt_lower) {
      ...
      INFO("median: " + str(median) +
           ", changed: " + str((median - previous_median) / previous_median));
      ...
      if (lower > 0.0 && ((median + 2.5 * mad) - (previous_median + 2.5 * previous_mad)) < 0.0 &&
          (cnt < cnt_upper))
        break;
    }
        ...
        for (auto l = Loop(0, 3)(input, int_roi); l; ++l) {
          if (int_roi.value()) {
            ...
            if (App::log_level >= 3) {
              ...
              eroded_zscore_image.value() =
                  (z > -zlower) && (dont_maskupper || z < zupper) ? z : NaN;
            }
            if (good)
              cnt++;
            int_roi.value() = good;
    ...
    for (auto l = Loop(0, 3)(input, mask, spatial_prior); l; ++l)
      mask.value() = spatial_prior.value() && input.value() >= lower &&
                     (dont_maskupper || input.value() <= upper);
...
protected:
  ...
  void calculate_median_mad(
      ImageType &image, MaskType &mask, size_t nvoxels, float &median, float &mad) {
