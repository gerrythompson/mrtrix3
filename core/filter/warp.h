...
// TODO if there is a use for this elsewhere then we should have threaded_copy4D convenience
// functions
class CopyKernel4D {
public:
...
void warp(ImageTypeSource &source,
  ...
  if (warp.transform().matrix() != destination.transform().matrix() ||
      !dimensions_match(warp, destination, 0, 3) || !spacings_match(warp, destination, 0, 3)) {
