 ...
 *
 *  Automatic oversampling is applied except for nearest-neighbour interpolation, which uses no
 * oversampling.
 *
 * Typical usage:
 * \code
...
public:
  ...
  void set_voxel_size(const vector<default_type> &voxel_size) {
    ...
    for (size_t j = 0; j < 3; ++j) {
      ...
      axes_[j].size =
          std::round(axes_[j].size * axes_[j].spacing / voxel_size[j] - 0.0001); // round down at .5
      // Here we adjust the translation to ensure the image extent is centered wrt the original
      // extent. This is important when the new voxel size is not an exact multiple of the original
      // extent
      for (size_t i = 0; i < 3; ++i)
        transform_(i, 3) += 0.5 *
                            ((voxel_size[j] - axes_[j].spacing) +
                             (original_extent[j] - (axes_[j].size * voxel_size[j]))) *
                            transform_(i, j);
