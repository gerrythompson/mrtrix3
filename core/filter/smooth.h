...
public:
  ...
  void set_extent(const vector<uint32_t> &new_extent) {
    if (new_extent.size() != 1 && new_extent.size() != 3)
      throw Exception("Please supply a single kernel extent value, or three values (one for each "
                      "spatial dimension)");
  ...
  //! ensure the image boundary remains zero. Used to constrain displacement fields during image
  //! registration
  ...
  void set_stdev(const vector<default_type> &std_dev) {
    ...
    if (std_dev.size() == 1) {
      ...
      if (std_dev.size() != 3)
        throw Exception("Please supply a single standard deviation value, or three values (one for "
  ...
  void operator()(InputImageType &input, OutputImageType &output) {
    std::shared_ptr<Image<ValueType>> in(
        make_shared<Image<ValueType>>(Image<ValueType>::scratch(input)));
    ...
    for (size_t dim = 0; dim < 3; dim++) {
      if (stdev[dim] > 0) {
        ...
        Adapter::Gaussian1D<Image<ValueType>> gaussian(
            *in, stdev[dim], dim, extent[dim], zero_boundary);
  ...
  template <class ImageType> void operator()(ImageType &in_and_output) {
    ...
    for (size_t dim = 0; dim < 3; dim++) {
      if (stdev[dim] > 0) {
        ...
        SmoothFunctor1D<ImageType> smooth(
            in_and_output, stdev[dim], dim, extent[dim], zero_boundary);
