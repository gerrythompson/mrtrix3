...
void SparseLegacy::load(const Header &header, size_t) {
  ...
  if (current_sparse_data_size) {
    ...
    DEBUG("Initialising output sparse data file " + file.name + ": new file size " +
          str(new_file_size) + " (" + str(init_sparse_data_size) +
          " of which is initial sparse data buffer)");
    ...
    // Any voxel that has its value initialised to 0 will point here, and therefore dereferencing of
    // any
...
void SparseLegacy::unload(const Header &header) {
  ...
  // Null the excess data before closing the memory map to prevent std::ofstream from giving an
  // error
  ...
  if (truncate_file_size) {
    DEBUG("truncating sparse image data file " + file.name + " to " + str(truncate_file_size) +
          " bytes");
...
uint64_t SparseLegacy::set_numel(const uint64_t old_offset, const uint32_t numel) {
  ...
  if (old_offset) {
    ...
    if (existing_numel >= numel) {
      ...
      memset(off2mem(old_offset) + sizeof(uint32_t) + (numel * class_size),
             0x00,
             ((existing_numel - numel) * class_size));
      // If this voxel is being cleared (i.e. no elements), rather than returning a pointer to a
      // voxel with
      //   zero elements, instead return a pointer to the start of the sparse data, where there's a
      //   single uint32_t(0) which can be used for any and all voxels with zero elements
      return numel ? old_offset : 0;

    } else {

      // Existing memory allocation for this voxel is not sufficient; erase it
      // Note that the handler makes no attempt at re-using this memory later; new data is just
      // concatenated
  ...
  if (data_end + requested_size > size()) {
    ...
    // Memory error arises due to the uninitialised data between the end of the sparse data and the
    // end
