...
const OptionGroup SelectOptions =
    ...
    Option("pe",
           ...
           "this can be three comma-separated values (for i,j,k components of vector direction) or "
...
Eigen::MatrixXd get_scheme(const Header &header) {
  ...
  try {
    ...
    if (opt_eddy.size()) {
      if (opt_table.size())
        throw Exception("Phase encoding table can be provided using either -import_pe_table or "
                        "-import_pe_eddy option, but NOT both");
...
Eigen::MatrixXd eddy2scheme(const Eigen::MatrixXd &config,
                            const Eigen::Array<int, Eigen::Dynamic, 1> &indices) {
  if (config.cols() != 4)
  ...
  for (ssize_t row = 0; row != indices.size(); ++row) {
    if (indices[row] > config.rows())
      throw Exception("Malformed EDDY-style phase-encoding information: Index exceeds number of "
                      "config entries");
