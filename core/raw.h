...
inline typename std::enable_if<std::is_fundamental<ValueType>::value && sizeof(ValueType) == 1,
                               ValueType>::type
swap(ValueType v) {
  return v;
}
...
template <typename ValueType>
inline typename std::enable_if<std::is_fundamental<ValueType>::value && sizeof(ValueType) == 2,
                               ValueType>::type
swap(ValueType v) {
  union {
    ValueType v;
...
template <typename ValueType>
inline typename std::enable_if<std::is_fundamental<ValueType>::value && sizeof(ValueType) == 4,
                               ValueType>::type
swap(ValueType v) {
  union {
    ValueType v;
...
template <typename ValueType>
inline typename std::enable_if<std::is_fundamental<ValueType>::value && sizeof(ValueType) == 8,
                               ValueType>::type
swap(ValueType v) {
  union {
    ValueType v;
...
template <typename ValueType> const ValueType *as(const void *p) {
...
template <typename ValueType>
inline ValueType fetch_(const void *address, bool is_big_endian = MRTRIX_IS_BIG_ENDIAN) {
  return ByteOrder::swap(*as<ValueType>(address), is_big_endian);
}

template <typename ValueType> inline ValueType fetch__native(const void *address) {
...
template <typename ValueType>
inline void
store(const ValueType value, void *data, size_t i, bool is_big_endian = MRTRIX_IS_BIG_ENDIAN) {
  as<ValueType>(data)[i] = ByteOrder::swap(value, is_big_endian);
}

//! store \a value in native format at offset \a i from \a data
template <typename ValueType>
inline void store_native(const ValueType value, void *data, size_t i) {
...
template <> inline bool fetch<bool>(const void *data, size_t i, bool) {
