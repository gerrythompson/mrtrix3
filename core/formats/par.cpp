...
std::unique_ptr<ImageIO::Base> PAR::read(Header &H) const {
  ...
  if (cols.ang >= 0 && cols.pos >= 0) {
    ...
    if (slices[0].seq == 2) { // sagittal
      ...
      throw Exception("Images detected in coronal orientation - not yet supported. Please contact "
                      "MRtrix3 team for support");
    }

    Eigen::Vector3d p(-slices[0].pos[2], -slices[0].pos[0], slices[0].pos[1]);
    p -= M * Eigen::Vector3d(((H.size(0) - 1) * H.spacing(0)) / 2.0,
                             ((H.size(1) - 1) * H.spacing(1)) / 2.0,
                             0.0);
    M.pretranslate(p);
  ...
  if (G.size()) {
    if (G.size() != size_t(nvols))
      throw Exception(
          "mismatch between number of volumes and number of b-values in PAR/REC file \"" +
          H.name() + "\"");
