...
std::unique_ptr<ImageIO::Base> PNG::read(Header &H) const {
  ...
  case PNG_COLOR_TYPE_RGB_ALPHA:
    ...
    break;
  default:
    throw Exception("Unsupported color type in PNG image \"" + H.name() + "\" (" +
  ...
  case 1:
    if (png.get_colortype() == PNG_COLOR_TYPE_PALETTE) {
      ...
      WARN("Bitwise PNG being read with width not a factor of 8; will be converted to UInt8 "
           "datatype");
      H.datatype() = DataType::UInt8;
    } else {
  ...
  case 16:
    ...
    break;
  default:
    throw Exception("Unexpected bit depth (" + str(png.get_bitdepth()) + ") in PNG image \"" +
                    H.name() + "\"");
  }
...
bool PNG::check(Header &H, size_t num_axes) const {
  ...
  if (H.ndim() == 4 && H.size(3) > 4)
    throw Exception(
        "A 4D image written to PNG must have between one and four volumes (requested: " +
        str(H.size(3)) + ")");
  ...
  if (H.ndim() - num_axes > 1)
    throw Exception(
        "Cannot nominate more than one axis using square-bracket notation for PNG format");
  switch (num_axes) {
  case 1:
  ...
  case 3:
    if (H.size(1) == 1) {
      ...
      if (H.ndim() == 3 && H.size(2) > 1)
        throw Exception("Cannot export 3D image to PNG format if all three axes have size greater "
                        "than 1 and square-bracket notation is not used");
  ...
  case 4:
    ...
    if (axis < 0)
      throw Exception("Cannot export 4D image to PNG format if all three spatial axes have size "
                      "greater than 1 and square-bracket notation is not used");
    if (!axis_to_zero)
      width_axis = 1;
    break;
  default:
  ...
  if (H.datatype() == DataType::Bit && H.size(width_axis) % 8) {
    WARN("Cannot write bitwise PNG image with width not a factor of 8; will instead write with "
         "8-bit depth");
    H.datatype() = DataType::UInt8;
  }
