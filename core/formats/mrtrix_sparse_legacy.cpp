...
std::unique_ptr<ImageIO::Base> MRtrix_sparse::read(Header &H) const {
  ...
  // Although the endianness of the image data itself (the sparse data offsets) actually doesn't
  // matter
  //   (the Image class would deal with this conversion), the sparse data itself needs to have
  //   the correct endianness for the system. Since MRtrix_sparse::create() forces the endianness of
  //   the offset data to be native, this is the easiest way to verify that the sparse data also has
...
#endif
  if (H.datatype() != dt)
    throw Exception("Cannot open sparse image file " + H.name() + " due to type mismatch; expect " +
...
std::unique_ptr<ImageIO::Base> MRtrix_sparse::create(Header &H) const {
  ...
  const auto name_it = H.keyval().find(Fixel::Legacy::name_key);
  if (name_it == H.keyval().end())
    throw Exception("Cannot create sparse image " + H.name() +
                    "; no knowledge of underlying data class type");

  const auto size_it = H.keyval().find(Fixel::Legacy::size_key);
  if (size_it == H.keyval().end())
    throw Exception("Cannot create sparse image " + H.name() +
                    "; no knowledge of underlying data class size");
