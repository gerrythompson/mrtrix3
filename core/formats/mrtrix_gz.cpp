...
std::unique_ptr<ImageIO::Base> MRtrix_GZ::read(Header &H) const {
  ...
  if (first_line != "mrtrix image") {
    zf.close();
    throw Exception("invalid first line for compressed image \"" + H.name() +
  ...
  write_offset = offset;
  if (fname != H.name())
    throw Exception("GZip-compressed MRtrix format images must have image data within the same "
                    "file as the header");
