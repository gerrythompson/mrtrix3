...
std::unique_ptr<ImageIO::Base> MRI::read(Header &H) const {
  ...
  while (current <= last) {
    ...
    case MRI_TRANSFORM:
      for (size_t i = 0; i < 3; ++i)
        for (size_t j = 0; j < 4; ++j)
          H.transform()(i, j) =
              Raw::fetch_<float32>(data(current) + (i * 4 + j) * sizeof(float32), is_BE);
      break;
    case MRI_DWSCHEME: {
    ...
    default:
      WARN("unknown header entity (" + str(type(current, is_BE)) + ", offset " +
           str(current - fmap.address()) + ") in image \"" + H.name() + "\" - ignored");
      break;
    }
