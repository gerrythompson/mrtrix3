...
void Calibrator::finalize(const size_t num_volumes, const bool is_integer) {
  if (!std::isfinite(bin_width)) {
    if (num_bins) {
      ...
      // Will need to revisit if mrstats gets capability to compute statistics across all volumes
      // rather than splitting
      bin_width = 2.0 * get_iqr() *
                  std::pow(static_cast<default_type>(data.size() / num_volumes), -1.0 / 3.0);
...
default_type Data::entropy() const {
  ...
  for (size_t i = 0; i < size_t(list.size()); i++) {
    const default_type probability =
        static_cast<default_type>(list[i]) / static_cast<default_type>(totalFrequency);
...
Matcher::Matcher(const Data &input, const Data &target)
  ...
  for (size_t input_index = 1; input_index != size_t(cdf_input.size()); ++input_index) {
    while (upper_target_index < size_t(cdf_target.size()) &&
           cdf_target[upper_target_index] < cdf_input[input_index])
      ++upper_target_index;
