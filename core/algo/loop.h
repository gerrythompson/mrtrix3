...
struct set_pos {
  ...
  template <class ImageType> FORCE_INLINE void operator()(ImageType &vox) {
    vox.index(axis) = index;
  }
  size_t axis;
  ssize_t index;
};
...
struct LoopAlongSingleAxisProgress {
  ...
  template <class... ImageType> struct Run {
    ...
    const ssize_t size0;
    FORCE_INLINE
    Run(const std::string &text, const size_t axis, const std::tuple<ImageType &...> &vox)
        : progress(text, std::get<0>(vox).size(axis)),
          axis(axis),
          vox(vox),
          size0(std::get<0>(vox).size(axis)) {
      MR::apply(set_pos(axis, 0), vox);
    }
    FORCE_INLINE operator bool() const { return std::get<0>(vox).index(axis) < size0; }
...
struct LoopAlongAxisRange {
  ...
  template <class... ImageType> struct Run {
    ...
    bool ok;
    FORCE_INLINE
    Run(const size_t axis_from, const size_t axis_to, const std::tuple<ImageType &...> &vox)
        : from(axis_from),
...
struct LoopAlongAxisRangeProgress : public LoopAlongAxisRange {
  ...
  template <class... ImageType> struct Run : public LoopAlongAxisRange::Run<ImageType...> {
    MR::ProgressBar progress;
    FORCE_INLINE Run(const std::string &text,
                     const size_t from,
                     const size_t to,
                     const std::tuple<ImageType &...> &vox)
...
struct LoopAlongAxesProgress {
  const std::string text;
  template <class... ImageType>
  FORCE_INLINE LoopAlongAxisRangeProgress::Run<ImageType...>
  operator()(ImageType &...images) const {
...
struct LoopAlongDynamicAxesProgress : public LoopAlongDynamicAxes {
  ...
  template <class... ImageType> struct Run : public LoopAlongDynamicAxes::Run<ImageType...> {
    MR::ProgressBar progress;
    FORCE_INLINE
    Run(const std::string &text, const vector<size_t> &axes, const std::tuple<ImageType &...> &vox)
        : LoopAlongDynamicAxes::Run<ImageType...>(axes, vox),
          progress(text, MR::voxel_count(std::get<0>(vox), axes)) {}
    FORCE_INLINE void operator++() {
...
FORCE_INLINE LoopAlongAxisRangeProgress Loop(const std::string &progress_message,
                                             size_t axis_from,
                                             size_t axis_to) {
...
FORCE_INLINE LoopAlongDynamicAxesProgress Loop(const std::string &progress_message,
                                               const vector<size_t> &axes) {
...
Loop(const ImageType &source,
     ...
     typename std::enable_if<std::is_class<ImageType>::value &&
                                 !std::is_same<ImageType, std::string>::value,
                             int>::type = 0) {
