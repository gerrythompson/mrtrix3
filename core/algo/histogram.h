...
public:
  ...
  typename std::enable_if<std::is_arithmetic<value_type>::value, bool>::type
  ...
  template <class T>
  FORCE_INLINE typename std::enable_if<!std::is_arithmetic<T>::value, bool>::type
...
public:
  ...
  Data(const Calibrator &calibrate)
      : info(calibrate), list(vector_type::Zero(info.get_num_bins())) {}
...
template <class ImageType> void calibrate(Calibrator &result, ImageType &image) {
  ...
  result.finalize(image.ndim() > 3 ? image.size(3) : 1,
                  std::is_integral<typename ImageType::value_type>::value);
}
