...
struct RandomThreadedLoopRunInner<0, Functor, ImageType...> {
  ...
  RandomThreadedLoopRunInner(const vector<size_t> &outer_axes,
    ...
    random_engine =
        std::default_random_engine{static_cast<std::default_random_engine::result_type>(seed)};
  ...
  void run_outer(Functor &&functor, const double voxel_density, const vector<size_t> &dimensions) {
    if (Thread::threads_to_execute() == 0) {
      for (auto i = outer_loop(iterator); i; ++i) {
        // std::cerr << "outer: " << str(iterator) << " " << voxel_density << " " << dimensions <<
        // std::endl;
        functor(iterator);
      }
      return;
...
inline RandomThreadedLoopRunOuter<decltype(Loop(vector<size_t>()))> RandomThreadedLoop(
    const HeaderType &source, const vector<size_t> &outer_axes, const vector<size_t> &inner_axes) {
  return {source, Loop(outer_axes), inner_axes};
}

template <class HeaderType>
inline RandomThreadedLoopRunOuter<decltype(Loop(vector<size_t>()))> RandomThreadedLoop(
    const HeaderType &source, const vector<size_t> &axes, size_t num_inner_axes = 1) {
