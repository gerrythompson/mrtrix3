...
template <class T> inline std::string str(const T &value, int precision = 0) {
  ...
  stream << value;
  if (stream.fail())
    throw Exception(std::string("error converting type \"") + typeid(T).name() +
                    "\" value to string");
  return stream.str();
}
...
template <class T> inline T to(const std::string &string) {
  ...
  if (stream.fail()) {
    ...
    throw Exception("incomplete use of string \"" + string + "\" in conversion to type \"" +
                    typeid(T).name() + "\"");
  }
  return value;
}
...
vector<IntType> parse_ints(const std::string &spec,
