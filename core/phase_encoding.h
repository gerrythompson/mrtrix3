...
void scheme2eddy(const MatrixType &PE,
  ...
  for (ssize_t PE_row = 0; PE_row != PE.rows(); ++PE_row) {
    for (ssize_t config_row = 0; config_row != config.rows(); ++config_row) {
      bool dir_match =
          PE.template block<1, 3>(PE_row, 0).isApprox(config.block<1, 3>(config_row, 0));
...
Eigen::MatrixXd transform_for_image_load(const MatrixType &pe_scheme, const HeaderType &H) {
  ...
  if (perm[0] == 0 && perm[1] == 1 && perm[2] == 2 && !flip[0] && !flip[1] && !flip[2]) {
    INFO("No transformation of external phase encoding data required to accompany image \"" +
         H.name() + "\"");
    return pe_scheme;
  }
...
Eigen::MatrixXd transform_for_nifti_write(const MatrixType &pe_scheme, const HeaderType &H) {
  ...
  Eigen::Matrix<default_type, Eigen::Dynamic, Eigen::Dynamic> result(pe_scheme.rows(),
                                                                     pe_scheme.cols());
  for (ssize_t row = 0; row != pe_scheme.rows(); ++row) {
    Eigen::VectorXd new_line = pe_scheme.row(row);
    for (ssize_t axis = 0; axis != 3; ++axis)
      new_line[axis] = pe_scheme(row, order[axis]) && flip[axis] ? -pe_scheme(row, order[axis])
...
template <class HeaderType>
Eigen::MatrixXd
load_eddy(const std::string &config_path, const std::string &index_path, const HeaderType &header) {
