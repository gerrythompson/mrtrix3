...
//! This class provides access to the voxel intensities of an image using cubic spline
//! interpolation.
...
public:
  ...
  SplineInterp(const ImageType &parent,
               value_type value_when_out_of_bounds = SplineBase::default_out_of_bounds_value())
      : SplineInterpBase<ImageType, SplineType, Math::SplineProcessingType::Value>(
  ...
  value_type value() {
    ...
    ssize_t c[] = {ssize_t(std::floor(P[0]) - 1),
                   ssize_t(std::floor(P[1]) - 1),
                   ssize_t(std::floor(P[2]) - 1)};
...
public:
  using SplineBase =
      SplineInterpBase<ImageType, SplineType, Math::SplineProcessingType::Derivative>;
  ...
  SplineInterp(const ImageType &parent,
      ...
      : SplineInterpBase<ImageType, SplineType, Math::SplineProcessingType::Derivative>(
            parent, value_when_out_of_bounds),
        out_of_bounds_vec(
            value_when_out_of_bounds, value_when_out_of_bounds, value_when_out_of_bounds),
  ...
  //! Returns the image gradient at the current position, defined with respect to the scanner
  ...
  //! Collectively interpolates gradients along axis 3, defined with respect to the scanner
  //! coordinate frame of reference.
...
class SplineInterp<ImageType, SplineType, Math::SplineProcessingType::ValueAndDerivative>
    : public SplineInterpBase<ImageType,
                              SplineType,
...
public:
  ...
  void value_and_gradient_wrt_scanner(value_type &value,
                                      Eigen::Matrix<value_type, 1, 3> &gradient) {
...
// This allows an interface that's consistent with other interpolators that all have one template
// argument
template <typename ImageType>
using Cubic = SplineInterp<ImageType,
                           Math::HermiteSpline<typename ImageType::value_type>,
                           Math::SplineProcessingType::Value>;

template <typename ImageType>
using CubicUniform = SplineInterp<ImageType,
                                  Math::UniformBSpline<typename ImageType::value_type>,
                                  Math::SplineProcessingType::Value>;
