...
protected:
  ...
  template <class VectorType> bool set_out_of_bounds(const VectorType &pos) {
    return ((out_of_bounds = (pos[0] <= -0.5 || pos[0] >= bounds[0] || pos[1] <= -0.5 ||
                              pos[1] >= bounds[1] || pos[2] <= -0.5 || pos[2] >= bounds[2])));
  }
  ...
  template <class VectorType> Eigen::Vector3d intravoxel_offset(const VectorType &pos) {
    ...
    return Eigen::Vector3d(
        pos[0] - std::floor(pos[0]), pos[1] - std::floor(pos[1]), pos[2] - std::floor(pos[2]));
  }
};
