...
void display_func_terminal(const ProgressBar &p) {
  __need_newline = true;
  if (p.show_percent())
    __print_stderr(printf(WRAP_OFF_CODE "\r%s: [%3" PRI_SIZET
                                        "%%] %s%s" CLEAR_LINE_CODE WRAP_ON_CODE,
                          App::NAME.c_str(),
                          p.value(),
                          p.text().c_str(),
                          p.ellipsis().c_str()));
...
void done_func_terminal(const ProgressBar &p) {
  if (p.show_percent())
    __print_stderr(
        printf("\r%s: [100%%] %s" CLEAR_LINE_CODE "\n", App::NAME.c_str(), p.text().c_str()));
  else
    __print_stderr(
        printf("\r%s: [done] %s" CLEAR_LINE_CODE "\n", App::NAME.c_str(), p.text().c_str()));
  ...
  // text is static - can simply append to the current line:
    ...
    if (p.show_percent()) {
      if (p.first_time) {
        p.first_time = false;
        __print_stderr(
            printf("%s: %s%s [", App::NAME.c_str(), p.text().c_str(), p.ellipsis().c_str()));
        ;
      } else
