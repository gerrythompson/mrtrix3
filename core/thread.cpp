...
__Backend::__Backend() : refcount(0) {
  ...
  if (!a.is_lock_free())
    WARN("atomic operations on uint8_t are not lock-free - this may introduce instabilities in "
         "operation!");

  if (sizeof(a) != sizeof(uint8_t))
    WARN("size of atomic<uint8_t> differs from that of uint8_t - this may introduce instabilities "
         "in operation!");
