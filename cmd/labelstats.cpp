...
void usage() {
  ...
  +Option("output",
      ...
      + Option("voxelspace",
               "report parcel centres of mass in voxel space rather than scanner space");
}
...
void run() {
  ...
  std::cout << std::setw(8) << std::right << "index"
            ...
            << std::setw(com_width / 2 + std::string("centre of mass").size() / 2) << std::right
            << "centre of mass"
            << "\n";
  for (ssize_t i = 0; i != masses.size(); ++i) {
    if (masses[i]) {
      std::cout << std::setw(8) << std::right << i + 1 << " " << std::setw(8) << std::right
                << masses[i] << " " << std::setw(4 + com_width) << std::right << com_strings[i]
                << "\n";
    }
  }
}
