...
void usage() {
  ...
  SYNOPSIS = "Examine the values in spherical harmonic images to estimate (and optionally change) "
             "the SH basis used";
...
template <typename value_type> void check_and_update(Header &H, const conv_t conversion) {
  ...
  for (size_t l = 2; l <= lmax; l += 2) {
    ...
    for (image.index(3) = ssize_t(Math::SH::NforL(l - 2));
         image.index(3) != ssize_t(Math::SH::NforL(l));
         ++image.index(3)) {
      double sum = 0.0;
  ...
  case 6:
    regression =
        std::make_pair(ratios[1] - 2 * (ratios[2] - ratios[1]), 0.5 * (ratios[2] - ratios[1]));
  ...
  if ((power_ratio > (5.0 / 3.0)) && (power_ratio < (7.0 / 3.0))) {
    ...
    case FORCE_NEWTOOLD:
      WARN("Refusing to convert image \"" + H.name() +
           "\" from new to old basis, as data appear to already be in the old non-orthonormal "
           "basis");
      return;
    }
    grad_threshold *= 2.0;
    ...
    WARN("Cannot make unambiguous decision on SH basis of image \"" + H.name() +
         "\" (power ratio " + (lmax < 8 ? "in" : "regressed to") + " " + str(l_for_decision) +
         " is " + str(power_ratio) + ")");

    if (conversion == FORCE_OLDTONEW) {
      WARN("Forcing conversion of image \"" + H.name() +
           "\" from old to new SH basis on user request; however NO GUARANTEE IS PROVIDED on "
           "appropriateness of this conversion!");
      multiplier = Math::sqrt1_2;
    } else if (conversion == FORCE_NEWTOOLD) {
      WARN("Forcing conversion of image \"" + H.name() +
           "\" from new to old SH basis on user request; however NO GUARANTEE IS PROVIDED on "
           "appropriateness of this conversion!");
      multiplier = Math::sqrt2;
    }
  }
  ...
  if (regression.second)
    DEBUG("Gradient of regression is " + str(regression.second) + "; threshold is " +
          str(grad_threshold));
  if (abs(regression.second) > grad_threshold) {
    WARN("Image \"" + H.name() +
         "\" may have been derived from poor directional encoding, or have some other underlying "
         "data problem");
    WARN("(m!=0 to m==0 power ratio changing by " + str(2.0 * regression.second) +
         " per even order)");
  }
