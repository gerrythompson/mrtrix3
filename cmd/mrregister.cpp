...
const OptionGroup multiContrastOptions = OptionGroup("Multi-contrast options") +
                                         Option("mc_weights",
                                                "relative weight of images used for multi-contrast "
                                                "registration. Default: 1.0 (equal weighting)") +
                                         Argument("weights").type_sequence_float();

void usage() {
  AUTHOR =
      "David Raffelt (david.raffelt@florey.edu.au) & Max Pietsch (maximilian.pietsch@kcl.ac.uk)";

  SYNOPSIS = "Register two images together using a symmetric rigid, affine or non-linear "
  ...
  +"By default this application will perform an affine, followed by non-linear registration."
      ...
      + "FOD registration (with apodised point spread reorientation) will be performed by default "
        ...
        "in the 4th dimension equals the number of coefficients in an antipodally symmetric "
        "spherical harmonic series (e.g. 6, 15, 28 etc). "
      ...
      "Similar to Avants (2008) Med Image Anal. 12(1): 26–41, registration is performed by "
      "matching both the image1 and image2 in a 'midway space'. "
      "Warps can be saved as two deformation fields that map directly between image1->image2 and "
      "image2->image1, or if using -nl_warp_full as a single 5D file "
      "that stores all 4 warps image1->mid->image2, and image2->mid->image1. The 5D warp format "
      "stores x,y,z deformations in the 4th dimension, and uses the 5th dimension "
      "to index the 4 warps. The affine transforms estimated (to midway space) are also stored as "
      "comments in the image header. The 5D warp file can be used to reinitialise "
      "subsequent registrations, in addition to transforming images to midway space (e.g. for "
      "intra-subject alignment in a 2-time-point longitudinal analysis).";
  ...
  ARGUMENTS
  +Argument("image1 image2", "input image 1 ('moving') and input image 2 ('template')")
          .type_image_in() +
      Argument("contrast1 contrast2",
               "optional list of additional input images used as additional contrasts. "
               "Can be used multiple times. contrastX and imageX must share the same coordinate "
               "system. ")
          .type_image_in()
          .optional()
          .allow_multiple();
  ...
  +Option("type",
          ...
          "rigid, affine, nonlinear, rigid_affine, rigid_nonlinear, affine_nonlinear, "
      ...
      + Option("transformed",
               ...
               "Note that -transformed needs to be repeated for each contrast if multi-contrast "
      ...
      Option("transformed_midway",
             ...
             "Note that -transformed_midway needs to be repeated for each contrast if "
...
void run() {
  ...
  // reorientation_forbidden required for output of transformed images because do_reorientation
  // might change
  ...
  for (size_t i = 1; i < n_images; i++) {
    if (!trafo.isApprox(MR::Transform(input1[i]).scanner2voxel.linear(), 1e-5))
      WARN("Multi contrast image has different header transformation from first image. Ignoring "
           "transformation of " +
           str(input1[i].name()));
  }
  ...
  for (size_t i = 0; i < n_images; i++) {
    ...
    if (nvols1 != nvols2)
      throw Exception("input images do not have the same number of volumes: " +
                      str(input2[i].name()) + " and " + str(input1[i].name()));
      ...
      if (do_reorientation && nvols1 > 1 && SH::NforL(SH::LforN(nvols1)) == nvols1) {
        ...
        if (!directions_cartesian.cols())
          directions_cartesian =
              Math::Sphere::spherical2cartesian(DWI::Directions::electrostatic_repulsion_60())
                  .transpose();
      } else {
  ...
  ssize_t max_mc_image_lmax =
      std::max_element(mc_params.begin(),
                       mc_params.end(),
                       [](const Registration::MultiContrastSetting &x,
                          const Registration::MultiContrastSetting &y) { return x.lmax < y.lmax; })
          ->lmax;

  do_reorientation =
      std::any_of(mc_params.begin(),
                  mc_params.end(),
  ...
  if (opt.size()) {
    if (!do_rigid)
      throw Exception(
          "rigid transformation output requested when no rigid registration is requested");
    output_rigid = true;
  ...
  if (opt.size()) {
    if (!do_rigid)
      throw Exception(
          "midway rigid transformation output requested when no rigid registration is requested");
  ...
  if (opt.size()) {
    if (init_rigid_matrix_set)
      throw Exception(
          "options -rigid_init_matrix and -rigid_init_translation are mutually exclusive");
  ...
  if (opt.size()) {
    if (!do_rigid)
      throw Exception("the rigid multi-resolution scale factors were input when no rigid "
  ...
  if (opt.size()) {
    if (!do_rigid)
      throw Exception(
          "the rigid sparsity factor was input when no rigid registration is requested");
  ...
  if (opt.size()) {
    if (!do_rigid)
      throw Exception(
          "the number of rigid iterations have been input when no rigid registration is requested");
  ...
  if (opt.size()) {
    if (!do_rigid)
      throw Exception(
          "the -rigid_lmax option has been set when no rigid registration is requested");
    ...
    for (size_t i = 0; i < rigid_lmax.size(); ++i)
      if (rigid_lmax[i] > max_mc_image_lmax) {
        WARN("the requested -rigid_lmax exceeds the lmax of the input images, setting it to " +
  ...
  if (opt.size()) {
    if (!do_affine)
      throw Exception(
          "affine transformation output requested when no affine registration is requested");
    output_affine = true;
  ...
  if (opt.size()) {
    if (!do_affine)
      throw Exception(
          "midway affine transformation output requested when no affine registration is requested");
  ...
  if (opt.size()) {
    if (init_rigid_matrix_set)
      throw Exception(
          "you cannot initialise registrations with both rigid and affine transformations");
    if (do_rigid)
      throw Exception("you cannot initialise with -affine_init_matrix since a rigid registration "
                      "is being performed");
  ...
  if (opt.size()) {
    if (init_affine_matrix_set)
      throw Exception(
          "options -affine_init_matrix and -affine_init_translation are mutually exclusive");
  ...
  if (opt.size()) {
    if (init_affine_matrix_set)
      throw Exception(
          "options -affine_init_matrix and -affine_init_rotation are mutually exclusive");
  ...
  if (opt.size()) {
    if (!do_affine)
      throw Exception("the affine multi-resolution scale factors were input when no affine "
                      "registration is requested");
  ...
  if (opt.size()) {
    if (!do_affine)
      throw Exception(
          "the affine sparsity factor was input when no affine registration is requested");
  ...
  if (opt.size()) {
    if (!do_affine)
      throw Exception("the number of affine iterations have been input when no affine registration "
                      "is requested");
  ...
  if (opt.size()) {
    if (!do_affine)
      throw Exception(
          "the -affine_lmax option has been set when no affine registration is requested");
    if (max_mc_image_lmax == 0)
  ...
  if (opt.size()) {
    if (!do_affine)
      throw Exception(
          "the -affine_log option has been set when no rigid registration is requested");
    linear_logstream.open(opt[0][0]);
  ...
  if (opt.size()) {
    if (!do_nonlinear)
      throw Exception(
          "Non-linear warp output requested when no non-linear registration is requested");
  ...
  if (opt.size()) {
    ...
    if (!Path::is_mrtrix_image(warp_full_filename) &&
      ...
      throw Exception("nl_warp_full output requires .mif/.mih or NIfTI file format with "
                      "NIfTIAutoSaveJSON config option set.");
  }
  ...
  if (opt.size()) {
    ...
    if (!do_nonlinear)
      throw Exception("the non linear initialisation option -nl_init cannot be used when no non "
                      "linear registration is requested");
    ...
    if (!Path::is_mrtrix_image(opt[0][0]) && !(Path::has_suffix(opt[0][0], {".nii", ".nii.gz"}) &&
      ...
      WARN("nl_init input requires warp_full in original .mif/.mih file format or in NIfTI file "
           "format with associated JSON. "
           "Converting to other file formats may remove linear transformations stored in the image "
           "header.");

    Image<default_type> input_warps = Image<default_type>::open(opt[0][0]);
    if (input_warps.ndim() != 5)
      throw Exception("non-linear initialisation input is not 5D. Input must be from previous "
                      "non-linear output");

    nl_registration.initialise(input_warps);

    if (do_affine) {
      WARN("no affine registration will be performed when initialising with non-linear non-linear "
           "warps");
      do_affine = false;
    }
    if (do_rigid) {
      WARN("no rigid registration will be performed when initialising with non-linear non-linear "
           "warps");
      do_rigid = false;
    }
    if (init_affine_matrix_set)
      WARN("-affine_init has no effect since the non-linear init warp also contains the linear "
           "transform in the image header");
    if (init_rigid_matrix_set)
      WARN("-rigid_init has no effect since the non-linear init warp also contains the linear "
           "transform in the image header");
  }
  ...
  if (opt.size()) {
    if (!do_nonlinear)
      throw Exception("the non-linear multi-resolution scale factors were input when no non-linear "
                      "registration is requested");
    vector<default_type> scale_factors = parse_floats(opt[0][0]);
    if (nonlinear_init) {
      WARN("-nl_scale option ignored since only the full resolution will be performed when "
           "initialising with non-linear warp");
    } else {
  ...
  if (opt.size()) {
    if (!do_nonlinear)
      throw Exception("the number of non-linear iterations have been input when no non-linear "
                      "registration is requested");
    vector<uint32_t> iterations_per_level = parse_ints<uint32_t>(opt[0][0]);
    if (nonlinear_init && iterations_per_level.size() > 1)
      throw Exception("when initialising the non-linear registration the max number of iterations "
                      "can only be defined for a single level");
    else
  ...
  if (opt.size()) {
    if (!do_nonlinear)
      throw Exception("the warp update field smoothing parameter was input when no non-linear "
                      "registration is requested");
  ...
  if (opt.size()) {
    if (!do_nonlinear)
      throw Exception("the displacement field smoothing parameter was input when no non-linear "
                      "registration is requested");
  ...
  if (opt.size()) {
    if (!do_nonlinear)
      throw Exception(
          "the initial gradient step size was input when no non-linear registration is requested");
  ...
  if (opt.size()) {
    if (!do_nonlinear)
      throw Exception(
          "the -nl_lmax option has been set when no non-linear registration is requested");
    if (max_mc_image_lmax == 0)
  ...
  if (opt.size()) {
    ...
    if (do_nonlinear) {
      ...
      if (MR::abs(sm - n_images) > 1.e-6)
        WARN("mc_weights do not sum to the number of contrasts. This changes the regularisation of "
             "the nonlinear registration.");
    }
    ...
    if (images2.ndim() == 4) {
      ...
      if (rigid_metric == Registration::Diff) {
        if (rigid_estimator == Registration::None) {
          ...
          Registration::Metric::
              DifferenceRobust4D<Image<value_type>, Image<value_type>, Registration::Metric::L1>
                  metric(images1, images2, estimator);
          rigid_registration.run_masked(metric, rigid, images1, images2, im1_mask, im2_mask);
        } else if (rigid_estimator == Registration::L2) {
          Registration::Metric::L2 estimator;
          Registration::Metric::
              DifferenceRobust4D<Image<value_type>, Image<value_type>, Registration::Metric::L2>
                  metric(images1, images2, estimator);
          rigid_registration.run_masked(metric, rigid, images1, images2, im1_mask, im2_mask);
        } else if (rigid_estimator == Registration::LP) {
          Registration::Metric::LP estimator;
          Registration::Metric::
              DifferenceRobust4D<Image<value_type>, Image<value_type>, Registration::Metric::LP>
                  metric(images1, images2, estimator);
          rigid_registration.run_masked(metric, rigid, images1, images2, im1_mask, im2_mask);
        } else
  ...
  if (do_affine) {
    ...
    if (output_affine_2tomid)
      save_transform(
          affine.get_transform_half_inverse(), affine.get_centre(), affine_2tomid_filename);
  ...
  if (do_nonlinear) {
    ...
    if (warp1_filename.size()) {
      ...
      Registration::Warp::compute_full_deformation(
          nl_registration.get_im2_to_mid_linear().inverse(),
    ...
    if (warp2_filename.size()) {
      ...
      Registration::Warp::compute_full_deformation(
          nl_registration.get_im1_to_mid_linear().inverse(),
    ...
    for (size_t idx = 0; idx < im1_transformed_paths.size(); idx++) {
      CONSOLE("... " + im1_transformed_paths[idx]);
      {
        ...
        Image<value_type> im1_transformed =
            Image<value_type>::create(im1_transformed_paths[idx], transformed_header);

        const size_t nvols = im1_image.ndim() == 3 ? 1 : im1_image.size(3);
        const bool reorient_output =
            !reorientation_forbidden && (nvols > 1) && SH::NforL(SH::LforN(nvols)) == nvols;

        if (do_nonlinear) {
          Filter::warp<Interp::Cubic>(
              im1_image, im1_transformed, deform_field, out_of_bounds_value);
          if (reorient_output)
  ...
  if (input1_midway_transformed_paths.size() and input2_midway_transformed_paths.size()) {
    ...
    if (do_nonlinear) {
      ...
      Registration::Warp::compose_linear_deformation(nl_registration.get_im1_to_mid_linear(),
                                                     *(nl_registration.get_im1_to_mid()),
                                                     im1_deform_field);
    }
    ...
    for (size_t idx = 0; idx < input1_midway_transformed_paths.size(); idx++) {
      CONSOLE("... " + input1_midway_transformed_paths[idx]);
      {
        ...
        if (do_nonlinear) {
          auto im1_midway =
              Image<default_type>::create(input1_midway_transformed_paths[idx], midway_header);
    ...
    if (do_nonlinear) {
      ...
      Registration::Warp::compose_linear_deformation(nl_registration.get_im2_to_mid_linear(),
                                                     *(nl_registration.get_im2_to_mid()),
                                                     im2_deform_field);
    }
    ...
    for (size_t idx = 0; idx < input2_midway_transformed_paths.size(); idx++) {
      CONSOLE("... " + input2_midway_transformed_paths[idx]);
      {
        ...
        if (do_nonlinear) {
          auto im2_midway =
              Image<default_type>::create(input2_midway_transformed_paths[idx], midway_header);
