...
const OptionGroup ConnectOption =
    ...
    + Option("axes",
             ...
             "the first 3 axes are included. The axes should be provided as a comma-separated list "
             "of values.") +
    ...
    + Option("minsize",
             "impose minimum size of segmented components (Default: select all components)") +
    Argument("value").type_integer(1, 1e6);
...
void usage() {
  AUTHOR =
      ...
      "Thijs Dhollander (thijs.dhollander@gmail.com) and J-Donald Tournier (jdtournier@gmail.com)";
  ...
  +Argument("input", "the input mask.").type_image_in() +
      Argument("filter",
               "the name of the filter to be applied; options are: " + join(filters, ", "))
          .type_choice(filters) +
...
void run() {
  ...
  if (filter_index == 0) { // Mask clean
    Filter::MaskClean filter(input_image,
                             std::string("applying mask cleaning filter to image ") +
  ...
  if (filter_index == 1) { // Connected components
    Filter::ConnectedComponents filter(
        input_image,
        std::string("applying connected-component filter to image ") + Path::basename(argument[0]));
  ...
  if (filter_index == 2) { // Dilate
    Filter::Dilate filter(
        input_image, std::string("applying dilate filter to image ") + Path::basename(argument[0]));
  ...
  if (filter_index == 3) { // Erode
    Filter::Erode filter(
        input_image, std::string("applying erode filter to image ") + Path::basename(argument[0]));
    auto opt = get_options("npass");
    if (opt.size())
      filter.set_npass(int(opt[0][0]));
  ...
  if (filter_index == 4) { // Fill
    Filter::Fill filter(input_image,
                        std::string("filling interior of image ") + Path::basename(argument[0]));
    auto opt = get_options("axes");
    if (opt.size()) {
      const vector<int> axes = opt[0][0];
      filter.set_axes(axes);
  ...
  if (filter_index == 5) { // Median
    Filter::Median filter(
        input_image, std::string("applying median filter to image ") + Path::basename(argument[0]));
