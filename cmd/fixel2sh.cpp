...
void run() {
  ...
  auto in_directions_image = Fixel::find_directions_header(Fixel::get_fixel_directory(argument[0]))
                                 .get_image<float>()
                                 .with_direct_io();
  ...
  for (auto l1 = Loop("converting fixel image to spherical harmonic image",
                      in_index_image)(in_index_image, sh_image);
       l1;
       ++l1) {
