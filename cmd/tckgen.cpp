...
void usage() {
  ...
  +"References based on streamlines algorithm used:"
      ...
      + "* FACT:\n"
        ...
        "Three-dimensional tracking of axonal projections in the brain by magnetic resonance "
        "imaging. "
      ...
      + "* iFOD2:\n"
        "Tournier, J.-D.; Calamante, F. & Connelly, A. " // Internal
        "Improved probabilistic streamlines tractography by 2nd order integration over fibre "
      ...
      + "* Tensor_Prob:\n"
        "Jones, D. "
        "Tractography Gone Wild: Probabilistic Fibre Tracking Using the Wild Bootstrap With "
      ...
      + "* -act, -backtrack, -seed_gmwmi:\n"
        ...
        "through effective use of anatomical information. "
...
void run() {
  ...
  if (properties.seeds.is_finite()) {
    ...
    if (properties["max_num_tracks"].size())
      WARN("Overriding -select option (desired number of successful streamline selections), as "
           "seeds can only provide a finite number");
    properties["max_num_tracks"] = str(properties.seeds.get_total_count());

    if (properties["max_num_seeds"].size())
      WARN("Overriding -seeds option (maximum number of seeds that will be attempted to track "
           "from), as seeds can only provide a finite number");
