...
void usage() {
  ...
  +"DWI data denoising and noise map estimation by exploiting data redundancy in the PCA domain "
   ...
   "noise level estimate then determines the optimal cut-off for PCA denoising."
      ...
      + "Important note: image denoising must be performed as the first step of the image "
        "processing pipeline. "
        "The routine will fail if interpolation or smoothing has been applied to the data prior to "
        "denoising."
  ...
  +Option("mask", "Only process voxels within the specified binary brain mask image.") +
      ...
      + Option("datatype",
               ...
               "For complex input data, this will select complex float32 or complex float64 "
               "datatypes.") +
  ...
  COPYRIGHT = "Copyright (c) 2016 New York University, University of Antwerp, and the MRtrix3 "
              "contributors \n \n"
              "Permission is hereby granted, free of charge, to any non-commercial entity "
              "('Recipient') obtaining a copy of this software and "
              "associated documentation files (the 'Software'), to the Software solely for "
              "non-commercial research, including the rights to "
              "use, copy and modify the Software, subject to the following conditions: \n \n"
              "\t 1. The above copyright notice and this permission notice shall be included by "
              "Recipient in all copies or substantial portions of "
              "the Software. \n \n"
              ...
              "consequential damages in connection with the Software. "
              "Recipient will defend, indemnify and hold NYU harmless from any claims or liability "
              "resulting from the use of the Software by recipient. \n \n"
              "\t 4. Neither anything contained herein nor the delivery of the Software to "
              "recipient shall be deemed to grant the Recipient any right or "
              "licenses under any patents or patent application owned by NYU. \n \n"
              "\t 5. The Software may only be used for non-commercial research and may not be used "
              "for clinical care. \n \n"
              "\t 6. Any publication by Recipient of research involving the Software shall cite "
...
public:
  ...
  template <typename ImageType> void operator()(ImageType &dwi, ImageType &out) {
    ...
    if (cutoff_p > 0) {
      ...
      if (m <= n)
        X.col(n / 2) = eig.eigenvectors() *
                       (s.cast<F>().asDiagonal() * (eig.eigenvectors().adjoint() * X.col(n / 2)));
      else
        X.col(n / 2) = X * (eig.eigenvectors() *
                            (s.cast<F>().asDiagonal() * eig.eigenvectors().adjoint().col(n / 2)));
    }
...
void run() {
  ...
  if (std::min<uint32_t>(dwi.size(3), extent[0] * extent[1] * extent[2]) < 15) {
    WARN(
        "The number of volumes or the patch size is small. This may lead to discretisation effects "
