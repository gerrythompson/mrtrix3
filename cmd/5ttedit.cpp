...
void usage() {
  ...
  +Option("cgm", "provide a mask of voxels that should be set to cortical grey matter") +
      ...
      + Option("none",
               "provide a mask of voxels that should be cleared (i.e. are non-brain); note that "
               "this will supersede all other provided masks") +
      Argument("image").type_image_in();
}
...
public:
  Modifier(Image<float> &input_image, Image<float> &output_image)
      : v_in(input_image), v_out(output_image) {}
