...
void reslice(size_t interp,
  ...
  case 0:
    Filter::reslice<Interp::Nearest>(
        input, output, trafo, Adapter::AutoOverSample, out_of_bounds_value);
    DEBUG("Nearest");
    break;
  case 1:
...
void usage() {
  AUTHOR =
      "David Raffelt (david.raffelt@florey.edu.au) and Max Pietsch (maximilian.pietsch@kcl.ac.uk)";
  ...
  +Option("space",
      ...
      + Option("interp",
               ...
               "cubic, sinc. Default: linear).") +
      Argument("method").type_choice(interp_choices)

      + Option("metric",
               "define the dissimilarity metric used to calculate the cost. "
               "Choices: diff (squared differences), cc (non-normalised negative cross correlation "
               "aka negative cross covariance). Default: diff). "
...
void run() {
  ...
  auto input1 =
      Image<value_type>::open(argument[0]).with_direct_io(Stride::contiguous_along_axis(3));
  auto input2 =
      Image<value_type>::open(argument[1]).with_direct_io(Stride::contiguous_along_axis(3));
  ...
  Eigen::Matrix<value_type, Eigen::Dynamic, 1> sos =
      Eigen::Matrix<value_type, Eigen::Dynamic, 1>::Zero(volumes, 1);
  if (space == 0) {
    INFO("per-voxel");
    ...
    evaluate_voxelwise_msq(
        input1, input2, mask1, mask2, dimensions, use_mask1, use_mask2, n_voxels, sos);
  } else {
    DEBUG("scanner space");
    ...
    if (space == 1) {
      ...
      output2mask = Header::scratch(input1, "-").get_image<bool>();
      {
        ...
        if (use_mask2)
          Filter::reslice<Interp::Nearest>(
              mask2, output2mask, Adapter::NoTransform, Adapter::AutoOverSample, 0);
      }
      evaluate_voxelwise_msq(output1,
                             output2,
                             output1mask,
                             output2mask,
                             dimensions,
    ...
    if (space == 2) {
      ...
      output2mask = mask2;
      {
        ...
        if (use_mask1)
          Filter::reslice<Interp::Nearest>(
              mask1, output1mask, Adapter::NoTransform, Adapter::AutoOverSample, 0);
      }
    ...
    if (space == 3) {
      ...
      using LinearInterpolatorType2 =
          Interp::LinearInterp<Image<value_type>, Interp::LinearInterpProcessingType::Value>;
      using CubicInterpolatorType1 =
          Interp::SplineInterp<ImageType1,
                               Math::UniformBSpline<typename ImageType1::value_type>,
                               Math::SplineProcessingType::Value>;
      using CubicInterpolatorType2 =
          Interp::SplineInterp<ImageType2,
      ...
      if (interp == 1 or interp == 2) {
        if (metric_type == MetricType::MeanSquared) {
          if (dimensions == 3) {
            ...
            Registration::Metric::MeanSquaredVectorNoGradient4D<ImageType1, ImageType2> metric(
                input1, input2);
            if (interp == 1) {
              LinearParamType parameters(transform, input1, input2, midway_image, mask1, mask2);
              Registration::Metric::ThreadKernel<decltype(metric), LinearParamType> kernel(
