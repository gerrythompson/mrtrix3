...
void usage() {
  ...
  +"For the TFNBS algorithm, default parameters for statistical enhancement "
   ...
   "Vinokur, L.; Zalesky, A.; Raffelt, D.; Smith, R.E. & Connelly, A. A Novel Threshold-Free "
   "Network-Based Statistics Method: Demonstration using Simulated Pathology. OHBM, 2015, 4144; \n"
   "and: \n"
   "Vinokur, L.; Zalesky, A.; Raffelt, D.; Smith, R.E. & Connelly, A. A novel threshold-free "
   "network-based statistical method: Demonstration and parameter optimisation using in vivo "
   "simulated pathology. In Proc ISMRM, 2015, 2846. \n"
  ...
  REFERENCES +
      ...
      "Zalesky, A.; Fornito, A. & Bullmore, E. T. Network-based statistic: Identifying differences "
      "in brain networks. \n"
      "NeuroImage, 2010, 53, 1197-1207"

      + "* If using the TFNBS algorithm: \n"
        "Baggio, H.C.; Abos, A.; Segura, B.; Campabadal, A.; Garcia-Diaz, A.; Uribe, C.; Compta, "
        "Y.; Marti, M.J.; Valldeoriola, F.; Junque, C. Statistical inference in brain graphs using "
        "threshold-free network-based statistics."
        "HBM, 2018, 39, 2289-2302"

      + "* If using the -nonstationary option: \n"
        "Salimi-Khorshidi, G.; Smith, S.M. & Nichols, T.E. Adjusting the effect of nonstationarity "
        "in cluster-based and TFCE inference. \n"
...
void run() {
  ...
  for (size_t i = 1; i < importer.size(); ++i) {
    if (importer[i]->size() != importer[0]->size())
      throw Exception("Size of connectome for subject " + str(i) + " (file \"" +
                      importer[i]->name() + "\" does not match that of first subject");
  }
  ...
  const default_type empirical_skew =
      get_option_value("skew_nonstationarity", EMPIRICAL_SKEW_DEFAULT);
  ...
  if (extra_columns.size()) {
    ...
    if (nans_in_columns)
      CONSOLE("Non-finite values detected in element-wise design matrix columns; individual rows "
              "will be removed from edge-wise design matrices accordingly");
  }
  ...
  if (hypotheses[0].cols() != num_factors)
    throw Exception(
        ...
        (extra_columns.size()
             ? " (taking into account the " + str(extra_columns.size()) + " uses of -column)"
             : ""));
...

  {
    ...
    for (size_t i = 0; i != num_hypotheses; ++i) {
      if (!hypotheses[i].is_F()) {
        save_matrix(mat2vec.V2M(abs_effect_size.col(i)),
                    output_prefix + "abs_effect" + postfix(i) + ".csv");
        ++progress;
        if (num_vgs == 1)
          save_matrix(mat2vec.V2M(std_effect_size.col(i)),
  ...
  if (extra_columns.size() || nans_in_data) {
    if (variance_groups.size())
      ...
      glm_test.reset(new GLM::TestVariableHomoscedastic(
  ...
  if (do_nonstationarity_adjustment) {
    ...
    Stats::PermTest::precompute_empirical_stat(
        glm_test, enhancer, empirical_skew, empirical_statistic);
    for (size_t i = 0; i != num_hypotheses; ++i)
      save_matrix(mat2vec.V2M(empirical_statistic.col(i)),
  ...
  for (size_t i = 0; i != num_hypotheses; ++i) {
    save_matrix(mat2vec.V2M(default_statistic.col(i)),
    ...
    save_matrix(mat2vec.V2M(default_enhanced.col(i)),
                output_prefix + "enhanced" + postfix(i) + ".csv");
  }
  ...
  if (!get_options("notest").size()) {
    ...
    for (size_t i = 0; i != num_hypotheses; ++i) {
      save_matrix(mat2vec.V2M(pvalue_output.col(i)),
                  output_prefix + "fwe_1mpvalue" + postfix(i) + ".csv");
      save_matrix(mat2vec.V2M(uncorrected_pvalues.col(i)),
                  output_prefix + "uncorrected_1mpvalue" + postfix(i) + ".csv");
      save_matrix(mat2vec.V2M(null_contributions.col(i)),
