...
void usage() {
  ...
  COPYRIGHT =
      "Copyright (C) 2015 KU Leuven, Dept. Electrical Engineering, ESAT/PSI,\n"
  ...
  +Example("Basic usage",
           "tckglobal dwi.mif wmr.txt -riso csfr.txt -riso gmr.txt -mask mask.mif -niter 1e9 -fod "
           "fod.mif -fiso fiso.mif tracks.tck",
           "dwi.mif is the input image, wmr.txt is an anisotropic, multi-shell response function "
           "for WM, "
           "and csfr.txt and gmr.txt are isotropic response functions for CSF and GM. The output "
           "tractogram is "
           "saved to tracks.tck. Optional output images fod.mif and fiso.mif contain the predicted "
           "WM fODF and "
           "isotropic tissue fractions of CSF and GM respectively, estimated as part of the global "
           "optimization "
  ...
  +OptionGroup("Input options")
      ...
      + Option("riso", "set one or more isotropic response functions. (multiple allowed)")
            .allow_multiple() +
      ...
      + Option("lmax",
               "set the maximum harmonic order for the output series. (default = " +
                   str(DEFAULT_LMAX) + ")") +
      Argument("order").type_integer(2, 30)

      + Option("length",
               "set the length of the particles (fibre segments). (default = " +
                   str(DEFAULT_LENGTH, 2) + "mm)") +
      Argument("size").type_float(1e-6)

      + Option("weight",
               "set the weight by which particles contribute to the model. (default = " +
                   str(DEFAULT_WEIGHT, 2) + ")") +
      Argument("w").type_float(1e-6, 1.0)

      + Option("ppot",
               "set the particle potential, i.e., the cost of adding one segment, relative to the "
               "particle weight. (default = " +
                   str(DEFAULT_PPOT, 2) + ")") +
      Argument("u").type_float(0.0, 1.0)

      + Option("cpot",
               "set the connection potential, i.e., the energy term that drives two segments "
               "together. (default = " +
                   str(DEFAULT_CPOT, 2) + ")") +
      Argument("v").type_float(0.0)

      + Option("t0",
               "set the initial temperature of the metropolis hastings optimizer. (default = " +
                   str(DEFAULT_T0, 2) + ")") +
      Argument("start").type_float(1e-6, 1e6)

      + Option("t1",
               "set the final temperature of the metropolis hastings optimizer. (default = " +
                   str(DEFAULT_T1, 2) + ")") +
      ...
      + Option("balance",
               "balance internal and external energy. (default = " + str(DEFAULT_BALANCE, 2) +
                   ")\n"
                   "Negative values give more weight to the internal energy, positive to the "
                   "external energy.") +
      Argument("b").type_float(-100.0, 100.0)

      + Option("density",
               "set the desired density of the free Poisson process. (default = " +
                   str(DEFAULT_DENSITY, 2) + ")") +
      ...
      + Option("beta",
               "set the width of the Hanning interpolation window. (in [0, 1], default = " +
                   str(DEFAULT_BETA, 2) +
                   ")\n"
                   "If used, a mask is required, and this mask must keep at least one voxel "
                   "distance to the image bounding box.") +
...
void run() {
  ...
  EnergySumComputer *Esum = new EnergySumComputer(
      ...
      properties.lam_int,
      Eext,
      properties.lam_ext / (wmscale2 * properties.weight * properties.weight));
  ...
  MHSampler mhs(dwi,
                ...
                pgrid,
                Esum,
                mask); // All EnergyComputers are recursively destroyed upon destruction of mhs,
                       // except for the shared data.
  ...
  ftfileprops.comments.push_back("segment weight = " +
                                 std::to_string((long double)properties.weight));
  ftfileprops.comments.push_back("");
  ...
  if (opt.size()) {
    if (properties.resp_ISO.size() > 0) {
      ...
      WARN("Ignore saving file " + opt[0][0] +
           ", because no isotropic response functions were provided.");
    }
  }
