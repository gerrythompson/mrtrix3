...
void usage() {
  ...
  +Option("plane", "fill one or more planes on a particular image axis").allow_multiple() +
      Argument("axis").type_integer(0, 2) + Argument("coord").type_sequence_int() +
      Argument("value").type_float()

      + Option("sphere", "draw a sphere with radius in mm").allow_multiple() +
      Argument("position").type_sequence_float() + Argument("radius").type_float() +
      Argument("value").type_float()
      ...
      + Option("scanner",
               "indicate that coordinates are specified in scanner space, rather than as voxel "
               "coordinates");
}
...
void run() {
  ...
  if (inplace) {
    ...
    if (std::string(argument[1]) ==
      ...
      throw Exception("Do not provide same image as input and output; instad specify image to be "
                      "edited in-place");
  ...
  for (auto p : opt) {
    ...
    const std::array<size_t, 2> loop_axes{
        {axis == 0 ? size_t(1) : size_t(0), axis == 2 ? size_t(1) : size_t(2)}};
    for (auto c : coords) {
      out.index(axis) = c;
  ...
  for (auto v : opt) {
    ...
    if (position.size() != H.ndim())
      throw Exception("Image has " + str(H.ndim()) + " dimensions, but -voxel option position " +
                      std::string(v[0]) + " provides only " + str(position.size()) +
                      " coordinates");
    if (scanner) {
      ...
      for (size_t axis = 3; axis != out.ndim(); ++axis) {
        if (std::round(position[axis]) != position[axis])
          throw Exception(
              "Non-spatial coordinates provided using -voxel option must be provided as integers");
      ...
      for (size_t axis = 0; axis != out.ndim(); ++axis) {
        if (std::round(position[axis]) != position[axis])
          throw Exception(
              "Voxel coordinates provided using -voxel option must be provided as integers");
        out.index(axis) = position[axis];
      }
    }
    out.value() = value;
