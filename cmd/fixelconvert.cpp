...
void usage() {
  ...
  +Example("Convert from the old file format to the new directory format",
      ...
      + Example("Convert from the new directory format to the old file format",
                "fixelconvert new_fixels/ old_fixels.msf -value parameter.mif -in_size "
                "new_fixels/afd.mif",
  ...
  +OptionGroup("Options for converting from old to new format") +
      Option("name",
             "assign a different name to the value field output (Default: value). Do not include "
             "the file extension.") +
      ...
      Option("template",
             "specify an existing fixel directory (in the new format) to which the new output "
             "should conform") +
...
void convert_old2new() {
  ...
  auto index_image = Image<index_type>::create(
      Path::join(output_fixel_directory, "index" + file_extension), header);
  auto directions_image =
      Image<float>::create(Path::join(output_fixel_directory, "directions" + file_extension),
                           directions_header)
          .with_direct_io();
  auto value_image = Image<float>::create(
      Path::join(output_fixel_directory, value_name + file_extension), data_header);
  Image<float> size_image;
  if (output_size)
    size_image = Image<float>::create(Path::join(output_fixel_directory, "size" + file_extension),
                                      data_header);
...
void run() {
  ...
  if (is_old_format(argument[0])) {
    if (is_old_format(argument[1]))
      throw Exception("fixelconvert can only be used to convert between old and new fixel formats; "
                      "NOT to convert images within the old format");
    convert_old2new();
  } else {
    if (!is_old_format(argument[1]))
      throw Exception("fixelconvert can only be used to convert between old and new fixel formats; "
                      "NOT to convert within the new format");
    convert_new2old();
  }
}
