...
void run() {
  ...
  auto directions_image =
      Image<float>::create(Path::join(argument[1], "directions.mif"), directions_header);
  ...
  for (auto l = Loop("converting peaks to fixel format", 0, 3)(input_directions, index_image); l;
       ++l) {
    auto dirs = get(input_directions);
