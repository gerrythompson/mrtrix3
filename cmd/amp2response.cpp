...
void usage() {
  ...
  +Argument("amps", "the amplitudes image").type_image_in() +
      Argument("mask",
          ...
          .type_image_in() +
      Argument("directions", "a 4D image containing the estimated fibre directions")
          .type_image_in() +
  ...
  +Option("isotropic", "estimate an isotropic response function (lmax=0 for all shells)")
      ...
      + Option("directions",
               "provide an external text file containing the directions along which the amplitudes "
               "are sampled") +
  ...
  +"Smith, R. E.; Dhollander, T. & Connelly, A. " // Internal
   "Constrained linear least squares estimation of anisotropic response function for spherical "
   "deconvolution. "
...
Eigen::Matrix<default_type, 3, 3> gen_rotation_matrix(const Eigen::Vector3d &dir) {
  ...
  // Here the other two axes are determined at random (but both are orthogonal to the FOD peak
  // direction)
...
void run() {
  ...
  if (get_options("isotropic").size()) {
    ...
    if ((*shells)[0].is_bzero() && lmax.front()) {
      WARN("Non-zero lmax requested for " +
           ((*shells)[0].get_mean() ? "first shell (mean b=" + str((*shells)[0].get_mean()) +
  ...
  if (dir_image.ndim() < 4 || dir_image.size(3) < 3)
    throw Exception("input direction image \"" + std::string(argument[2]) +
  ...
  CONSOLE(std::string("estimating response function using ") +
          (use_ols ? "ordinary" : "constrained") + " least-squares from " + str(num_voxels) +
          " voxels");
  ...
  for (size_t shell_index = 0; shell_index != dirs_azel.size(); ++shell_index) {
    ...
    {
      auto transform = Math::ZSH::init_amp_transform<default_type>(dirs_azel[shell_index].col(1),
                                                                   lmax[shell_index]);
      if (!transform.allFinite()) {
        ...
        if (!shell_index && (*shells)[0].is_bzero())
          e.push_back("  (this appears to be a b=0 shell, and therefore lmax should be set to 0 "
                      "for this shell)");
        throw e;
      }
    }
    ...
    if (!lmax[shell_index] || use_ols) {
      ...
      // We are going to both constrain the amplitudes to be non-negative, and constrain the
      // derivatives to be non-negative
      ...
      Eigen::MatrixXd constraints(amp_transform.rows() + deriv_transform.rows(),
                                  amp_transform.cols());
      constraints.block(0, 0, amp_transform.rows(), amp_transform.cols()) = amp_transform;
      constraints.block(amp_transform.rows(), 0, deriv_transform.rows(), deriv_transform.cols()) =
          deriv_transform;

      // Initialise the problem solver
      auto problem = Math::ICLS::Problem<default_type>(
          shared.M, constraints, Eigen::VectorXd(), 0, 1e-10, 1e-10, 0, 0.0, true);
    ...
    CONSOLE("  b=" + str((*shells)[shell_index].get_mean(), 4) + ": [" +
            str(rf.transpose().cast<float>()) + "]");
