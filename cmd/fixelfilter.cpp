...
void usage() {
  ...
  DESCRIPTION
  +"If the first input to the command is a specific fixel data file, then a filtered version of "
   "only that file "
   "will be generated by the command. Alternatively, if the input is the location of a fixel "
   "directory, then the "
   "command will create a duplicate of the fixel directory, and apply the specified filter "
   "operation to all "
  ...
  ARGUMENTS
  +Argument("input", "the input: either a fixel data file, or a fixel directory (see Description)")
          .type_various() +
  ...
  OPTIONS
  +Option("matrix",
          "provide a fixel-fixel connectivity matrix for filtering operations that require it")
          .required() +
...
void run() {
  ...
  std::set<std::string> option_list{
      "threhsold_value", "threshold_connectivity", "fwhm", "minweight", "mask"};
...

  {
    ...
    case 0: {
      const float value =
          get_option_value("threshold_value", float(DEFAULT_FIXEL_CONNECT_VALUE_THRESHOLD));
      const float connect = get_option_value("threshold_connectivity",
                                             float(DEFAULT_FIXEL_CONNECT_CONNECTIVITY_THRESHOLD));
    ...
    case 1: {
      ...
      const float threshold =
          get_option_value("minweight", float(DEFAULT_FIXEL_SMOOTHING_MINWEIGHT));
  ...
  if (single_file.valid()) {
    ...
    CONSOLE(std::string("Applying \"") + filters[argument[1]] +
            "\" operation to fixel data file \"" + single_file.name() + "\"");
    ...
    for (auto &H : multiple_files) {
      ...
      auto output_image =
          Image<float>::create(Path::join(argument[2], Path::basename(H.name())), H);
