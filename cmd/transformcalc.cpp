...
void usage() {
  ...
  +Example("Invert a transformation", "transformcalc matrix_in.txt invert matrix_out.txt", "")
      ...
      + Example("Calculate the transformation matrix from an original image and an image with "
                "modified header",
      ...
      + Example("Decompose transformation matrix M into translation, rotation and stretch and "
                "shear (M = T * R * S)",
                ...
                "angles: list of Euler angles about static x, y, z axes in radians in the range "
      ...
      + Example("Calculate transformation that aligns two images based on sets of corresponding "
                "landmarks",
                "transformcalc input moving.txt fixed.txt align_vertices_rigid rigid.txt",
                "Similary, 'align_vertices_rigid_scale' produces an affine matrix (rigid and "
                "global scale). "
                "Vertex coordinates are in scanner space, corresponding vertices must be stored in "
                "the same row "
                "of moving.txt and fixed.txt. Requires 3 or more vertices in each file. "
                "Algorithm: Kabsch 'A solution for the best rotation to relate two sets of "
                "vectors' DOI:10.1107/S0567739476001873");
}

template <typename T> int sgn(T val) { return (T(0) < val) - (val < T(0)); }

transform_type align_corresponding_vertices(const Eigen::MatrixXd &src_vertices,
                                            const Eigen::MatrixXd &trg_vertices,
                                            bool scale) {
  //  this function aligns two sets of vertices which must have corresponding vertices stored in the
  //  same row
  //
  //  scale == false --> Kabsch
...
void run() {
  ...
  case 5: { // interpolate
    ...
    transform_out.translation() =
        ((1.0 - t) * transform1.translation() + t * transform2.translation());
    Qout = Q1.slerp(t, Q2);
  ...
  case 8: { // align_vertices_rigid and align_vertices_rigid_scale
    ...
    const transform_type T =
        align_corresponding_vertices(moving_vertices, target_vertices, op == 8);
