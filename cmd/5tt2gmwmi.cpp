...
void usage() {
  ...
  SYNOPSIS = "Generate a mask image appropriate for seeding streamlines on the grey matter-white "
             "matter interface";

  REFERENCES
  +"Smith, R. E.; Tournier, J.-D.; Calamante, F. & Connelly, A. " // Internal
   "Anatomically-constrained tractography:"
   "Improved diffusion MRI streamlines tractography through effective use of anatomical "
   "information. "
  ...
  OPTIONS
  +Option("mask_in",
          "Filter an input mask image according to those voxels that lie upon the grey matter - "
          "white matter boundary. "
          "If no input mask is provided, the output will be a whole-brain mask image calculated "
          "using the anatomical image only.") +
  ...
  bool operator()(Image<float> &input, Image<float> &output) {
    ...
    if (process_voxel) {
      ...
      for (size_t axis = 0; axis != 3; ++axis) {
        ...
        gradient += Math::pow2(multiplier * std::min(abs(pos.get_gm() - neg.get_gm()),
                                                     abs(pos.get_wm() - neg.get_wm())));
      }
