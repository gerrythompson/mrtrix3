...
void usage() {
  ...
  +"Calamante, F.; Smith, R.E.; Liang, X.; Zalesky, A.; Connelly, A " // Internal
   "Track-weighted dynamic functional connectivity (TW-dFC): a new method to study time-resolved "
...
void run() {
  ...
  if (opt.size()) {
    ...
    case 4: // hamming
      for (ssize_t i = 0; i != window_width; ++i)
        window[i] =
            0.53836 - (0.46164 * std::cos(2.0 * Math::pi * i / default_type(window_width - 1)));
      break;
  ...
  if (opt.size()) {
    ...
    if (voxel_size.empty())
      throw Exception("please specify either a template image using the -template option, or the "
                      "desired voxel size using the -vox option");
