...
void usage() {
  AUTHOR =
      "J-Donald Tournier (jdtournier@gmail.com) and Robert E. Smith (robert.smith@florey.edu.au)";
  ...
  +OptionGroup("Options for manipulating fundamental image properties")
      ...
      + Option("clear_property", "remove the specified key from the image header altogether.")
            .allow_multiple() +
      Argument("key").type_text()

      + Option("set_property", "set the value of the specified key in the image header.")
            .allow_multiple() +
      Argument("key").type_text() + Argument("value").type_text()

      + Option("append_property",
               "append the given value to the specified key in the image header (this adds the "
               "value specified as a new line in the header value).")
            .allow_multiple() +
      Argument("key").type_text() + Argument("value").type_text()

      + Option("copy_properties",
               "clear all generic properties and replace with the properties from the image / file "
               "specified.") +
...
void permute_DW_scheme(Header &H, const vector<int> &axes) {
  ...
  out.block(0, 3, out.rows(), out.cols() - 3) = in.block(
      0, 3, in.rows(), in.cols() - 3); // Copy b-values (and anything else stored in dw_scheme)
...
void permute_PE_scheme(Header &H, const vector<int> &axes) {
  ...
  out.block(0, 3, out.rows(), out.cols() - 3) =
      in.block(0,
               3,
               in.rows(),
               in.cols() - 3); // Copy total readout times (and anything else stored in pe_scheme)
  for (int row = 0; row != in.rows(); ++row)
...
void run() {
  ...
  try {
    ...
    if (get_options("grad").size() || get_options("fslgrad").size() ||
        get_options("bvalue_scaling").size())
      throw;
    e.display(2);
  }
  ...
  if (header_in.datatype().is_complex() && !header_out.datatype().is_complex())
    WARN("requested datatype is real but input datatype is complex - imaginary component will be "
         "ignored");
    ...
    if (str(opt[0][0]) != "NULL") {
      try {
        ...
        try {
          ...
          throw Exception("Unable to obtain header key-value entries from spec \"" +
                          str(opt[0][0]) + "\"");
        }
      }
    }
  ...
  if (opt.size()) {
    if (header_out.datatype().is_integer()) {
      ...
      if (scaling.size() != 2)
        throw Exception(
            "-scaling option expects comma-separated 2-vector of floating-point values");
