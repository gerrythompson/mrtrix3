...
const OptionGroup OutputDimOption =
    ...
    + Option(
          "tod",
          "generate a Track Orientation Distribution (TOD) in each voxel; need to specify the "
          "maximum "
...
const OptionGroup TWIOption =
    ...
    Option("image",
           ...
           "'scalar_map_count' contrast, or the spherical harmonics image for 'fod_amp' contrast") +
    Argument("image").type_image_in()
    ...
    + Option("map_zero",
             "if a streamline has zero contribution based on the contrast & statistic, typically "
             "it is not mapped; "
             "use this option to still contribute to the map even if this is the case "
             "(these non-contributing voxels can then influence the mean value in each voxel of "
             "the map)")

    + Option("backtrack",
             "when using -stat_tck ends_*, if the streamline endpoint is outside the FoV, "
             "backtrack along "
...
const OptionGroup MappingOption =
    ...
    + Option("precise",
             "use a more precise streamline mapping strategy, that accurately quantifies the "
             "length through each voxel "
...
void usage() {
  ...
  AUTHOR =
      "Robert E. Smith (robert.smith@florey.edu.au) and J-Donald Tournier (jdtournier@gmail.com)";
  ...
  +"* For TDI or DEC TDI:\n"
   ...
   "NeuroImage, 2010, 53, 1233-1243"
      ...
      + "* If using -contrast length and -stat_vox mean:\n"
        ...
        "The average pathlength map: A diffusion MRI tractography-derived index for studying brain "
        "pathology. "
        "NeuroImage, 2011, 55, 133-141"

      + "* If using -dixel option with TDI contrast only:\n"
        "Smith, R.E., Tournier, J-D., Calamante, F., Connelly, A. " // Internal
        "A novel paradigm for automated segmentation of very large whole-brain probabilistic "
      ...
      + "* If using -dixel option with any other contrast:\n"
        ...
        "Incorporating directional information in diffusion tractography derived maps: angular "
        "track imaging (ATI). "
        "In Proc. ISMRM, 2012, 20, 1912"

      + "* If using -tod option:\n"
        "Dhollander, T., Emsell, L., Van Hecke, W., Maes, F., Sunaert, S., Suetens, P. " // Internal
        "Track Orientation Density Imaging (TODI) and Track Orientation Distribution (TOD) based "
        "tractography. "
  ...
  OPTIONS
  +OutputHeaderOption + OutputDimOption + TWIOption + MappingOption +
      Tractography::TrackWeightsInOption;
}

MapWriterBase *
make_writer(Header &H, const std::string &name, const vox_stat_t stat_vox, const writer_dim dim) {
...
DataType determine_datatype(const DataType current_dt,
  ...
  if (current_dt == DataType::Undefined) {
    ...
    WARN("Cannot use non-floating-point datatype with " + str(Mapping::contrasts[contrast]) +
         " contrast" + (precise ? " and precise mapping" : "") + "; defaulting to " +
...
void run() {
  ...
  if (voxel_size.size() == 1) {
    ...
    throw Exception("voxel size must either be a single isotropic value, or a list of 3 "
                    "comma-separated voxel dimensions");
  ...
  if (opt.size()) {
    if (stat_tck != GAUSSIAN) {
      WARN("Overriding per-track statistic to Gaussian as a full-width half-maximum has been "
           "provided.");
      stat_tck = GAUSSIAN;
    }
    gaussian_fwhm_tck = opt[0][0];
  } else if (stat_tck == GAUSSIAN) {
    throw Exception("If using Gaussian per-streamline statistic, need to provide a full-width "
                    "half-maximum for the Gaussian kernel using the -fwhm option");
  }

  bool backtrack = false;
  if (get_options("backtrack").size()) {
    if (stat_tck == ENDS_CORR || stat_tck == ENDS_MAX || stat_tck == ENDS_MEAN ||
        stat_tck == ENDS_MIN || stat_tck == ENDS_PROD)
      backtrack = true;
    else
  ...
  case LENGTH:
    if (stat_tck != T_MEAN)
      WARN("Cannot use track statistic other than default for length-weighted TDI generation - "
           "ignoring");
    stat_tck = T_MEAN;
    break;
  ...
  case INVLENGTH:
    if (stat_tck != T_MEAN)
      WARN("Cannot use track statistic other than default for inverse-length-weighted TDI "
           "generation - ignoring");
    stat_tck = T_MEAN;
    break;
  ...
  case VECTOR_FILE:
    if (stat_tck != T_MEAN)
      WARN("Cannot use track statistic other than default when providing contrast from an external "
           "data file - ignoring");
    stat_tck = T_MEAN;
    break;
  ...
  if (opt.size()) {
    ...
    // If accurately calculating the length through each voxel traversed, need a higher upsampling
    // ratio
    //   (1/10th of the voxel size was found to give a good quantification of chordal length)
    // For all other applications, making the upsampled step size about 1/3rd of a voxel seems
    // sufficient
    try {
  ...
  std::unique_ptr<TrackMapperTWI> mapper((stat_tck == GAUSSIAN)
  ...
  if (contrast == SCALAR_MAP || contrast == SCALAR_MAP_COUNT || contrast == FOD_AMP) {
    ...
    if (!opt.size()) {
      if (contrast == SCALAR_MAP || contrast == SCALAR_MAP_COUNT)
        throw Exception("If using 'scalar_map' or 'scalar_map_count' contrast, must provide the "
                        "relevant scalar image using -image option");
      else
        throw Exception("If using 'fod_amp' contrast, must provide the relevant spherical harmonic "
                        "image using -image option");
    }
    ...
    if (!opt.size())
      throw Exception("If using 'vector_file' contrast, must provide the relevant data file using "
                      "the -vector_file option");
