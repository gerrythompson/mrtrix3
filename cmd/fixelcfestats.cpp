...
void usage() {
  ...
  SYNOPSIS = "Fixel-based analysis using connectivity-based fixel enhancement and non-parametric "
             "permutation testing";

  DESCRIPTION
  +"Unlike previous versions of this command, where a whole-brain tractogram file would be "
   "provided as input "
   "in order to generate the fixel-fixel connectivity matrix and smooth fixel data, this version "
   "expects to be "
   "provided with the directory path to a pre-calculated fixel-fixel connectivity matrix (likely "
   "generated using "
   "the MRtrix3 command fixelconnectivity), and for the input fixel data to have already been "
   "smoothed (likely "
   "using the MRtrix3 command fixelfilter)."

      + "Note that if the -mask option is used, the output fixel directory will still contain the "
        "same set of fixels as that "
        "present in the input fixel template, in order to retain fixel correspondence. However a "
        "consequence of this is that "
        "all fixels in the template will be initialy visible when the output fixel directory is "
        "loaded in mrview. Those fixels "
        "outside the processing mask will immediately disappear from view as soon as any "
        "data-file-based fixel colouring or "
  ...
  REFERENCES
  +"Raffelt, D.; Smith, RE.; Ridgway, GR.; Tournier, JD.; Vaughan, DN.; Rose, S.; Henderson, R.; "
   "Connelly, A. " // Internal
   "Connectivity-based fixel enhancement: Whole-brain statistical analysis of diffusion MRI "
   "measures in the presence of crossing fibres."
  ...
  +Argument("in_fixel_directory",
            "the fixel directory containing the data files for each subject (after obtaining fixel "
            "correspondence")
          .type_directory_in()

      + Argument("subjects",
                 "a text file listing the subject identifiers (one per line). This should "
                 "correspond with the filenames "
                 "in the fixel directory (including the file extension), and be listed in the same "
                 "order as the rows of the design matrix.")
            .type_image_in()
  ...
  +Option(
      "mask",
      "provide a fixel data file containing a mask of those fixels to be used during processing") +
      ...
      + Option("cfe_dh",
               "the height increment used in the cfe integration (default: " +
                   str(DEFAULT_CFE_DH, 2) + ")") +
...
public:
  SubjectFixelImport(const std::string &path)
    ...
    for (size_t axis = 1; axis < data.ndim(); ++axis) {
      if (data.size(axis) > 1)
        throw Exception("Image file \"" + path +
                        "\" does not contain fixel data (wrong dimensions)");
    }
  }
...
void run() {
  ...
  const default_type empirical_skew =
      get_option_value("skew_nonstationarity", DEFAULT_EMPIRICAL_SKEW);
  ...
  for (size_t i = 0; i != importer.size(); ++i) {
    if (!Fixel::fixels_match(index_header,
                             dynamic_cast<SubjectFixelImport *>(importer[i].get())->header()))
      throw Exception("Fixel data file \"" + importer[i]->name() +
                      "\" does not match template fixel image");
  }
  ...
  if (extra_columns.size()) {
    ...
    if (nans_in_columns)
      CONSOLE("Non-finite values detected in element-wise design matrix columns; individual rows "
              "will be removed from fixel-wise design matrices accordingly");
  }
  ...
  // Load hypotheses
  const vector<Math::Stats::GLM::Hypothesis> hypotheses =
      Math::Stats::GLM::load_hypotheses(argument[3]);
  ...
  if (hypotheses[0].cols() != num_factors)
    throw Exception(
        ...
        (extra_columns.size()
             ? " (in addition to the " + str(extra_columns.size()) + " uses of -column)"
             : "") +
  ...
  if (nans_in_data) {
    CONSOLE("Non-finite values present in data; rows will be removed from fixel-wise design "
            "matrices accordingly");
    if (!extra_columns.size()) {
      CONSOLE(
          "(Note that this will result in slower execution than if such values were not present)");
    }
  }

  // Only add contrast matrix row number to image outputs if there's more than one hypothesis
  auto postfix = [&](const size_t i) -> std::string {
...

  {
    ...
    ProgressBar progress("Outputting beta coefficients, effect size and standard deviation",
                         num_factors + (2 * num_hypotheses) + num_vgs +
                             (nans_in_data || extra_columns.size() ? 1 : 0));
    ...
    if (num_vgs == 1) {
      write_fixel_output(
          Path::join(output_fixel_directory, "std_dev.mif"), stdev.row(0), mask, output_header);
    } else {
  ...
  if (extra_columns.size() || nans_in_data) {
    ...
    if (variance_groups.size())
      glm_test.reset(new Math::Stats::GLM::TestFixedHeteroscedastic(
          data, design, hypotheses, variance_groups));
    else
  ...
  std::shared_ptr<Stats::EnhancerBase> cfe_integrator(
      new Stats::CFE(matrix, cfe_dh, cfe_e, cfe_h, cfe_c, !cfe_legacy));
  ...
  if (do_nonstationarity_adjustment) {
    Stats::PermTest::precompute_empirical_stat(
        glm_test, cfe_integrator, empirical_skew, empirical_cfe_statistic);
  ...
  Stats::PermTest::precompute_default_permutation(glm_test,
                                                  cfe_integrator,
                                                  ...
                                                  default_enhanced);
  for (size_t i = 0; i != num_hypotheses; ++i) {
    write_fixel_output(Path::join(output_fixel_directory,
                                  (hypotheses[i].is_F() ? std::string("F") : std::string("t")) +
                                      "value" + postfix(i) + ".mif"),
  ...
  if (!get_options("notest").size()) {
    ...
    ProgressBar progress("Outputting final results",
                         (fwe_strong ? 1 : num_hypotheses) + 1 + 3 * num_hypotheses);
    ...
    if (fwe_strong) {
      ...
      for (size_t i = 0; i != num_hypotheses; ++i) {
        save_vector(null_distribution.col(i),
                    Path::join(output_fixel_directory, "null_dist" + postfix(i) + ".txt"));
        ++progress;
      }
    }

    const matrix_type pvalue_output =
        MR::Math::Stats::fwe_pvalue(null_distribution, default_enhanced);
    ++progress;
    for (size_t i = 0; i != num_hypotheses; ++i) {
      ...
      ++progress;
      write_fixel_output(
          Path::join(output_fixel_directory, "uncorrected_1mpvalue" + postfix(i) + ".mif"),
          uncorrected_pvalues.col(i),
          mask,
          output_header);
      ++progress;
      write_fixel_output(
          Path::join(output_fixel_directory, "null_contributions" + postfix(i) + ".mif"),
