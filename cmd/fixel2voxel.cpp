...
void usage() {
  AUTHOR =
      "Robert E. Smith (robert.smith@florey.edu.au) & David Raffelt (david.raffelt@florey.edu.au)";
  ...
  +"Fixel data can be reduced to voxel data in a number of ways:" +
      "- Some statistic computed across all fixel values within a voxel: mean, sum, product, min, "
      "max, absmax, magmax" +
  ...
  +"* Reference for 'complexity' operation:\n"
   ...
   "Beyond Fractional Anisotropy: Extraction of bundle-specific structural metrics from crossing "
   "fibre models. "
   "NeuroImage, 2014, 100, 176-191";

  ARGUMENTS
  +Argument("fixel_in", "the input fixel data file").type_image_in() +
      Argument("operation", "the operation to apply, one of: " + join(operations, ", ") + ".")
  ...
  +Option("number",
      ...
      + Option("fill",
               "for \"none\" operation, specify the value to fill when number of fixels is fewer "
               "than the maximum (default: 0.0)") +
      Argument("value").type_float()

      + Option("weighted",
               "weight the contribution of each fixel to the per-voxel result according to its "
               "volume.") +
...
struct LoopFixelsInVoxelWithMax {
  ...
  template <class... DataType> struct Run {
    ...
    FORCE_INLINE operator bool() const {
      return max_fixels ? (fixel_index < max_fixels) : (fixel_index < num_fixels);
    }
  ...
  void operator()(Image<index_type> &index, Image<float> &out) {
    ...
    if (vol.valid()) {
      for (auto f = Base::Loop(index)(data, vol, dir); f; ++f) {
        if (!f.padding())
          sum_dec += Eigen::Vector3d(abs(dir.row(1)[0]), abs(dir.row(1)[1]), abs(dir.row(1)[2])) *
...
void run() {
  ...
  if (in_data.size(2) != 1)
    throw Exception("Input fixel data file must have a single scalar value per fixel (i.e. have "
                    "dimensions Nx1x1)");
  ...
  if (max_fixels && op == 7)
    throw Exception("\"count\" statistic is meaningless if constraining the number of fixels per "
                    "voxel using the -number option");
  ...
  if (op == 7) { // count
    ...
    if (max_fixels) {
      ...
      for (auto l = Loop("determining largest fixel count", in_index_image, 0, 3)(in_index_image);
           l;
           ++l)
  ...
  if (op == 10 || op == 11) // dec
    in_directions = Fixel::find_directions_header(Fixel::get_fixel_directory(in_data.name()))
                        .get_image<float>()
                        .with_direct_io();
  ...
  if (op == 2 || op == 3 || op == 4 || op == 5 || op == 6 || op == 7 || op == 8 || op == 9 ||
      op == 12) {
    if (in_vol.valid())
      WARN("Option -weighted has no meaningful interpretation for the operation specified; "
           "ignoring");
  }
