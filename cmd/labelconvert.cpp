...
void usage() {
  ...
  EXAMPLES
  +Example("Convert a Desikan-Killiany parcellation image as provided by FreeSurfer to have nodes "
           "incrementing from 1",
           "labelconvert aparc+aseg.mgz FreeSurferColorLUT.txt "
           "mrtrix3//share//mrtrix3//labelconvert//fs_default.txt nodes.mif",
  ...
  +Argument("path_in", "the input image").type_image_in() +
      Argument("lut_in", "the connectome lookup table corresponding to the input image")
          .type_file_in() +
      Argument("lut_out", "the target connectome lookup table for the output image")
          .type_file_in() +
      Argument("image_out", "the output image").type_image_out();

  OPTIONS
  +Option("spine",
          "provide a manually-defined segmentation of the base of the spine where the streamlines "
          "terminate, so that "
...
void run() {
  ...
  if (*std::max_element(mapping.begin(), mapping.end()) == 0)
    throw Exception("Mapping between input and output LUTs is empty, i.e. no common node names "
                    "between these two LUTs");
  ...
  if (opt.size()) {
    ...
    if (duplicates)
      throw Exception("Cannot add spine node: \"" + SPINE_NODE_NAME +
                      "\" appears multiple times in output LUT");
    if (!spine_index)
      throw Exception("Cannot add spine node: \"" + SPINE_NODE_NAME +
                      "\" not present in output LUT");
    ...
    if (dimensions_match(in_spine, out)) {
      ...
      WARN("Spine node is being created from the mask image provided using -spine option using "
           "nearest-neighbour interpolation;");
      WARN("recommend using the parcellation image as the basis for this mask so that "
