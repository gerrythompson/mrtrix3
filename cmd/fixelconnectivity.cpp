...
void usage() {
  ...
  +Argument("fixel_directory",
      ...
      + Argument("matrix", "the output fixel-fixel connectivity matrix directory path")
            .type_directory_out();
  ...
  +OptionGroup("Options that influence generation of the connectivity matrix / matrices")
      ...
      + Option("threshold",
               "a threshold to define the required fraction of shared connections to be included "
               "in the neighbourhood (default: " +
      ...
      + Option("mask",
               "provide a fixel data file containing a mask of those fixels to be computed; fixels "
               "outside the mask will be empty in the output matrix") +
...
void run() {
  ...
  const value_type angular_threshold =
      get_option_value("angle", value_type(DEFAULT_ANGLE_THRESHOLD));
  ...
  if (opt.size()) {
    ...
    if (!Fixel::fixels_match(index_header, fixel_mask))
      throw Exception(
          "Mask image provided using -mask option does not match input fixel directory");
  } else {
  ...
  auto connectivity_matrix =
      Fixel::Matrix::generate(argument[1], index_image, fixel_mask, angular_threshold);
