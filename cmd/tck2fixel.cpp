...
void usage() {
  ...
  +Argument("tracks", "the input tracks.").type_tracks_in() +
      Argument("fixel_folder_in",
               "the input fixel folder. Used to define the fixels and their directions")
          .type_directory_in() +
      Argument("fixel_folder_out",
               "the fixel folder to which the output will be written. This can be the same as the "
               "input folder if desired")
          .type_text() +
...
void run() {
  ...
  {
    auto directions_data = Fixel::find_directions_header(input_fixel_folder)
                               .get_image<default_type>()
                               .with_direct_io();
    // Load template fixel directions
    Transform image_transform(index_image);
    for (auto i = Loop("loading template fixel directions and positions", index_image, 0, 3)(
             index_image);
         i;
         ++i) {
      const Eigen::Vector3d vox((default_type)index_image.index(0),
...

  {
    ...
    DWI::Tractography::Mapping::TrackLoader loader(
        track_file, num_tracks, "mapping tracks to fixels");
    DWI::Tractography::Mapping::TrackMapperBase mapper(index_image);
    mapper.set_upsample_ratio(
        DWI::Tractography::Mapping::determine_upsample_ratio(index_header, properties, 0.333f));
