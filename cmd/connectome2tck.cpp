...
const OptionGroup TrackOutputOptions =
    ...
    + Option("nodes",
             "only select tracks that involve a set of nodes of interest (provide as a "
             "comma-separated list of integers)") +
    Argument("list").type_sequence_int()

    + Option("exclusive",
             "only select tracks that exclusively connect nodes from within the list of nodes of "
             "interest")
    ...
    + Option("exemplars",
             ...
             "(the parcellation node image must be provided in order to constrain the exemplar "
             "endpoints)") +
    Argument("image").type_image_in()

    + Option("keep_unassigned",
             "by default, the program discards those streamlines that are not successfully "
             "assigned to a node. "
             "Set this option to generate corresponding outputs containing these streamlines "
             "(labelled as node index 0)")

    + Option("keep_self",
             "by default, the program will not output streamlines that connect to the same node at "
             "both ends. "
...
void usage() {
  ...
  const OptionGroup TrackWeightsOptions =
      OptionGroup("Options for importing / exporting streamline weights") +
  ...
  DESCRIPTION
  +"The compulsory input file \"assignments_in\" should contain a text file where there is one row "
   "for each streamline, "
   "and each row contains a list of numbers corresponding to the parcels to which that streamline "
   "was assigned "
   "(most typically there will be two entries per streamline, one for each endpoint; but this is "
   "not strictly a requirement). "
   "This file will most typically be generated using the tck2connectome command with the "
   "-out_assignments option.";

  EXAMPLES
  +Example("Default usage",
           "connectome2tck tracks.tck assignments.txt edge-",
           "The command will generate one track file for every edge in the connectome, with the "
           "name of each file "
           "indicating the nodes connected via that edge; for instance, all streamlines connecting "
           "nodes "
           "23 and 49 will be written to file \"edge-23-49.tck\".")

      + Example(
            "Extract only the streamlines between nodes 1 and 2",
            "connectome2tck tracks.tck assignments.txt tracks_1_2.tck -nodes 1,2 -exclusive -files "
            "single",
            "Since only a single edge is of interest, this example provides only the two nodes "
            "involved "
            "in that edge to the -nodes option, adds the -exclusive option so that only "
            "streamlines for which "
            "both assigned nodes are in the list of nodes of interest are extracted (i.e. only "
            "streamlines connecting "
      ...
      Example("Extract the streamlines connecting node 15 to all other nodes in the parcellation, "
              ...
              "The command will generate the same number of track files as there are nodes in the "
              "parcellation: "
      ...
      Example(
          ...
          "connectome2tck tracks.tck assignments.txt unassigned.tck -nodes 0 -keep_self -files "
          "single",
          ...
          "use of the -keep_self option), the single output track file will contain all "
          "streamlines "
      ...
      Example("Generate a single track file containing edge exemplar trajectories",
              "connectome2tck tracks.tck assignments.txt exemplars.tck -files single -exemplars "
              "nodes.mif",
  ...
  +Argument("tracks_in", "the input track file").type_file_in() +
      Argument("assignments_in",
               "input text file containing the node assignments for each streamline")
          .type_file_in() +
...
void run() {
  ...
  if (assignments_lists.size() != count)
    throw Exception("Assignments file contains " + str(assignments_lists.size()) +
                    " entries; track file contains " + str(count) + " tracks");
  ...
  if (opt.size()) {
    ...
    for (auto i : data) {
      if (i > max_node_index) {
        WARN("Node of interest " + str(i) + " is above the maximum detected node index of " +
             str(max_node_index));
      } else {
        nodes.push_back(i);
        if (!i)
  ...
  if (opt.size()) {
    ...
    // Generate exemplars - these can _only_ be done per edge, and requires a mutex per edge to
    // multi-thread
    ...
    for (node_t index = 1; index <= max_node_index; ++index) {
      if (volumes[index])
        COMs[index] = (transform.voxel2scanner *
                       (COMs[index] * (1.0f / float(volumes[index]))).cast<default_type>())
                          .cast<float>();
      else
...

    {
      ...
      if (assignments_pairs.size()) {
        ...
        Thread::run_queue(loader,
                          Thread::batch(Tractography::Connectome::Streamline_nodepair()),
                          Thread::multi(worker));
      } else {
        ...
        Thread::run_queue(loader,
                          Thread::batch(Tractography::Connectome::Streamline_nodelist()),
                          Thread::multi(worker));
      }
    }
    ...
    if (file_format == 0) { // One file per edge
      ...
      for (vector<node_t>::const_iterator n = nodes.begin(); n != nodes.end(); ++n) {
        generator.write(*n,
                        prefix + str(*n) + ".tck",
                        weights_prefix.size() ? (weights_prefix + str(*n) + ".csv") : "");
        ++progress;
      }
    ...
    case 0: // One file per edge
      ...
      INFO("A total of " + str(writer.file_count()) +
           " output track files will be generated (one for each edge)");
      break;
    case 1: // One file per node
      for (vector<node_t>::const_iterator i = nodes.begin(); i != nodes.end(); ++i)
        writer.add(*i,
                   prefix + str(*i) + ".tck",
                   weights_prefix.size() ? (weights_prefix + str(*i) + ".csv") : "");
      INFO("A total of " + str(writer.file_count()) +
           " output track files will be generated (one for each node)");
      break;
    case 2: // Single file
      std::string path = prefix;
      if (path.rfind(".tck") != path.size() - 4)
