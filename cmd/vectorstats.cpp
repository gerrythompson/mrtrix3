...
public:
  SubjectVectorImport(const std::string &path)
      : SubjectDataImportBase(path), data(load_vector(path)) {}
...
void run() {
  ...
  try {
    ...
    for (size_t i = 0; i != importer.size(); ++i) {
      if (importer[i]->size() != num_elements)
        throw Exception("Subject file \"" + importer[i]->name() +
                        "\" contains incorrect number of elements (" + str(importer[i]) +
                        "; expected " + str(num_elements) + ")");
    }
  ...
  if (nans_in_data) {
    INFO("Non-finite values present in data; rows will be removed from element-wise design "
  ...
  for (size_t i = 0; i != num_hypotheses; ++i) {
    save_matrix(default_statistic.col(i),
  ...
  if (!get_options("notest").size()) {
    ...
    for (size_t i = 0; i != num_hypotheses; ++i) {
      ...
      save_vector(uncorrected_pvalues.col(i),
                  output_prefix + "uncorrected_1mpvalue" + postfix(i) + ".csv");
      save_vector(null_contributions.col(i),
