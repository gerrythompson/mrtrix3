...
void usage() {
  AUTHOR =
      "David Raffelt (david.raffelt@florey.edu.au) & Rami Tabarra (rami.tabarra@florey.edu.au)";

  SYNOPSIS = "Crop/remove fixels from sparse fixel image using a binary fixel mask";

  DESCRIPTION
  +"The mask must be input as a fixel data file the same dimensions as the fixel data file(s) to "
   "be cropped."
  ...
  +Argument("input_fixel_directory",
      ...
      Argument("input_fixel_mask",
          ...
          .type_image_in() +
      Argument("output_fixel_directory",
...
void run() {
  ...
  auto out_index_image = Image<index_type>::create(
      Path::join(out_fixel_directory, Path::basename(in_index_image.name())), out_header);
  ...
  for (auto &in_data_header : in_headers) {
    ...
    out_data_images.push_back(
        Image<float>::create(Path::join(out_fixel_directory, Path::basename(in_data_header.name())),
                             out_data_header)
            .with_direct_io());
  }
