...
void usage() {
  AUTHOR =
      "David Raffelt (david.raffelt@florey.edu.au) and J-Donald Tournier (jdtournier@gmail.com)";

  SYNOPSIS = "Evaluate the amplitude of an image of spherical harmonic functions along specified "
             "directions";
  ...
  OPTIONS
  +Option("nonnegative", "cap all negative amplitudes to zero") + DWI::GradImportOptions() +
...
public:
  SH2Amp(const Eigen::MatrixXd &transform, bool nonneg)
      : transform(transform), nonnegative(nonneg) {}
...
void run() {
  ...
  try {
    ...
    try {
      ...
      if (directions.cols() < 4)
        throw("unable to interpret file \"" + std::string(argument[1]) +
              "\" as a directions or gradient file");
    } catch (Exception &E) {
  ...
  if (directions.cols() == 2) { // single-shell:
    ...
    dir_stream << directions(directions.rows() - 1, 0) << ","
               << directions(directions.rows() - 1, 1);
    ...
    if (shells.count() > 1) {
      if (sh_data.ndim() < 5)
        throw Exception(
            "multiple shells detected in gradient scheme, but only one shell in input data");
    ...
    for (size_t n = 0; n < shells.count(); ++n) {
      ...
      if (shells[n].is_bzero()) {
        ...
        for (size_t idx = 0; idx < shells[n].count(); ++idx)
          Math::Sphere::cartesian2spherical(directions.row(shells[n].get_volumes()[idx]).head(3),
                                            dirs.row(idx));
      }
