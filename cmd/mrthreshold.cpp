...
void usage() {
  ...
  +"The threshold value to be applied can be determined in one of a number of ways:"
      ...
      + "The four operators available through the \"-comparison\" option (\"lt\", \"le\", \"ge\" "
        "and \"gt\") "
  ...
  +OptionGroup("Threshold determination mechanisms")
      ...
      + Option("percentile",
               "determine threshold based on some percentile of the image intensity distribution") +
      Argument("value").type_float(0.0, 100.0)

      +
      Option(
          "top",
          "determine threshold that will result in selection of some number of top-valued voxels") +
      Argument("count").type_integer(1)

      + Option(
            "bottom",
            "determine & apply threshold resulting in selection of some number of bottom-valued "
            "voxels "
      ...
      + Option("allvolumes",
               "compute a single threshold for all image volumes, rather than an individual "
  ...
  if (std::isfinite(abs)) {
    ...
    if (index < 0 || index >= ssize_t(data.size()))
      throw Exception("Number of valid input image values (" + str(data.size()) +
                      ") less than number of voxels requested via -" +
                      (bottom >= 0 ? "bottom" : "top") + " option (" +
...
void apply(Image<value_type> &in,
  ...
  if (mask_out) {
    ...
    for (auto l = Loop(in, 0, max_axis)(in, mask, out); l; ++l)
      out.value() = !std::isnan(static_cast<value_type>(in.value())) && mask.value() &&
                            func(in.value(), threshold)
                        ? true_value
                        : false_value;
  } else {
    for (auto l = Loop(in, 0, max_axis)(in, out); l; ++l)
      out.value() = !std::isnan(static_cast<value_type>(in.value())) && func(in.value(), threshold)
                        ? true_value
                        : false_value;
  }
}
...
void execute(Image<value_type> &in,
    ...
    if (to_cout) {
      ...
      for (auto l = Loop(3, in.ndim())(in); l; ++l) {
        ...
        const default_type threshold =
            calculate(in, mask, 3, abs, percentile, bottom, top, ignore_zero);
        std::cout << threshold;
      }

    } else {

      for (auto l = Loop("Determining and applying per-volume thresholds", 3, in.ndim())(in); l;
           ++l) {
        LogLevelLatch latch(App::log_level - 1);
        const default_type threshold =
            calculate(in, mask, 3, abs, percentile, bottom, top, ignore_zero);
  ...
  const default_type threshold =
      calculate(in, mask, in.ndim(), abs, percentile, bottom, top, ignore_zero);
  if (to_cout)
    std::cout << threshold;
  else
...
void run() {
  ...
  operator_type comp = opt.size() ? operator_type(int(opt[0][0]))
