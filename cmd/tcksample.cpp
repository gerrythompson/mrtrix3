...
public:
  SamplerNonPrecise(Image<value_type> &image,
      ...
      : interp(image),
        mapper(precalc_tdi.valid() ? new DWI::Tractography::Mapping::TrackMapperBase(image)
                                   : nullptr),
        tdi(precalc_tdi),
        statistic(statistic) {
    if (mapper)
  ...
  bool operator()(DWI::Tractography::Streamline<value_type> &tck,
                  std::pair<size_t, value_type> &out) {
...
public:
  SamplerPrecise(Image<value_type> &image,
                 const stat_tck statistic,
                 const Image<value_type> &precalc_tdi)
      : image(image),
...
public:
  Receiver_Statistic(const size_t num_tracks)
      : ReceiverBase(num_tracks), vector_data(vector_type::Zero(num_tracks)) {}
  ...
  void save(const std::string &path) { MR::save_vector(vector_data, path); }
...
void run() {
  ...
  const interp_type interp =
      nointerp ? interp_type::NEAREST : (precise ? interp_type::PRECISE : interp_type::LINEAR);
  const size_t num_tracks =
  ...
  if (get_options("use_tdi_fraction").size()) {
    if (statistic == stat_tck::NONE)
      throw Exception(
          "Cannot use -use_tdi_fraction option unless a per-streamline statistic is used");
  ...
  if (statistic == stat_tck::NONE) {
    ...
    case interp_type::NEAREST:
      execute_nostat<Interp::Nearest<Image<value_type>>>(
          reader, properties, num_tracks, image, argument[2]);
      break;
