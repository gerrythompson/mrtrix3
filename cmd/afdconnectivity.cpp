...
void usage() {
  ...
  SYNOPSIS = "Obtain an estimate of fibre connectivity between two regions using AFD and "
  ...
  +"This estimate is obtained by determining a fibre volume (AFD) occupied by the pathway "
   "of interest, and dividing by the streamline length."
      ...
      + "Note that the sum of the AFD is normalised by streamline length to "
        "account for subject differences in fibre bundle length. This normalisation results in a "
        "measure "
        "that is more related to the cross-sectional volume of the tract (and therefore "
        "'connectivity'). "
        "Note that SIFT-ed tract count is a superior measure because it is unaffected by "
        "tangential yet unrelated "
        "fibres. However, AFD connectivity may be used as a substitute when Anatomically "
        "Constrained Tractography "
        "is not possible due to uncorrectable EPI distortions, and SIFT may therefore not be as "
        "effective."
...
public:
  ...
  value_type get_selected_volume(const value_type l) const {
    return get_TD() ? (get_FOD() * (l / get_TD())) : 0.0;
  }
  value_type get_selected_volume() const {
    return get_TD() ? (get_FOD() * (length / get_TD())) : 0.0;
  }
...
public:
  AFDConnectivity(Image<value_type> &fod_buffer,
    ...
    mapper.set_upsample_ratio(
        DWI::Tractography::Mapping::determine_upsample_ratio(fod_buffer, tck_path, 0.1));
...
value_type AFDConnectivity::get(const std::string &path) {
  ...
  DWI::Tractography::Mapping::TrackLoader loader(
      reader, track_count, "summing apparent fibre density within track");

  // If WBFT is provided, this is the sum of (volume/length) across streamlines
  // Otherwise, it's a sum of lengths of all streamlines (for later scaling by mean streamline
  // length)
...
void run() {
  ...
  // output the AFD sum using std::cout. This enables output to be redirected to a file without the
  // console output.
