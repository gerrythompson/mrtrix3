...
void usage() {
  ...
  EXAMPLES
  +Example("Convert a transformation matrix produced by FSL's flirt command into a format usable "
           "by MRtrix3",
           "transformconvert transform_flirt.mat flirt_in.nii flirt_ref.nii flirt_import "
           "transform_mrtrix.txt",
           "The two images provided as inputs for this operation must be in the correct order: "
           "first the image "
           "that was provided to flirt via the -in option, second the image that was provided to "
           "flirt via the "
           "-ref option.")

      + Example(
            "Convert a plain text transformation matrix file produced by ITK's affine registration "
...
void parse_itk_trafo(const std::string &itk_file,
  ...
  while (file.next()) {
    if (file.key() == "Transform") {
      if (std::find(supported_transformations.begin(),
                    supported_transformations.end(),
                    file.value()) == supported_transformations.end())
        throw Exception("The " + file.value() +
                        " transform type is currenly not supported or tested");
    } else if (file.key() == "Parameters") {
      line = file.value();
      std::replace(line.begin(), line.end(), ' ', ',');
      vector<default_type> parameters(parse_floats(line));
      if (parameters.size() != 12)
        throw Exception("Expected itk file with 12 parameters but has " + str(parameters.size()) +
                        " parameters.");
...
void run() {
  ...
  case 0: { // flirt_import
    ...
    transform_type forward_transform =
        dest_flirt_to_scanner * transform * src_flirt_to_scanner.inverse();
  ...
  case 1: { // ITK import
    ...
    transform.translation() =
        transform.translation() + centre_of_rotation - transform.linear() * centre_of_rotation;
