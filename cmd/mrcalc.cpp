...
#define UNARY_OP(OPTION, FEEDBACK, FLAGS, DESCRIPTION, REAL_OPERATION, COMPLEX_OPERATION)          \
  ...
  public:                                                                                          \
    Op_##OPTION()                                                                                  \
        : OpUnary(FEEDBACK, FLAGS & COMPLEX_MAPS_TO_REAL, FLAGS & REAL_MAPS_TO_COMPLEX) {}         \
    complex_type R(real_type v) const REAL_OPERATION complex_type                                  \
        Z(complex_type v) const COMPLEX_OPERATION                                                  \
  };
...
#define BINARY_OP(OPTION, FEEDBACK, FLAGS, DESCRIPTION, REAL_OPERATION, COMPLEX_OPERATION)         \
  ...
  public:                                                                                          \
    Op_##OPTION()                                                                                  \
        : OpBinary(FEEDBACK, FLAGS & COMPLEX_MAPS_TO_REAL, FLAGS & REAL_MAPS_TO_COMPLEX) {}        \
...
#define TERNARY_OP(OPTION, FEEDBACK, FLAGS, DESCRIPTION, REAL_OPERATION, COMPLEX_OPERATION)        \
  ...
  public:                                                                                          \
    Op_##OPTION()                                                                                  \
        : OpTernary(FEEDBACK, FLAGS & COMPLEX_MAPS_TO_REAL, FLAGS & REAL_MAPS_TO_COMPLEX) {}       \
...
BINARY_OP(
    ...
    "(%1 - %2)",
    NORMAL,
    "subtract nth operand from (n-1)th",
    { return a - b; },
    { return a - b; })
BINARY_OP(
...
BINARY_OP(
    ...
    "(%1 > %2)",
    NORMAL,
    "greater-than operator (true=1, false=0)",
    { return a > b; },
    NOT_IMPLEMENTED)
BINARY_OP(
    le,
    "(%1 <= %2)",
    NORMAL,
    "less-than-or-equal-to operator (true=1, false=0)",
    { return a <= b; },
    NOT_IMPLEMENTED)
BINARY_OP(
    ge,
...
BINARY_OP(
    ...
    "%1^%2",
    NORMAL,
    "raise (n-1)th operand to nth power",
    { return std::pow(a, b); },
    { return std::pow(a, b); })
...
BINARY_OP(
    ...
    "(%1 /_ %2)",
    REAL_MAPS_TO_COMPLEX,
    "create complex number using the last two operands as magnitude,phase components (phase in "
    "radians)",
...
public:
  BinaryEvaluator(const std::string &name,
                  Operation operation,
  ...
  virtual Chunk &evaluate(Chunk &a, Chunk &b) const {
    ...
    if (operands[0].is_complex() || operands[1].is_complex()) {
      ...
      for (size_t n = 0; n < out.size(); ++n)
        out[n] =
            op.R(a.size() ? a[n].real() : a.value.real(), b.size() ? b[n].real() : b.value.real());
    }
    return out;
  }
};
...
public:
  ...
  virtual Chunk &evaluate(Chunk &a, Chunk &b, Chunk &c) const {
    ...
    if (operands[0].is_complex() || operands[1].is_complex() || operands[2].is_complex()) {
      for (size_t n = 0; n < out.size(); ++n)
        out[n] =
            op.Z(a.size() ? a[n] : a.value, b.size() ? b[n] : b.value, c.size() ? c[n] : c.value);
    } else {
      for (size_t n = 0; n < out.size(); ++n)
...
void unary_operation(const std::string &operation_name,
...
template <class Operation>
void binary_operation(const std::string &operation_name,
                      vector<StackEntry> &stack,
                      Operation operation) {
  if (stack.size() < 2)
...
template <class Operation>
void ternary_operation(const std::string &operation_name,
                       vector<StackEntry> &stack,
                       Operation operation) {
  if (stack.size() < 3)
    throw Exception("not enough operands in stack for operation \"" + operation_name + "\"");
  ...
  b.load();
  c.load();
  if (a.evaluator || a.image || a.rng || b.evaluator || b.image || b.rng || c.evaluator ||
      c.image || c.rng) {
...
void get_header(const StackEntry &entry, Header &header) {
  ...
  for (size_t n = 0; n < std::min<size_t>(header.ndim(), entry.image->ndim()); ++n) {
    ...
    if (!voxel_grids_match_in_scanner_space(header, *(entry.image), 1.0e-4) &&
        !transform_mis_match_reported) {
...
class ThreadFunctor {
public:
  ThreadFunctor(const vector<size_t> &inner_axes,
...
class OpBase {
public:
  OpBase(const char *format_string,
         bool complex_maps_to_real = false,
         bool real_map_to_complex = false)
...
public:
  OpUnary(const char *format_string,
          bool complex_maps_to_real = false,
          bool real_map_to_complex = false)
...
public:
  OpBinary(const char *format_string,
           bool complex_maps_to_real = false,
           bool real_map_to_complex = false)
      : OpBase(format_string, complex_maps_to_real, real_map_to_complex) {}
...
public:
  OpTernary(const char *format_string,
            bool complex_maps_to_real = false,
            bool real_map_to_complex = false)
      : OpBase(format_string, complex_maps_to_real, real_map_to_complex) {}
