...
void usage() {
  ...
  +Option("bg", "image intensity of background (default: " + str(VALUE_DEFAULT_BG, 2) + ")") +
      ...
      + Option("cgm",
               "image intensity of cortical grey matter (default: " + str(VALUE_DEFAULT_CGM, 2) +
                   ")") +
      Argument("value").type_float(0.0, 1.0)

      +
      Option("sgm",
             "image intensity of sub-cortical grey matter (default: " + str(VALUE_DEFAULT_SGM, 2) +
                 ")") +
      Argument("value").type_float(0.0, 1.0)
      ...
      + Option("path",
               "image intensity of pathological tissue (default: " + str(VALUE_DEFAULT_PATH, 2) +
                   ")") +
...
void run() {
  ...
  auto f = [&](decltype(input) &in, decltype(output) &out) {
    ...
    out.value() = (bg_multiplier * bg) + (cgm_multiplier * t.get_cgm()) +
                  (sgm_multiplier * t.get_sgm()) + (wm_multiplier * t.get_wm()) +
                  (csf_multiplier * t.get_csf()) + (path_multiplier * t.get_path());
  };
