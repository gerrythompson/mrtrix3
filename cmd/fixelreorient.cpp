...
void usage() {
  ...
  +"Reorientation is performed by transforming the vector representing "
   "the fixel direction with the Jacobian (local affine transform) computed at each voxel in the "
   "warp, "
  ...
  +Argument("fixel_in", "the input fixel directory").type_directory_in() +
      Argument("warp",
               ...
               "Reorientation is performed by applying the Jacobian affine transform in each voxel "
               "in the warp, "
               "then re-normalising the vector representing the fixel direction")
          .type_image_in() +
      Argument("fixel_out",
               "the output fixel directory. If the the input and output directories are the same, "
               "the existing directions file will "
               "be replaced (providing the -force option is supplied). If a new directory is "
               "supplied then the fixel directions and all "
...
void run() {
  ...
  auto output_directions_image =
      Image<float>::create(Path::join(output_fixel_directory, output_directions_filename),
                           input_directions_image)
          .with_direct_io();

  for (auto i = Loop("reorienting fixel directions", input_index_image, 0, 3)(input_index_image,
                                                                              jacobian);
       i;
       ++i) {
    ...
    if (num_fixels_in_voxel) {
      ...
      for (index_type f = 0; f < num_fixels_in_voxel; ++f) {
        ...
        output_directions_image.row(1) =
            (transform * Eigen::Vector3f(input_directions_image.row(1))).normalized();
      }
    }
  }
