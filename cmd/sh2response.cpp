...
void usage() {
  ...
  ARGUMENTS
  +Argument("SH", "the spherical harmonic decomposition of the diffusion-weighted images")
          .type_image_in() +
      Argument("mask",
               "the mask containing the voxels from which to estimate the response function")
          .type_image_in() +
      Argument("directions",
               "a 4D image containing the direction vectors along which to estimate the response "
               "function")
          .type_image_in() +
      Argument("response", "the output axially-symmetric spherical harmonic coefficients")
          .type_file_out();
  ...
  +Option("lmax", "specify the maximum harmonic degree of the response function to estimate") +
      ...
      Option("dump",
             "dump the m=0 SH coefficients from all voxels in the mask to the output file, rather "
             "than their mean") +
...
void run() {
  ...
  if (dir.size(3) != 3)
    throw Exception("input direction image \"" + std::string(argument[2]) +
                    "\" must contain precisely 3 volumes");
  ...
  for (auto l = loop(mask, SH, dir); l; ++l) {
    ...
    if (!d.allFinite()) {
      WARN("voxel with invalid direction [ " + str(dir.index(0)) + " " + str(dir.index(1)) + " " +
           str(dir.index(2)) + " ]; skipping");
      continue;
    }
    d.normalize();
    // Uncertainty regarding Eigen's behaviour when normalizing a zero vector; may change behaviour
    // between versions
    if (!d.allFinite() || !d.squaredNorm()) {
      WARN("voxel with zero direction [ " + str(dir.index(0)) + " " + str(dir.index(1)) + " " +
           str(dir.index(2)) + " ]; skipping");
      continue;
    }
