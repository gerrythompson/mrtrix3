...
void usage() {
  ...
  SYNOPSIS = "Modify the grid of an image without interpolation (cropping or padding) or by "
             "regridding to an image grid with modified orientation, location and or resolution. "
             "The image content remains in place in real world coordinates.";

  DESCRIPTION
  +"- regrid: This operation performs changes of the voxel grid that require interpolation of the "
   "image such as changing the resolution or location and orientation of the voxel grid. "
   "If the image is down-sampled, the appropriate smoothing is automatically applied using "
   "Gaussian smoothing unless nearest neighbour interpolation is selected or oversample is changed "
   "explicitly. The resolution can only be changed for spatial dimensions. " +
      "- crop: The image extent after cropping, can be specified either manually for each axis "
      "dimensions, or via a mask or reference image. "
      "The image can be cropped to the extent of a mask. "
      "This is useful for axially-acquired brain images, where the image size can be reduced by a "
      "factor of 2 by removing the empty space on either side of the brain. Note that cropping "
      "does not extend the image beyond the original FOV unless explicitly specified (via "
      "-crop_unbound or negative -axis extent)." +
      "- pad: Analogously to cropping, padding increases the FOV of an image without image "
      "interpolation. Pad and crop can be performed simultaneously by specifying signed specifier "
      "argument values to the -axis option." +
      "This command encapsulates and extends the functionality of the superseded commands 'mrpad', "
      "'mrcrop' and 'mrresize'. Note the difference in -axis convention used for 'mrcrop' and "
      "'mrpad' (see -axis option description).";

  EXAMPLES
  +Example("Crop and pad the first axis",
           "mrgrid in.mif crop -axis 0 10,-5 out.mif",
           "This removes 10 voxels on the lower and pads with 5 on the upper bound, which is "
           "equivalent to "
      ...
      Example("Right-pad the image to the number of voxels of a reference image",
              ...
              "The headers of in.mif and ref.mif are ignored and the output image uses NAN values "
              "to fill in voxels outside the original range of in.mif.")
      ...
      + Example("Regrid and interpolate to match the voxel grid of a reference image",
                ...
                "The -scale option overwrites the voxel scaling factor yielding voxel sizes in the "
                "third dimension that are "
                "twice as coarse as those of the template image.");

  ARGUMENTS
  +Argument("input", "input image to be regridded.").type_image_in() +
      Argument("operation",
               "the operation to be performed, one of: " + join(operation_choices, ", ") + ".")
  ...
  +OptionGroup("Regridding options (involves image interpolation, applied to spatial axes only)") +
      Option("template",
             "match the input image grid (voxel spacing, image size, header transformation) to "
             "that of a reference image. "
             "The image resolution relative to the template image can be changed with one of "
             "-size, -voxel, -scale.") +
      ...
      + OptionGroup("Pad and crop options (no image interpolation is performed, header "
                    "transformation is adjusted)") +
      Option("as",
             "pad or crop the input image on the upper bound to match the specified reference "
             "image grid. "
             "This operation ignores differences in image transformation between input and "
             "reference image.") +
      ...
      + Option("mask",
               ...
               "The mask must share a common voxel grid with the input image but differences in "
               "image transformations are "
               "ignored. Note that even though only 3 dimensions are cropped when using a mask, "
               "the bounds are computed by "
               "checking the extent for all dimensions. "
               "Note that by default a gap of 1 voxel is left at all edges of the image to allow "
               "valid trilinear interpolation. "
               "This gap can be modified with the -uniform option but by default it does not "
               "extend beyond the FOV unless -crop_unbound is used.") +
      ...
      + Option("axis",
               ...
               "specifier argument "
               "defines the number of voxels added or removed on the lower or upper end of the "
               "axis (-axis index delta_lower,delta_upper) "
               "or acts as a voxel selection range (-axis index start:stop). In both modes, values "
               "are relative to the input image "
               "(overriding all other extent-specifying options). Negative delta specifier values "
               "trigger the inverse operation "
               "(pad instead of crop and vice versa) and negative range specifier trigger padding. "
               "Note that the deprecated commands 'mrcrop' and 'mrpad' used range-based and "
      ...
      Option("fill",
             "Use number as the out of bounds value. nan, inf and -inf are valid arguments. "
             "(Default: 0.0)") +
...
void run() {
  ...
  if (op == 0) { // regrid
    ...
    if (!resize_option_count and !template_option_count)
      throw Exception("please use either the -scale, -voxel, -resolution or -template option to "
                      "regrid the image");
    if (resize_option_count > 1)
      throw Exception("only a single method can be used to resize the image (image resolution, "
                      "voxel size or scale factor)");
    ...
    if (opt.size()) {
      ...
      struct BoundsCheck {
        ...
        BoundsCheck(vector<vector<ssize_t>> &overall_bounds)
      ...
      for (size_t axis = 0; axis != 3; ++axis) {
        if ((input_header.size(axis) - 1) != bounds[axis][1] or bounds[axis][0] != 0)
          INFO("cropping to mask changes axis " + str(axis) +
               " extent from 0:" + str(input_header.size(axis) - 1) + " to " +
               str(bounds[axis][0]) + ":" + str(bounds[axis][1]));
      }
    ...
    if (opt.size()) {
      if (crop_pad_option_count)
        throw Exception(str(operation_choices[op]) +
                        " can be performed using either a mask or a template image");
      ++crop_pad_option_count;
      ...
      for (size_t axis = 0; axis != 3; ++axis) {
        if (bounds[axis][0] < 0 || bounds[axis][1] > input_header.size(axis) - 1) {
          if (ignore.find(axis) == ignore.end())
            INFO("operation: crop without -crop_unbound: restricting padding on axis " + str(axis) +
                 " to valid FOV " + str(std::max<ssize_t>(0, bounds[axis][0])) + ":" +
    ...
    for (size_t i = 0; i != opt.size(); ++i) {
      ...
      if (axis >= input_header.ndim())
        throw Exception("-axis " + str(axis) + " larger than image dimensions (" +
                        str(input_header.ndim()) + ")");
      ...
      if (end == std::string::npos) { // spec = delta_lower,delta_upper
        ...
        try {
          ...
          throw Exception(E,
                          "-axis " + str(axis) + ": can't parse integer sequence specifier \"" +
                              spec + "\"");
        }
    ...
    for (size_t axis = 0; axis != 3; ++axis) {
      if (bounds[axis][1] < bounds[axis][0])
        throw Exception("axis " + str(axis) + " is empty: (" + str(bounds[axis][0]) + ":" +
                        str(bounds[axis][1]) + ")");
    }
    ...
    for (size_t axis = 0; axis < nd; axis++) {
      if (bounds[axis][0] != 0 || input_header.size(axis) != size[axis]) {
        changed_axes++;
        CONSOLE("changing axis " + str(axis) + " extent from 0:" +
                str(input_header.size(axis) - 1) + " (n=" + str(input_header.size(axis)) + ") to " +
                str(bounds[axis][0]) + ":" + str(bounds[axis][1]) + " (n=" + str(size[axis]) + ")");
      }
    }
    if (!changed_axes)
