...
OptionGroup CommonOptions =
    OptionGroup("Options common to more than one algorithm")
...
void usage() {
  AUTHOR =
      "J-Donald Tournier (jdtournier@gmail.com) and Ben Jeurissen (ben.jeurissen@uantwerpen.be)";
  ...
  +Example("Perform single-shell single-tissue CSD",
      ...
      + Example("Perform multi-shell multi-tissue CSD",
                "dwi2fod msmt_csd dwi.mif response_wm.txt wmfod.mif response_gm.txt gm.mif "
  ...
  +Argument("algorithm",
      ...
      Argument("response odf", "pairs of input tissue response and output ODF images")
          .allow_multiple();
...
public:
  ...
  void operator()(Image<float> &dwi_image) {
    ...
    if (sdeconv.niter >= sdeconv.shared.problem.max_niter) {
      INFO("voxel [ " + str(dwi_image.index(0)) + " " + str(dwi_image.index(1)) + " " +
           str(dwi_image.index(2)) + " ] did not reach full convergence");
    }
...
void run() {
  ...
  if (algorithm == 0) {
    ...
    if (argument.size() != 4)
      throw Exception(
          "CSD algorithm expects a single input response function and single output FOD image");
    ...
    try {
      ...
      throw Exception(
          e, "CSD algorithm expects second argument to be the input response function file");
    }
    ...
    if (argument.size() % 2)
      throw Exception("MSMT_CSD algorithm expects pairs of (input response function & output FOD "
    ...
    try {
      ...
      throw Exception(e,
                      "MSMT_CSD algorithm expects the first file in each argument pair to be an "
                      "input response function file");
    }
    ...
    auto dwi = header_in.get_image<float>().with_direct_io(3);
    ThreadedLoop("performing MSMT CSD (" + str(shared.num_shells()) + " shell" +
