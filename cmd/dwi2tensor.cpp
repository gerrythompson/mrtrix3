...
void usage() {
  ...
  +"By default, the diffusion tensor (and optionally the kurtosis tensor) is fitted to "
   ...
   "be altered in two ways:"
      ...
      + "By default, the diffusion tensor (and optionally the kurtosis tensor) is fitted using "
        ...
        "e.g. parameters that represent negative apparent diffusivities or negative apparent "
        "kurtoses, "
  ...
  +Option("ols", "perform initial fit using an ordinary least-squares (OLS) fit (see Description).")
      ...
      + Option("constrain",
               "constrain fit to non-negative diffusivity and kurtosis as well as monotonic signal "
               "decay (see Description).")
  ...
  +"References based on fitting algorithm used:"
      ...
      + "* IWLS:\n"
        ...
        "Weighted linear least squares estimation of diffusion MRI parameters: strengths, "
        "limitations, and pitfalls. "
        "NeuroImage, 2013, 81, 335-346"

      + "* any of above with constraints:\n"
        "Morez, J.; Szczepankiewicz, F; den Dekker, A. J.; Vanhevel, F.; Sijbers, J. &  Jeurissen, "
        "B. " // Internal
...
template <class MASKType, class B0Type, class DTType, class DKTType, class PredictType>
inline Processor<MASKType, B0Type, DTType, DKTType, PredictType>
processor(const Eigen::MatrixXd &A,
          const Eigen::MatrixXd &Aneq,
          const bool ols,
          const int iter,
...
void run() {
  ...
  if (constrain) {
    ...
    if (dki) {
      ...
      Aneq.block(tmp.rows() * 1, 7, tmp.rows(), 15) =
          (maxb / 3.0) * 6.0 * tmp.block(0, 7, tmp.rows(), 15);
    } else {
  ...
  ThreadedLoop("computing tensors", dwi, 0, 3)
      .run(processor(A, Aneq, ols, iter, mask, b0, dt, dkt, predict), dwi);
}
