...
void usage() {
  ...
  +"Supported operations are:"
      ...
      + "mean, median, sum, product, rms (root-mean-square value), norm (vector 2-norm), var "
        "(unbiased variance), "
...
void run() {
  ...
  if (opt.size()) {
    ...
    if (axis >= image_in.ndim())
      throw Exception("Cannot perform operation along axis " + str(axis) + "; image only has " +
                      str(image_in.ndim()) + " axes");
    ...
    if (num_inputs < 2)
      throw Exception(
          "mrmath requires either multiple input images, or the -axis option to be provided");
    ...
    for (size_t i = 1; i != num_inputs; ++i) {
      ...
      if (temp.ndim() < header.ndim())
        throw Exception("Image " + path + " has fewer axes than first imput image " +
                        header.name());
      for (size_t axis = 0; axis != header.ndim(); ++axis) {
        if (temp.size(axis) != header.size(axis))
          throw Exception("Dimensions of image " + path +
                          " do not match those of first input image " + header.name());
      }
      for (size_t axis = header.ndim(); axis != temp.ndim(); ++axis) {
        if (temp.size(axis) != 1)
          throw Exception("Image " + path +
                          " has axis with non-unary dimension beyond first input image " +
                          header.name());
      }
