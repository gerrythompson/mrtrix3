...
void usage() {
  ...
  SYNOPSIS = "Thoroughly check that one or more images conform to the expected ACT "
  ...
  OPTIONS
  +Option("voxels",
          "output mask images highlighting voxels where the input does not conform to 5TT "
          "requirements") +
...
void run() {
  ...
  const std::string vox_option_suggestion =
      get_options("masks").size()
          ? (" (suggest checking " +
             std::string(argument.size() > 1 ? "outputs from" : "output of") + " -masks option)")
          : " (suggest re-running using the -masks option to see voxels where tissue fractions do "
            "not sum to 1.0)";

  if (major_error_count) {
    if (argument.size() > 1)
      throw Exception(str(major_error_count) + " input image" +
                      (major_error_count > 1 ? "s do" : " does") + " not conform to 5TT format");
    else
    ...
    if (argument.size() > 1) {
      ...
      WARN("Input image does not perfectly conform to 5TT format, but may still be applicable" +
           vox_option_suggestion);
    }
  } else {
