...
void usage() {
  ...
  SYNOPSIS = "Invert the polarity of individual directions so as to optimise a unipolar "
  ...
  OPTIONS
  +Option("permutations",
          "number of permutations to try (default: " + str(default_permutations) + ")") +
...
public:
  ...
  bool update(value_type eddy, const vector<int> &signs) {
    ...
    if (eddy < best_eddy) {
      ...
      best_signs = signs;
      progress.set_text(
          "optimising directions for eddy-currents (current best configuration: energy = " +
          str(best_eddy) + ")");
    }
    ++num_permutations;
    ++progress;
...
public:
  Processor(Shared &shared)
      : shared(shared), signs(shared.get_init_signs()), uniform(0, signs.size() - 1) {}
