...
void usage() {
  AUTHOR =
      ...
      "David Raffelt (david.raffelt@florey.edu.au), Jonas Rosnarho-Tornstrand "
      "(jonas.rosnarho-tornstrand@kcl.ac.uk) "
  ...
  OPTIONS
  +Option("mask",
          "the mask defines the data used to compute the intensity normalisation. This option is "
          "mandatory.")
          .required() +
      Argument("image").type_image_in()

      + Option("order",
               "the maximum order of the polynomial basis used to fit the normalisation field in "
               "the log-domain. "
               "An order of 0 is equivalent to not allowing spatial variance of the intensity "
      ...
      + Option("niter",
               "set the number of iterations. The first (and potentially only) entry applies to "
               "the main loop. "
               "If supplied as a comma-separated list of integers, the second entry applies to the "
               "inner loop to update the balance factors "
               "(default: " +
                   str(DEFAULT_MAIN_ITER_VALUE) + "," + str(DEFAULT_BALANCE_MAXITER_VALUE) + ").") +
      Argument("number").type_sequence_int()

      + Option("reference",
               "specify the (positive) reference value to which the summed tissue compartments "
               "will be normalised. "
               "(default: " +
      ...
      + Option("check_norm",
               "output the final estimated spatially varying intensity level that is used for "
               "normalisation.") +
  ...
  REFERENCES
  +"Raffelt, D.; Dhollander, T.; Tournier, J.-D.; Tabbara, R.; Smith, R. E.; Pierre, E. & "
   "Connelly, A. " // Internal
   "Bias Field Correction and Intensity Normalisation for Quantitative Analysis of Apparent Fibre "
   "Density. "
   "In Proc. ISMRM, 2017, 26, 3541" +
      "Dhollander, T.; Tabbara, R.; Rosnarho-Tornstrand, J.; Tournier, J.-D.; Raffelt, D. & "
      "Connelly, A. " // Internal
      "Multi-tissue log-domain intensity and inhomogeneity normalisation for quantitative apparent "
      "fibre density. "
...
Eigen::MatrixXd initialise_basis(IndexType &index, size_t num_voxels, int order) {
  struct BasisInitialiser {
    BasisInitialiser(const Transform &transform,
...
size_t detect_outliers(double outlier_range,
  ...
  double lower_outlier_threshold =
      lower_quartile - outlier_range * (upper_quartile - lower_quartile);
  double upper_outlier_threshold =
      upper_quartile + outlier_range * (upper_quartile - lower_quartile);
...
void compute_balance_factors(const Eigen::MatrixXd &data,
  ...
  balance_factors.noalias() = llt.compute(HtH.triangularView<Eigen::Lower>())
                                  .solve(scaled_data * Eigen::VectorXd::Ones(field.size()));
...
void update_field(const double log_norm_value,
  ...
  Eigen::LLT<Eigen::MatrixXd> llt;
  field_coeffs.noalias() =
      llt.compute(HtH.selfadjointView<Eigen::Lower>()).solve(basis.transpose() * logsum);

  field.noalias() = (basis * field_coeffs).array().exp().matrix();
}

ImageType
compute_full_field(int order, const Eigen::VectorXd &field_coeffs, const IndexType &index) {
  Header header(index);
...
void write_weights(const Eigen::VectorXd &data,
                   IndexType &index,
...
void run() {
  if (argument.size() % 2)
    throw Exception("The number of arguments must be even, provided as pairs of each input and its "
                    "corresponding output file.");
  if (argument.size() == 2)
    WARN("Only one contrast provided. If multi-tissue CSD was performed, provide all components to "
         "mtnormalise.");
  ...
  if (num_non_finite > 0) {
    WARN("Input data contain " + str(num_non_finite) + " non-finite voxel" +
         (num_non_finite > 1 ? "s" : ""));
    ...
    while (++iter <= max_iter) {
      ...
      do {
        ...
        if (n_tissue_types > 1) {
          ...
          INFO("  balance factors (" + str(balance_iter) +
               "): " + str(balance_factors.transpose()));
        }
