...
void usage() {
  ...
  ARGUMENTS
  +Argument("type", "type of histogram matching to perform; options are: " + join(choices, ","))
          .type_choice(choices) +
      Argument("input", "the input image to be modified").type_image_in() +
      Argument("target", "the input image from which to derive the target histogram")
          .type_image_in() +
  ...
  +OptionGroup("Image masking options") +
      ...
      Option("mask_target",
             "only generate target histogram based on a specified binary mask image") +
      Argument("image").type_image_in()
  ...
  +"* If using inverse contrast normalization for inter-modal (DWI - T1) registration:\n"
   ...
   "Co-registration and distortion correction of diffusion and anatomical images based on inverse "
...
void match_linear(Image<float> &input,
  ...
  // Ax=b
  // A: Input data
  // x: Model parameters; in the "scale" case, it's a single multiplier; if "linear", include a
  // column of ones and estimate an intercept b: Output data (or actually, interpolated
  // histogram-matched output data)
  Eigen::Matrix<default_type, Eigen::Dynamic, Eigen::Dynamic> input_matrix(
      input_data.size(), estimate_intercept ? 2 : 1);
  ...
  for (size_t input_index = 0; input_index != input_data.size() - 1; ++input_index) {
    ...
    const default_type output_position =
        (target_data.size() - 1) *
        (default_type(input_index) / default_type(input_data.size() - 1));
  ...
  auto parameters = (input_matrix.transpose() * input_matrix)
                        .llt()
                        .solve(input_matrix.transpose() * output_vector)
                        .eval();
...
void match_nonlinear(Image<float> &input,
  ...
  INFO("Input histogram ranges from " + str(calib_input.get_min()) + " to " +
       str(calib_input.get_max()) + "; using " + str(calib_input.get_num_bins()) + " bins");
  ...
  INFO("Target histogram ranges from " + str(calib_target.get_min()) + " to " +
       str(calib_target.get_max()) + "; using " + str(calib_target.get_num_bins()) + " bins");
