...
void usage() {
  ...
  +Example("Default usage",
           "tck2connectome tracks.tck nodes.mif connectome.csv -tck_weights_in weights.csv "
      ...
      Example(
          ...
          "tck2connectome tracks.tck nodes.mif mean_FA_connectome.csv -scale_file "
          "mean_FA_per_streamline.csv -stat_edge mean",
      ...
      + Example(
            ...
            "rather than a matrix, since each streamline is assigned to only one node rather than "
            "two.");
  ...
  +MR::DWI::Tractography::Connectome::AssignmentOptions +
      MR::DWI::Tractography::Connectome::MetricOptions + MR::Connectome::MatrixOutputOptions
      ...
      + Option("keep_unassigned",
               "By default, the program discards the information regarding those streamlines that "
               "are not successfully assigned to a node pair. "
               "Set this option to keep these values (will be the first row/column in the output "
               "matrix)")
  ...
  REFERENCES
  +"If using the default streamline-parcel assignment mechanism (or -assignment_radial_search "
   "option): " // Internal
   "Smith, R. E.; Tournier, J.-D.; Calamante, F. & Connelly, A. "
   "The effects of SIFT on the reproducibility and biological accuracy of the structural "
   "connectome. "
...
template <typename T>
void execute(Image<node_t> &node_image,
             const node_t max_node_index,
             const std::set<node_t> &missing_nodes) {
  ...
  Tractography::Connectome::Matrix<T> connectome(
      max_node_index, statistic, vector_output, track_assignments);
...
void run() {
  ...
  if (missing_nodes.size()) {
    ...
    for (++i; i != missing_nodes.end(); ++i)
      list += ", " + str(*i);
    WARN(list);
    WARN("(This may indicate poor parcellation image preparation, use of incorrect or incomplete "
         "LUT file(s) in labelconvert, or very poor registration)");
  }
