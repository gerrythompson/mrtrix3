...
void usage() {
  ...
  +"This command will accept as inputs:" +
      "- directions file in spherical coordinates (ASCII text, [ az el ] space-separated values, "
      "one per line);" +
      "- directions file in Cartesian coordinates (ASCII text, [ x y z ] space-separated values, "
      "one per line);" +
      "- DW gradient files (MRtrix format: ASCII text, [ x y z b ] space-separated values, one per "
      "line);" +
      "- image files, using the DW gradient scheme found in the header (or provided using the "
      "appropriate command line options below)."
  ...
  if (directions.cols() >= 4) {
    ...
    if (get_options("shells").empty() && shells.has_bzero() && shells.count() > 1) {
      ...
      if (get_options("output").empty())
        print(std::string(argument[0]) + " (b=0) [ " + str(shells.smallest().count(), precision) +
              " volumes ]\n\n");
    }
...
Metrics compute(Eigen::MatrixXd &directions) {
  ...
  for (ssize_t i = 0; i < directions.rows() - 1; ++i) {
    for (ssize_t j = i + 1; j < directions.rows(); ++j) {
      double cos_angle =
          directions.row(i).head(3).normalized().dot(directions.row(j).head(3).normalized());
  ...
  for (const auto &x : select) {
    ...
    if (xl == "uet")
      ...
      if (order >= metrics.SH.size())
        throw Exception(
            "spherical harmonic order requested is too large given number of directions");
...
void report(const std::string &title, Eigen::MatrixXd &directions) {
  ...
  output += "    energy: total = " + str(metrics.BE[0], precision) +
            ", mean = " + str(metrics.BE[1], precision) + ", range [ " +
            str(metrics.BE[2], precision) + " - " + str(metrics.BE[3], precision) + " ]\n";
  ...
  output += "    energy: total = " + str(metrics.UE[0], precision) +
            ", mean = " + str(metrics.UE[1], precision) + ", range [ " +
            str(metrics.UE[2], precision) + " - " + str(metrics.UE[3], precision) + " ]\n";
  ...
  output += "\n  Asymmetry of sampling:\n    norm of mean direction vector = " +
            str(metrics.ASYM, precision) + "\n";
  if (metrics.ASYM >= 0.1)
    output += std::string("    WARNING: sampling is ") +
