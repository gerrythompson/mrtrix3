...
const OptionGroup SIFT2RegularisationOption =
    ...
    + Option("reg_tikhonov",
             "provide coefficient for regularising streamline weighting coefficients (Tikhonov "
             "regularisation) (default: " +
                 str(SIFT2_REGULARISATION_TIKHONOV_DEFAULT, 2) + ")") +
    Argument("value").type_float(0.0)

    + Option("reg_tv",
             "provide coefficient for regularising variance of streamline weighting coefficient to "
             "fixels along its length (Total Variation regularisation) (default: " +
...
const OptionGroup SIFT2AlgorithmOption =
    ...
    + Option("min_td_frac",
             ...
             "if the reconstructed streamline density is below this fraction, the fixel is "
    ...
    + Option("min_iters",
             ...
             "this can prevent premature termination at early iterations if the cost function "
             "increases slightly "
             "(default: " +
    ...
    + Option("min_factor",
             ...
             "if the factor falls below this number the streamline will be rejected entirely "
             "(factor set to zero) "
             "(default: " +
    ...
    + Option("min_coeff",
             ...
             "similar to the '-min_factor' option, but using the exponential coefficient basis of "
             "the SIFT2 model; "
             "these parameters are related as: factor = e^(coeff). "
             "Note that the -min_factor and -min_coeff options are mutually exclusive - you can "
             "only provide one. "
             "(default: " +
    ...
    + Option("max_coeff",
             ...
             "similar to the '-max_factor' option, but using the exponential coefficient basis of "
             "the SIFT2 model; "
             "these parameters are related as: factor = e^(coeff). "
             "Note that the -max_factor and -max_coeff options are mutually exclusive - you can "
             "only provide one. "
             "(default: " +
    ...
    + Option("min_cf_decrease",
             "minimum decrease in the cost function (as a fraction of the initial value) that must "
             "occur each iteration for the algorithm to continue "
             "(default: " +
    ...
    +
    Option("linear",
           "perform a linear estimation of streamline weights, rather than the standard non-linear "
           "optimisation "
...
void usage() {
  ...
  SYNOPSIS = "Optimise per-streamline cross-section multipliers to match a whole-brain tractogram "
             "to fixel-wise fibre densities";

  REFERENCES
  +"Smith, R. E.; Tournier, J.-D.; Calamante, F. & Connelly, A. " // Internal
   "SIFT2: Enabling dense quantitative assessment of brain white matter connectivity using "
...
void run() {
  ...
  if (get_options("linear").size()) {
    ...
    const float reg_tikhonov =
        get_option_value("reg_tikhonov", SIFT2_REGULARISATION_TIKHONOV_DEFAULT);
