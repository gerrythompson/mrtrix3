...
void usage() {
  ...
  AUTHOR = "J-Donald Tournier (d.tournier@brain.org.au) and Robert E. Smith "
  ...
  +"By default, all information contained in each image header will be printed to the console in "
   "a reader-friendly format."
      ...
      +
      "Alternatively, command-line options may be used to extract specific details from the "
      "header(s); "
      ...
      "will be printed in the order in which they appear in the help page, with all requested "
      "details "
  ...
  +Option("all", "print all properties, rather than the first and last 2 of each.") +
      ...
      Option("format", "image file format") + Option("ndim", "number of image dimensions") +
      ...
      Option(
          "transform",
          "the transformation from image coordinates [mm] to scanner / real world coordinates [mm]")
...
void run() {
  ...
  if (export_grad && argument.size() > 1)
    throw Exception(
        "can only export DW gradient table to file if a single input image is provided");
  if (export_pe && argument.size() > 1)
    throw Exception(
        "can only export phase encoding table to file if a single input image is provided");

  std::unique_ptr<nlohmann::json> json_keyval(get_options("json_keyval").size() ? new nlohmann::json
                                                                                : nullptr);
  std::unique_ptr<nlohmann::json> json_all(get_options("json_all").size() ? new nlohmann::json
                                                                          : nullptr);
  ...
  const bool print_full_header =
      !(format || ndim || size || spacing || datatype || strides || offset || multiplier ||
        properties.size() || transform || dwgrad || export_grad || shell_bvalues || shell_sizes ||
        shell_indices || export_pe || petable || json_keyval || json_all);
  ...
  for (size_t i = 0; i < argument.size(); ++i) {
    ...
    Eigen::MatrixXd grad;
    if (export_grad || check_option_group(GradImportOptions) || dwgrad || shell_bvalues ||
        shell_sizes || shell_indices) {
    ...
    if (json_keyval)
      File::JSON::write(
          header, *json_keyval, (argument.size() > 1 ? std::string("") : std::string(argument[0])));
