...
void usage() {
  ...
  +Option("fc",
      ...
      Argument("output_fixel_directory").type_text() + Argument("output_fixel_data").type_text()

      +
      Option("jmat",
             "output a Jacobian matrix image stored in column-major order along the 4th dimension."
             "Note the output jacobian describes the warp gradient w.r.t the scanner space "
             "coordinate system") +
...
void run() {
  ...
  if (opt.size()) {
    ...
    fixel_template_directions = Fixel::find_directions_header(template_fixel_directory)
                                    .get_image<value_type>()
                                    .with_direct_io();
  ...
  for (auto i = Loop("outputting warp metric(s)", jacobian, 0, 3)(jacobian); i; ++i) {
    ...
    if (fc_output_data.valid()) {
      ...
      for (auto f = Fixel::Loop(fixel_template_index)(fixel_template_directions, fc_output_data); f;
           ++f) {
