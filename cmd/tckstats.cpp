...
void usage() {
  ...
  +Option("output",
      ...
      + Option("ignorezero",
               "do not generate a warning if the track file contains streamlines with zero length")
...
void run() {
...

  {
    ...
    if ((!std::isfinite(step_size) || !step_size) && get_options("histogram").size()) {
      WARN("Do not have streamline step size with which to bin histogram; histogram will be "
           "generated using 1mm bin widths");
    }
    ...
    if (opt.size())
      dump.reset(
          new File::OFStream(std::string(opt[0][0]), std::ios_base::out | std::ios_base::trunc));
    ...
    while (reader(tck)) {
      ...
      if (std::isfinite(length)) {
        ...
        const size_t index =
            std::isfinite(step_size) ? std::round(length / step_size) : std::round(length);
  ...
  const float stdev =
      sum_weights ? (std::sqrt(ssd / (((count - 1) / default_type(count)) * sum_weights))) : NaN;
  ...
  if (fields.size()) {
    ...
    std::cout << " " << std::setw(width) << std::right << (mean_length) << " " << std::setw(width)
              << std::right << (median_length) << " " << std::setw(width) << std::right << (stdev)
              << " " << std::setw(width) << std::right << (min_length) << " " << std::setw(width)
              << std::right << (max_length) << " " << std::setw(width) << std::right << (count)
              << "\n";
  }
