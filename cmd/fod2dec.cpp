...
void usage() {
  ...
  COPYRIGHT =
      "Copyright (C) 2014 The Florey Institute of Neuroscience and Mental Health, Melbourne, "
      "Australia. "
      "This is free software; see the source for copying conditions. "
      "There is NO warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.";

  SYNOPSIS = "Generate FOD-based DEC maps, with optional panchromatic sharpening and/or "
             "luminance/perception correction";

  DESCRIPTION
  +"By default, the FOD-based DEC is weighted by the integral of the FOD. To weight by another "
   "scalar map, use the -contrast option. This option can also be used for panchromatic "
   "sharpening, e.g., by supplying a T1 (or other sensible) anatomical volume with a higher "
   "spatial resolution.";
  ...
  +Option("mask", "Only perform DEC computation within the specified mask image.") +
      ...
      + Option("contrast",
               "Weight the computed DEC map by the provided image contrast. If the contrast has a "
               "different image grid, the DEC map is first resliced and renormalised. To achieve "
               "panchromatic sharpening, provide an image with a higher spatial resolution than "
               "the input FOD image; e.g., a T1 anatomical volume. Only the DEC is subject to the "
               "mask, so as to allow for partial colouring of the contrast image. \nDefault when "
               "this option is *not* provided: integral of input FOD, subject to the same "
               "mask/threshold as used for DEC computation.") +
      Argument("image").type_image_in()

      + Option("lum",
               "Correct for luminance/perception, using default values Cr,Cg,Cb = " +
                   str(DEFAULT_LUM_CR, 2) + "," + str(DEFAULT_LUM_CG, 2) + "," +
                   str(DEFAULT_LUM_CB, 2) + " and gamma = " + str(DEFAULT_LUM_GAMMA, 2) +
                   " (*not* correcting is the theoretical equivalent of Cr,Cg,Cb = 1,1,1 and gamma "
                   "= 2).")
      ...
      + Option("lum_gamma",
               ...
               "switches on luminance/perception correction, using a default Cr,Cg,Cb = " +
                   str(DEFAULT_LUM_CR, 2) + "," + str(DEFAULT_LUM_CG, 2) + "," +
                   str(DEFAULT_LUM_CB, 2) + " unless specified otherwise.") +
      ...
      + Option("no_weight",
               "Do not weight the DEC map; just output the unweighted colours. Reslicing and "
               "renormalising of colours will still happen when providing the -contrast option as "
               "a template.");
}
...
void run() {
  ...
  if (get_options("lum").size() || optlc.size() || optlg.size()) {
    ...
    if (optlc.size()) {
      ...
      if (lc.size() != 3)
        throw Exception("expecting exactly 3 coefficients for the lum_coefs option, provided as a "
                        "comma-separated list Cr,Cg,Cb ; e.g., " +
                        str(DEFAULT_LUM_CR, 2) + "," + str(DEFAULT_LUM_CG, 2) + "," +
                        str(DEFAULT_LUM_CB, 2) + "");
      coefs(0) = lc[0];
      coefs(1) = lc[1];
      coefs(2) = lc[2];
    }
...

  {
...

    {
      ...
      ThreadedLoop("computing colours", fod_img, 0, 3)
          .run(DecComputer(
                   DecTransform(Math::SH::LforN(fod_img.size(3)), dirs, thresh), mask_img, w_img),
               fod_img,
               dec_img);
    }
    ...
    if (needtoslice)
      Filter::reslice<Interp::Cubic>(
          dec_img, out_img, Adapter::NoTransform, Adapter::AutoOverSample, UNIT);
    else
