...
void usage() {
  ...
  +"* If not using the -threshold command-line option:\n"
   ...
   "Threshold-free cluster enhancement: Addressing problems of smoothing, threshold dependence and "
   "localisation in cluster inference. "
   "NeuroImage, 2009, 44, 83-98"

      + "* If using the -nonstationary option:\n"
        "Salimi-Khorshidi, G. Smith, S.M. Nichols, T.E. Adjusting the effect of nonstationarity in "
        "cluster-based and TFCE inference. "
        "Neuroimage, 2011, 54(3), 2006-19";

  ARGUMENTS
  +Argument("input", "a text file containing the file names of the input images, one file per line")
          .type_file_in()
...
void run() {
  ...
  for (size_t i = 0; i != importer.size(); ++i) {
    if (!dimensions_match(dynamic_cast<SubjectVoxelImport *>(importer[i].get())->header(),
                          mask_header))
  ...
  if (extra_columns.size()) {
    ...
    if (nans_in_columns)
      CONSOLE("Non-finite values detected in element-wise design matrix columns; individual rows "
              "will be removed from voxel-wise design matrices accordingly");
  }
  ...
  if (nans_in_data) {
    INFO("Non-finite values present in data; rows will be removed from voxel-wise design matrices "
         "accordingly");
    if (!extra_columns.size()) {
...

  {
    ...
    for (size_t i = 0; i != num_hypotheses; ++i) {
      if (!hypotheses[i].is_F()) {
        write_output(abs_effect_size.col(i),
                     *v2v,
                     prefix + "abs_effect" + postfix(i) + ".mif",
                     output_header);
        ++progress;
        if (num_vgs == 1)
          write_output(std_effect_size.col(i),
                       *v2v,
  ...
  if (do_nonstationarity_adjustment) {
    if (!use_tfce)
      throw Exception("Nonstationarity adjustment is not currently implemented for threshold-based "
                      "cluster analysis");
    Stats::PermTest::precompute_empirical_stat(
        glm_test, enhancer, empirical_skew, empirical_enhanced_statistic);
    for (size_t i = 0; i != num_hypotheses; ++i)
      write_output(empirical_enhanced_statistic.col(i),
                   *v2v,
                   prefix + "empirical" + postfix(i) + ".mif",
                   output_header);
  }
