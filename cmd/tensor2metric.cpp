...
void usage() {
  ...
  AUTHOR = "Ben Jeurissen (ben.jeurissen@uantwerpen.be), Thijs Dhollander "
           "(thijs.dhollander@gmail.com) & J-Donald Tournier (jdtournier@gmail.com)";
...
public:
  Processor(Image<bool> &mask_img,
      ...
      : mask_img(mask_img),
        ...
        mk_dirs(mk_dirs),
        rk_ndirs(rk_ndirs),
        need_eigenvalues(value_img.valid() || vector_img.valid() || ad_img.valid() ||
                         rd_img.valid() || cl_img.valid() || cp_img.valid() || cs_img.valid() ||
...
private:
  ...
  double kurtosis(const BMatType &bmat, const DTType &dt, const DKTType &dkt) {
    return -6.0 * ((bmat.template middleCols<15>(7) * dkt).array() /
                   (bmat.template middleCols<6>(1) * dt).array().square())
                      .mean();
  }
};
...
void run() {
  ...
  if (dki_metric_count && !dkt_img.valid()) {
    throw Exception("Cannot calculate diffusion kurtosis metrics; must provide the kurtosis tensor "
                    "using the -dkt input option");
  }

  if (!metric_count)
    throw Exception("No output specified; must request at least one metric of interest using the "
