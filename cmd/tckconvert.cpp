...
template <class T>
void loadLines(vector<int64_t> &lines, std::ifstream &input, int number_of_line_indices) {
...
public:
  ASCIIWriter(const std::string &file) {
    ...
    parser.parse(file);
    if (parser.ndim() != 1)
      throw Exception("output file specifier should contain one placeholder for numbering (e.g. "
                      "output-[].txt)");
...
public:
  ...
  bool operator()(const Streamline<float> &intck) {
    ...
    // to handle the increment, we want to keep the first 2 and last 2 points, but we can skip
    // inside
    ...
    for (size_t idx = 1; idx < tck.size() - 1; ++idx) {
      ...
      for (auto sideIdx = 0; sideIdx < nSides; sideIdx++) {
        ...
        vertexOF << (int)(255 * fabs(T[0])) << " " << (int)(255 * fabs(T[1])) << " "
                 << (int)(255 * fabs(T[2])) << "\n";
        if (!isLast) {
          faceOF << "3"
                 << " " << num_vertices + faces(sideIdx, 0) << " "
                 << num_vertices + faces(sideIdx, 1) << " " << num_vertices + faces(sideIdx, 2)
                 << "\n";
          faceOF << "3"
                 << " " << num_vertices + faces(sideIdx, 3) << " "
                 << num_vertices + faces(sideIdx, 4) << " " << num_vertices + faces(sideIdx, 5)
                 << "\n";
          num_faces += 2;
        }
      }
      ...
      if (isFirst) {
        for (auto sideIdx = nSides - 1; sideIdx >= 2; --sideIdx) {
          faceOF << "3"
                 << " " << num_vertices + sideIdx << " " << num_vertices + sideIdx - 1 << " "
                 << num_vertices << "\n";
        }
        num_faces += nSides - 2;
        isFirst = false;
      ...
      if (isLast) {
        for (auto sideIdx = 2; sideIdx <= nSides - 1; ++sideIdx) {
          faceOF << "3"
                 << " " << num_vertices + sideIdx - 1 << " " << num_vertices + sideIdx << " "
                 << num_vertices << "\n";
        }
        num_faces += nSides - 2;
      }
    ...
    try {
      ...
      out << "ply\n"
          ...
          << "\n"
             ...
             "comment the coordinate system and scale is taken from directly from the input and is "
             "not adjusted\n"
             "element vertex "
          << num_vertices
          << "\n"
