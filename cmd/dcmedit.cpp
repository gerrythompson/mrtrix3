...
void usage() {
  ...
  +Option("anonymise",
      ...
      + Option("tag", "replace specific tag.").allow_multiple() + Argument("group") +
      Argument("element") + Argument("newvalue");
}
...
void run() {
  ...
  for (size_t n = 0; n < tags.size(); ++n)
    INFO("replacing tag (" + hex(tags[n].group) + "," + hex(tags[n].element) + ") with value \"" +
         tags[n].newvalue + "\"");
  ...
  while (item.read()) {
    ...
    for (size_t n = 0; n < tags.size(); ++n) {
      if (item.is(tags[n].group, tags[n].element)) {
        memset(item.data, 32, item.size);
        memcpy(
            item.data, tags[n].newvalue.c_str(), std::min<int>(item.size, tags[n].newvalue.size()));
      }
    }
  }
}
