...
void usage() {
  ...
  +Example(
      ...
      "The wildcard characters will find all images in the current working directory with names "
      "that "
      "begin with \"volume\" and end with \".mif\"; the mrcat command will receive these as a list "
      "of "
...
void write(vector<Header> &in, const size_t axis, Header &header_out) {
  ...
  for (size_t i = 0; i != in.size(); i++) {
    ...
    ThreadedLoop("concatenating \"" + image_in.name() + "\"",
                 image_in,
                 0,
...
void run() {
  ...
  const size_t axis = get_option_value(
      "axis", std::max(size_t(3), size_t(std::max(ssize_t(0), max_axis_nonunity))));
