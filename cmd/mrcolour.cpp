...
void usage() {
  ...
  +Option("upper", "manually set the upper intensity of the colour mapping") +
      ...
      + Option("colour",
               "set the target colour for use of the 'colour' map (three comma-separated "
...
void run() {
  ...
  if (colourmap.is_colour) {
    ...
    if (!opt.size())
      throw Exception(
          "For \'colour\' colourmap, target colour must be specified using the -colour option");
    ...
    if (get_options("lower").size()) {
      WARN("Option -lower ignored: not compatible with \'rgb\' colourmap (a minimum of 0.0 is "
           "assumed)");
    }
  } else {
    if (!(H_in.ndim() == 3 || (H_in.ndim() == 4 && H_in.size(3) == 1)))
  ...
  auto scale = [&](const float value) {
    return std::max(0.0f, std::min(1.0f, multiplier * (value - lower)));
  };
  ...
  if (colourmap.is_colour) {
    ...
    for (auto l_outer = Loop("Applying fixed RGB colour to greyscale image", H_in)(in, out);
         l_outer;
         ++l_outer) {
    ...
    for (auto l_outer = Loop("Mapping intensities to RGB colours", H_in)(in, out); l_outer;
         ++l_outer) {
