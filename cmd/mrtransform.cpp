...
void usage() {
  ...
  +"If a linear transform is applied without a template image the command "
   "will modify the image header transform matrix"
      ...
      + "The output image intensity can be modulated using the (local or global) volume change "
        ...
        "apparent fibre density across the fibre bundle width and can only be applied if FOD "
        "reorientation "
        "is used. Alternatively, non-directional scaling by the Jacobian determinant can be "
        "applied to "
        "any image type. "
  ...
  +"* If FOD reorientation is being performed:\n"
   ...
   "Magnetic Resonance in Medicine, 2012, 67, 844-855"
      ...
      + "* If FOD modulation is being performed:\n"
        "Raffelt, D.; Tournier, J.-D.; Rose, S.; Ridgway, G.R.; Henderson, R.; Crozier, S.; "
        "Salvado, O.; Connelly, A.; " // Internal
  ...
  +OptionGroup("Affine transformation options")
      ...
      + Option("flip",
               "flip the specified axes, provided as a comma-separated list of indices (0:x, 1:y, "
               "2:z).") +
      ...
      + Option("half",
               "apply the matrix square root of the transformation. This can be combined with the "
               "inverse option.")
      ...
      + Option("midway_space",
               "reslice the input image to the midway space. Requires either the -template or "
               "-warp option. If "
               "used with -template and -linear option the input image will be resliced onto the "
               "grid halfway between the input and template. "
               "If used with the -warp option the input will be warped to the midway space defined "
               "by the grid of the input warp "
               "(i.e. half way between image1 and image2)")

      + Option("interp",
               "set the interpolation method to use when reslicing (choices: nearest, linear, "
               "cubic, sinc. Default: cubic).") +
      Argument("method").type_choice(interp_choices)

      + Option("oversample",
               "set the amount of over-sampling (in the target space) to perform when regridding. "
               "This is particularly "
               "relevant when downsamping a high-resolution image to a low-resolution image, to "
               "avoid aliasing artefacts. "
               "This can consist of a single integer, or a comma-separated list of 3 integers if "
               "different oversampling "
               "factors are desired along the different axes. Default is determined from ratio of "
               "voxel dimensions (disabled "
      ...
      Option(
          "warp",
          "apply a non-linear 4D deformation field to warp the input image. Each voxel in the "
          "deformation field must define "
          "the scanner space position that will be used to interpolate the input image during "
          "warping (i.e. pull-back/reverse warp convention). "
          "If the -template image is also supplied the deformation field will be resliced first to "
          "the template image grid. If no -template "
          "option is supplied then the output image will have the same image grid as the "
          "deformation field. This option can be used in "
          "combination with the -affine option, in which case the affine will be applied first)") +
      Argument("image").type_image_in()

      + Option("warp_full",
               "warp the input image using a 5D warp file output from mrregister. Any linear "
               "transforms in the warp image header "
               "will also be applied. The -warp_full option must be used in combination with "
               "either the -template option or the -midway_space option. "
               "If a -template image is supplied then the full warp will be used. By default the "
               "image1->image2 transform will be applied, "
               "however the -from 2 option can be used to apply the image2->image1 transform. Use "
               "the -midway_space option to warp the input "
               "image to the midway space. The -from option can also be used to define which warp "
               "to use when transforming to midway space") +
      Argument("image").type_image_in()
      ...
      + Option("modulate",
               ...
               "fod: modulate FODs during reorientation to preserve the apparent fibre density "
               "across fibre bundle widths before and after the transformation. \n"
               "jac: modulate the image intensity with the determinant of the Jacobian of the warp "
               "of linear transformation "
               "to preserve the total intensity before and after the transformation.") +
      Argument("method").type_choice(modulation_choices)

      + Option("directions",
               "directions defining the number and orientation of the apodised point spread "
               "functions used in FOD reorientation "
               "(Default: 300 directions)") +
      Argument("file", "a list of directions [az el] generated using the dirgen command.")
          .type_file_in()
      ...
      + Option("reorient_fod",
               ...
               "antipodally symmetric spherical harmonic series with lmax >= 2 (i.e. 6, 15, 28, "
               "45, 66 volumes).") +
...
void apply_warp(Image<float> &input,
  ...
  switch (interp) {
  case 0:
    Filter::warp<Interp::Nearest>(
        input, output, warp, out_of_bounds_value, oversample, jacobian_modulate);
    break;
  case 1:
    Filter::warp<Interp::Linear>(
        input, output, warp, out_of_bounds_value, oversample, jacobian_modulate);
    break;
  case 2:
...
void apply_linear_jacobian(Image<float> &image, transform_type trafo) {
  ...
  for (auto i = Loop("applying global intensity modulation", image, 0, image.ndim())(image); i;
       ++i) {
    image.value() *= det;
  }
}
...
void run() {
  ...
  if (opt.size()) {
    ...
    try {
      ...
      try {
        ...
        throw Exception("Unable to extract transform matrix from -replace file \"" +
                        str(opt[0][0]) + "\"");
      }
    }
  }
  ...
  if (opt.size()) {
    if (!Path::is_mrtrix_image(opt[0][0]) && !(Path::has_suffix(opt[0][0], {".nii", ".nii.gz"}) &&
      ...
      WARN("warp_full image is not in original .mif/.mih file format or in NIfTI file format with "
           "associated JSON.  "
    ...
    if (linear)
      throw Exception(
          "the -warp_full option cannot be applied in combination with -linear since the "
  ...
  if (opt.size()) {
    ...
    if (warp.size(3) != 3)
      throw Exception(
          "the input -warp file must have 3 volumes in the 4th dimension (x,y,z positions)");
  }
  ...
  if (inverse) {
    ...
    if (replace)
      throw Exception(
          "cannot use -inverse option in conjunction with -replace or -identity options");
    if (warp.valid())
      if (warp.ndim() == 4)
    ...
    for (size_t i = 0; i < axes.size(); ++i) {
      if (axes[i] < 0 || axes[i] > 2)
        throw Exception("axes supplied to -flip are out of bounds (" + std::string(opt[0][0]) +
                        ")");
      flip(axes[i], 3) +=
          flip(axes[i], axes[i]) * input_header.spacing(axes[i]) * (input_header.size(axes[i]) - 1);
  ...
  // Detect FOD image
  bool is_possible_fod_image =
      input_header.ndim() == 4 && input_header.size(3) >= 6 &&
  ...
  if (fod_reorientation && (linear || warp.valid() || template_header.valid()) &&
      is_possible_fod_image) {
  ...
  const std::string reorient_msg =
      str("reorienting") + str((modulate_fod ? " with FOD modulation" : ""));
  if (modulate_fod)
    add_line(output_header.keyval()["comments"], std::string("FOD modulation applied"));
  if (modulate_jac)
    add_line(output_header.keyval()["comments"],
             std::string("Jacobian determinant modulation applied"));
  ...
  if (modulate_jac) {
    if (fod_reorientation) {
      WARN("Input image being interpreted as FOD data (user requested FOD reorientation); "
           "FOD-based modulation would be more appropriate for such data than the requested "
           "Jacobian modulation.");
    } else if (is_possible_fod_image) {
      WARN("Jacobian modulation performed on possible SH series image. Did you mean FOD "
           "modulation?");
    }
    if (!linear && !warp.valid())
      ...
      try {
        ...
        if (lines.size() != size_t(input_header.size(3)))
          throw Exception("Number of lines in header entry \"directions\" (" + str(lines.size()) +
                          ") does not match number of volumes in image (" +
        ...
        for (size_t l = 0; l != lines.size(); ++l) {
          ...
          if (!result.cols()) {
            if (!(v.size() == 2 || v.size() == 3))
              throw Exception(
                  "Malformed \"directions\" field (expected matrix with 2 or 3 columns; data has " +
                  str(v.size()) + " columns)");
  ...
  if (opt.size()) {
    if (!template_header.valid() && !warp)
      throw Exception("-oversample option applies only to regridding using the template option or "
    ...
    if (oversample.size() == 1)
      ...
      throw Exception("-oversample option requires either a single integer, or a comma-separated "
                      "list of 3 integers");
  ...
  if (template_header.valid() && !warp) {
    ...
    if (get_options("midway_space").size()) {
      ...
      if (!half)
        WARN("regridding to midway_space assumes the linear transformation to be a transformation "
             "from input to midway space. Use -half if the input transformation is a full "
             "transformation.");
      transform_type linear_transform_inverse;
      linear_transform_inverse.matrix() = linear_transform.inverse().matrix();
      auto midway_header = compute_minimum_average_header(
          input_header, template_header, linear_transform_inverse, linear_transform);
      for (size_t i = 0; i < 3; ++i) {
    ...
    case 0:
      Filter::reslice<Interp::Nearest>(
          input, output, linear_transform, oversample, out_of_bounds_value);
      break;
    case 1:
      Filter::reslice<Interp::Linear>(
          input, output, linear_transform, oversample, out_of_bounds_value);
      break;
    case 2:
    ...
    if (fod_reorientation)
      Registration::Transform::reorient(reorient_msg.c_str(),
                                        output,
                                        output,
                                        linear_transform,
                                        directions_cartesian.transpose(),
                                        modulate_fod);
    ...
    if (warp.ndim() == 5) {
      ...
      apply_warp(
          input, output, warp_composed, interp, out_of_bounds_value, oversample, modulate_jac);
      if (fod_reorientation)
        Registration::Transform::reorient_warp(reorient_msg.c_str(),
                                               output,
                                               warp_composed,
    ...
    if (!temp.isIdentity(0.001))
      WARN("the input linear transform is not orthonormal and therefore applying this without the "
           "-template "
    ...
    if (fod_reorientation || modulate_jac) {
      ...
      if (fod_reorientation)
        Registration::Transform::reorient(
            "reorienting", output, output, transform, directions_cartesian.transpose());
      if (modulate_jac)
