...
void usage() {
  ...
  AUTHOR = "David Raffelt (david.raffelt@florey.edu.au) and Thijs Dhollander "
           "(thijs.dhollander@gmail.com) and Robert E. Smith (robert.smith@florey.edu.au)";
  ...
  +Argument("input", "the input DW image.").type_image_in() +
      Argument("output", "the output image (diffusion-weighted volumes by default).")
          .type_image_out();

  OPTIONS
  +Option("bzero",
          "Output b=0 volumes (instead of the diffusion weighted volumes, if -singleshell is not "
          "specified).") +
      Option("no_bzero",
             "Output only non b=0 volumes (default, if -singleshell is not specified).") +
      Option("singleshell",
             "Force a single-shell (single non b=0 shell) output. This will include b=0 volumes, "
             "if present. Use with -bzero to enforce presence of b=0 volumes (error if not "
             "present) or with -no_bzero to exclude them.") +
      DWI::GradImportOptions() + DWI::ShellsOption + DWI::GradExportOptions() +
      PhaseEncoding::ImportOptions + PhaseEncoding::SelectOptions + Stride::Options;
}
...
void run() {
  ...
  if (get_options("shells").size() || get_options("singleshell").size()) {
    ...
    shells.select_shells(get_options("singleshell").size(),
                         get_options("bzero").size(),
                         get_options("no_bzero").size());
    for (size_t s = 0; s != shells.count(); ++s) {
      DEBUG("Including data from shell b=" + str(shells[s].get_mean()) + " +- " +
    ...
    for (ssize_t row = 0; row != grad.rows(); ++row) {
      if ((bzero && (grad(row, 3) < bzero_threshold)) ||
          (!bzero && (grad(row, 3) > bzero_threshold)))
  ...
  if (opt.size()) {
    ...
    if (!(filter.size() == 3 || filter.size() == 4))
      throw Exception(
          "Phase encoding filter must be a comma-separated list of either 3 or 4 numbers");
