...
void usage() {
  ...
  +"This application attempts to remove Gibbs ringing artefacts from MRI images using the method "
   "of local subvoxel-shifts proposed by Kellner et al. (see reference below for details). By "
   "default, "
   "the original 2D slice-wise version is used. If the -mode 3d option is provided, the program "
   "will run "
   "the 3D version as proposed by Bautista et al. (also in the reference list below)."

      + "This command is designed to run on data directly after it has been reconstructed by the "
        "scanner, "
        "before any interpolation of any kind has taken place. You should not run this command "
        "after any "
        "form of motion correction (e.g. not after dwifslpreproc). Similarly, if you intend "
        "running dwidenoise, "
      ...
      + "Note that this method is designed to work on images acquired with full k-space coverage. "
        "Running this method on partial Fourier ('half-scan') or filtered data may not remove all "
        "ringing "
        "artefacts. Users are encouraged to acquired full-Fourier data where possible, and disable "
        "any "
  ...
  +Option("mode",
      ...
      + Option("axes",
               "select the slice axes (default: 0,1 - i.e. x-y). Select all 3 spatial axes for 3D "
               "operation, "
...
void run() {
  ...
  header.datatype() = DataType::from_command_line(
      header.datatype().is_complex() ? DataType::CFloat32 : DataType::Float32);
  ...
  if (slice_encoding_it != header.keyval().end()) {
    if (mode == 1) {
      WARN("running 3D volume-wise unringing, but image header contains \"SliceEncodingDirection\" "
           "field");
      ...
      try {
        ...
        if (axes_set_manually) {
          if (slice_axes == auto_slice_axes) {
            INFO("User's manual selection of within-slice axes consistent with "
          ...
          if (slice_axes == auto_slice_axes) {
            INFO("\"SliceEncodingDirection\" field in image header is consistent with default "
                 "selection of first two axes as being within-slice");
          } else {
            slice_axes = auto_slice_axes;
            CONSOLE("Using axes { " + str(slice_axes[0]) + ", " + str(slice_axes[1]) +
                    " } for Gibbs ringing removal based on \"SliceEncodingDirection\" field in "
                    "image header");
          }
        }
      } catch (...) {
        WARN("Invalid value for field \"SliceEncodingDirection\" in image header (" +
             slice_encoding_it->second + "); ignoring");
      }
    }
  }
