...
void usage() {
  ...
  DESCRIPTION
  +"Any linear transforms must be supplied as a 4x4 matrix in a text file (e.g. as per the output "
   "of mrregister). "
   "Any warp fields must be supplied as a 4D image representing a deformation field (e.g. as "
   "output from mrrregister -nl_warp)."

      + "Input transformations should be provided to the command in the order in which they would "
        "be applied "
        "to an image if they were to be applied individually."

      + "If all input transformations are linear, and the -template option is not provided, then "
        "the file output by "
        "the command will also be a linear transformation saved as a 4x4 matrix in a text file. If "
        "a template image is "
        "supplied, then the output will always be a deformation field. If at least one of the "
        "inputs is a warp field, "
        "then the output will be a deformation field, which will be defined on the grid of the "
        "last input warp image "
        "supplied if the -template option is not used.";

  ARGUMENTS
  +Argument("input", "the input transforms (either linear or non-linear warps).")
          .type_file_in()
          .allow_multiple() +
      Argument("output",
               "the output file (may be a linear transformation text file, or a deformation warp "
               "field image, depending on usage)")
          .type_various();
...
void run() {
  ...
  if (opt.size()) {
    ...
    // no template is supplied and there are input warps, then make sure the last transform in the
    // list is a warp
  } else if (template_header) {
    if (!dynamic_cast<Warp *>(transform_list[transform_list.size() - 1].get()))
      throw Exception("Output deformation field grid not defined. When composing warps either use "
                      "the -template "
                      "option to define the output deformation field grid, or ensure the last "
                      "input transformation is a warp.");
  }

  // all inputs are linear so compose and output as text file
  if (!template_header) {

    transform_type composed =
        dynamic_cast<Linear *>(transform_list[transform_list.size() - 1].get())->transform;
