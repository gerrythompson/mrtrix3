...
const OptionGroup OutputOptions =
    ...
    + Option("disp",
             "output a measure of dispersion per fixel as the ratio between FOD lobe integral and "
...
void usage() {
  ...
  SYNOPSIS = "Perform segmentation of continuous Fibre Orientation Distributions (FODs) to produce "
             "discrete fixels";
  ...
  +OutputOptions
      ...
      + Option("dirpeak",
               ...
               "weighted mean direction (the default)");
}
...
public:
  Segmented_FOD_receiver(const Header &header,
                         const index_type maxnum = 0,
                         bool dir_from_peak = false)
  ...
  public:
    Primitive_FOD_lobes(const FOD_lobes &in, const index_type maxcount, bool dir_from_peak)
      ...
      for (index_type i = 0; i != N; ++i) {
        ...
        if (dir_from_peak)
          this->emplace_back(
              lobe.get_peak_dir(0).cast<float>(), lobe.get_integral(), lobe.get_max_peak_value());
        else
          this->emplace_back(
              lobe.get_mean_dir().cast<float>(), lobe.get_integral(), lobe.get_max_peak_value());
      }
    }
    Eigen::Array3i vox;
  };
...
void Segmented_FOD_receiver::commit() {
  ...
  if (dir_path.size()) {
    ...
    dir_image = make_unique<DataImage>(
        DataImage::create(Path::join(fixel_directory_path, dir_path), dir_header));
    dir_image->index(1) = 0;
  ...
  if (afd_path.size()) {
    ...
    afd_image = make_unique<DataImage>(
        DataImage::create(Path::join(fixel_directory_path, afd_path), afd_header));
    afd_image->index(1) = 0;
  ...
  if (disp_path.size()) {
    ...
    disp_image = make_unique<DataImage>(
        DataImage::create(Path::join(fixel_directory_path, disp_path), disp_header));
...
void run() {
  ...
  if (opt.size()) {
    ...
    if (!dimensions_match(fod_data, mask, 0, 3))
      throw Exception("Cannot use image \"" + str(opt[0][0]) +
  ...
  Thread::run_queue(
      writer, Thread::batch(SH_coefs()), Thread::multi(fmls), Thread::batch(FOD_lobes()), receiver);
  receiver.commit();
}
