...
void usage() {
  ...
  DESCRIPTION
  +"This command is designed to enable CFE-based statistical analysis to be performed on "
   "voxel-wise measures." +
      Fixel::format_description;

  ARGUMENTS
  +Argument("image_in", "the input image.").type_image_in() +
      Argument("fixel_directory_in",
               "the input fixel directory. Used to define the fixels and their directions")
          .type_directory_in() +
      Argument("fixel_directory_out",
               "the fixel directory where the output will be written. This can be the same as the "
               "input directory if desired")
          .type_text() +
...
void run() {
  ...
  for (auto v = Loop("mapping voxel scalar values to fixels", 0, 3)(scalar, input_fixel_index); v;
       ++v) {
