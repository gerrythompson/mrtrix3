...
void run() {
  ...
  if (multi_in1.size() != multi_in2.size())
    throw Exception("Mismatched number of mesh objects (" + str(multi_in1.size()) + " - " +
  ...
  for (size_t mesh_index = 0; mesh_index != multi_in1.size(); ++mesh_index) {
    ...
    //  throw Exception ("Mismatched vertex count (" + str(in1.num_vertices()) + " - " +
    //  str(in2.num_vertices()) + ") - test FAILED");
    if (in1.num_triangles() != in2.num_triangles())
      throw Exception("Mismatched triangle count (" + str(in1.num_triangles()) + " - " +
                      str(in2.num_triangles()) + ") - test FAILED");
    if (in1.num_quads() != in2.num_quads())
      throw Exception("Mismatched quad count (" + str(in1.num_quads()) + " - " +
                      str(in2.num_quads()) + ") - test FAILED");
