    ...
    for (size_t iter_fillvalue = 0; iter_fillvalue != 2; ++iter_fillvalue) {
      ...
      for (size_t i = 0; i != bits; ++i) {
        ...
        test(data.count() == target_count,
             "Data " + str(data) + " (count " + str(target_count) +
                 ") erroneously reported as count " + str(data.count()));
      }
      switch (iter_fillvalue) {
      case 0:
      ...
      test(identical(data, duplicate, bits),
           "Duplicated sets pre-resize of size " + str(bits) +
               " did not lead to identical data: " + str(data) + " " + str(duplicate));
      // Change one bit within the final byte; make sure duplicate is no longer equivalent according
      // to operator==()
      if (bits) {
        size_t index_to_toggle;
        do {
        ...
        test(data == duplicate,
             "Reversion of one changed bit in size " + str(bits) +
                 " not reported as equal: " + str(data) + " " + str(duplicate));
      }
      ...
      test(identical(data, duplicate, bits),
           "Duplicated sets post-resize of size " + str(bits) +
               " did not lead to identical data: " + str(data) + " " + str(duplicate));
      switch (iter_fillvalue) {
      case 0:
        test(valid_last(data, bits, false),
             "Resized (0's) full set of size " + str(bits) +
                 " contains invalid zeroed excess data: " + str(data));
        test(valid_last(duplicate, bits, true),
             "Resized (1's) full set of size " + str(bits) +
                 " contains invalid filled excess data: " + str(duplicate));
        break;
      case 1:
        test(valid_last(data, bits, false),
             "Resized (0's) empty set of size " + str(bits) +
                 " contains invalid zeroed excess data: " + str(data));
        test(valid_last(duplicate, bits, true),
