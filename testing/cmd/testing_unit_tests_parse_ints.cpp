...
void run() {
  ...
  for (const auto &t : tests) {
    try {
      if (parse_ints<int>(t.str) != t.result)
        failures.push_back("\"" + std::string(t.str) + "\" to " + str(t.result) +
                           " failed (produced " + str(parse_ints<int>(t.str)) + ")");
    } catch (Exception &e) {
      if (t.result.size())
        failures.push_back("\"" + std::string(t.str) + "\" to " + str(t.result) +
                           " failed: " + e[0]);
  ...
  if (failures.size()) {
    Exception e(str(failures.size()) + " of " + str(sizeof(tests) / sizeof(Test)) +
                " tests failed:");
