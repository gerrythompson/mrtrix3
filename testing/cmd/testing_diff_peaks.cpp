...
void run() {
  ...
  if (in1.size(3) % 3)
    throw Exception("images \"" + in1.name() + "\" and \"" + in2.name() +
                    "\" do not contain XYZ peak directions");
  for (size_t i = 0; i < in1.ndim(); ++i) {
    if (std::isfinite(in1.size(i)))
      if (in1.size(i) != in2.size(i))
        throw Exception("images \"" + in1.name() + "\" and \"" + in2.name() +
                        "\" do not have matching voxel spacings " + str(in1.size(i)) + " vs " +
                        str(in2.size(i)));
  }
        ...
        for (size_t i = 0; i != size_t(a.size(3)); i += 3) {
          ...
          if (norma && normb && (1.0 - dp > tol))
            throw Exception("images \"" + a.name() + "\" and \"" + b.name() +
                            ...
                            str(veca.transpose().cast<float>()) + "] vs [" +
                            str(vecb.transpose().cast<float>()) + "], norms [" + str(norma) + " " +
                            str(normb) + "], dot product = " + str(dp) + ")");
        }
      },
      in1,
      in2);
