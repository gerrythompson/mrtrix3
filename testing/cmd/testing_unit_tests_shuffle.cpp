...
void run() {
      ...
      for (const auto &b1 : blocks) {
        ...
        for (const auto &b2 : blocks) {
          if (b2.find(first_out) != b2.end()) {
            // Index at the start of block b1 in the shuffled data originated from block b2 before
            // shuffling Ensure that ALL indices in block b1 in the shuffled data originated from
            // block b2
            for (auto i : b1) {
  ...
  auto test_kernel = [&](const size_t requested_number,
    ...
    if (eb_within.size())
      test_permutation_within(
          temp, "Broken within-block permutation; " + error_string + "; " + test_string);
    if (eb_whole.size()) {
      if (error_type == Shuffler::error_t::EE || error_type == Shuffler::error_t::BOTH)
        test_permutation_whole(
            temp, "Broken whole-block exchangeability; " + error_string + "; " + test_string);
      if (error_type == Shuffler::error_t::ISE || error_type == Shuffler::error_t::BOTH)
        test_signflip_whole(
            temp, "Broken whole-block sign-flipping; " + error_string + "; " + test_string);
    }
    if (test_uniqueness)
  ...
  for (size_t exchange_index = 0; exchange_index != 3; ++exchange_index) {
    ...
    const index_array_type eb_within(
        exchange_t(exchange_index) == exchange_t::WITHIN ? block_indices : index_array_type());
    const index_array_type eb_whole(
        exchange_t(exchange_index) == exchange_t::WHOLE ? block_indices : index_array_type());
    ...
    for (size_t error_index = 0; error_index != 2; ++error_index) {
      const Shuffler::error_t error_type(error_index ? Shuffler::error_t::ISE
                                                     : Shuffler::error_t::EE);
