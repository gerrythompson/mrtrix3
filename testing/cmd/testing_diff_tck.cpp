...
void usage() {
  ...
  OPTIONS
  +Option("distance",
          "maximum permissible Hausdorff distance in mm (default: " + str(DEFAULT_HAUSDORFF) +
              "mm)") +
...
inline bool within_haussdorf(const DWI::Tractography::Streamline<> &tck1,
                             const DWI::Tractography::Streamline<> &tck2,
                             float tol) {
  tol *= tol;
